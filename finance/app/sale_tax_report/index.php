...
{ 
    //if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "generate_report") {
			...
			$str='';
			$qry='select inv.invoice_id,inv.invoice_no,inv.invoice_date,led.l_name,led.gst_no,sum(invtrn.product_amount) as taxable,inv.round_off,inv.g_total from tbl_invoice as inv 
			left join tbl_invoicetrn as invtrn on invtrn.invoice_id = inv.invoice_id  
			left join tbl_ledger as led on led.l_id = inv.cust_id
			where inv.invoice_status=0 and invtrn.trancation_status=0 and inv.invoice_date>="'.date('Y-m-d',strtotime($s_date[0])).'" and inv.invoice_date<="'.date('Y-m-d',strtotime($s_date[1])).'" and inv.company_id='.$_SESSION['company_id'].' '.$where.' group by inv.invoice_id order by invoice_no';
			$result=$dbcon->query($qry);

			$query = "SELECT led.l_name,gen.ledger_id FROM `tbl_general_book` as gen left join tbl_ledger as led on led.l_id = gen.ledger_id WHERE gen.module_name='Invoice' and led.l_group=31 and gen.company_id = ".$_SESSION['company_id']." group by gen.ledger_id";
			$rs_tax=$dbcon->query($query);
            $tax_col=mysqli_num_rows($rs_tax);
			$tax_row = brp_mysqli_fetch_all($rs_tax);
            
			$query_sun = "SELECT led.l_name,gen.ledger_id FROM `tbl_general_book` as gen left join tbl_ledger as led on led.l_id = gen.ledger_id WHERE gen.module_name='Invoice' and led.l_group in (16,19) and gen.company_id = ".$_SESSION['company_id']." group by gen.ledger_id";
            ...
            $str .='<div id="report" class="col-md-12" name="report" width="100%">
				...
				if(mysqli_num_rows($result)>0)
				{
					...
					{	
						...
						$k=0;
						foreach($sun_row as $row1){
							$query_sundry_value = "SELECT gen.ledger_id,gen.amount FROM `tbl_general_book` as gen left join tbl_ledger as led on led.l_id = gen.ledger_id WHERE gen.module_name='Invoice' and led.l_group in (16,19) and gen.company_id = ".$_SESSION['company_id']." and gen.module_id=".$re['invoice_id']." and ledger_id=".$row1['ledger_id'];
						...
						$i=0;
						foreach($tax_row as $row){
							$query_tax_value = "SELECT gen.ledger_id,gen.amount FROM `tbl_general_book` as gen left join tbl_ledger as led on led.l_id = gen.ledger_id WHERE gen.module_name='Invoice' and led.l_group=31 and gen.company_id = ".$_SESSION['company_id']." and gen.module_id=".$re['invoice_id']." and ledger_id=".$row['ledger_id'];
				 		...
				 		$str .='<td data-label="Total Amount" style="text-align:right">'.indian_number($re["round_off"]).'</td>
						 <td data-label="Total Amount" style="text-align:right">'.indian_number($re["g_total"]).'</td>
						</tr>';				
						$j++;
						$total=$total+$re["g_total"];
						$roundof = $roundof + $re['round_off'];
