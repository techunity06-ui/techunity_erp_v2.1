...
{	
	...
	$company_state = get_company_data($dbcon,$_SESSION['company_id']);
	 ...
	 $chkother = "select sum(sundry_amount) as other_amount from tbl_bill_sundry_transaction as b left join tbl_ledger as tl on b.sundry_ledger_id=tl.l_id where b.sundry_voucher_id='".$invoice_id."' and b.sundry_voucher_table='tbl_invoice' and b.isdelete='0' and tl.l_form='tax_form' and tl.default_sundry='0'";
	...
	while($getsql = brp_mysqli_fetch_assoc($chksql)){
	 ...
	 $other_amount=$getother['other_amount'];
	$gtotal_TotInvVal=$gtotal_TotInvVal+$other_amount;
	...
	$ValDtls = array(
		...
		"OthChrg"=> $other_amount,
		"RndOffAmt"=> 0,
		"TotInvVal"=> round($gtotal_TotInvVal),
