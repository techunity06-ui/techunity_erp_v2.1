...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$where.="  and s.debitnote_date >= '".date('Y-m-d',strtotime($s_date[0]))."' AND s.debitnote_date <= '".date('Y-m-d',strtotime($s_date[1]))."'";
		...
		// branch , comapny , user check end - dhaval 
			...
			$appData = array();
			$i=1;
			$aColumns = array('s.debitnote_id','s.debitnote_no','cust.l_name','s.debitnote_date','s.g_total','s.cdate','s.userid','s.vender_id');
			$sIndexColumn = "debitnote_id";
			$isWhere = array("s.debit_note_status = 0 and s.is_without_item=0  ".$where);
			$sTable = "tbl_debitnote as s";	
			$isJOIN = array('inner join tbl_ledger cust on s.vender_id=cust.l_id');
			$hOrder = "s.debitnote_date desc";
			...
			foreach($sqlReturn as $row) {
				...
				if(in_array(FINANCE_PURCHASE_RETURN_UPDATE,$bulkAccessArray)){
					$edit_btn='<a class="btn btn-xs btn-warning" data-original-title="Edit" data-toggle="tooltip" data-placement="top" href="'.ROOT.FINANCE_ROOT.'debitnoteedit/'.$row['debitnote_id'].'"><i class="fa fa-pencil"></i></a>';
				}
				if(in_array(FINANCE_PURCHASE_RETURN_UPDATE,$bulkAccessArray)){
					$delete_btn='<button class="btn btn-xs btn-danger" data-original-title="Delete" data-toggle="tooltip" data-placement="top" onClick="delete_debit_note('.$row['debitnote_id'].')"><i class="fa fa-trash-o"></i></button>';
				}
				...
				$sql=$dbcon->query("SELECT * FROM print_setup_mst WHERE print_type = 29 AND approve_status = 1 AND status = 0 ORDER BY priority");
				while($res = mysqli_fetch_assoc($sql)){
					if(in_array($res['id'],$menu_show_permissions)) {
						$print.='<a class="btn btn-xs btn-primary" data-original-title="'.$res['print_name'].'" data-toggle="tooltip" data-placement="top" target="_blank"  href="'.ROOT.PRINT_ROOT.$res['page_path'].'/'.$row['debitnote_id'].'?'.time().'" style="background: '.$res['icon_color'].'; border-color: '.$res['icon_color'].';"><i class="'.$res['fa_icon'].'"></i></a>';
				    }
				}
	        ...
	        $info['vender_id']			= $POST['cust_id'];
	        $info['debitnote_no'] 		= $POST['voucher_no']; //Added new by dhaval
	        $info['material_center']	= $POST['mat_center'];
	        ...
	        $info['cost_center_enable']	= (isset($POST['enable_cost_center']) && ($POST['enable_cost_center']=='yes')) ? 1 : 0;
	        $info['eway_enable']		= (isset($POST['enable_ewaybill']) && ($POST['enable_ewaybill']=='yes')) ? 1 : 0;
	        $info['salesman_enable']	= (isset($POST['enable_salesman']) && ($POST['enable_salesman']=='yes')) ? 1 : 0;
	        
			$info['eway_bill_no']		= $POST['eway_bill_no'];
	        $info['eway_bill_date']		= $POST['eway_bill_date'];
	        $info['narration']			= $POST['remark'];
			...
			$insertid=add_record('tbl_debitnote', array_merge($info,$curncy_trn), $dbcon);
			
			/*Update sale return Trn Table Start by Dhaval */
	        if($insertid){
				$inv_trn['debitnote_id']	= $insertid;
				$inv_trn['debitnote_trn_status'] = 0;
				$updatetrnid=update_record('tbl_debitnote_trn', array_merge($inv_trn,$curncy_trn)," debitnote_id=0 and debitnote_trn_status='1' and user_id=".$_SESSION['user_id'] , $dbcon,'');
			}
	        ...
	        if($insertid && $POST['enable_cost_center']=='yes'){
				...
				$updatecosttrnid=update_record('tbl_cost_center_transaction', array_merge($cost_trn,$curncy_trn),"isdelete=0 and cost_center_table='tbl_debitnote' and  user_id=".$_SESSION['user_id']  , $dbcon);
			}
			
			/** Insert in general book table By Dhaval **/
			if($insertid)
			{
            	add_general_book_entry($dbcon,"tbl_debitnote",$insertid,1,$POST['sales_ledger_id'],$POST['total'],'',$POST['debitnote_date'],'',$curncy_trn);
				
            	add_general_book_entry($dbcon,"tbl_debitnote",$insertid,2,$POST['cust_id'],$POST['g_total'],'',$POST['debitnote_date'],'',$curncy_trn);
            	...
            	foreach ($POST['bill_sundry_tax'] as $bill_sundry_tax_id => $bill_sundry_tax_amount) {
					...
					$info_sundry['sundry_voucher_type']=PURCHASE_RETURN_VOUCHER;
					$info_sundry['sundry_voucher_table']='tbl_debitnote';
					$info_sundry['cdate']	= date("Y-m-d H:i:s");
			        $info_sundry['user_id']	= $_SESSION['user_id'];
			        $info_sundry['company_id']	= $_SESSION['company_id'];
					
					$sundry_insert=add_record('tbl_bill_sundry_transaction', array_merge($info_sundry,$curncy_trn), $dbcon,'');
					
            		add_general_book_entry($dbcon,"tbl_bill_sundry_transaction",$sundry_insert,1,$bill_sundry_tax_id,$bill_sundry_tax_amount,'',$POST['debitnote_date'],$POST['branch_id'],$curncy_trn);
            	}
            	...
            	foreach ($POST['bill_sundry_addon'] as $bill_sundry_addon_id => $bill_sundry_addon_amount) {
					...
					$info_sundry_addon['sundry_voucher_type']=PURCHASE_RETURN_VOUCHER;
					$info_sundry_addon['sundry_voucher_table']='tbl_debitnote';
					...
					if($bill_sundry_addon_amount < 0){
						add_general_book_entry($dbcon,"tbl_bill_sundry_transaction",$sundry_insert_addon,2,$bill_sundry_addon_id,abs($bill_sundry_addon_amount),'',$POST['debitnote_date'],'',$curncy_trn);

						$info_gen1['table_name']	= 'tbl_debitnote';
						$info_gen1['table_id']		= $insertid;
						$info_gen1['entry_type']	= 1;
						$info_gen1['ref_date']		= date('Y-m-d',strtotime($POST['debitnote_date']));
						...
						add_general_book_entry($dbcon,"tbl_bill_sundry_transaction",$sundry_insert_addon,1,$bill_sundry_addon_id,$bill_sundry_addon_amount,'',$POST['debitnote_date'],'',$curncy_trn);

						$info_gen2['table_name']	= 'tbl_debitnote';
						$info_gen2['table_id']		= $insertid;
						$info_gen2['entry_type']	= 2;
						$info_gen2['ref_date']		= date('Y-m-d',strtotime($POST['debitnote_date']));
            		...
            		//add_general_book_entry($dbcon,"tbl_bill_sundry_transaction",$sundry_insert_addon,1,$bill_sundry_addon_id,$bill_sundry_addon_amount,'',$POST['sale_return_date'],$POST['branch_id'],$curncy_trn);
            	}
            	...
            	foreach($POST['bill_sundry_addon_tax'] as $addon_id=>$addon_value){
            		...
            		$updateaddontaxid=update_record('tbl_bill_sundry_transaction', $info_addon,"sundry_voucher_table='tbl_debitnote' and isdelete=0 and sundry_voucher_id=".$insertid." and sundry_ledger_id=".$addon_id." " , $dbcon);
            	}
			...
			if($insertid){
				...
				$updatetcstrnid=update_record('tbl_tax_trn', array_merge($tax_trn,$curncy_trn),"tx_transaction_type='tbl_debitnote_trn' and tx_status = 3" , $dbcon);
			}
        	...
        	if($insertid){
        		...
        		$sel_gen = $dbcon->query("select trn.*,p.product_type,p.ledger_id 
        			from tbl_debitnote_trn as trn 
        			left join product_mst as p on trn.product_id=p.product_id
        			where trn.debitnote_id='$insertid' and trn.debitnote_trn_status!='2'");

        		while($r_gen = brp_mysqli_fetch_assoc($sel_gen))
        		{
        			add_general_book_entry($dbcon,"tbl_debitnote_trn",$r_gen['debitnote_trn_id'],1,$r_gen['ledger_id'],$r_gen['product_amount'],'',$POST['debitnote_date'],'',$curncy_trn); 
        		}
        	}
			...
			if($insertid){
				...
				$updatetcstrnid=update_record('tbl_salesman_transaction',$sales_trn,"transaction_voucher_type='3' and transaction_table_id = 0" , $dbcon);
			}
			...
			//echo '<pre>'; print_r($eway_row);exit;
			$inserinvoiceid=update_record('tbl_debitnote', array_merge($info,$curncy_trn),"debitnote_id=".$POST['eid'] , $dbcon);

			$query1="select * from tbl_bill_sundry_transaction where sundry_voucher_id='$POST[eid]'  and isdelete=0 and sundry_voucher_table='tbl_debitnote'  ";
			$rel1=brp_mysqli_fetch_all($dbcon->query($query1));

			foreach ($rel1 as $bill_sundry_addon){
				$info_general_sundry['ref_date'] = date('Y-m-d',strtotime($POST['debitnote_date']));
			...
			$info_invoice_sundry['ref_date'] = date('Y-m-d',strtotime($POST['debitnote_date']));
			update_record("tbl_general_book",$info_invoice_sundry,"table_name='tbl_debitnote' 
			...
			$query="select * from tbl_debitnote_trn where debitnote_id=".$POST['eid'];
			$result=$dbcon->query($query);
			while($row=mysqli_fetch_assoc($result)){		 
				$info_de['stock_status']=2;
				$updateid1=update_record("tbl_stock_trn", $info_de,"ref_name='purchase_return_trn' and ref_id=".$row['trancation_id'] ,$dbcon);
			}		 
			...
			$updateinvoiceid=update_record('tbl_general_book', $info_gen,"table_name='tbl_debitnote' and table_id=".$POST['eid'] , $dbcon);	

			//Bill Sundry Transaction			
			$info_bsun['isdelete'] = 1;
			$update_id1=update_record("tbl_bill_sundry_transaction", $info_bsun, "sundry_voucher_table='tbl_debitnote' and sundry_voucher_id='$POST[eid]'", $dbcon);
			
			$sel_bsun = $dbcon->query("select * from tbl_bill_sundry_transaction where sundry_voucher_table='tbl_debitnote' and sundry_voucher_id='$POST[eid]' and isdelete='1'");
			while($r_bsun=brp_mysqli_fetch_array($sel_bsun))
			{
				$info_bsun_general['genral_book_status'] = 2;
				$update_id1=update_record("tbl_general_book", $info_bsun_general, "table_name='tbl_bill_sundry_transaction' and table_id='$r_bsun[sundry_id]'", $dbcon);
			}
			
			
			$sel_itrn = $dbcon->query("select * from  tbl_debitnote_trn where debitnote_id='$POST[eid]' and debitnote_trn_status='2'");
			while($r_itrn=brp_mysqli_fetch_array($sel_itrn))
			{
				$info_tax_trn['tx_status']=2;
				update_record("tbl_tax_trn", $info_tax_trn,"tx_transaction_id='$r_itrn[debitnote_trn_id]' and tx_transaction_type='tbl_debitnote_trn'" ,$dbcon);

				$info_general['genral_book_status'] = 2;
				update_record('tbl_general_book', $info_general,"table_name='tbl_debitnote_trn' and table_id=".$r_itrn['debitnote_trn_id'] , $dbcon);
			}
		...
		$log_entry=common_log_entry($dbcon,"invoice_add",3,"tbl_invoice",$POST['eid']);
			...
			{
				$query="SELECT sales_ledger_id,vender_id FROM tbl_debitnote where debitnote_id=".$POST['invoice_id']." ";
				...
				update_record("tbl_debitnote",$update_invoice," debitnote_id=".$POST['invoice_id'] ,$dbcon);

				//Update Basic total in General book for debit table - purchase ledger entry
				$info_gen['amount'] = $POST['basic_total'];
				$info_gen['ref_date'] = date('Y-m-d',strtotime($POST['invoice_date']));
				update_record("tbl_general_book",$info_gen," table_id=".$POST['invoice_id']." and ledger_id=".$rel['sales_ledger_id']." and table_name='tbl_debitnote'" ,$dbcon);

				//Update Basic total in General book for debit table - customer ledger entry
				$info_gen1['amount'] = $POST['g_total'];
				$info_gen1['ref_date'] = date('Y-m-d',strtotime($POST['invoice_date']));
				update_record("tbl_general_book",$info_gen1," table_id=".$POST['invoice_id']." and ledger_id=".$rel['vender_id']." and ref_by='' and genral_book_status=0  and table_name='tbl_debitnote'" ,$dbcon);
				...
				foreach ($bill_sundry_tax as $bill_sundry_tax_id => $bill_sundry_tax_amount) {
					...
					update_record("tbl_bill_sundry_transaction",$info_sundry_tax," sundry_ledger_id=".$bill_sundry_tax_id." and sundry_voucher_table='tbl_debitnote' and sundry_voucher_id='$POST[invoice_id]'" ,$dbcon);

					$query1="select * from tbl_bill_sundry_transaction where sundry_voucher_id='$POST[invoice_id]' and sundry_voucher_table='tbl_debitnote' and sundry_ledger_id=".$bill_sundry_tax_id." and isdelete=0  ";
			...
			if($company_state['stateid'] == $POST['cust_stateid']){
				...
				$info1['debitnote_invoice_no']		= $POST['invoice_number'];
				$info1['grn_id']			= $POST['grn_id'];
				$info1['product_id']		= $POST['product_id'];
				$info1['description']		= $POST['description'];
				...
				$info1['purchase_return_cgst_tax_per']		= isset($cgst_tax_per) ? $cgst_tax_per : 0 ;
				$info1['purchase_return_cgst_tax_amt']		= isset($cgst_tax_rate) ? $cgst_tax_rate : 0 ;
				$info1['purchase_return_sgst_tax_per']		= isset($sgst_tax_per) ? $sgst_tax_per : 0 ;
				$info1['purchase_return_sgst_tax_amt']		= isset($sgst_tax_rate) ? $sgst_tax_rate : 0 ;
				$info1['purchase_return_igst_tax_per']		= isset($igst_tax_per) ? $igst_tax_per : 0 ;
				$info1['purchase_return_igst_tax_amt']		= isset($igst_tax_rate) ? $igst_tax_rate : 0 ;
				
				$info1['product_tax_cat']	= $sale_gst['tax_cat_id'];
				$info1['company_id']		= $_SESSION['company_id'];
				$info1['purchase_return_total_amount']	= $total_amount;
				...
				if(!empty($POST['eid'])){
					$info1['debitnote_id']= $POST['eid'];
					$info1['debitnote_trn_status']	= 0;
				}
				else{
					$info1['debitnote_trn_status']	= 1;
				}
			//	print_r($POST);
				if(empty($POST['edit_id'])){

				$inserid=add_record($table, $info1, $dbcon, $POST['branch_id']);

					if(!empty($POST['eid'])) {

					$sel_gen = $dbcon->query("select trn.*,p.product_type,p.ledger_id,s.debitnote_date from tbl_debitnote_trn as trn 
        			left join product_mst as p on trn.product_id=p.product_id
        			left join tbl_debitnote as s on s.debitnote_id = trn.debitnote_id
        			where trn.debitnote_trn_id='$inserid'");

        			$r_gen=brp_mysqli_fetch_assoc($sel_gen);


        			add_general_book_entry($dbcon,"tbl_debitnote_trn",$r_gen['debitnote_trn_id'],1,$r_gen['ledger_id'],$POST['product_amount'],'',$r_gen['debitnote_date'],'',$curncy_trn); 
					...
					$updateid=update_record($table, $info1,$tableid."=".$POST['edit_id'] , $dbcon, $POST['branch_id']);	
					
					$inserid=$POST['edit_id'];

					$sel_edit = $dbcon->query("select general_book_id from tbl_general_book where table_name='tbl_debitnote_trn' and table_id='$POST[edit_id]'");
					$r_edit = brp_mysqli_fetch_assoc($sel_edit);

					$info_gen1['amount'] = $POST['product_amount'];

					update_record("tbl_general_book",$info_gen1," table_id=".$POST['edit_id']."  and table_name='tbl_debitnote_trn'" ,$dbcon);
				...
				if( ($cgst_tax_per != 0) && ($cgst_tax_rate != 0) ){
					...
					$insert_tax = add_tax_transaction_record($dbcon,$cl_id['l_id'],$cgst_tax_per,$cgst_tax_rate,$inserid,"tbl_debitnote_trn",$POST['product_id'],3,$POST['edit_id']);
				}
				if( ($sgst_tax_per != 0) && ($sgst_tax_rate != 0) ){
					$cl_id = get_ledger_by_name($dbcon,'SGST');
					$insert_tax = add_tax_transaction_record($dbcon,$cl_id['l_id'],$sgst_tax_per,$sgst_tax_rate,$inserid,"tbl_debitnote_trn",$POST['product_id'],3,$POST['edit_id']);
				}
				if( ($igst_tax_per != 0) && ($igst_tax_rate != 0) ){
					$cl_id = get_ledger_by_name($dbcon,'IGST');
					$insert_tax = add_tax_transaction_record($dbcon,$cl_id['l_id'],$igst_tax_per,$igst_tax_rate,$inserid,"tbl_debitnote_trn",$POST['product_id'],3,$POST['edit_id']);
				}
				
				$count_add_tax=get_check_addition_tax($dbcon,$sale_gst['tax_cat_id'],$POST['product_amount'],$inserid,$POST['product_id'],$POST['edit_id'],$POST['branch_id'],'tbl_debitnote_trn');
		...
		else if(strtolower($POST['mode'])== "load_creditnoteno")
		{
			$row=array();
			...
			if($POST['eid']){
				$query="select mst.*,grn.grn_no,product.product_name,product.product_type,cat.unit_name as base_unit,ccat.unit_name as conv_unit,rcat.unit_name as rat_unit,po.po_no from  tbl_debitnote_trn as mst
					...
					left join tbl_pono as po on po.po_id=mst.debitnote_invoice_no	
					left join tbl_grn as grn on grn.grn_id = mst.grn_id			
					where debitnote_id='$POST[eid]' and debitnote_trn_status=0";
			}
			else{
				$query="select mst.*,grn.grn_no,product.product_name,product.product_type,cat.unit_name as base_unit,ccat.unit_name as conv_unit,rcat.unit_name as rat_unit,po.po_no from  tbl_debitnote_trn as mst
					left join unit_mst as cat on cat.unitid=mst.unit_id 
					...
					left join tbl_pono as po on po.po_id=mst.debitnote_invoice_no	
					left join tbl_grn as grn on grn.grn_id = mst.grn_id				
					where debitnote_id=0 and debitnote_trn_status=1 and  mst.user_id=".$_SESSION['user_id'];
			}
			//echo $query;
				...
				if($rel['unit_id'] != $rel['conv_unit_id']){
			 	...
			 	echo '<tr id="fieldtr'.$id.'" >
					...
					<td style="vertical-align:top">
						<button type="button" class="btn btn-round btn-warning btn-xs" onclick="edit_data('.$rel['debitnote_trn_id'].',\' tbl_debitnote_trn\',\'debitnote_trn_id\');" id="fieldedit'.$i.'"><i class="fa fa-pencil"></i></button>
						<button type="button" class="btn btn-round btn-danger btn-xs" onclick="delete_data('.$rel['debitnote_trn_id'].',\' tbl_debitnote_trn\',\'debitnote_trn_id\');" id="fieldremove'.$i.'"><i class="fa fa-times"></i></button>
					</td>	
				</tr>';
				$i++;
			}
		...
		{
			$q = $dbcon -> query("SELECT mst.*,(select sum(product_qty) from tbl_potrancation where po_id=mst.debitnote_invoice_no and product_id=mst.product_id and potrancation_status=0) as purchase_qty,(select sum(product_qty) from tbl_debitnote_trn where debitnote_trn_id !=mst.debitnote_trn_id and debitnote_invoice_no=mst.debitnote_invoice_no and debitnote_trn_status!=2) as used_qty FROM tbl_debitnote_trn as mst 
			WHERE mst.debitnote_trn_id = '$POST[id]'");
			
			$r = $q->fetch_assoc();
			$r['remained_qty'] = $r['purchase_qty']-$r['used_qty'];
		...
		else if(strtolower($POST['mode'])== "delete_data")
		{
			...
			$updatetax=update_record("tbl_tax_trn", $info_tax, "tx_transaction_id=".$POST['eid']." and tx_transaction_type='tbl_debitnote_trn'" , $dbcon);
			
			$info_general_book['genral_book_status'] = 2;

			update_record('tbl_general_book', $info_general_book,"table_name='tbl_debitnote_trn' and table_id=".$POST['eid'] , $dbcon);
		...
		else if(strtolower($POST['mode'])== "load_sale_productdata")
		{
			...
			$q = $dbcon -> query("SELECT trn.product_id,trn.potrancation_status,trn.po_id,trn.product_rate,unit.unit_name,(select sum(product_qty) from tbl_debitnote_trn where debitnote_trn_status != 2 and product_id ='$product_id' and debitnote_invoice_no = '$invoice_number')as used_qty, trn.product_qty, p.product_desc,p.product_hsn,p.product_base_unit,h.hsn_id,h.sale_gst,h.hsn_code from tbl_potrancation as trn 
				...
				where trn.product_id='$product_id' and trn.po_id='$invoice_number' and trn.potrancation_status='0' ");
			...
			$row['sale_qty'] = $product_qty.' '.$resp['unit_name'];
	...
	//dhaval upadhyay : start 
			...
			$where="";
			if($grn_id){
				$where=" and grn_id=".$grn_id;
			}
			
			echo get_purchase_invoice_by_cust($dbcon,$cust,$eid,$where);
			...
			echo get_product_from_purchase_invoice($dbcon,$invoice_no,$product_id);
	...
	{
		$debitnote_id=$POST['eid'];
		$resp='';
		
		$query="SELECT purchase_return_cgst_tax_per,sum(purchase_return_cgst_tax_amt) as cgst_rate,purchase_return_sgst_tax_per,sum(purchase_return_sgst_tax_amt) as sgst_rate,purchase_return_igst_tax_per,sum(purchase_return_igst_tax_amt) as igst_rate,product_amount FROM `tbl_debitnote_trn` where debitnote_id='$debitnote_id' and debitnote_trn_status!=2 group by purchase_return_cgst_tax_per,purchase_return_sgst_tax_per,purchase_return_igst_tax_per";

		//echo $query;
		$rs_prel=$dbcon->query($query);
		
		$rs_prel_fetch=brp_mysqli_fetch_assoc($dbcon->query("SELECT purchase_return_cgst_tax_per,sum(purchase_return_cgst_tax_amt) as cgst_rate,purchase_return_sgst_tax_per,sum(purchase_return_sgst_tax_amt) as sgst_rate,purchase_return_igst_tax_per,sum(purchase_return_igst_tax_amt) as igst_rate,product_amount FROM `tbl_debitnote_trn` where debitnote_id='$debitnote_id' and debitnote_trn_status!=2"));
		...
		foreach($taxRate as $taxdetail) {
			$gst_tax_per = ($taxdetail['purchase_return_cgst_tax_per'] != 0 || $taxdetail['purchase_return_sgst_tax_per'] != 0) ? ($taxdetail['purchase_return_cgst_tax_per']+$taxdetail['purchase_return_sgst_tax_per']) : $taxdetail['purchase_return_igst_tax_per'];
			$gst_tax_rate = ($taxdetail['cgst_rate'] != 0 || $taxdetail['sgst_rate'] != 0) ? ($taxdetail['cgst_rate']+$taxdetail['sgst_rate']) : $taxdetail['igst_rate'];

			if($taxdetail['purchase_return_cgst_tax_per'] != 0 || $taxdetail['purchase_return_sgst_tax_per'] != 0){
				$resp.='<tr>
				<th class="text-center">'.$cnt.'</th>
				<th class="text-center">'.$gst_tax_per.'%'.'</th>
				<th class="text-center">'.($taxdetail['product_amount']).'</th>
				<th class="text-center">'.$gst_tax_rate.'</th>
				<th class="text-center">'.($taxdetail['purchase_return_cgst_tax_per']).'%'.'</th>
				<th class="text-center">'.($taxdetail['purchase_return_sgst_tax_per']).'%'.'</th>
				</tr>';
			...
			if($taxdetail['purchase_return_igst_tax_per'] != 0){
				$resp.='<tr>
				<th class="text-center">'.$cnt.'</th>
				<th class="text-center">'.$gst_tax_per.'%'.'</th>
				<th class="text-center">'.($taxdetail['product_amount']).'</th>
				<th class="text-center">'.$gst_tax_rate.'</th>
				<th class="text-center">'.($taxdetail['purchase_return_igst_tax_per']).'%'.'</th>
				</tr>';
	...
	else if(strtolower($POST['mode'])== "get_invoice_total_tax")
	{
		...
		$resp='';
		$query="SELECT sum(purchase_return_cgst_tax_amt) as cgst_rate,sum(purchase_return_sgst_tax_amt) as sgst_rate,sum(purchase_return_igst_tax_amt) as igst_rate,sum(product_amount) as product_amount FROM `tbl_debitnote_trn` where debitnote_id='$debitnote_id' and debitnote_trn_status!=2";
		...
		foreach ($get_bill_sundry as $billsundry) {
			...
			if(($billsundry['ledger_alias'] == 'TDS_PURCHASE') && ($getCompanyConfig['enable_tds_reporting'] == 1) && ($custLedgerDetails['enable_tds']==1) && ($POST['gross'] > $getCompanyConfig['gross_balance_tds_limit'])){
				$row['isTds']="1";
				$total_tds_calculate = $rs_prel['product_amount']+$rs_prel['igst_rate'];
				$resp.='<div class="form-group">
					<label class="col-md-5 control-label">'.$billsundry['l_name'].'</label>
					<div class="col-md-5 col-xs-11">
						<input id="'.$billsundry['l_name'].'" name="bill_sundry_tax['.$billsundry['l_id'].']" type="number" class="form-control gst" title="'.$billsundry['l_name'].'"  value="'.round((($total_tds_calculate*$billsundry['tax_value'])/100),2).'" placeholder="'.$billsundry['l_name'].'" readonly >
						<input type="hidden" name="tds_amount" id="tds_amount" value="'.round((($total_tds_calculate*$billsundry['tax_value'])/100),2).'" >
						<input type="hidden" name="tds_per" id="tds_per" value="'.$billsundry['tax_value'].'" >
					</div>
				</div>';
			}
		...
		$qry_add = $dbcon->query("select sum((tc.tax_per*trn.product_amount)/100) as add_sum , trn.*,l.l_name,l.l_id,t.tax_cat_id from tbl_debitnote_trn as trn left join tbl_tax_category as t on t.tax_cat_id=trn.product_tax_cat 
		left join tbl_tax_category_details as tc on tc.tax_cat=t.tax_cat_id 
		left join tbl_ledger as l on l.l_id=tc.tax_id
		where tc.tax_additional='1' and trn.debitnote_id='$debitnote_id' and trn.debitnote_trn_status!=2 and tc.isdelete='0' group by tc.tax_id 
		");
	...
	else if(strtolower($POST['mode'])== "get_bill_sundry_details")
	{
		...
		if($invoice_id>0)
		{
			...
			$info_gen2['ref_date']		= $invoice_date;
			$info_gen2['ledger_id']		= $POST['cust_id'];
	...
	else if(strtolower($POST['mode'])== "tcs_reversal_trn")
	{
		...
		else
		{
			...
			$where.="  and s.sale_ledger_id='$POST[sale_ledger_id]' and s.sale_invoice_id='$POST[debitnote_id]'";
			$appData = array();
			$i=1;
	...
	else if(strtolower($POST['mode'])== "tcs_reversal_add")
	{
		...
		$query = $dbcon->query("select debitnote_id from tbl_tcs_reverse where sale_return_ledger_id='$sale_ledger_id' and sale_return_status!='2'");
	...
	else if(strtolower($POST['mode'])== "get_all_bill_sundry")
	{
		...
		$update_gen_cusid=update_record('tbl_general_book', $info_general,"table_id=".$invoice_id." and ledger_id=".$cust_ledger_id." and amount=".$resp['amount']." and ref_by='tbl_addon_bill_sundry' and  table_name='tbl_debitnote'" , $dbcon);
