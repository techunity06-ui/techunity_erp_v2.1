...
<html lang="en">
        ...
        .link_dash
	{
        ...
        <?php include_once($include.'left_menu.php');?>
                            ...
                            <section class="panel">
                                ...
                                <div class="">
                                    <ul class="breadcrumb">
										...
										<li><a href="<?=ROOT.FINANCE_ROOT.'sales_monthly_view'?>"> Monthly Sales Report</a>  </li>
										<li><?=$form?> Report</li>
                                    </ul>
                                </div>
                            </section>
            ...
            <?php
				   $ca_qry = "SELECT pbill.invoice_id,pbill.invoice_date,pbill.invoice_no,pbill.g_total,ledger.l_name FROM tbl_invoice as pbill left join tbl_ledger as ledger on ledger.l_id=pbill.cust_id where month(pbill.invoice_date)=".$month_m." and year(pbill.invoice_date)= ".$year_y." and pbill.invoice_status=0 and pbill.company_id = ".$_SESSION[company_id]." Order by pbill.invoice_date ASC";
                ...
                if($ca_result){
                    foreach ($ca_result as $value)
					{
                        //echo '<pre>';        print_r($ca_result); //exit;
                        $str.='<tr>
                                    <td data-label="DATE" style="text-align:left">'.date('d/m/Y',strtotime($value["invoice_date"])).'</td>
									<td  style="text-align:left"><a style="color: inherit;" href="'.ROOT.FINANCE_ROOT.'invoiceedit/'.$value['invoice_id'].'">'.$value["l_name"].'</a></td>
									<td  style="text-align:left">'.$value["l_name"].'</td>
