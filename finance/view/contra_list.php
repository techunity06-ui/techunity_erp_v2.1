<?php 
	...
	include_once(COMMON_FUNCTION_PATH."finance_common_functions.php");
        ...
        if(!in_array(FINANCE_CONTRA_LIST,$bulkAccessArray)){
...
<html lang="en">
	...
	<body>
												...
												<div class="input-group date form_datetime-component">
													...
													<span class="input-group-btn">
														<button type="button" class="btn btn-danger date-set"><i class="fa fa-calendar"></i></button>
                                                                        ...
                                                                        <?php if(in_array(FINANCE_CONTRA_CREATE, $bulkAccessArray)){ ?>
									<span class="tools pull-right respadr_15">
										<a href="<?=ROOT.FINANCE_ROOT.'contra_entry'?>" ><button class="btn btn-success btn-flat" >Add <?=$form?></button></a>
									</span>
                                                                        <?php } ?>
								</header>	
		...
		<script src="<?=ROOT.FINANCE_ROOT?>js/app/contra_entry.js?<?=time()?>"></script>
		<script>
                        $(".select2").select2({
                            width: '100%'
                        });
