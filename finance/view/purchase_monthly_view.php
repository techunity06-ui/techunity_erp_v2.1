...
$form="Purchase Details By Month ";

$query = "SELECT sum(g_total) as g_totall,MONTHNAME(order_date) month_name, MONTH(order_date) as month,YEAR(order_date) as year FROM tbl_pono where status=0 and company_id = ".$_SESSION[company_id]." GROUP BY month ORDER BY order_date";
							...
							<th width="15%" style="text-align:center">Credit</th>
                                            ...
                                            //if($ca_entries && !empty($ca_entries)){ 
                                                ...
                                                foreach ($report as $month => $data) {
                                                    ...
                                                    ?>
                                                    <tr>
                                                        <td tabindex="<?= $i ?>"><a style="color: inherit;" href="purchase_monthly_detail_view.php?month=<?=$data['month']?>-<?=$data['year'] ?>"><?= $month ."-". $data['year']?></a></td>
