...
if (!in_array(FINANCE_INVOICE_LIST, $bulkAccessArray)) {
	header("Location: " . DOMAIN . "permission_access");
}
$financial_year = get_financial_year_new($dbcon);
$eway_bill_start_date = date("Y-m-d");
$companyConfiguration = getCompanyConfiguration($dbcon);
$crm_user_type = $companyConfiguration['crm_user_type'];
$amnts = get_invoice_taxable_total($dbcon);
$cnyts = explode(",", $amnts);
...
<body>
	<section id="container">
								...
								<div class="col-lg-12 centeral-align">
									<div class="icons">
										<div class="icon1 success">
											...
											<h3 style="font-size:20px;color:white;padding-top:5px;" id="invoiceamount">
											</h3>
										</div>
									</div>
									<div class="icons">
										<div class="icon1 info">
											...
											<h3 style="font-size:20px;color:white;padding-top:5px;"
												id="invoicetaxamount"></h3>
										</div>
									</div>
								</div>
							</div>
						...
						<section class="panel">
							<header class="panel-heading">
									...
									<div class="form-group">
										...
										<div class="col-md-7">
											<div class="input-group date form_datetime-component">
												...
												<input type="text" id="rep_date" onChange="reload_data();;"
													class="form-control datepikerdemo" value="">
								...
								<?php if (in_array(FINANCE_INVOICE_CREATE, $bulkAccessArray)) { ?>
									<span class="tools pull-right">
										<a href="<?= ROOT . FINANCE_ROOT . 'invoice' ?>"><button
												class="btn btn-success btn-flat">Create Invoice</button></a>
									</span>
								<?php } ?>
		...
		<?php include_once($include1 . 'add_eway_bill_new.php'); ?>
		<?php include_once($include1 . 'add_einvoice.php'); ?>
		<?php include_once($include1 . 'preview_update_user.php'); ?>
		<!--footer end-->
	</section>
	...
	<script>
		...
		$('.default-eway-date').datepicker({
			format: 'dd-mm-yyyy',
			autoclose: true,
			startDate: '<?php echo date("d-m-Y", strtotime($eway_bill_start_date)) ?>',
