<?php 
	...
	$form="Bank Reconsilation All Entries";
	...
	//echo $usertype;
	$sel1=$dbcon->query("select l.l_name,l.l_group,g.g_name from tbl_ledger as l left join tbl_group as g on g.g_id=l.l_group where l.l_id='$id'");
	$row1=mysqli_fetch_array($sel1);
	
	$financial_year=get_financial_year_new($dbcon);
	
	$ledgerbank1=$dbcon->query("select l_id,l_name from tbl_ledger where l_form='bank_form' and l_status=0 ");
	$ledgerbank=brp_mysqli_fetch_all($ledgerbank1);
	//echo '<pre>';print_r($ledgerbank);exit;
?>

<!DOCTYPE html>
<html lang="en">
<head>
	<title>BANK RECONSILATION ALL ENTRIES</title>
<?php include_once($include.'include_css_file.php');?>
</head>
<body onload="bank_reco_all_entries_report()">
  <section id="container" >
					  ...
					  <header class="panel-heading">
						 ...
						 <?php include_once($include.'reporthead_menu.php');?>
						  ...
						  <ul class="breadcrumb">
							  ...
							  <li><a href="<?=ROOT.FINANCE_ROOT.'finance_report_list'?>"> Bank Reconsilation Reports</a></li>
							  <li ><?=$form." <strong style='color:red'></strong>"; ?></li>
					...
					<div class="row">
									 ...
									 <?
									  ...
									  ?>
										 ...
										 <input type="hidden" id="to_date"  value="<?=date('t-m-Y')?>">
         					 		        <input type="text" id="rep_date"  onChange="bank_reco_all_entries_report();" class="form-control datepikerdemo" value="">
						...
						<div class="col-md-6">
							<div class="form-group">
								<label class="col-md-1 control-label">Select Bank</label>
								<div class="col-md-8">
									<select class="select2" name="b_l_id" id="b_l_id" onChange="bank_reco_all_entries_report();" >
											<option value="">Select Bank</option>
											<?php foreach($ledgerbank as $bank){ ?>
												<option value="<?= $bank['l_id'] ?>"><?= $bank['l_name'] ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
	...
	include_once($include.'include_js_file.php');
 	include_once($include1.'clear_entry.php');
	?>   
   <script src="<?=ROOT.FINANCE_ROOT ?>js/app/bank_reconsilation.js?<?=time()?>"></script>
