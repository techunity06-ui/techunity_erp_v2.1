<?php 
	...
	include_once(COMMON_FUNCTION_PATH."finance_common_functions.php");
        ...
        $bulkAccessArray = canCheckPermissionAccess($dbcon, [
            ...
            FINANCE_RECEIPT_LIST,
            FINANCE_RECEIPT_DELETE,
            FINANCE_RECEIPT_PRINT
        ]);
        
        if(!in_array(FINANCE_RECEIPT_LIST,$bulkAccessArray)){
            header("Location: ".DOMAIN."permission_access");
        }
        
	$form="Received Payment";
...
<html lang="en">
<head>
	<title>RECEIVED PAYMENT LIST</title>
...
<section class="panel">
	<header class="panel-heading respadlr0">
		...
		<?php if(in_array(FINANCE_RECEIPT_CREATE,$bulkAccessArray)) { ?>
		<span class="tools pull-right respadr_15">
			<a href="<?=ROOT.FINANCE_ROOT.'recipt'?>" ><button class="btn btn-success btn-flat" >Add <?=$form?></button></a>					
		</span>
                <?php } ?>
		<br/>
	</header>	
				...
				<thead>
					<tr>
						...
						<th style="color:blue">Payment Date</th>
                                                <?php if(in_array(FINANCE_RECEIPT_PRINT,$bulkAccessArray) && in_array(FINANCE_RECEIPT_DELETE,$bulkAccessArray)) { ?>
