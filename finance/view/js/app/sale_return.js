	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(response.trim() == '1') {
				...
				if ($("#save_print").val() == '1'){
					window.location=root_domain+finance_root_domain+'invoicereceipt/'+arr.eid+'/'+arr.printstatus;
				}
				else{
					window.location=root_domain+finance_root_domain+'sale_return';
				}
			}
...
{	
		//alert('hiii');
		var product_qty_hid = Number($('#product_qty_hid').val());
		
		if($("#product_qty").val()>product_qty_hid)
		{
			toastr.warning("QTY MUST BE LESS THAN SALE QTY", "WARNING");
...
function update_total()
{
	...
	var sale_return_date = $('#sale_return_date').val();
	...
	$.ajax({
		...
		branch_id:branch_id , bill_sundry_tax:gst1,bill_sundry_tax1:gst2 ,invoice_date:sale_return_date},
		url:root_domain+finance_root_domain+'app/salereturn/',
...
function load_productdetail(val) 
{
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			$('.hsncode').show();
			$('#sale_remained').text(obj.sale_qty);
			$('.sale_remained').show();
...
function add_field()
{
	...
	$.ajax({
			type: "POST",
			url: root_domain+finance_root_domain+'app/salereturn/',
			data: { mode : "fieldadd",edit_id:$("#edit_id").val(),product_id:$("#product_id").val(),
			product_qty:$("#product_qty").val(),product_rate:$("#product_rate").val(),unit_id:$("#unit_id").val(),product_amount:$("#product_amount").val(),eid:$("#eid").val(),sale_gst:$('#product_tax').val(),invoice_number:$('#invoice_number').val(),cust_stateid:$('#cust_stateid').val(),product_hsn_code:$("#hsncode").text(),currency_enable:currency_enable,currency_rate:$('#currency_rate').val(),currency_id:$('#currency_id').val() },
			success: function(response)
			{
				console.log(response);
...
function load_datatable()
{
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+finance_root_domain+'app/salereturn/',
...
function get_invoice_total_tax(){
	...
	$.ajax({
        type: "POST",
        url: root_domain + finance_root_domain +'app/salereturn/',
        data: { mode : "get_invoice_total_tax",cust_id:$('#cust_id').val(),gross:0,inv_total:$('#total').val(),eid:eid,addontax1:addontax1 },
        success: function(response)
        {
	...
	Loading();
			$.ajax({
				...
				success: function(response)
				{
					...
					$("#invoice_number").select2("val",data.sale_return_invoice_no);
					get_product_from_invoice(data.sale_return_invoice_no,data.sale_return_product);
					//$("#product_id").select2("val",data.sale_return_product);
					$("#product_qty").val(data.sale_return_qty);
					$("#product_qty_hid").val(data.remained_qty);
					$("#product_tax").val(data.product_sale_gst);
					$("#product_rate").val(data.sale_return_rate);
					$("#unit_id").select2("val",data.sale_return_unit);
					$("#product_amount").val(data.sale_return_amount);
					$('#sale_remained').text(data.remained_qty);
					$('.sale_remained').show();
					$('#hsncode').text(data.sale_return_hsn);
					$('.hsncode').show();
					$("#edit_id").val(id);
					$('#addrow').val('Update');
...
function get_invoice_by_cust(cust,return_div,eid='')
{
	//alert(return_div);
	$.ajax({
		...
		type:'POST',
		url:root_domain+finance_root_domain+'app/salereturn/',
		data:{ mode:'get_invoice_by_cust', cust:cust ,eid:eid },
		success:function(response)
		{
			//alert(response);
			//console.log(response);
...
function addBillSundry(){
	...
	if(bill_sundry_value == 0)
	{
		...
		$.ajax({
	        ...
	        basic_amount:basic_amount,netamount:netamount,default_amount:bill_sundry_amount,invoice_id:eid,currency_enable:currency_enable,currency_id:currency_id,currency_rate:currency_rate,invoice_date:$('#sale_return_date').val(),cust_id:cust_id},
	        success: function(response)
	        {
...
function get_symbol(){
	...
	var symbl = $("#currency_conv").find(':selected').attr("data-currency-symbol");
...
function add_tcs_reversal_trn()
{
	...
	if($('#ref_id').val()=='')
	{
		toastr.warning("Please Select Ref No", "ERROR");
		$('#ref_id').focus();
	}
	else if($('#amt_reversed').val()=='')
	{
		toastr.warning("Select Amount Reversed", "ERROR");
		$('#amt_reversed').focus();
	}
	else
	{
