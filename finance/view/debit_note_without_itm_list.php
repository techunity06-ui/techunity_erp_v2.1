...
<html lang="en">
	<head>
		<title>DEBIT NOTE WOTHOUT ITEM LIST</title>
	...
	<body>
							...
							<section class="panel">
                                ...
                                <?php if(in_array(FINANCE_JOURNAL_CREATE,$bulkAccessArray)){ ?>
                                    <span class="tools pull-right respadr_15">
                                            <a href="<?=ROOT.FINANCE_ROOT.'debit_note_without_itm'?>" ><button class="btn btn-success btn-flat" >Add <?=$form?></button></a>
                                    </span>
                                <?php } ?>
								</header>	
								<div class="panel-body">
									<div class="adv-table">
										<table  class="display table table-bordered table-striped" id="debit_note-table">
											<thead>
												<tr>
													<th>Debit Note Entry No</th>
													<th>Debit Note Entry Date</th>                                                                         <?php if(in_array(FINANCE_JOURNAL_EDIT,$bulkAccessArray) && in_array(FINANCE_JOURNAL_DELETE,$bulkAccessArray)) { ?>
										...
										<style>
										  @media screen and (max-width:992px){
											#debit_note-table td:before{
													color:red
												}
											#debit_note-table td:nth-of-type(1):before { content: "Journal Entry No:"; }
											#debit_note-table td:nth-of-type(2):before { content: "Journal Entry Date:"; }
											#debit_note-table td:nth-of-type(3):before { content: "Action:"; }
		...
		<script src="<?=ROOT.FINANCE_ROOT?>js/app/debit_note_without_itm.js?<?=time()?>"></script>
		<script>
                        $(".select2").select2({
                            width: '100%'
                        });
