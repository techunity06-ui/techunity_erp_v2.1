			...
			<section class="wrapper">
				...
				<div class="row">			
					<div class="col-sm-12">
						...
						$querymenu1re1="SELECT menumaster.*, CASE WHEN parent_id = 0 THEN id ELSE parent_id END AS sort FROM menu_master_access as menumaster WHERE menumaster.status=0 and menu_path='#crm_report_list' ORDER BY sort, id";
											...
											<?php if(in_array($rel_menu1re['accessdata'], $user_access_permission)){ ?>
												<div class="col-sm-6 col-md-4" style="padding-top:10px;font-size:20px;">
													<li class="btn btn-shadow btn-info btn-lg btn-block btn-align">
														<a  class="two"  href="<?=ROOT.strtolower($rel_menu1re['route_path_name'])?>"><?=ucwords(strtolower($rel_menu1re['menu_name']))?></a></li>
													</div>
												<?php } ?>
											<?php } ?>
										</ul>
