...
function load_inquiry_reg_datatable(){
	...
	$("#inq-reg-datatable").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain+crm_domain+'app/pending_so_report/',
		...
		"fnFooterCallback": function ( nRow, aaData, iStart, iEnd, aiDisplay ) {
			...
			{
				iQty += aaData[i][8]*1;
				iPending += aaData[i][9]*1;
				iAllocate += aaData[i][10]*1;
				iRate += aaData[i][12]*1;
				iValue += aaData[i][13]*1;
			}

			var nCells = nRow.getElementsByTagName('th');
			nCells[1].innerHTML = parseFloat(iQty ).toFixed(4);
			nCells[2].innerHTML = parseFloat(iPending ).toFixed(2);
			nCells[3].innerHTML = parseFloat(iAllocate ).toFixed(2);
			nCells[5].innerHTML = parseFloat(iRate ).toFixed(2);
			nCells[6].innerHTML = parseFloat(iValue ).toFixed(2);
		}
	}).fnSetFilteringDelay();
