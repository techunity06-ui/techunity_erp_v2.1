...
$("#forecast_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain + crm_domain + 'app/forecast_byuser_pro/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
		success: function(response)
		{
			...
			if(msg.trim() == '1') {				
				...
				window.location=root_domain + crm_domain +'forecast_byuser_pro_list';
			}
			else if(msg.trim() == '2') {				
...
function load_forecast_datatable(){
	...
	$("#forecast-datatable").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain + crm_domain +'app/forecast_byuser_pro/',
