...
$(document).ready(function() {
	...
	$("#task_add").validate({
		...
		messages: {
			...
			"assign_user_ids[]": "Please Select User"
		}
	}); 
}); 
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				...
				window.location= root_domain + crm_domain+'general_task_list';
			}
			else if(arr.msg == '0') {
...
function load_task_datatable(){
	...
	var assign_user = $("#assign_user_id").val(); 
	var owner_user = $("#user_id").val(); 
	var is_general = true;
	...
	datatable = $("#task-table").dataTable({
		...
		"fnServerParams": function ( aoData ) {
			aoData.push( {"name": "mode", "value": "fetch"}, {"name": "date", "value": date}, {"name": "task_status", "value": task_status }, {"name": "is_general", "value": is_general }, {"name": "assign_user", "value": assign_user }, {"name": "owner_user", "value": owner_user } );
		},
...
function open_follow_up(task_id,inq_name, current_status = '') {
	...
	$('.addFollowupBlock').show();

	if(current_status == '1') {
		$('.addFollowupBlock').hide();
	}
        ...
        "fnStateLoad": function (oSettings) {
			...
			"fnServerParams": function ( aoData ) {
				aoData.push( { "name": "mode", "value": "load_pend_task" },
								...
								{"name": "is_general", "value": is_general });
			},
			"fnDrawCallback": function( oSettings ) {
				$('.ttip, [data-toggle="tooltip"]').tooltip();
			}
...
function add_task_attch_field() {
	...
	form_data.append('mode', "add_generaltask_attch_field");
	form_data.append('task_id', $("#eid").val());
	form_data.append('attachment_name', $("#attachment_name").val());
...
function show_task_attach_data() {
	...
	$.ajax({
		type: "POST",
		url: root_domain + crm_domain + 'app/task/',
		data: { mode : "show_generaltask_attach_data", task_id:eid,modee:chkmode },
...
function delete_task_attach_data(task_attach_id){
	...
	if(r) {
		...
		$.ajax({
			type: "POST",
			url: root_domain + crm_domain + 'app/task/',
			data: { mode:"delete_generaltask_attach_data", task_attach_id:task_attach_id },
			success: function(response)
			{
				//console.log(response);
...
function open_doc_modal(task_id,inq_name, current_status = '') {

	$('#preview_doc_details_modal').modal('show');
	$('#flp_task_id').val(task_id);
	//Amish Soni End 18-03-2021

	show_doc_hist(task_id);
}
function show_doc_hist(task_id) {
	//var eid = $('#eid').val();
	var chkmode = 'modalview';
	Loading();
	$.ajax({
		type: "POST",
		url: root_domain + crm_domain + 'app/task/',
		data: { mode : "show_generaltask_attach_data", task_id:task_id,modee:chkmode },
		success: function(resp){
			//console.log(resp);
			$('#preview_doc_details_div').html(resp);
			Unloading();
		}		 
	}); 
}
function followup_read(flp_id,status,user_type){
	Loading();
	$.ajax({
		type: "POST",
		url: root_domain + crm_domain + 'app/task/',
		data: { mode : "read_flp", flp_id:flp_id,status:status,user_type:user_type },
		success: function(resp){
			//console.log(resp);
			Unloading();
			show_flp_hist();
