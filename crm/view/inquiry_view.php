						...
						<div class="col-md-12" style="padding-top:10px;">
							...
							<div style="overflow:auto;">
								<table class="display table table-bordered table-striped">
									...
									where trn.quot_trn_status=0 and quo.quotation_status=0 and quo.inquiry_id=".$rel['inquiry_id']." 
									order by quo.quotation_id DESC LIMIT 0,1";
							...
							<table class="display table table-bordered table-striped">
								...
								if(mysqli_num_rows($get_quot_qry_rs)){
									while($get_quot_rel=mysqli_fetch_assoc($get_quot_qry_rs)){
										...
										<tr>
											...
											<td>
												<?php $menusql = $dbcon->query("SELECT * FROM `print_permission` WHERE `status` = '0' AND `company_id`='".$_SESSION['company_id']."'");
												...
												while($res = mysqli_fetch_assoc($sql)){
													if(in_array($res['id'],$menu_show_permissions)) { ?>
														<a href="<?=ROOT.PRINT_ROOT.$res['page_path'].'/'.$get_quot_rel['quotation_id']?>" type="button" class="btn btn-primary" target="_blank"> <i class="fa fa-eye"></i> View</a>
													<?php } } ?>
												</td>
											</tr>
										<?php	}
								...
								<div style="overflow:auto;">
									<table class="display table table-bordered table-striped">
										...
										<?
										$get_apt_qry="select tsk.*,sub.mcd_name as subject,usr.user_name,prior.task_priority_name,user.user_name as assign_name from tbl_task as tsk 
