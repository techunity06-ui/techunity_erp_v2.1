...
$quotaion_id=$dbcon->real_escape_string($_REQUEST['poid']);
$query="select po.*,state.state_name,l.l_name as vender_name,country.country_name,l.m_address as vender_address,l.gst_no as tin_no,l.cust_mobile as vender_mobile,l.stateid,state.gst_state_code,city.city_name from po_quotation as po 
...
where po.po_quotation_id=$quotaion_id";
...
if($rel['currency_name']!='0'){$currency_name = $rel['currency_name']; }else{$currency_name = $_SESSION['currency_name'];}
if($rel['currency_id']==$_SESSION['currency_id']){
  $total_amount = $rel['g_total'];
}else{
  $total_amount = $rel['currency_total'];
}
    ...
    <section class="wrapper">
     ...
     <div class="">
         <ul class="breadcrumb">
          ...
          <li><a href="<?=ROOT.'po_quotation_vendor_list/'?><?=$approve_indent_id?>"><?=$form?> List</a></li>
      </ul>
  </div>
</section>
<!--breadcrumbs end -->
...
<div class="col-md-4">
   ...
   <a href="<?=ROOT.'po_quotation_vendor_list/'?><?=$approve_indent_id?>" type="button" class="btn btn-danger"><i class="fa fa-ban"></i> Cancel</a>
...
<table width="100%" class="maintable" style="font-size: 12px;margin-top: 5px;" id="invoice_type" >
     ...
     <tr>
      <th colspan="11" style="padding:0px !important;">
       <table style="font-size:12px;border-collapse: collapse;border-top:none;" cellpadding="0" cellspacing="0" width="100%" >
        <tr>
         <td colspan="4" width="100%" style="text-align:center;width:100%;"> 
  ...
  <tr >
     <td width="50%"  rowspan="4" style="vertical-align:top;border:1px solid;">
      <b>To, </b><br/>
      ...
      <td style="vertical-align:top;border:1px solid;width: 25%; border-right: none">
       Quotation No
   </td>
...
<tbody style="border: 1px solid;">
 <?php
 $query="select pro.product_name,unit.unit_name,arpro.approve_qty from approve_indent as arpro
 ...
 $result=mysqli_fetch_assoc($dbcon->query($query));
 $i=1;
...
<?php
$totalqty=$result['approve_qty'];
$totalrate=$rel['product_rate'];
$total = $totalrate; 
...
for($j=0; $j<$pr; $j++)
{
  ...
  <tr style="height:20px">
      ...
      <td style="text-align:center;border-top:1px solid;border-right:1px solid;"><strong><?=number_format($totalrate,2,".","")?></strong></td>
  ...
  <tr>
      ...
      <td colspan="2" style="text-align:right;" width="10%"><?=number_format($total,2,".","")?></td>
  </tr>
  <tr height="20px">
  ...
  <td colspan="2" style="text-align:right; border-top:1px solid;border-right:1px solid; "><?=number_format('0.00',2,".","")?></td>
</tr>
<tr height="20px">
 <td style="border-right:1px solid;border-top:1px solid;" colspan="<?=$colspan?>">
...
<td colspan="2" style="text-align:right; border-top:1px solid;font-size:13px;border-right:1px solid; "><strong><?=number_format($total,0,".","").'.00'?></strong></td>
</tr>
<tr>
