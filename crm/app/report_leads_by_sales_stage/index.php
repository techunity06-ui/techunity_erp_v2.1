...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "generate_report_product_service1")
	{
        ...
        else if(strtolower($POST['mode']) == "generate_graph") {
                $source_id=$POST['source_id'];
                $s_date=explode(' - ',$POST['date']);
                if($source_id){
                    $where = "and e.sales_stage_id= ".$source_id;
                }

                $where .= " and DATE(e.cdate) between '".date("Y-m-d",strtotime($s_date[0]))."' and '".date("Y-m-d",strtotime($s_date[1]))."'";
                $query="select COUNT(*)as led,e.sales_stage_id,et.mcd_name as sales_stage
                        from tbl_inquiry as e 
                        left join tbl_master_category_detail as et on et.mcd_id = e.sales_stage_id 
                        where e.inquiry_status=0 AND e.company_id IN (0,$_SESSION[company_id])".$where."
                        group by e.sales_stage_id";
                $result=$dbcon->query($query);
                $i=0;
                while($row=mysqli_fetch_assoc($result))
                {	
                        $row1[$i]['y']= (int)$row['led'];
                        $row1[$i]['label']=$row['sales_stage'];
                        $row1[$i]['sales_stage_id']=$row['sales_stage_id'];	
                        $i++;
                }	
