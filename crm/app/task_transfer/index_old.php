...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$info['lost_reason']		= $_POST['lost_reason'];
...
WHERE task.task_status=0 and task.entry_type=1 and task.alert_date_time!="0000-00-00 00:00:00" and task.alert_date_time!="1970-01-01 05:30:00"  and alert_date_time<"'.date('Y-m-d',strtotime($POST['fil_due_date'])).'" '.$whr.' '.$ftp.' order by task.task_priority_id,task.alert_date_time';
		...
		if($POST['log_user_id']){
			...
			if(!empty($POST['c_user_id'])){
				...
				//$ftp=' and task.assign_user_ids in ('.$fis.')';
			}else{
				$user_id=$_SESSION['user_id'];
				//$ftp=check_crm_find_in_set($dbcon,$user_id,1);
				//$fis=check_crm_find_in_set($dbcon,$user_id,1);
    ...
    END
			...
			$isWhere = array('task.task_status=0 and task.entry_type=1 and task.alert_date_time!="0000-00-00 00:00:00" and task.alert_date_time!="1970-01-01 05:30:00" and DATE_FORMAT(alert_date_time,"%Y-%m-%d")<="'.date('Y-m-d',strtotime($POST['fil_due_date'])).'" '.$whr.$ftp);
			$sTable = "tbl_task as task";			
...
left join (SELECT inquiry_id,approve_status,quotation_id FROM `tbl_quotation` WHERE quotation_status=0 and revise_status=0) as qt_aprv on qt_aprv.inquiry_id=task.inquiry_id','left join tbl_customer as cust on cust.cust_id=inq.c_id','left join state_mst as state on state.stateid=(SELECT c_add_state FROM `tbl_cust_address` WHERE cust_id=inq.c_id and c_add_status=0 limit 1)','left join city_mst as city on city.cityid=(SELECT c_add_city FROM `tbl_cust_address` WHERE cust_id=inq.c_id and c_add_status=0 limit 1)','left join tbl_opportunity_mst as stage on stage.opp_id=(SELECT opp_id from tbl_inquiry where inquiry_id=task.inquiry_id)');
			$hOrder = "task.task_priority_id,task.alert_date_time";
			$hGroupby = array("task.inquiry_id");
			include('../../include/pagging.php');
			$appData = array();
			$id=1;
			foreach($sqlReturn as $row) {
				...
				if($row['task_rel_id']=='5'){//Fixed Type Inquiry
					...
					if($row['task_type_id']=='20'){
						$add_flp_btn='';$add_apt_btn='';
						$add_flp_btn='<a class="btn btn-xs btn-info" data-original-title="Revise Quotation" data-toggle="tooltip" data-placement="top" href="'.ROOT.'quotation_revise/'.$row['quotation_id'].'"><i class="fa fa-repeat"></i></a>';
					}
				}
				...
				$row_data[] = $row['task_rel_name'];
				$row_data[] = '<strong>'.$row['rel_name'].'</strong>';
				$row_data[] = ''.$row['opp_stage'].'('.$row['opp_probability'].'%).';
				$row_data[] = $row['state_name'];
				$row_data[] = '<strong>'.$row['city_name'].'</strong>';
