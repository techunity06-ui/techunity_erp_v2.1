...
if (strtolower($POST['mode']) == "fetch") {
    $appData = array();
    $i=1;
    $aColumns = array('estimate.task_transfer_id','users.user_name','nusers.user_name as new_user','estimate.approve_status');
    $sIndexColumn = "estimate.task_transfer_id";
    $isWhere = array("estimate.status = 0 and estimate.company_id IN (0,$_SESSION[company_id])");
    $sTable = "tbl_task_transfer as estimate";            
    $isJOIN = array(
         'left join users as users on users.user_id=estimate.old_user_id',
        'left join users as nusers on nusers.user_id=estimate.new_user_id'
    );
    $hOrder = "estimate.task_transfer_id desc";
    //$hGroupby = array("estimate.task_transfer_id");
    /*END*/
    ...
    foreach($sqlReturn as $row) {
        ...
        $row_data[] = $row['new_user'];
        $btn="";
        if($row['approve_status']==1){
            $row_data[] = '<button class="btn btn-xs btn-success" data-original-title="Approve" data-toggle="tooltip" data-placement="top" >Approved</button>';
        }else if($row['approve_status']==2){
            $row_data[] = '<button class="btn btn-xs btn-success" data-original-title="Approve" data-toggle="tooltip" data-placement="top" >Rejected</button>';
        }else{
            $row_data[] = '<button class="btn btn-xs btn-success" data-original-title="Approve" data-toggle="tooltip" data-placement="top" >Approve Pending</button>';

            $btn='<button class="btn btn-xs btn-success" data-original-title="Approve" data-toggle="tooltip" data-placement="top" onclick="task_transfer('.$row['task_transfer_id'].');" >Approve</button>';
        }
    ...
    $info['old_user_id']        = $POST['old_user_id'];
    $info['new_user_id']        = $POST['new_user_id'];
    $info['cdate']              = date("Y-m-d H:i:s");
    $info['user_id']            = $_SESSION['user_id'];
    $info['company_id']         = $_SESSION['company_id'];
    $ins_task_id = add_record('tbl_task_transfer', $info, $dbcon);
    if($ins_task_id){
        $arr['msg'] = "1";
    }else{
        $arr['msg'] = "0";
...
}else if (strtolower($POST['mode']) == "task_transfer") {

     $querybcc="select * from tbl_task_transfer where task_transfer_id=".$POST['task_trasfer_id'];
    $resultbdd=$dbcon->query($querybcc);
    $rel1=brp_mysqli_fetch_assoc($resultbdd);
     $show_user_ids                  = show_user_ids($dbcon,$rel1['new_user_id']);

    $quer="select * from tbl_task where task_status=0 and entry_type=1 and assign_user_ids=".$rel1['old_user_id'];
    $resul=$dbcon->query($quer);
    $s=0;
    while($rel=brp_mysqli_fetch_assoc($resul)){

         $task_info['assign_user_ids']  = $rel1['new_user_id'];
         $task_info['user_id']          = $rel1['new_user_id'];
         $task_info['show_user_ids']    = $show_user_ids;
         $task_info['perent_id']        = $rel['task_id'];
         $task_info['transfer_flag'] = 1;


        
        $task_info['task_type_id']              = $rel['task_type_id'];
        $task_info['task_rel_id']               = $rel['task_rel_id'];
        $task_info['task_name']                 = $rel['task_name'];
        $task_info['c_con_id']                  = $rel['c_con_id'];
        $task_info['cust_id']                   = $rel['cust_id'];
        $task_info['gt_id']                     = $rel['gt_id'];
        $task_info['inquiry_id']                = $rel['inquiry_id'];
        $task_info['task_location']             = $rel['task_location'];
        $task_info['full_day_event']            = $rel['full_day_event'];
        $task_info['appointment_start_time']    = $rel['appointment_start_time'];
        $task_info['appointment_end_time']      = $rel['appointment_end_time'];
        $task_info['appointment_subject']       = $rel['appointment_subject'];
        $task_info['task_remark']               = $rel['task_remark'];
       /// $task_info['assign_user_ids']           = $rel['assign_user_ids'];
        $task_info['task_priority_id']          = $rel['task_priority_id'];
        $task_info['task_due_date']             = $rel['task_due_date'];
        $task_info['task_alert_id']             = $rel['task_alert_id'];
        $task_info['create_date']               = $rel['create_date'];
        $task_info['alert_date_time']           = $rel['alert_date_time'];
        $task_info['task_completion_date']      = $rel['task_completion_date'];
        $task_info['entry_type']                = $rel['entry_type'];
        $task_info['company_id']                = $rel['company_id'];
        $task_info['branch_id']                 = $rel['branch_id'];
        $task_info['opp_id']                    = $rel['opp_id'];
        $task_info['sales_stage_id']            = $rel['sales_stage_id'];
        $task_info['stage_prob']                = $rel['stage_prob'];
        $task_info['lost_reason']               = $rel['lost_reason'];
        $task_info['email_template_id']         = $rel['email_template_id'];
        $task_info['is_read']                   = $rel['is_read'];
        $task_info['is_read_by_admin']          = $rel['is_read_by_admin'];
        $task_info['is_delete']                 = $rel['is_delete'];
        $task_info['quotation_followup_lock']   = $rel['quotation_followup_lock'];
        $task_info['followup_start_date']       = $rel['followup_start_date'];
        $task_info['quotation_to_quotation_followup_lock']        = $rel['quotation_to_quotation_followup_lock'];
        $task_info['quotation_followup_start_date']        = $rel['quotation_followup_start_date'];
        ...
        if($ins_task_id){
            ...
            $infoinq['task_status'] = 1;
            $infoinq['task_completion_date'] = date("Y-m-d H:i:s");

            $updateinqid = update_record('tbl_task', $infoinq, "task_id=" .$rel['task_id'], $dbcon);  
            $s++;  
        }
        
    }
    if($s>0){
         $infoinq2['approve_status'] = 1;
            $updateinqid3 = update_record('tbl_task_transfer', $infoinq2, "task_transfer_id=" .$rel1['task_transfer_id'], $dbcon);
    }
    if($updateinqid3){
        echo 1;
    }else{
...
$info['gt_id'] = $POST['gt_id'];
    ...
    $qry=$dbcon->query("SELECT * FROM tbl_task WHERE inquiry_id = '".$POST['inquiry_id']."' AND task_id != '".$POST['task_id']."' ORDER BY task_id DESC LIMIT 1");
    $res = mysqli_fetch_assoc($qry);

    $info1['task_status'] = 0;
    $info1['cdate'] = date("Y-m-d H:i:s");
    $updateids = update_record('tbl_task', $info1, "task_id=" . $res['task_id'], $dbcon);
