    ...
    if($_SESSION['user_type'] == 2 || $companyConfiguration['automatic_approval_proforma']==1){
    	...
    	if($mail_template && $to_email_id) {
    		...
    		if($getspecialConfiguration['umaboy_permission']==1){
    			...
    			final_send_email($from_email_id, $to_email_id, $rel11['email_cc'],$rel12['email_bcc'], $subject, $content, $files);
    		}
    	}
    }else{
...
{
	// echo $POST['cust_id']; echo"<Br>";
	// echo $_SESSION['company_id'];echo"<Br>";
	// echo $POST['eid'];
	...
	left join tbl_ledger as cust on cust.l_id=".$POST['cust_id']." where popro.product_id=".$POST['eid'];
...
else if(strtolower($POST['mode'])== "load_invoiceno")
{
	...
	if($performa_invoice_type=='3'){
		if(empty($POST['eid'])){
			$query="SELECT mst.*,hsn.hsn_code,product.product_name,cat.unit_name as base_unit, ccat.unit_name as conv_unit, rcat.unit_name as rat_unit,product.product_name,mst.description, product_qty, product_rate,product_disc, product_amount, categ.cat_name from  tbl_proforma_trn as mst 
		...
		$query="SELECT trancation_id,hsn.hsn_code,product.product_name,cat.unit_name as base_unit, ccat.unit_name as conv_unit, rcat.unit_name as rat_unit,product.product_name,mst.description,product_qty,product_rate,product_disc,mst.*,product_amount, categ.cat_name from  tbl_proforma_trn as mst 
		left join unit_mst as cat on cat.unitid=mst.unit_id 
		left join unit_mst as ccat on ccat.unitid=mst.conv_unit_id 
		left join unit_mst as rcat on rcat.unitid=mst.rate_unit 
		left join product_mst as product on product.product_id=mst.product_id 
...
else if(strtolower($POST['mode'])== "get_grossbalance")
{
	...
	$gst_keys = isset($_POST['bill_sundry_tax']) ? $_POST['bill_sundry_tax'] : [];
	$gst_vals = isset($_POST['bill_sundry_tax1']) ? $_POST['bill_sundry_tax1'] : [];

	$bill_sundry_tax = [];

	if (is_array($gst_keys) && is_array($gst_vals) && count($gst_keys) === count($gst_vals)) {
		$bill_sundry_tax = array_combine($gst_keys, $gst_vals);
	}
...
<table>
  ...
  .page{
  	...
  	$result=$dbcon->query($qry);    
  	$i=1;$total=0;$discount=0;$totalqty=0;$totalsqr=0;$charges_qty=0;$charges_qty1=0;$total_product_amount=0;$totaltaxable=0;$total_packing=0;$final_amount=0;$total_cs_gst=0;$total_i_gst=0;$gst_per = 0;$gst_rate = 0;
