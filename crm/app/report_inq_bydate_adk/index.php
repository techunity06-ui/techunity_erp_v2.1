...
if($_POST != NULL) {
	...
	if($POST['stage_id']){
		$whr.=' and inq.opp_id='.$POST['stage_id'];
	}

	if($POST['sales_stage_id']){
		$whr.=' and inq.sales_stage_id='.$POST['sales_stage_id'];
	}

	if($POST['source_id']){
		$whr.=' and src.rb_id='.$POST['source_id'];
	}
	...
	$whr .= ' AND inq.opp_id NOT IN(12,13)';
	//$whr .= ' and task.task_status = 0';

	$c=1;
	$qry="SELECT inq.*, group_concat(product_name SEPARATOR ',<br>') as product_name,coun.country_name,state.state_name,city.city_name,updated_by_usr.user_name as updated_user_name, assign_usr.user_name as assign_users,ter.t_name,cust.cust_name,stage.opp_stage,src.rb_name,ind.ci_name,inq_cat.mcd_name as inq_cat_name,inq_sale_stage.mcd_name as inq_sale_stage_name,inq_type.mcd_name as inq_type_name,cur.currency_code,IF((select count(cust_id) from tbl_inquiry where inquiry_status=0 and cust_id=inq.cust_id)>1, 'Existing Customer', 'New Customer') as ctype,owner_ausr.user_name as owner_user,addr.c_add_address,(select count(quotation_id) from tbl_quotation as quot where quot.inquiry_id=inq.inquiry_id) as quo_sta from tbl_inquiry as inq
	left join tbl_inquiry_trn as inq_trn on inq_trn.inquiry_id=inq.inquiry_id
	left join product_mst as pro on pro.product_id  = inq_trn.product_id 
	left join users as assign_usr on assign_usr.user_id=inq.user_id
	left join users as updated_by_usr on updated_by_usr.user_id=inq.updated_by_userid
	...
	WHERE inq.inquiry_status=0 AND inq.company_id IN (0,".$_SESSION['company_id'].")".$whr." Group by inq.inquiry_id, inq.inquiry_no ".$hav." order by inq.inquiry_date";
	...
	if(mysqli_num_rows($qry_rs)){
		while($rel=mysqli_fetch_assoc($qry_rs)){
			...
			$str.='<tr>
				...
				<td class="text-left" style="white-space:nowrap;">'.date("d-m-Y",strtotime($rel['inquiry_date'])).'</td>
				...
				<td class="text-left">'.$contact_per['c_con_fname'].' '.$contact_per['c_con_lname'].'</td>
				<td class="text-left">'.$contact_per['c_con_mobile'].'</td>
				<td class="text-left">'.$contact_per['c_con_email'].'</td>
				...
				<td class="text-left" style="white-space:nowrap;">'.date("d-m-Y h:i A",strtotime($task_rel['create_date'])).'</td>
				<td class="text-left">'.nl2br($task_rel['task_remark']).'</td>
				<td class="text-left">'.$rel['updated_user_name'].'</td>
				<td class="text-left">'.$rel['inq_desc'].'</td>
			</tr>';
			$c++;
		}
	}
