...
$POST = ($_POST != NULL)? bulk_filter($dbcon,$_POST) : bulk_filter($dbcon,$_GET);
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$where='';
		if($branch_id){
	        $where .= check_branch('apt',$branch_id);
	    }
		...
		$appData = array();
		$i=1;
		$aColumns = array('apt.task_id', 'apt.appointment_subject','apt.task_status','apt.appointment_start_time','apt.appointment_end_time','regrd.task_rel_name', 'apt.task_name', 'apt.company_id', 'usr.user_name', 'apt.create_date', 'inq.inquiry_name', 'cust.cust_name', 'per.c_con_fname', 'per.c_con_lname', 'apt.cdate', 'apt.task_rel_id');
		$sIndexColumn = "apt.task_id";
		$isWhere = array("apt.entry_type=2  and apt.company_id in (0,$_SESSION[company_id])".$where_status.$where.check_user('apt'));
		...
		foreach($sqlReturn as $row) {
			...
			if(in_array(APPOINTMNET_SLUG_EDIT,$bulkAccessArray)){
				if($row['task_rel_id']=='5'){//Inquiry
					...
					$uu=$row['c_con_fname'].' '.$row['c_con_lname'];
				}
				else{
					$uu=$row['task_name'];
				}
				
				$row_data[] = '<a class="" data-original-title="Edit '.$uu.'" data-toggle="tooltip" data-placement="top" href="'.ROOT.CRM_ROOT.'appointment_edit/'.$row['task_id'].'">'.$row["sr"].'</a>';
				$row_data[] = '<a class="" data-original-title="Edit '.$uu.'" data-toggle="tooltip" data-placement="top" href="'.ROOT.CRM_ROOT.'appointment_edit/'.$row['task_id'].'">'.$row["appointment_subject"].'</a>';
				$row_data[] = '<a class="" data-original-title="Edit '.$uu.'" data-toggle="tooltip" data-placement="top" href="'.ROOT.CRM_ROOT.'appointment_edit/'.$row['task_id'].'">'.$row["task_rel_name"].'</a>';
				
				$row_data[] = '<a class="" data-original-title="Edit '.$uu.'" data-toggle="tooltip" data-placement="top" href="'.ROOT.CRM_ROOT.'appointment_edit/'.$row['task_id'].'">'.$uu.'</a>';
			...
			$edit='';$delete='';
			if(in_array(APPOINTMNET_SLUG_EDIT,$bulkAccessArray)) {
				$edit='<a class="btn btn-xs btn-warning" data-original-title="Edit" data-toggle="tooltip" data-placement="top" href="'.ROOT.CRM_ROOT.'appointment_edit/'.$row['task_id'].'"><i class="fa fa-pencil"></i></a>';
			}
			if(in_array(APPOINTMNET_SLUG_DELETE,$bulkAccessArray)) {
