...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "generate_report_product_service1")
	{
		...
		if($POST['start_date']){
			$va=" and e.inquiry_status=0 and et.task_status=0 and et.entry_type=1 and e.closing_date >= '".date('Y-m-d',strtotime($POST['start_date']))."' AND e.closing_date <= '".date('Y-m-d',strtotime($POST['end_date']))."'";
		}else{
			$va=" and e.inquiry_status=5";
		}
		if($leaddate){
			$va=" and e.inquiry_status=0 and et.task_status=0 and et.entry_type=1 and e.closing_date='".$leaddate."'";
		}
		$appData = array();
		//'mc.mcd_name as sales_stage'
	...
	else if(strtolower($POST['mode']) == "generate_report_leads_closing_date") {
			$whr.=" and e.closing_date between '".date("Y-m-d",strtotime($POST['start_date']))."' and '".date("Y-m-d",strtotime($POST['end_date']))."'";
			$query="select COUNT(*)as led,e.closing_date,e.inquiry_name as oppurtunity_name from tbl_inquiry as e left join tbl_task  as et on et.inquiry_id=e.inquiry_id where e.inquiry_status=0 and et.task_status=0 and et.entry_type=1 ".$whr." AND e.company_id IN (0,".$_SESSION['company_id'].") group by e.closing_date";
			//var_dump($query);
			...
			{	
					...
					$row1[$i]['label']=$row['closing_date'];
					//$row1[$i]['label']=$POST['rep_date'];
					$row1[$i]['closing_date']=$row['closing_date'];	
					$i++;
			}	
