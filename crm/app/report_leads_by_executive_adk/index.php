...
//print_r($_POST);	
	if(strtolower($POST['mode']) == "generate_report_product_service1")
	{
		...
		//'mc.mcd_name as sales_stage'
			$i=1;
			$aColumns = array('e.inquiry_no','mc.mcd_name as sales_stage','addr.c_add_address','city.city_name','state.state_name','cust.cust_name','con.c_con_fname','con.c_con_lname','con.c_con_mobile','con.c_con_email','us.user_name as lead_owner','typ.mcd_name as type_name','source.rb_name','op.opp_stage as stage','e.stage_prob as probablity','e.closing_date','e.cdate','e.inquiry_id','e.cust_id');
			...
			$isJOIN = array("left join tbl_task  as et on et.inquiry_id=e.inquiry_id",
							...
							"left join tbl_cust_address as addr on addr.cust_id = e.cust_id and addr.c_addr_defult=1",
							...
							"left join users as us on us.user_id=e.user_id",
							"left join tbl_opportunity_mst as op on op.opp_id=e.opp_id",
							"left join tbl_master_category_detail as mc on mc.mcd_id=e.sales_stage_id","left join tbl_master_category_detail as typ on typ.mcd_id = e.inquiry_type_id");
			$hOrder = "e.inquiry_id";
			...
			foreach($sqlReturn as $row) {
				...
				$row_data[] = brp_ucwords(strtolower($row['c_con_email']));
				$row_data[] = brp_ucwords(brp_strtolower($row['rb_name']));
