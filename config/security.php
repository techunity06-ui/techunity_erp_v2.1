...
//@End
function convert_number_to_words($num = false,$currency = "",$last_word = "") {
	...
	if($x > 0 )
	{
		...
		$fraction=  $tens.'-'.$singles.' '.$last_word;
   ...
   return  implode(' ', $words).$currency.' ' .$and .$fraction." Only " ;
}
...
function canCheckPermissionAccess($dbcon, $slugs,$company_id = "",$user_id ="") {
   ...
   $qry="SELECT `user_id`,`user_access_permission` FROM `users` 
    ...
    if($user_data->num_rows > 0) {
       ...
       $tempqry = "SELECT * FROM `menu_master_access_routes` 
        ...
        //echo "<pre>"; print_r($accessibleIds);
...
function getLeftMenu($dbcon,$parent_id){
    ...
    if($parent_data){
        foreach($parent_data as $menu_item){
        	//$has_childd = $dbcon->query("SELECT `menumaster`.`id`,`menumaster`.`parent_id`,`menumaster`.`menu_name`,`menumaster`.`menu_fa_icon`,`menumaster`.`user_id`,`menumaster`.`menu_path` FROM `menu_master_access` as `menumaster` WHERE `menumaster`.`status` = '0' and `menumaster`.`parent_id`=".$menu_item['id']." ORDER BY `menumaster`.`menu_order`");
            $kk="SELECT `menumaster`.`id`,`menumaster`.`parent_id`,`menumaster`.`menu_name`,`menumaster`.`menu_fa_icon`,`menumaster`.`user_id`,`menumaster`.`menu_path` FROM `menu_master_access` as `menumaster` WHERE `menumaster`.`status` = '0' and `menumaster`.`parent_id`=".$menu_item['id']." ORDER BY `menumaster`.`menu_order`";
            
            $rs_comp = $dbcon->query($kk);
            $count_child = mysqli_num_rows($rs_comp);
           // $has_child = $has_childd->fetch_assoc();
...
function convert_number_to_words_old($number = false,$currency = "",$last_word = "",$firstword = "")
{
    ...
    $paise = ($decimal > 0) ? "and " . ($words[$decimal / 10] . " " . $words[$decimal % 10]) .' '. $last_word . ' Only' : '';
    $only = $paise ? '' : 'Only ';
    return ($Rupees ? $Rupees . ucfirst(strtolower($firstword)) .' '.' '.$only : '') . ucfirst(strtolower($paise));
}
...
function convert_number_to_words_new($number = false,$currency = "",$last_word = "",$firstword = "")
{
    ...
    $Rupees = implode('', array_reverse($str)) . ' Rupees ';

if ($decimal > 0) {
            $paise = ' and '.  numberToWords($decimal) . ' Paise';
        }
