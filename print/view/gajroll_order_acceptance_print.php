...
<table>
<tr>
<td style="border: 0px; "><img src="'.DOMAIN_F.LOGO.$comp_rel['logo'].'" style="width:8.27in;height:100px;" /></td>
</tr>
</table>';
...
.page{
	...
	while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
		...
		if($delivery_type == 'product_wise'){
				$retu_date = "select sdate.*,unit.unit_name from tbl_salesorder_delivery_date as sdate left join unit_mst as unit on unit.unitid=sdate.unit_id where invoice_status=0 and sales_ordertrn_id=".$trn_rel['sales_ordertrn_id'];
		...
		<td style=" text-align:right;font-size:12px;">'.without_comma_two_digit_amount($trn_rel['discount_per']).' % <br>('.without_comma_two_digit_amount($trn_rel['product_discount']).')</td>
