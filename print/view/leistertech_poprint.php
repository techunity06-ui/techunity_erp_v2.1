...
$query="select po.*,state.state_name,modesup.mode_dispatch,payterms.payment_terms as payment_term,l.l_name as vender_name,country.country_name,l.m_address as vender_address,l.gst_no as tin_no,l.cust_cont_name as contac_per,l.cust_mobile as vender_mobile,l.cust_email as vender_email, l.stateid, state.gst_state_code, city.city_name, bmast.branch_address, city1.city_name as bcity, state1.state_name as bstate, country1.country_name as bcountry,comp.company_name,comp.address,god.gd_address, comp.company_name, le.l_name as con_ven, bm.branch_name as cons_bran, curr.currency_name from tbl_purchaseorder as po 
...
left join currency_mst as curr on curr.currencyid = po.currency_id
...
$po_del="";
if($rel['delivery_type'] == "po_wise"){
	$po_del = date('d-m-Y',strtotime($rel['purchaseorder_due_date']));
}
/*if(!empty($rel['gd_address'])){
	$baddress = $rel['gd_address'];
}
else{
...
<body>
							...
							<div class="panel-body">
								<!--<center>-->
									...
									<div class="col-lg-12 table-responsive" id="receipt_print">	<div class="col-md-12" style=" margin-top:10px;" id="print1">
										...
										<table width="100%" class="maintable" border="1" id="table_head" style="border-radius:2px;border-collapse: separate; border-width: 2px;border-color: black;" >
		<tr style="border:none;">
			<td width="100%" style="border:none;"> 
				<img src="<?=ROOT.LOGO.$set_head['logo']?>" style="width:100%;padding: 2px;"/>
			</td>
		</tr>
	</table>
	...
	<table width="100%" class="maintable" style="font-size: 12px;margin-top: 5px;" id="invoice_type" >
				...
				<th colspan="11" style="padding:0px !important;">
					<table style="font-size:12px;border-collapse: collapse;border-top:none;" cellpadding="0" cellspacing="0" width="100%" >
						...
						<tr>
							...
							?>
							<td width="50%" colspan="2" rowspan="<?=$rowsp?>" style="vertical-align:top;border:1px solid;">
								<b>To, </b><br/>
								...
								<span style="font-weight:normal;">   <br/>
									...
									<br/>Contact Person : <span style="font-weight:normal;"><?=$rel['contac_per']?>,</span>
		...
		//tax summary calculation start
				...
				$code="";
				$description = ($row['product_des']) ? stripcslashes(str_replace(array("\n", "\r", "\N", "\R"), '', $row['product_des'])) : '';
				if(!empty($row['scode'])){
					$code=" ( ".$row['scode'] .")";
				}
				?>
				...
				for($j=0; $j<$pr; $j++)
				{
					...
					<tr style="height:40px">
						...
						<tr style="height:20px">
							...
							<tr height="20px">
								<td style="border-right:1px solid;border-top:1px solid;" colspan="<?=$colspan?>">
									<strong><?=$rel['currency_name']?> :</strong>
									<?=ucwords(convert_number_to_words($rel['g_total'],$rel['currency_name']))?>
								</td>
