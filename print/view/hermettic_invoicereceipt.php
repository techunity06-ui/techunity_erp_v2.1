...
if(strtolower($type) == 'pdf') {
	...
	if($rel['is_sales_order']!=0){
		$qry = $dbcon->query("SELECT GROUP_CONCAT(sales_order_no) as sales_order_no, GROUP_CONCAT(DATE_FORMAT(sales_order_date,'%d-%m-%Y')) as sales_order_date, GROUP_CONCAT(po_no) as po_no, GROUP_CONCAT(DATE_FORMAT(po_date,'%d-%m-%Y')) as po_date FROM tbl_sales_order WHERE sales_order_id IN (".$rel['sales_order_id'].")");
		$rels = brp_mysqli_fetch_assoc($qry);
		$po_no = $rels['po_no'];
		$po_date = $rels['po_date'];
		$sales_order_no = $rels['sales_order_no'];
		$sales_order_date = $rels['sales_order_date'];
	}
	else
	{
		$po_no = $rel['order_no'];
	...
	.page{
		...
		$html.='<tr>
		<td style="width:15%;border:1px solid; border-right:none !important;"><strong>Invoice No </strong>
		</td>
		<td style="width:15%;border-bottom:1px solid; border-right:1px solid;border-top:1px solid;border-left:none !important; white-space: nowrap;">: <strong>'.$rel['invoice_no'].'</strong>
		</td>
		<td style="width:15%;border-bottom:1px solid;border-top:1px solid; border-right:none;"><strong>Date </strong>
		</td>
		<td style="width:15%;border-bottom:1px solid;border-right:1px solid;border-top:1px solid;border-left:none;">: <strong>'.date('d-m-Y',strtotime($rel['invoice_date'])).'</strong>
		</td>
		<td style="width:20%;border-bottom:1px solid;border-top:1px solid;border-right:none;"><strong>Transport Detail</strong>	
		</td>							
		<td style="width:20%;border-bottom:1px solid;border-top:1px solid;border-right:1px solid;border-left:none; white-space: nowrap;">: '.$r_t['transportation_name'].'</td>
		</tr>
		<tr>
		<td style="border-bottom:1px solid;border-left:1px solid;border-right:none;"><strong>Challan No </strong></td>
		<td style="border-bottom:1px solid;border-right:1px solid;border-left:none; white-space: nowrap;">: '.(($rel['challan_no']) ? $rel['challan_no'] : '').' </td>
		...
		<td style="border-bottom:1px solid;border-right:none;"><strong>Docket No. </strong></td>							
		...
		<tr>
		<td style="border-right:none;border-bottom:1px solid;border-left:1px solid;"><strong>SO No.</strong></td>
		<td colspan="3" style="border-left:none;border-bottom:1px solid;border-right:1px solid">: '.$sales_order_no.'</td>
		<td style="border-right:none;border-bottom:1px solid;"><strong>SO Date</strong></td>
		<td style="border-left:none;border-bottom:1px solid">: '.$sales_order_date.'</td>
		</tr>
		<tr>
		...
		if($rel['enable_consignee']==1) {
			...
			$html.='<td colspan="2" style="vertical-align:top;border-right:1px solid"><b>Consignee : </b><br><strong>'.$cons_data['company_name'].'</strong><span style="font-weight:normal;">'.$cons_data['cust_addres'].'<br/>'.$cons_data['city_name'].', '.$cons_data['state_name'].', '.$cons_data['country_name'];
		...
		<th width="9%" style="border:1px solid;border-top: none;"><strong>QTY.</strong></th>
		<th width="10%" style="border:1px solid;border-top: none;"><strong>Rate</strong></th>';
		$html.='<th width="10%" style="border:1px solid;border-top: none;"><strong>Taxable<br/>Value</strong></th>';
