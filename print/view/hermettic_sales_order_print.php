...
if($rel['consignee_id']!=0){
	$query_con="select cust.cust_name,cust.company_name,cust.cust_address,cust.cust_mobile,cust.cust_email,cust.cust_pincode,cust.gst_no,country.country_name,state.state_name,city.city_name,state.gst_state_code from tbl_custmer_consignee as cust 
		...
		$company_name=$rel_con['company_name'];
	$cust_address=$rel_con['cust_address'];
	$city_name=$rel_con['city_name'];
	$state_name=$rel_con['state_name'];
	$country_name=$rel_con['country_name'];
	$cust_pincode=$rel_con['cust_pincode'];
	$gst_no=$rel_con['gst_no'];
}
...
.page{
	...
	<strong style="font-size:16px">'.$rel['company_name'].'</strong><br/>
	'.$rel['cust_address'].'<br/>
	Phone No : '.$rel['cust_mobile'].'<br/>
	<strong>GST no - '.$rel['gst_no'].'</strong>              
	...
	<td style="text-align:left;border:1px solid;"> 
	'.(($rel['quotation_date']) ? date('d-M-Y', strtotime($rel['quotation_date'])) : '').'
	</td>
	</tr>
	';
	...
	'.$cust_address.'<br/>
	Phone No : '.$rel_con['cust_mobile'].'<br/>
