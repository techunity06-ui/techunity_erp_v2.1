...
$query="select invoice.*,country.country_name,state.state_name,cust.stateid,state.gst_state_code, city.city_name, cust.l_name, cust.m_address, cust.gst_no,cust.company_name,type.invoice_type,cust_pincode,cust_mobile,dispatch.mode_dispatch from tbl_invoice as invoice 
...
$place_of_supply=$rel['city_name'];
		...
		$currency = $rel['currency_id'];
		//echo $currency;
		if($currency!='1' && $currency!='0')
		{
			$currency_sel = $dbcon->query("select * from currency_mst where currencyid='$currency'");
			$r_sel = brp_mysqli_fetch_array($currency_sel);

			$currency_sign = "<br>(<i class='".$r_sel['currency_symbl']."'></i>)";
			$currency_name = '('.ucfirst(strtolower($r_sel['currency_code'])).')';
		$currency_word_start = ucfirst(strtolower($r_sel['currency_in_word']));
		$currency_word_end = ucfirst(strtolower($r_sel['currency_in_word_end']));
		}
		else
		{
			$currency_sign = '';
		...
		//transport details 
		$rel_trans = $dbcon->query("select tr.*,t.transportation_name 
			from tbl_transport_transaction as tr 
			left join transportation_details as t on t.id=tr.transport_id
			 where transport_transaction_table_id='$rel[invoice_id]'");
		if(brp_mysqli_num_rows($rel_trans)>0)
		{
			$r_trans = brp_mysqli_fetch_assoc($rel_trans);
			$transport_no = $r_trans['transportation_name'];
			$doc_no = $r_trans['transport_doc_no'];
			$vehicle_no = $r_trans['transport_vehicle_no'];
		}
		else
		{
			$transport_no='';
								...
								<section class="panel">
									...
									<div class="col-lg-12 " id="receipt_print">	
										<div class="col-md-12 breakout" style=" margin-top:10px;" id="print1">
											...
											<table  class="maintable headermain " style="margin-top: 23px;    padding: 17px 0 0;" id="table_head" width="100%">
															...
															<th colspan="11" style="padding:0px !important;">
																<table style="font-size:10px;border-collapse: collapse;border-top:none !important;" cellpadding="0" cellspacing="0" width="100%" >
																	...
																	<tr>
																		...
																		<td width="38%" style="vertical-align:top;border-bottom:1px solid;border-top:1px solid;border-right:1px solid;">: <?=$transport_no;?>						
																	</td>							
																</tr>
																<tr>
																	...
																	<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid" colspan="">: <?php if($rel['dc_enable']==1) { echo $rel['challan_no']; } else { echo "-";  } ?>
																</td>
																<td style="vertical-align:top;border-bottom:1px solid "><strong>Date </strong>
																</td>
																<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid" colspan="">: <?php if($rel['dc_enable']==1) { echo date("d/m/Y",strtotime($rel['challan_date'])); } else { echo "-";  } ?>
															</td>

															<td style="vertical-align:top;border-bottom:1px solid;white-space:nowrap;"><strong>Docket No. </strong>					
															</td>							
															<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid;">: <?=$doc_no ?></td>
														</tr>
														<tr>
															<td class="boderremoveres" style="vertical-align:top;border-bottom:1px solid;border-left:1px solid;"><strong>Po No </strong></td>
															<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid" colspan="">: <?php if($rel['po_enable']==1) { echo $rel['order_no']; } else { echo "-";  } ?>
														</td>
														<td style="vertical-align:top;border-bottom:1px solid "><strong>Date </strong>
														</td>
														<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid" colspan="">:<?php if($rel['po_enable']==1) { echo date("d/m/Y",strtotime($rel['order_date'])); } else { echo "-";  } ?>
													</td>
												...
												<td style="vertical-align:top;border-bottom:1px solid;white-space:nowrap;white-space:nowrap;"><strong>Payment Terms</strong>					
												</td>							
											...
											<tr>
												...
												<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid">: <?=$vehicle_no; ?>
											</td>
															...
															<tr  height="20px">
																...
																if($rel['stateid']==$set_head['stateid']){
																	...
																	<tr height="20px">
																		...
																		<td colspan="2" style="text-align:right  !important; border-top:1px solid;font-size:10px;border-left:1px solid; "><?=number_format(($total_cs_gst/2),2,".","")?> 
																			</td>
																	</tr>
																	<tr height="20px">
																		<td  style="border-right:1px solid;border-top:1px solid; font-size:10px;" colspan="5">
																...
																while($row11=mysqli_fetch_assoc($result11))
																	...
																	if($tax_name[1]) 
																		...
																		<?php if($rel['formulaid']){ 
																		 ...
																		 <td colspan="3" style="border-top:1px solid;border-right:1px solid;font-size:10px;text-align:left  !important">Sub Total</td>
																		 <td colspan="2" style="text-align:right  !important; border-top:1px solid;font-size:10px;border-right:1px solid; "><?=number_format($total,2,".","")?></td>
																		 </tr>--> 
																		 <tr height="20px">
																		 	<td class="borderleftadd" style="border-right:1px solid;border-top:1px solid;font-size:10px;" colspan="<?=$colspan?>"></td>
																		...
																		<tr height="20px">
																			<td style="border-right:1px solid;border-top:1px solid;font-size:10px;" colspan="<?=$colspan?>">
																				...
																				<?=ucfirst(convert_number_to_words_new($rel['g_total'],$currency_word_start,$currency_word_end))?>
																			</td>
																			<td colspan="3" style="border-top:1px solid;border-right:1px solid;font-size:10px;text-align:left  !important">
																				<strong>Grand Total 
																					<?=$currency_sign; ?></strong> :
																			</td>
																	...
																	// $query="select sum(product_amount) as product_amount,sum(tax_amount1) as tax_amt1,trn.product_hsn_code,sum(tax_amount2) as tax_amt2,tax_name1,tax_name2  FROM `tbl_invoicetrn` as trn where trancation_status=0 and invoice_id=".$rel['invoice_id']." group by trn.formulaid, trn.product_hsn_code";
																				...
																				{	
																					...
																					echo '<td style="vertical-align:top;text-align:center;border-bottom:1px solid;" >
