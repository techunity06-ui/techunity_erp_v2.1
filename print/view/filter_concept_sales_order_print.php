...
echo $sales_order_id = $_REQUEST['id'];	
...
//Quotation Data
	...
	<td style="text-align:left;" colspan="3">: '.$rel['project_name'].'</td>
	</tr>
	</table>';  
...
//$footer='<img src="'.DOMAIN_F.LOGO.$comp_rel['f_logo'].'" style="padding-left:0px !important;width:8.27in"/>';
	...
	.page{
		...
		<tbody>';
		$trn_qry="select trn.*,product.product_name, product.product_icode, product.product_alias_name FROM `tbl_sales_ordertrn` as trn 
		...
		while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
			...
			<td style="width:50%;text-align:left; vertical-align: top;"><strong>'.$trn_rel['product_name'].'</strong>'.$alias_name.'<br>'.nl2br($trn_rel['description']).'</td>
			<td style="width:10%;text-align:center;vertical-align:top;">'.$trn_rel['unit_name'].'</td>
			...
			if($delivery_type == 'product_wise'){
				...
				$html.='<tr>
				<td></td>
				<td style="vertical-align: top; white-space: nowrap;">Delivery Schedule : </td>
				<td><table style="font-size:12px; width: 50%">
				<tr style="border: none;border-top: 1px dashed; border-bottom: 1px dashed;">
				<td>No</td>
				<td>Delivery Date</td>
				<td>Qty</td>
				<td>Pend. Qty</td>
				...
				while($rowdate=brp_mysqli_fetch_array($resadate)){		
					...
					<td style="white-space: nowrap;">'.$rowdate['product_qty'].' '.$rowdate['unit_name'].'</td>
					<td style="white-space: nowrap;">'.$rowdate['used_qty'].' '.$rowdate['unit_name'].'</td>
					</tr>';		
					$dt++;
				}
				$html .='</table>
		...
		if($delivery_type == 'product_wise'){
			...
			<td colspan="5" style="text-align:left; font-weight: bold;">Delivery Date : '.date("d/m/Y", strtotime($rel['delivery_date'])).'</td>
			</tr>
			</tbody></table>';
		}
		...
		<tbody>
		<tr>
		<td colspan="3"><strong>Notes : </strong>'.$rel['remark'].'</td>
		</tr>
		<tr>
		<td style="width: 33%; text-align:left;padding:5px;">Issued By : '.$user_rel['user_name'].'</td>
	...
	// echo $html;exit;
		...
		$mpdf=new mPDF('','A4','0','calibri','10','10','68','10','1','1');
