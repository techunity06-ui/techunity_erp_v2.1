...
if(strtolower($type) == 'pdf') {
	...
	$prepared = "select us.user_name from tbl_purchaseorder_aprv_log as aplog 
	left join users as us on us.user_id = aplog.user_id
	where aplog.purchaseorder_id=$purchaseorder_id order by purchaseorder_aprv_id desc limit 1";
	$prepared_by=brp_mysqli_fetch_assoc($dbcon->query($prepared));

	$checked = "select us.user_name from tbl_purchaseorder_finance_aprv_log as aplog 
	left join users as us on us.user_id = aplog.user_id
	where aplog.purchaseorder_id=$purchaseorder_id order by purchaseorder_aprv_id desc limit 1";

	$checked_by=brp_mysqli_fetch_assoc($dbcon->query($checked));
...
<table>
<tr>
<td style="border: 0px; "><img src="'.DOMAIN_F.LOGO.$comp_rel["logo"].'" style="width:8in;" /></td>
</tr>
</table>';
...
.page{
	...
	<td colspan="4" style="border: 0px; font-size:14px;">Email : <span style="font-size:13px;">gajrollpur@gmail.com</span></td>
	...
	<td colspan="9" style="text-align:center;font-size:18px;font-weight:bold;padding-bottom:-15px;">Purchase Order</td>
	</tr>
	<tr style="border:none; border-left:1px solid; border-right:1px solid;margin-top:-10px; ">
	<td colspan="2" style="border: 0px; font-size:14px;">PO No : <span style="font-size:13px;">'.$rel['purchaseorder_no'].'</span></td>
	<td colspan="4" style="border: 0px; font-size:14px;"></td>
	<td colspan="3" style="border: 0px; font-size:14px;">PO Date :<span style="font-size:13px;">'.$purchaseorder_date.'</span></td>
	</tr>
	
	<tr style="background-color:#b3b3b3">
	<td colspan="4" style=" text-align:center;"><b>Vendor / Supplier Details</b> </td>
	<td colspan="5" style=" text-align:center;"><b>Ship to Address</b></td>
	</tr>
	<tr style="border:none; border-left:1px solid; border-right:1px solid;">
	<td colspan="4" rowspan="2" style="border-left:1px solid;border-bottom:none; text-align:left;">'.$party_address_billing.' </td>
	<td colspan="5" style="border-bottom:none;  text-align:left;vertical-align:top"><strong>'.$party_address_con.'</td>
	</tr>
	<tr style="border:none; border-left:1px solid; border-right:1px solid;">
	<td colspan="5" style="border-bottom:none;  text-align:left;">
	Payment Terms : '.$rel['payment_term_name'].'
	<br>Terms of delivery : '.date('d-m-Y',strtotime($rel['purchaseorder_due_date'])).'
	...
	<td style=" text-align:center;"><b>Net Amount</b> </td>
	</tr>
	</thead>
	<tbody>';
	$qry="select trn.*,product.product_name, product.product_icode, dr.drawing_number, product.product_alias_name,per.unit_name, unit.unit_name as unit_name1 FROM `tbl_purchaseordertrn` as trn 
	...
	left join unit_mst as unit on unit.unitid=trn.conv_unit_id
	...
	while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
		...
		if($trn_rel['unit_id']===$trn_rel['rate_unit']){
			...
			$cqty='';
			$unit = $trn_rel['unit_name'];
			$unit1 = '';
		}else{
			$sqty=$trn_rel['product_qty'];
			$cqty=$trn_rel['product_conv_qty'];
			$unit = $trn_rel['unit_name'];
			$unit1 = $trn_rel['unit_name1'];
		}
		$product_des = (!empty($trn_rel['product_des'])) ? $trn_rel['product_des'] : $trn_rel['description'];
		...
		$html.='<td border="0" style="border-radious: 0px; border: none!important;font-size:11px;'.$align.'"><strong>'.$trn_rel['product_icode'].'<br/>'.$trn_rel['product_name'].'</strong>  '.$drawing_number.' '.$alias.' '.$wno.'<br>'.$product_des.'</td> 
		</tr>
		</table>
		</td>
		...
		<td style=" text-align:right;font-size:12px;">'.without_comma_two_digit_amount($sqty).'<br>'.without_comma_two_digit_amount($cqty).'</td>
		<td style=" text-align:center;font-size:12px;">'.$unit.'<br>'.$unit1.'</td>
		<td style=" text-align:right;font-size:12px;"> '.without_comma_two_digit_amount($trn_rel['product_rate']).'</td>
	...
	$html.='
	<tr>
	<td colspan="8" style=" text-align:right;font-size:12px;">TOTAL  </td>
	<td  style=" text-align:right;font-size:12px;">'.without_comma_two_digit_amount($ttl_amt,2).'</td>
	</tr>
	';
	...
	{
		$html.='<tr>
		<td colspan="8" style=" text-align:right;font-size:12px;"><b>'.$row11['l_name'].'</b></td>
	...
	{
		$html.='<tr>
		<td colspan="8" style=" text-align:right;font-size:12px;"><b>'.$row12['l_name'].'</b></td>
		<td style="text-align:center;border:1px solid;"><b>
	...
	$html .= '
	<tr>
	<td colspan="8" style=" text-align:right;font-size:14px;background-color:#b3b3b3">Total Amount (In Figure) </td>
	<td  style=" text-align:right;font-size:14px;font-weight:bold;background-color:	#b3b3b3">'.without_comma_two_digit_amount(($rel['g_total']),2).' </td>
	</tr>';
	...
	while($purchase_data=mysqli_fetch_assoc($purchase_exec)){
		...
		'.without_comma_two_digit_amount($purchase_data['product_amount']).'
		</td>';
		...
		'.without_comma_two_digit_amount($total_taxable_amount).'
		</td>
		</tr>';
	}
	...
	if($rel['stateid']==$comp_rel['stateid']){
		...
		'.without_comma_two_digit_amount($gst_rate/2).'
		</td>
		...
		$html.='<td style="text-align:right;font-size:13px;font-weight:bold" colspan="2">
		'.without_comma_two_digit_amount($gst_rate).'
		</td>
		';
	}
	...
	<tr>
	<td colspan="8" style="border:none; vertical-align: top;">'.$rel['po_condition'].'</td>
	</tr>
	<tr style="border-bottom:none">
	<td colspan="2" style="height:50px;border-bottom:none;vertical-align:top;text-align:center;width:20%">'.$prepared_by['user_name'].'
	</td>
	<td colspan="2" style="height:50px;border-bottom:none;vertical-align:top;text-align:center;width:20%">'.$checked_by['user_name'].'</td>
	...
	/*if(!empty($rel['po_condition'])){
		$html.= '<pagebreak page-break-type="clonebycss" />
		<div style="padding-top:50px !important;">'.$rel['po_condition'].'</div>';
	}*/
