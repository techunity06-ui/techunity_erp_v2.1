...
$con="select comp.* from tbl_company_configuration as comp where company_id=".$rel['company_id'];
//CHEMITEK PROCESS EQUIPMENTS PRIVATE LIMITED		
$comp_config=mysqli_fetch_assoc($dbcon->query($con));
$header ='<table style="border:0; border-color:white">
<tr style="border:0;">
<td style="width:30%; text-align:right;border:0;"> 
<img src="../../view/img/logo/'.$comp_rel["logo"].'" style="height: 100px; " />
</td>
</tr>
<tr style="border:0;">
...
if ($getspecialConfiguration['adk_permission'] == 1) {
	...
	$party_address_con .= "<br>"."GSTIN : ".$rel['tin_no'];
}
...
.page{
    ...
    <tr style="border:none;">
        <th style="text-align: left;border: none;"><u>BY EMAIL</u><br>'.$rel['purchaseorder_no'].'<br>' .  date("F j, Y",strtotime($rel['purchaseorder_date'])). ' </th>
    </tr>

    <tr style="border:none;">
    <td style="text-align: left;border: none;width:350px;"> <br><b>' . $rel['vender_name'] . '</b><br>' . $rel['vender_address'] . '</br></br><br>MAIL ID:  '.$rel['cust_email'].'</td>
    </tr >   
    <tr style="border:none;">
        <td colspan="2" style="text-align: left; border: none;"><span ><b>Kind Attention </b></span>:	' . $rel['cust_contact_person_name'] . ' <br> <br>
        <span style=""><b>SUB.: PURCHASE ORDER FOR SUPPLY OF '.$rel['po_sub'].'</b>
            <br>
         </td>
        
    </tr>   
    <tr style="border:none;">
        <td colspan="2" style="text-align: left; border: none; "><br>'.$comp_config['po_header_content'].'<br>
            </td>           
    </tr>   
</table><br><br>
	...
	<td  style=" text-align:left;"><b> ITEM DESCRIPTION</b></td>
	...
	while($trn_rel=brp_mysqli_fetch_array($trn_qry_rs)){
		...
		$html.='<td border="0" style="border-radious: 0px; border: none!important;'.$align.'">'.$trn_rel['product_name'].' '.$drawing_number.' '.$item_code.' '.$alias.''.$sno.' '.$wno.'</td> 
		</tr>
		...
		<td style=" text-align:right;">'.without_comma_two_digit_amount($sqty).' </td>
		<td style=" text-align:center;">'.$trn_rel['unit_name'].' </td>
		<td style=" text-align:center;">'.$rel['currency_symbol'].' '.without_comma_two_digit_amount($trn_rel['product_currency_rate']).'</td>';
			if($get_disc['discount']>0){
			...
			$html.='<td style=" text-align:right;"> '.$rel['currency_symbol'].' '.without_comma_two_digit_amount($trn_rel['product_currency_amount']).'</td>
			</tr>';
	...
	$html.='
	<tr>
	<td colspan="'.$colspan_8.'" style=" text-align:right;">TOTAL  </td>
	<td  style=" text-align:right;">'.$rel['currency_symbol'].' '.without_comma_two_digit_amount($ttl_amt,2).'</td>
	</tr>
	';
	if($rel['stateid']==$comp_rel['stateid']){
		$html.='<tr>
		<td colspan="'.$colspan_8.'" style=" text-align:right;">CGST</td>
		<td  style=" text-align:right;">'.$rel['currency_symbol'].' '.number_format(($total_cs_gst/2),2,".","").'</td>
		</tr><tr>
		<td colspan="'.$colspan_8.'" style=" text-align:right;">SGST</td>
		<td  style=" text-align:right;">'.$rel['currency_symbol'].' '.number_format(($total_cs_gst/2),2,".","").'</td>
		</tr>';
	}else{
		$html.='<tr>
		<td colspan="'.$colspan_8.'" style=" text-align:right;">IGST</td>
		<td  style=" text-align:right;">'.$rel['currency_symbol'].' '.number_format(($total_i_gst),2,".","").'</td>
		</tr>';
	}		
	...
	{
		$html.='<tr>
		<td colspan="'.$colspan_8.'" style=" text-align:right;"><b>'.$row11['l_name'].'</b></td>
	...
	{
		$html.='<tr>
		<td colspan="'.$colspan_8.'" style=" text-align:right;"><b>'.$row12['l_name'].'</b></td>
		<td style="text-align:center;border:1px solid;"><b>
	...
	$html .= '
	<tr>
	<td colspan="'.$colspan_8.'" style=" text-align:right;">Total Amount (In Figure) </td>
	<td  style=" text-align:right;font-weight:bold;">'.$rel['currency_symbol'].' '.without_comma_two_digit_amount(($rel['g_total_conv']),2).' </td>
	</tr>';
    ...
    <tr style="border:none;" >
<td colspan="2" style="border:none;padding-top:25px; text-align:left;">THIS PURCHASE ORDER FOR SUPPLY OF<b> “SMC” MAKE CYLINDER </b>IS SENT TO YOU BY MAIL AND HENCE, YOU
ARE REQUEST TO RETURN BY MAIL, THE CONFIRMATION COPY OF THE PURCHASE ORDER DULY SIGNED ALONG
WITH YOUR THE COMPANYS SEAL AS TOKEN OF YOUR ACCEPTANCE OF THE PURCHASE ORDER IN TOTO.
</td>
</tr>
<tr style="border:none;" >
<td  style="border:none;text-align:left;vertical-align:top;"> 
THANKING YOU, <br><b>
FOR, SIDDHIVINAYAK ENGINEERING. 
<br>
...
<span style="font-weight:bold;">RECEIVED & ACCEPTED IN TOTAL<br>FOR, '.$comp_rel['company_name'].'</span><br>

';	
	if($set_head['authorized_signature']!=""){
		$html.='<img src="'.DOMAIN_F.'view/upload/signature/'.$comp_rel['authorized_signature'].'" style="height: 100px; "><br>';
	}else{
        $html.='<br>';
        $html.='<br>';
        $html.='<br>';
    }
	$html .= '<span style="font-weight:bold;"> (AUTHORIZED SIGNATORY)  
</span><br>
</td>
</tr>
';
...
$mpdf=new mPDF('','A4','0','calibri','10','10','55','10','1','1');
