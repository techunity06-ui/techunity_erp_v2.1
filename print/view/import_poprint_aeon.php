...
//$sundrytax_rels = mysqli_fetch_assoc($sundrytax);
	...
	.page{
		...
		$html.='<div>
				...
				<tr>
					...
					<th style="text-align:center;border:1px solid;">Unit Price <br>'.$rel['currency_code'].'</th>
					<th style="text-align:center;border:1px solid;">Sub Total <br>'.$rel['currency_code'].'</th>
					<th style="text-align:center;border:1px solid;">Group Total <br>'.$rel['currency_code'].'</th>
				</tr>
				</thead>
				<tbody> ';

		$cat_q = "select sum(product_currency_amount) as group_total,cat.cat_name,po.cat_id from tbl_purchaseordertrn as po 
			left join tbl_category as cat on cat.cat_id=po.cat_id
			where po.purchaseordertrn_status=0 and po.purchaseorder_id=".$rel['purchaseorder_id']." group by po.cat_id";
		...
		while($row_cat = brp_mysqli_fetch_array($result_cat)){
			...
			where purchaseordertrn_status=0 and purchaseorder_id=".$rel['purchaseorder_id']." and trn.cat_id=".$row_cat['cat_id']."  order by purchaseordertrn_id asc";

			$result=$dbcon->query($qry);
			$k=1;
			$total=0;$discount=0;$totalqty=0;$charges_qty=0;$pcount=1;$total_cs_gst=0;$total_i_gst=0;$gst_per=0;$gst_rate=0;
			...
			while($row=brp_mysqli_fetch_assoc($result))
			{
				...
				if($k==1){
					...
					<td rowspan="'.$cnt.'" style="text-align:center;border:1px solid;vertical-align:center !important;">'.$row_cat['cat_name'].'</td>
					<td style="text-align:center;border:1px solid;vertical-align:top;">'.$row['product_icode'].'</td>
					<td style="text-align:left;border:1px solid;vertical-align:top;">
					<strong>'.$row['product_name'].'</strong></td>
					...
					<td style="text-align:center;border:1px solid;vertical-align:top;">'.$rel['currency_symbol'].' '.number_format($product_rate,2,".","").'</td>
					<td style="text-align:right;border:1px solid;vertical-align:top;">'.$rel['currency_symbol'].' '.number_format($row['currency_total'],2,".","").'</td>
					<td rowspan="'.$cnt.'" style="text-align:right;border:1px solid;vertical-align:center !important;white-space:nowrap">'.$rel['currency_symbol'].' '.number_format($row_cat['group_total'],2,".","").'</td>
					</tr>';
				}else{
					$html.='<tr style="border:none;border-left:1px solid;border-right:1px solid;border-bottom:1px solid;">
					<td style="text-align:center;border:1px solid;vertical-align:top;">'.$i.'</td>
		...
		$html.='<table style="font-size:12px;border-collapse: collapse;width:100%;" cellpadding="3" cellspacing="3">
			...
			<td style="text-align:right;border:1px solid;font-weight: bold;width:25%">'.$rel['currency_symbol'].' '.number_format($total_product_amount,2,".","").'</td>
		</tr>';
		...
		if($rel['stateid']==$set_head['stateid']){
			...
			while($row12=mysqli_fetch_assoc($result12))
				{
					$html.='<tr>
						...
						<td style="text-align:right;border:1px solid;font-weight: bold;">'.$rel['currency_symbol'].' '.number_format($row12['sundry_amount'],2,".","").'</td>
						</tr>';
				...
				while($row11=mysqli_fetch_assoc($result11))
				{
					$html.='<tr>
						...
						<td style="text-align:right;border:1px solid;font-weight: bold;">'.$rel['currency_symbol'].' '.number_format($row11['add_sum'],2,".","").'</td>
						</tr>';
				}
			...
			$html.='<tr>
				...
				<td style="text-align:right;border:1px solid;font-weight: bold;">'.$rel['currency_symbol'].' '.number_format(($sub_total),2,".","").'</td>
			</tr>';
			if($rel['quot_type']==0){
				$html.='<tr>
					
					<td colspan="2" style="text-align:right;border:1px solid; font-weight: bold;">CGST </td>
					<td style="text-align:right;border:1px solid;font-weight: bold;">'.$rel['currency_symbol'].' '.number_format(($total_cs_gst/2),2,".","").'</td>
				</tr>
				<tr>
			...
			if($rel['quot_type']==0){
				$html.='<tr>
					...
					<td style="text-align:right;border:1px solid;font-weight: bold;">'.$rel['currency_symbol'].' '.number_format(($total_i_gst),2,".","").'</td>
				</tr>';
			}			
		}
		...
		$html.='<tr>
				...
				<td style="text-align:right;border:1px solid;font-weight: bold;">'.number_format($rel['round_of_conv'],2,".","").'</td>
				</tr>
				<tr>
				
				<td colspan="2" style="text-align:right;border:1px solid; font-weight: bold;">Total Amount</td>
				<td style="text-align:right;border:1px solid;font-weight: bold;">'.$rel['currency_symbol'].' '.number_format($rel['g_total_conv'],2,".","").'</td>
				</tr>
				<tr>
				
				<td colspan="3" style="border-top:1px solid;text-align:left;"><b>Amount in words : 
			'.(($set_head['currency_id']==$rel['currency_id']) ? ucfirst(convert_number_to_words_new($rel['g_total_conv'],$rel['currency_in_word'],$rel['currency_in_word_end'])) : ucfirst(convert_number_to_words($rel['g_total_conv'],$rel['currency_in_word'],$rel['currency_in_word_end']))).'</b></td>
	...
	///////////////////////////////////////////////////////////////////Tax Calculation End////////////////////////////////////////////////////////			
				...
				<div style="margin-top:50px !important">
					<img src="'.DOMAIN_F.'view/upload/signature/'.$set_head['authorized_signature'].'" style="height: 100px; width: 180px;"><br> Authorised Signatory
				</div>
				</td>
				</tr>
       ...
       $pro_im ="select trn.*, trn.cat_id as product_cat, trn.product_qty as sh_qty,trn.product_conv_qty as sh_conv_qty,product.*,product.product_desc as scode,per.unit_name,per1.unit_name as base_unit_name,per2.unit_name as conv_unit_name, ca.cat_name FROM `tbl_purchaseordertrn` as trn 
		...
		left join unit_mst as per1 on per1.unitid=product.product_base_unit 
		left join unit_mst as per2 on per2.unitid=product.product_conv_unit
		where purchaseordertrn_status=0 and purchaseorder_id=".$rel['purchaseorder_id']."  order by cat_id,purchaseordertrn_id asc";
