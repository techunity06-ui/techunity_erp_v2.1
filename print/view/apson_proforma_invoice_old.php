...
if(strtolower($type) == 'pdf') {
    ...
    if($rel_que['performa_invoice_type']=='1'){
        ...
        $query="select invoice.*,country.country_name,payterms.payment_terms as payment_trm,state.state_name,cust.stateid,state.gst_state_code, city.city_name, cust.l_name as company_name,cust.m_address as cust_address, type.invoice_type,cust_pincode,cust_mobile,gst_no,cur.currency_in_word_end, cur.currency_in_word 
        from tbl_proforma_invoice as invoice 
...
//echo "<pre>";print_r($set_head);die();
    ...
    $html='';
    //$header = get_header($dbcon,'text-align: center','2in','70px').'<table style="text-align:center;border:none">
    $header ='<table style:"border:none; border-color:white">
            <tr>
...
<div>
    ...
    <tr style="border:1px solid;background-color:#bfbfbf">
    <td> 
    ...
    <table>
    <tr id="rawnone">
    <td  width="50%" style="border:1px solid;vertical-align:top;">
    <span style="font-weight:bold;font-size: 14px;text-decoration: underline;">Supplier : </span><br>
    <strong style="font-weight:normal;font-size: 14px;font-weight: bold">'.$set_head['company_name'].'</strong>
    <span style="font-weight:normal;font-size: 14px;">   
    <br/>'. $set_head['address'].'</span>
    <br>
    <span style="font-size: 14px">PH. '.$set_head['contact_no'].'</span>   
    <br>
    <span style="font-size: 14px">GSTIN: '.$cons_gst_no.'</span>
    </td>

    <td width="50%" style="border:1px solid;vertical-align:top;">
    <span style="font-weight:bold;font-size: 14px;text-decoration: underline;">Buyer : </span><br>
    <strong style="font-weight:normal;font-size: 14px;font-weight: bold">'.$rel['company_name'].'</strong>
    <span style="font-weight:normal;font-size: 14px;">   
    <br/>'. $rel['cust_address'].'</span>
    <br>
    <span style="font-size: 14px">PH. '.$rel['cust_mobile'].'</span>   
    <br>
    <span style="font-size: 14px">GSTIN: '.$rel['gst_no'].'</span>
    </td>
    </tr>
...
<table style="text-align:center;border:none">
    <tr>
        <td style="border:1px solid;background-color:#bfbfbf;width:17%;font-size: 12px">
            QUOTATION NUMBER
        </td>
        <td style="border:1px solid;width:17%;font-size: 12px">
        </td>
        <td style="border:1px solid;background-color:#bfbfbf;width:17%;font-size: 12px">
        ...
        <td style="border:1px solid;background-color:#bfbfbf;width:16.5%;font-size: 12px">
            CUSTOMER NAME
        </td>
        <td  style="border:1px solid;width:16.5%;font-size: 12px">
        </td>
    </tr>
    <tr>
        <td style="border:1px solid;background-color:#bfbfbf;width:17%;font-size: 12px">
...
<th width="5%" style="text-align:center;border:1px solid;border-top: 1px solid;"><strong>SR</strong></th>
<th width="30.5%" style="width:400px;text-align:center !important; border:1px solid;border-top: 1px solid;font-size:14px;" >
<strong>PRODUCT DESCRIPTION </strong>
</th>
<th width="15%" style="border:1px solid;border-top: 1px solid;font-size:14px;">
<strong>HSN CODE</strong>
</th>
<th width="10%" style="border:1px solid;border-top: 1px solid;font-size:14px;">
<strong>MOQ</strong>
</th>
<th width="9%" style="border:1px solid;border-top: 1px solid;font-size:14px;">
<strong>UNIT</strong>
</th>
<th width="15%" style="border:1px solid;border-top: 1px solid;font-size:14px;">
<strong>AESSABLE VALUE PER UNIT</strong>
</th>
<th width="15%" style="border:1px solid;border-top: 1px solid;font-size:14px;">
...
<td style="text-align:center !important; vertical-align:top;border-left:1px solid;">';
...
$html.='<td style="width:400px;font-size:14px;vertical-align:top;border-left:1px solid;'.$clas.'" >
<strong>'.stripcslashes($row['product_name']).'</strong><br/>';
$html.= ($row['description']!='' && $row['description']!='0')?nl2br(stripcslashes($row['description'])):'';
$html.= '</td>
<td style="vertical-align:top; white-space:nowrap;font-size:14px;border-left:1px solid;border-bottom:none;text-align:right">'.$row['product_hsn'].'</td>
<td style="vertical-align:top; white-space:nowrap;font-size:14px;text-align:right;border-bottom:none;border-left:1px solid;" >';
if($row['product_type']!='8'){ 
...
$html.='</td>
<td style="vertical-align:top;font-size:14px;text-align:right;border-left:1px solid;" >';
if($row['product_type']!='8'){ 
 $html.=$row['unit_name'];
}
$html.='</td>
...
$html.='</td>
<td style=" vertical-align:top;font-size:14px;text-align:right;border-left:1px solid;border-right:1px solid;">
...
for($j=0; $j<$pr; $j++){
  ...
  $html.='<tr style="height:35px;border-bottom:none;border-top:none;border-left:1px solid;">
  ...
  $html.='<tr style="height:35px;border-bottom:none;border-top:1px solid;border-left:1px solid;border-right:1px solid;">
            <td colspan="3" style="border-left:1px solid;text-align:right;">Total</td>
            <td style="border-left:1px solid;text-align:right">'.$totalqty.'</td>
      <td style="border-left:1px solid;"></td>
      <td style="border-left:1px solid;"></td>
      
      <td style="border-left:1px solid;border-right:1px solid;">'.$final_amount.'</td>
  ';
  ...
  <tr>
  <td colspan="4" rowspan="'.$rowsp12.'" width="50%" style="border:1px solid">
  ...
  <br><br>
  <span style="font-weight:bold;font-size: 14px;">Payable Amount In Words : </span><br>
  <span style="font-weight:bold;font-size: 14px;">
    '.ucwords(convert_number_to_words_new($rel['payable_amt'],$rel['currency_id'],$rel['currency_in_word_end'],$rel['currency_in_word'])).' 
  </span>
  <br>
  <br>
...
<span style="text-align:right">'.number_format(($tax_amnt+$total_sundrytax),2,".","").'</span>';
$html.='
</td>
</tr>';
$html .='<tr>
...
<tr>
<td style="border:1px solid;font-weight:normal;font-size: 14px;">Advance Amount
</td>
<td style="border:1px solid;font-weight:normal;font-size: 14px;text-align:right">'.number_format($rel['advance_payment'],2,".","").'
</td>
</tr>
<tr>
<td style="border:1px solid;font-weight:normal;font-size: 14px;"><strong>Payable Amount ( '.$rel['payable_per'].' %)</strong>
</td>
<td style="border:1px solid;font-weight:normal;font-size: 14px;text-align:right"><strong>'.number_format($rel['payable_amt'],2,".","").'</strong>
</td>
</tr>
<tr>
<td style="border:1px solid;font-weight:normal;font-size: 14px;">Pending Amount
</td>
<td style="border:1px solid;font-weight:normal;font-size: 14px;text-align:right">'.number_format($rel['pending_amt'],2,".","").'
</td>
</tr>
<tr id="rawnone">
...
<tr id="rawnone">
<td colspan="6" width="50%" style="border:1px solid">
<span style="font-weight:bold;font-size: 14px"> Remarks : '.$rel['remark'].' </span>
</td>
</tr>
</tbody></table>';
