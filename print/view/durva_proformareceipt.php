...
//echo "<pre>";print_r($set_head);die();
    ...
    $header = get_header($dbcon,'text-align: center','','100px');
...
<table style="text-align:center;border:none">
    ...
    if(empty($rel['consignee_id'])){ 
      ...
      $html.= $cons_data['cust_address'];
      $html.=' <br/>';
      $html.=$cons_data['city_name'].','. $cons_data['state_name'].','. $cons_data['country_name'];
...
$qry="select trn.*,product.*,unit_name FROM `tbl_proforma_trn` as trn left join product_mst as product on product.product_id=trn.product_id left join unit_mst as per on per.unitid=trn.unit_id where trancation_status=0 and invoice_id=".$rel['invoice_id']." and pid=0 group by trancation_id order by product_type,trancation_id";
...
$qry1="select trn.*,product.*,unit_name FROM `tbl_proforma_trn` as trn left join product_mst as product on product.product_id=trn.product_id left join unit_mst as per on per.unitid=trn.unit_id where trancation_status=0 and invoice_id=".$rel['invoice_id']." and pid=".$row['trancation_id']." group by trancation_id order by product_type,trancation_id";
$result1=$dbcon->query($qry1);
$j=1;
while($row1=brp_mysqli_fetch_assoc($result1))
{
    $gst_per = $row1['cgst_tax_per']+$row1['sgst_tax_per']+$row1['igst_tax_per'];
    $gst_rate = $row1['cgst_tax_rate']+$row1['sgst_tax_rate']+$row1['igst_tax_rate'];

    if($row1['cgst_tax_rate'] != 0 || $row1['sgst_tax_rate'] !=0){
        $total_cs_gst += $gst_rate;
    }else{
        $total_i_gst += $gst_rate;
    }
        //tax summary calculation start
    if(!empty($row1['tax_val']))
    {
        $tax_num=explode(",",$row1['tax_val']);
        $tax_name=explode(",",$row1['tax_name']);
        $total_net_rate=($row1['product_qty']*$row1['product_rate'])-$row1['discount'];
        for($j=0;$j<count($tax_num);$j++)
        {
            if(!in_array($tax_name[$j],$tax['per']))
            {
    ...
    $html.='<tr style="height:35px;border-bottom:none;border-top:none;">
    <td style="text-align:center !important; vertical-align:top;">';
    if($row1['product_type']!='8'){ $html.= $i.'.'.$j;}
    $html.='</td>';
    $clas = '';
    if($row1['product_type']=='8'){
        $clas = 'text-align:right;padding-top:5px;vertical-align:top;';
    }
    $html.='<td style="width:400px;font-size:14px;border-right:1px solid;vertical-align:top;'.$clas.'" >
    <strong>'.stripcslashes($row1['product_name']).'</strong><br/>';
    $html.= ($row1['description']!='' && $row1['description']!='0')?nl2br(stripcslashes($row1['description'])):'';
    $html.= '</td>
    <td style="vertical-align:top; border-right:1px solid;white-space:nowrap;font-size:14px;text-align:right" >';
    if($row1['product_type']!='8'){ 
       $html.=$row1['product_qty'];
    }else{
        $charges_qty+=$row1['product_qty'];
    } 
    $html.='</td>
    <td style="vertical-align:top;font-size:14px;text-align:right" >';
    if($row1['product_type']!='8'){ 
    ...
    <td style="vertical-align:top;font-size:14px;text-align:right" >';
    if($row1['product_type']!='8'){ 
     $html.=number_format($row1['product_rate'],2,".","");
    }
    $html.='</td>
    <td style=" vertical-align:top;font-size:14px;text-align:right">
    '.number_format($row1['product_amount'],2,".","").'
    </td>
    </tr>';

    $j++; 
    $totalqty=$totalqty+$row1['product_qty']-$charges_qty;
    $totalsqr=$totalsqr+$row1['sqr_ft']-$charges_qty1;
    $total_product_amount+=($row1['product_qty']*$row1['product_rate']);
    $totaltaxable+=$row1['product_amount'];
    $total+=$total_net_rate;
    $total_packing +=$rel['packing'];
