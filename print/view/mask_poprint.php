...
//Quotation Data
	...
	$user_qry = "select user.user_name, user.user_mail, user.user_phone, user.user_type, led.common_email_id, user.authorized_signature from users as user
	left join tbl_ledger as led on led.l_id=user.employee_id
	where user.user_id=".$rel['userid']." and user.company_id=".$rel['company_id'];
...
//$header ='<div style="text-align:right;"><img src="'.DOMAIN_F.LOGO.'Hermettic_Equipments.png" style="width:8.27in;padding-top:20px;" /></div>';  
	...
	$header = get_header($dbcon,'text-align: left','100%','150px');
...
//$sundrytax_rels = mysqli_fetch_assoc($sundrytax);
	...
	.page{
		...
		while($row=mysqli_fetch_assoc($result))
		{
			...
			$html.='<tr>
			<td style="text-align:center;vertical-align:top;border-bottom:1px solid">'.$i.'</td>
			<td style="text-align:left;vertical-align:top;border-bottom:1px solid">
			...
			$html .='</td>
			<td style="text-align:center;vertical-align:top;border-bottom:1px solid">'.$row['product_hsn_code'].'</td>
			<td style="text-align:center;vertical-align:top;border-bottom:1px solid">
			'.$pqty.'
			</td>
			<td style="text-align:center;vertical-align:top;border-bottom:1px solid">'.number_format($product_rate,2,".","").'</td>';
			/*if($get_disc['discount'] > 0.00){
			...
			$html.='<td style="text-align:center;vertical-align:top;border-bottom:1px solid">'.$gst_per.' %</td>
			<td style="text-align:right;vertical-align:top;border-bottom:1px solid">'.number_format($gst_rate,2,".","").'</td>
			<td style="text-align:right;vertical-align:top;border-bottom:1px solid">'.number_format($row['currency_total'],2,".","").'</td>
			</tr>';
		...
		<tr>
			<td colspan="3" style="text-align:left;border:1px solid; font-weight: bold;">* Note : The Material Should Be Sent As Per The Qty In Nos Mentioned in Purchase Order. In Case Of Variable UOM i.e. Kgs. / Ltr / Mtr / Feet etc., The Amount Of Purchase Order Will Be Approximate And Will Be Apprximate And It Will Be Accepted On Actual Basis.</td>
		</tr>
		';
		...
		$html.='<tr>
				...
				<td colspan="3" style="border-left:1px solid;border-right:1px solid;text-align:left;"><b>Amount in words : 
	...
	///////////////////////////////////////////////////////////////////Tax Calculation End////////////////////////////////////////////////////////			
				if(empty($user_data['authorized_signature'])){
	                $signature = "<br><br><br><br>";
	            }else{
	                $signature = '<img src="'.DOMAIN_F.'view/upload/signature/'.$user_data['authorized_signature'].'" height="130" width="150" class="img-thumbnail" />';
	            }

				$html.='<table style="font-size:12px;border-collapse: collapse;width:100%;" cellpadding="3" cellspacing="3"><tr style="">
				<td colspan="" rowspan="0" style="height:80px; width:70%;  text-align:left; vertical-align:top; border-left:1px solid; border-bottom:1px solid !important;border-top:1px solid; font-weight: bold;"><strong>Remarks:</strong><br> '.$rel['remark'].'</td>
				<td colspan="" style="text-align:center;vertical-align:top;font-weight: bold;height:80px; width:30%;border-top:1px solid;border-bottom:1px solid;border-right:1px solid">For, '.$set_head['company_name'].'<br>
					'.$signature.'
				<div style="margin-top:50px !important"> Authorised Signatory</div>
				</td>
				</tr>
