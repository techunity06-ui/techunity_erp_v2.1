								...
								<div class="col-lg-12 table-responsive" id="receipt_print">	<div class="col-md-12" style=" margin-top:10px;" id="print1">
									...
									<table width="100%" class="maintable" style="font-size: 12px;margin-top: 5px;" id="invoice_type" >
										...
										<tbody style="border: 1px solid;">
											<?php$qry="select trn.*,product.product_name,product.product_hsn,per.unit_name, po.product_qty as poqty,po.product_conv_qty as poconvqty,cnv.unit_name as cnvunit FROM `tbl_grn_trn` as trn 
											left join product_mst as product on product.product_id=trn.product_id 
											left join unit_mst as per on per.unitid=trn.unit_id
											left join unit_mst as cnv on cnv.unitid=trn.product_conv_unit
											...
											{
												$pending_qty='';
												$pending_qty = $row['poqty'] - $row['product_qty'];
												$conv_pen_qty = '';
												if($row['unit_id'] != $row['product_conv_unit']){
													$conv_pen_qty = $row['poconvqty'] - $row['product_conv_qty'];
													$conv_qty = number_format($row['product_conv_qty'],2,".","").' '.$row['cnvunit'];
													$conv_pqty = number_format($conv_pen_qty,2,".","").' '.$row['cnvunit'];
													$poconvqt = number_format($row['poconvqty'],2,".","").' '.$row['cnvunit'];
												}
												?>
												<tr style="border-top: 1px solid #9a9a9a;border-bottom: 1px solid #9a9a9a;">
													...
													<td style="border-right:1px solid;vertical-align: top;" colspan="2">
														<?php //=$row['product_hsn']?>&nbsp;&nbsp;<strong><?=stripcslashes($row['product_name'])?></strong>
														</td>
														<td style="border-right:1px solid;vertical-align:top;text-align:center" >
															<?=number_format($row['poqty'],2,'.','').' '.$row['unit_name']?><br><?=$poconvqt?>
														</td>
														...
														<td style="text-align:center;vertical-align:top;border-right:1px solid;">
															<?=number_format($pending_qty,2,".","").' '.$row['unit_name']?>
															<br>
															<?=$conv_pqty?>
														</td>
														<td style="text-align:center;vertical-align:top;border-right:1px solid;">
												...
												<tr style="border-top: none;border-bottom: none;">
													...
													<td colspan="6">
														: <?=$rel['test_certificate']?>
													</td>
												</tr>
												<tr style="border-top: none;border-bottom: none;">
													<td colspan="4">
														<strong>Test certificate - Reviewed as Per Code</strong>
													</td>
													<td colspan="6">
														: <?=$rel['test_certificate_code']?>
													</td>
												</tr>
												<tr style="border-top: none;border-bottom: none;">
													<td colspan="4">
												...
												<tr style="height: 60px;vertical-align: top;border-top: 1px solid;">
													<td colspan="3" rowspan="2" style="border-right: 1px solid;width: 50%;">
