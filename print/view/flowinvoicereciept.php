...
if(strtolower($type) == 'pdf') {
	...
	if($company_config['finance_print_letterhead_per']==0){
		...
		<tr>
			...
			<td style="width:12%;text-align: left; border-right:none; border-bottom:none; font-size:14px;"><strong>GSTIN</strong></td>
			<td style="width:25%;text-align: left;border-left:none;border-right:none;border-bottom:none; font-size:14px;"> : <strong>'.$set_head['vatno'].'</strong></td>
			<td style="width:12%;text-align: left;border-left:none;border-right:none;border-bottom:none;font-size:14px;"><strong>Invoice No</strong></td>
			<td style="width:25%;text-align: left;border-left:none; border-right:none; border-bottom:none;font-size:14px;"> : '.$rel['invoice_no'].'</td>
			<td rowspan="7" style="width:26%;text-align: left;border-left:none;vertical-align:top;font-size:14px;">';

			if(!empty($rel['einv_SignedQRCode'])){
				$header.='<img style="height: 150px;width: 150px;"  src="data:image/png;base64,'.$rel['einv_SignedQRCode'] .'" />';
			}
			$header.='</td>
		</tr>

		<tr>
			<td colspan="2" style="text-align: left;border-top:none; border-bottom:none;border-right:none;font-size:14px;"><strong>'.$set_head['company_name'].'</strong></td>
			...
			<td style="text-align: left;border-top:none; border-bottom:none; border-right:none; border-left:none;font-size:14px;"> : '.date('d-m-Y',strtotime($rel['invoice_date'])).'</td>
		</tr>
		<tr>
			<td colspan="2" rowspan="2" style="text-align: left;border-top:none; border-right:none;border-bottom:none;font-size:14px;">'.$set_head['address'].'</td>
			<td style="text-align: left;border:none;font-size:14px;"><strong>Challan No</strong></td>
			<td style="text-align: left;border:none;font-size:14px;"> : '.$rel['challan_no'].'</td>
		</tr>
		<tr>
			<td style="text-align: left;white-space:nowrap;border:none;font-size:14px;"><strong>Challan Date</strong></td>
			<td style="text-align: left;border:none;font-size:14px;"> : '.date('d-m-Y',strtotime($rel['challan_date'])).'</td>
		</tr>
		
		<tr>
			<td style="border-top:none;border-right:none;border-bottom:none;font-size:14px;"><strong>IRN No.</strong></td>
			<td colspan="3" style="border:none;font-size:14px;"> : '.$rel['einv_Irn'].'</td>
		</tr>
		<tr>
			<td style="border-top:none;border-right:none;font-size:14px;"><strong>Ack No.</strong></td>
			<td colspan="2" style="border-left:none;border-top:none;border-right:none;font-size:14px;"> : '.$rel['einv_AckNo'].' &nbsp;&nbsp; <strong>Ack Date : </strong>'.$einv_AckDate.' </td>
			<td style="border-top:none; border-left:none; border-right:none"> </td>
		</tr>

		<tr>
			<td colspan="3" style="border-right:none;vertical-align:top;font-size:13px;height:200px">
			...
			<td colspan="2" style="border-left:none; vertical-align:top; font-size:14px;">
	...
	.page{
		...
		$qry="select trn.*,so.po_no,so.po_date,product.product_name,unit_name FROM `tbl_invoicetrn` as trn 
		...
		while($row=brp_mysqli_fetch_assoc($result))
		{
			...
			<td style="border-right:1px solid;vertical-align:top;border-bottom:none;border-top:none"><strong>'.$row['product_name'].'</strong><br>PO NO & PO Date : '.$row['po_no'].' & '.date('d-m-Y',strtotime($row['po_date']));
		...
		$html .='<tr class="amt_summary">
			<td colspan="4" style="border-bottom:none"><strong>Transporter : </strong> '.$r_t['transportation_name'].' &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <strong>L.R. No :</strong>'.$r_t['transport_gr_no'].' </td>
		...
		if($rel['stateid']==$set_head['stateid'] && ($custLedgerDetails['enable_sez'] == 0)){
			...
			$html.='<tr style="height:25px;" class="amt_summary">
				...
				<td class="borderleftadd" colspan="4" style="vertical-align:top; border-bottom:none; border-top:none"><strong>Vehicle No. :</strong> '.$r_t['transport_vehicle_no'].' &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;  <strong>L.R. Date :</strong> '.$transport_gr_date.'</td>
			...
			<tr style="height:25px;" class="amt_summary">
				<td colspan="4" style="border-right:1px solid;border-top:none; border-bottom:none; border-top:none"></td>
			...
			$html.='<tr style="height:25px;" class="amt_summary">
				<td colspan="4" style="border-right:1px solid;border-bottom:none;border-top:none"><strong>Vehicle No. :</strong> '.strtoupper($r_t['transport_vehicle_no']).' &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <strong>L.R. Date :</strong> '.$transport_gr_date.'</td>
		...
		foreach ($get_bill_sundry as $billsundry) {
			...
			if(($billsundry['l_name'] == 'TCS') && ($company_config['enable_tcs_reporting'] == 1) && ($custLedgerDetails['enable_tcs']==1) && ($arr >= $company_config['gross_balance_limit'])){
				...
				$html.='<tr style="height:25px;" class="amt_summary">
				<td colspan="4" style="border-right:1px solid; border-top:none; border-bottom:none;"></td>
		...
		while($row12=brp_mysqli_fetch_assoc($result12)){
			$html.='<!-- Added By Dhruv -->
			<tr style="height:25px;" class="amt_summary">
				<td colspan="4" style="border-right:1px solid; border-bottom:none; border-top:none"></td>
		...
		$html.='<tr class="amt_summary">
			<td style="border-top:none;border-right:1px solid;border-left:1px solid; text-align:center;" colspan="4"></td>
		...
		$mpdf=new mPDF('','A4','0','calibri','10','10','140','40','1','1');
		...
		//$mpdf->SetFooter($footer.'|{PAGENO} of {nbpg}|');
