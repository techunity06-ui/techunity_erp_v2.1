...
while($sumsundrytax=mysqli_fetch_assoc($sundrytax)){
	...
	<table>
	<tr>
	<td style="border: 0px; "><img src="'.DOMAIN_F.LOGO.$comp_rel["logo"].'" style="width:8.27in;padding-top:25px;" /></td>
	</tr>
	</table>';
	...
	.page{
		...
		<td colspan="9" style=" text-align:center;"><b>Vendor / Supplier Details</b> </td>
		</tr>
		<tr style="border:none; border-left:1px solid; border-right:1px solid;">
		<td colspan="9" style="border-left:1px solid;border-bottom:none; text-align:left;">'.$party_address_billing.' </td>
		</tr>
		</table>
		</div>
		...
		<tbody>';
		$qry="SELECT trn.*,product.*,dr.drawing_number,trn.product_conv_qty as trn_conv_qty, per.unit_name, unit.unit_name as unit_name1 FROM `tbl_purchaseordertrn` as trn 
		...
		while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
			...
			<td style="vertical-align: top; border: none;text-align:left;font-size:12px;">
			...
			$html.='<td border="0" style="border-radious: 0px; border: none!important;font-size:11px;font-weight:bold'.$align.'">'.$trn_rel['product_icode'].'<br/>'.$trn_rel['product_name'].' '.$drawing_number.' '.$alias.' '.$wno.''.$trn_rel['product_des'].'</td> 
			</tr>
			<tr border="0" style="border-radious: 0px; border: none;">
			<td border="0" style="border-radious: 0px; border: none!important;font-size:11px;'.$align.'">'.$trn_rel['product_des'].'</td>
			</tr>
			</table>';
			...
			$html .='</td>
			<td colspan="2" style="vertical-align: top; text-align:center;font-size:12px;"> '.$trn_rel['product_hsn_code'].'</td>

			<td style="vertical-align: top; text-align:center;font-size:12px;">'.$purchaseorder_due_date.'</td>
			
			<td style="vertical-align: top; text-align:right;font-size:12px;">'.($sqty).'<br>'.($cqty).'</td>
			<td style="vertical-align: top; text-align:center;font-size:12px;">'.$unit.'<br>'.$unit1.'</td>
			<td style="vertical-align: top; text-align:right;font-size:12px;"> '.without_comma_two_digit_amount($trn_rel['product_rate']).'</td>
    ...
    //$round_off = round($final_total)-$final_total;
		...
		if($rel['stateid']==$comp_rel['stateid']){
			...
			'.without_comma_two_digit_amount($total_cs_gst/2).'
			</td>
			<td style="text-align:right;font-size:13px;font-weight:bold" colspan="2">
			'.without_comma_two_digit_amount($total_cs_gst/2).'
			</td>
			<td colspan="2" style=" text-align:right;font-size:13px;font-weight:bold">
		'.without_comma_two_digit_amount($total_cs_gst).'
		</td>';
		} else{
			$html.='<td style="text-align:right;font-size:13px;font-weight:bold" colspan="2">
			'.without_comma_two_digit_amount($total_i_gst).'
			</td>
			<td colspan="2" style=" text-align:right;font-size:13px;font-weight:bold">
		'.without_comma_two_digit_amount($total_i_gst).'
		</td>';
		...
		<tr>
		<td colspan="8">Terms & Conditions : <br>'.(($rel['po_condition']) ? $rel['po_condition'] : '').'</td>
		</tr>
		<tr style="border-bottom:none">
		<td colspan="2" style="height:50px;border-bottom:none;vertical-align:top;text-align:center;width:20%">'.$user_rel['user_name'].'
		</td>
