...
//p($rel);
		...
		$header ='<div style="text-align:center;">
			<h1>'.$comp_rel['company_name'].'</h1>
			<h3>GST NO '.$comp_rel['vatno'].'</h3>
		</div>';
...
//Amish Soni Start 16-03-2021
		...
		if($rel['quot_header']){
			...
			$quotation_print_content = $quotation_print_content;
		}
...
//Amish Soni End 16-03-2021
		...
		.page{
			...
			<tr style="border: 0;">
			<td style="border: 0; width:50%;text-align: left; font-size: 16px;">Ref No : '.$rel['quotation_no'].'</td>
			<td style="border: 0; width:50%;text-align: left; font-size: 16px;">Date: '.date("d-M-Y",strtotime($rel['quotation_date'])).'</td>
			</tr>
			</table>
			...
			<tr style="border: 0;">
				<td style="border: 0;text-align:center"><span style="font-size:16px">
				KIND ATTN : '.$rel['c_con_fname'].' '.$rel['c_con_lname'].'
			</span></td>
			</tr>
			<tr style="border: 0;">
				<td style="border: 0;text-align:center"><span style="font-size:16px">
				SUB : '.$rel['quot_subject'].'
			</span></td>
			</tr>
			<tr style="border: 0;">
			<td style="border: 0;">
			'.stripslashes($quotation_print_content).'<br><br>
			...
			while($row_im=brp_mysqli_fetch_array($pro_im_rs)){
				if(!empty($row_im['image_name'])){
					$html .= '<div><center style="text-align:center"><h3>'.$row_im['product_name'].'</h3></center><img src="'.DOMAIN_F.'view/upload/product_images/'.$row_im['image_name'].'" style="width:800px;height:600px"></div><center class="nextpage"></center>';
				}else{
					$html .='';
				}
			}
		...
		//tax summary calculation start
			...
			if(mysqli_num_rows($terms_qry_rs)){
				...
				<center style="text-align:center"><span style="font-size:16px">ANNEXURE VII – TERMS & CONDITIONS FOR SALE</span></center><br>';
				while($term_rel=mysqli_fetch_assoc($terms_qry_rs)){
					$string=(nl2br($term_rel['tc_details']));
					$html.='<strong style="font-size:16px">'.$term_rel['tc_name'].'</strong><br>
