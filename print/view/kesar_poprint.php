								...
								<!--<center>-->
									...
									<div class="col-lg-12 table-responsive" id="receipt_print">	<div class="col-md-12" style=" margin-top:10px;" id="print1">
										...
										<table width="100%" class="maintable" border="1" id="table_head">
											<?=get_header($dbcon,'text-align: center','5.7in','150px');?>
										</table>
			...
			<tr>
				<th colspan="11" style="padding:0px !important;">
					<table style="font-size:12px;border-collapse: collapse;border-top:none;" cellpadding="0" cellspacing="0" width="100%" >
						<tr>
							<td colspan="2" style="text-align:right;width:100%;"><strong style="font-size:14px;"><?=$form?></strong></td>
							<td colspan="2" style="text-align:right;width:100%;"><input type="checkbox">&nbsp;Original&nbsp;&nbsp;<input type="checkbox">&nbsp;Duplicate&nbsp;&nbsp;<input type="checkbox">&nbsp;Triplicate</td>
						</tr>
						<tr>
							...
							<td width="50%" colspan="2" style="vertical-align:top;border:1px solid;">
								...
								<td width="50%" colspan="2" style="vertical-align:top;border:1px solid;">
									...
									<!-- <span style="font-weight:normal;">   <br/>
										<?=$cons_cust_address?>
										<br/>
										<?=$cons_city_name?>, <?=$cons_state_name?>, <?=$cons_country_name?></span>
										<br/>GST No. : <?=$cons_gst_no?> -->
									</td>
								</tr>
							</table>
        	...
        	$wno = "<br> <strong>Work Order No : ".$rw['work_order_no']."</strong>";
				...
				<tr style="height:40px">
					...
					<td style="border-bottom-color:#FFFFFF; border-right:1px solid;vertical-align:top;" colspan="2">
						...
						<?php if($row['product_alias_name']){?>
							...
							<br/><strong>S.O No :</strong><?=stripcslashes($rw['so_order_no'])?>
							<br/><strong>W.O NO :</strong><?=stripcslashes($rw['work_order_no'])?>
							<?php} ?>
				...
				for($j=0; $j<$pr; $j++)
				{
					...
					<tr style="height:40px">
						...
						<tr style="height:20px">
							...
							<?phpif($set_head['show_disc']=='1'){?>
								...
								<td style="border-top:1px solid;border-right:1px solid;text-align:right;"><strong><?=number_format($gst_rate,2,".","")?></strong></td>
								...
								<td colspan="11" style="padding: 0px !important;border:1px solid">
									<table class="footer-table" width="100%">
										<tr width="61.6%" height="20px">
											<td style="border-right:1px solid;" colspan="<?=$colspan?>"></td>
										...
										<?php if($rel['stateid']==$set_head['stateid']){?>
											<tr height="20px">
												...
												<td colspan="3" style="border-top:1px solid;border-right:1px solid;text-align:left" >CGST</td>
												<td colspan="2" style="text-align:right; border-top:1px solid;border-left:1px solid; "><?=number_format(($total_cs_gst/2),2,".","")?></td>
											</tr>
											<tr height="20px">
												<td style="border-right:1px solid;border-top:1px solid;" colspan="<?=$colspan?>"></td>
												<td colspan="3" style="border-top:1px solid;border-right:1px solid;text-align:left" >SGST</td>
												<td colspan="2" style="text-align:right; border-top:1px solid;border-left:1px solid; "><?=number_format(($total_cs_gst/2),2,".","")?></td>
											</tr>
										<?php } else { ?>
											<tr height="20px">
												<td style="border-right:1px solid;border-top:1px solid;" colspan="<?=$colspan?>"></td>
												<td colspan="3" style="border-top:1px solid;border-right:1px solid;text-align:left" >IGST</td>
												<td colspan="2" style="text-align:right; border-top:1px solid;border-left:1px solid; "><?=number_format(($total_i_gst),2,".","")?></td>
											</tr>
										<?php }
										...
										while($row11=mysqli_fetch_assoc($result11)) { ?>
											<tr height="20px">
												<td style="border-right:1px solid;border-top:1px solid;" colspan="<?=$colspan?>"></td>
												<td colspan="3" style="border-top:1px solid;border-right:1px solid;text-align:left" ><?=$row11['l_name'];?></td>
												<td colspan="2" style="text-align:right; border-top:1px solid;border-left:1px solid; "><?=number_format(($row11['add_sum']),2,".","")?></td>
											</tr>
										<?php }
										...
										while($row12=mysqli_fetch_assoc($result12)) { ?>
											<tr height="20px">
												<td style="border-right:1px solid;border-top:1px solid;" colspan="<?=$colspan?>"></td>
												<td colspan="3" style="border-top:1px solid;border-right:1px solid;text-align:left" ><?=$row12['l_name'];?></td>
												<td colspan="2" style="text-align:right; border-top:1px solid;border-left:1px solid; "><?=number_format(($row12['sundry_amount']),2,".","")?></td>
											</tr>
										<?php }
										...
										<tr height="20px">
											...
											<td colspan="2" style="text-align:right; border-top:1px solid;border-left:1px solid; "><?=number_format($r+$rel['round_off'],2,".","")?></td>
										</tr>

										<tr height="20px">
											<td style="border-right:1px solid;border-top:1px solid;" colspan="<?=$colspan?>">
												<strong>Rupees:</strong>
												<?=ucwords(convert_number_to_words(round($rel['g_total'])))?>
											</td>
											<td colspan="3" style="border-top:1px solid;border-right:1px solid;text-align:left"><strong>Grand Total</strong> </td>
											<td colspan="2" style="text-align:right; border-top:1px solid;font-size:13px;border-left:1px solid; "><strong><?=number_format($rel['g_total'],0,".","").'.00'?></strong></td>
										</tr>
										<tr>
											<td colspan="<?=$colspan?>" style="vertical-align:top;font-size:10px;text-align:left;border-top:1px solid;" class="con">
												...
												<?phpif(!empty($set_head['po_condition'])){ ?>
													<strong>Terms and Conditions:</strong><br> <?=(($set_head['po_condition']) ? $set_head['po_condition'] : '')?>
												<?php} ?>	

												<strong>Terms and Conditions:</strong><br> <?=(($rel['po_condition']) ? $rel['po_condition'] : '')?>
					...
					<tr style="border: none;">
						<td colspan="<?=5+$colspan?>" style="border:none;padding:0px 0px !important;width:100%;"> 
