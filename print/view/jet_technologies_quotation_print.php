...
//Quotation Data
		$query="select quot.*,per.c_con_fname,per.c_con_lname,per.c_con_mobile,per.c_con_email,cust.cust_name, cust.cust_email, cust.cust_mobile, inq.inquiry_no,inq.inquiry_date, ref.rb_name, cadd.c_add_state, usr.user_name from tbl_quotation as quot
...
//p($rel);
		...
		$header ='<div style="text-align:center;"><img src="'.DOMAIN_F.LOGO.$comp_rel["logo"].'" style="height:1.57in" /></div>';
		if(!empty($comp_rel["f_logo"])){
			$footer ='<div style="text-align:center;"><img src="'.DOMAIN_F.LOGO.$comp_rel["f_logo"].'" style="width:8.27in;" />123</div>';
		}
		$approve_status='';
...
//Amish Soni End 16-03-2021
		...
		.page{
			...
			if(!empty($quotation_print_content)){
				$html.='<div>
			...
			<br />
			<br />
			<strong>Kind Attn. : '.$rel['c_con_fname'].',</strong><br/>
			<strong>Contact Detail : '.$rel['c_con_mobile'].',</strong>
			</td>
		...
		//tax summary calculation start
			...
			'.(($rel['quot_remark']) ? $rel['quot_remark'] : '').'</td> -->
			</tr></tbody></table></div>';
			...
			if(!empty($comp_rel['authorized_signature'])){
				$authorized_signature = '<img src="'.DOMAIN_F.'view/upload/signature/'.$comp_rel['authorized_signature'].'" style="height: 100px; width: 100px;">';
			}
			$html .='<table cellpadding="5" cellspacing="5" border="0" style="font-size: 14px; font-family: Proxia Nova; border: 0;">
						<tr style="border:none">
					<td style="text-align:right;font-weight:bold;border:none;font-size:18px">Best Regards, <br>'.$comp_rel['company_name']. '<br><!--<span style="font-size:12px">'.$rel['user_name']. '</span><br>--></td>
				</tr>
				<tr style="border:none;">
