...
//echo "<pre>";print_r($rel);die();
	...
	.page{
		...
		<tr style="border:none; border-left:none ; border-right:none ; ">
		<td style=" text-align:left; border-right:1px solid ;">Payment Terms: '.$rel['payment_terms'].'</td>
		...
		<td style=" text-align:left; width:50%;"><b>Part Code / Goods Description</b></td>
		...
		<td style=" text-align:center; width:15%;"><b>Unite Rate <br>'.$currency_name.'</b> </td>';
		if($disc_qrys['discount'] > 0){
			$html.='<td style=" text-align:center;"><b>Disc<br>'.$currency_name.'</b></td>';
		}
		$html.='<td style=" text-align:center;"><b>Amount<br>'.$currency_name.'</b> </td>
		</tr>
		</thead>
		<tbody></table>';
		...
		while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
			...
			<td style="border: none;text-align:left; vertical-align: top;  width:52%;">'.$trn_rel['product_name'].''.$alias_name.'<br>'.$product_desc;
			if($delivery_type == 'product_wise'){
			...
			<td style=" text-align:center;font-size:16px; vertical-align: top;  width:10%;"> '.without_comma_two_digit_amount($trn_rel['product_rate']).'</td>';
			if($disc_qrys['discount'] > 0){
				$html.='<td style=" text-align:center;font-size:16px; vertical-align: top;">'.without_comma_two_digit_amount($trn_rel['discount_per']).' % <br>('.without_comma_two_digit_amount($trn_rel['product_discount']).')</td>';
			}
			$html.='<td style=" text-align:center;font-size:16px; vertical-align: top;  width:15%;"> '.without_comma_two_digit_amount($trn_rel['product_amount']).'</td>
			</tr>';
		...
		while($row12=mysqli_fetch_assoc($result12))
		{
			...
			<td  style=" text-align:center;font-size:16px;">'.number_format($row12['sundry_amount'],2,".","").'</td>
			</tr>';
		}
		...
		<td  style=" text-align:center;font-size:16px;font-weight:bold;">'.indian_number((round($rel['g_total'])),2).' </td>
		</tr>
		<tr style="">
		<td colspan="'.($colspan+1).'" style=" text-align:left;font-size:16px;">Total Order Value (In Words): <span style="font-weight:bold;font-size:16px;">'.convert_number_to_words_new(round($rel['g_total'])).'</span></td>
		</tr>
		<tr style="border:none;  border-left:none ; border-right:none ;   border-bottom:1px solid ;">
		<td colspan="'.($colspan+2).'" style=" text-align:left;font-size:16px;">Total Tax (In Words): <span style="font-weight:bold;font-size:16px;">'.convert_number_to_words_new($total_cs_gst + $total_i_gst + $total_sundrytax).'</span></td>
		</tr>
