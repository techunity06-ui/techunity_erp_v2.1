...
//Quotation Data
	...
	left join pay_terms as pt on pt.terms_id = invoice.payment_terms 
...
//echo "<pre>";print_r($rel);die();
	...
	if($rel['consignee_id']==0){
		...
		$contact_person_no = $rel['cust_mobile'];
		...
		$contact_person_no = $rel_con['cust_mobile'];
		$party_address_con="
...
//>Plot No : 117 , Nr. GETCO Sub -Station , Old GIDC , Gundlav Valsad - 396035, Gujarat, India
	...
	<span style="font-size:12px;">'.$comp_rel["address"].'</span><br>
	<span style="font-size:12px;">CIN : '.$comp_rel["cin"].'</span>
	</td>
	</tr>
	</table>';
	$footer='';
	...
	.page{
		...
		<tr style="border:none; border-left:1px solid; border-right:1px solid; ">
		<td colspan="2" style=" text-align:left;">
		Payment Terms: '.$rel['pay_term'].'</td>
		<td style=" text-align:left;">
		Customer PO Date:'.$po_date.'</td>
		</tr>
		<tr style="border:none; border-left:1px solid; border-right:1px solid; ">
		<td colspan="2" style=" text-align:left;">
		Contact Person:'.$contact_person.'<br>Contact Person No. : '.$contact_person_no.'</td>
		...
		<tbody>';
		$trn_qry="select *,product.product_name, hsn.hsn_code as product_hsn_code, product.image_name, product.product_icode FROM `tbl_sales_ordertrn` as trn 
		...
		while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
			...
			$html.='<td border="0" style="border-radious: 0px; border: none!important;font-size:12px;'.$align.'">'.$trn_rel['product_name'].''.$item_code.'<br/>'.$product_desc.'</td> 
			</tr>
			</table>';
			...
			<td style=" text-align:center;font-size:12px;">'.without_comma_two_digit_amount($trn_rel['product_qty']).' </td>
			<td style=" text-align:center;font-size:12px;"> '.without_comma_two_digit_amount($trn_rel['product_rate_conv']).'</td>
			<td style=" text-align:center;font-size:12px;">'.without_comma_two_digit_amount($trn_rel['discount_per']).' % <br>('.without_comma_two_digit_amount($trn_rel['product_discount_conv']).')</td>
			<td style=" text-align:center;font-size:12px;"> '.without_comma_two_digit_amount($trn_rel['product_amount_conv']).'</td>
			</tr>';
		...
		<td  style=" text-align:center;font-size:14px;font-weight:bold;background-color:	#b3b3b3">'.indian_number(($rel['g_total_conv']),2).' </td>
		</tr>';
		...
		Total Order Value (In Words): <span style="font-weight:bold;font-size:13px;">'.convert_number_to_words_new($rel['g_total_conv'],$rel['currency_id'],$currency_word_end,$currency_word_start).'</span></td>
		</tr>
		<tr style="">
		<td colspan="9" style=" text-align:left;font-size:14px;">
		Total Tax (In Words): <span style="font-weight:bold;font-size:13px;">'.convert_number_to_words_new($rel['g_total_conv'],$rel['currency_id'],$currency_word_end,$currency_word_start).'</span></td>
		</tr>
		<tr style="">
		<td colspan="9" style="border-bottom: 1px;text-align:left;font-size:14px;">
		Remarks :'.$rel['quot_remark'].'</td>
		</tr>
		...
		$trn_qry_tax="select sum(product_amount_conv) as product_amount_conv,product_hsn_code,cgst_tax_per,sum(cgst_tax_rate_conv) as cgst_tax_rate_conv,sgst_tax_per,sum(sgst_tax_rate) as sgst_tax_rate_conv,igst_tax_per,sum(igst_tax_rate) as igst_tax_rate_conv FROM `tbl_sales_ordertrn` where sales_ordertrn_status=0 and sales_order_id='".$rel['sales_order_id']."' group by product_hsn_code";
		...
		while($trn_rel_tax=brp_mysqli_fetch_assoc($trn_qry_rs_tax)){
			...
			$total1+=$row_total=$trn_rel_tax['cgst_tax_rate_conv']+$trn_rel_tax['sgst_tax_rate_conv']+$trn_rel_tax['igst_tax_rate_conv'];
			...
			'.without_comma_two_digit_amount($trn_rel_tax['product_amount_conv']).'
			</td>';
			...
			if($rel['stateid']==$comp_rel['stateid']){
				...
				'.number_format($trn_rel_tax['cgst_tax_rate_conv'],2,".","").'
				</td>
				<td style="text-align:right;font-size:12px;">
				'.str_replace("SGST","",$trn_rel_tax['cgst_tax_per']).'
				</td>
				<td style="text-align:right;font-size:12px;">
				'.number_format($trn_rel_tax['sgst_tax_rate_conv'],2,".","").'
				</td>';
				$k++;
				...
				<td style="text-align:right;font-size:12px;">
				'.number_format($trn_rel_tax['igst_tax_rate_conv'],2,".","").'
				</td>';
				$k++;
			...
			$totalamt+=$trn_rel_tax['product_amount_conv'];
			$totaltaxamt1+=$trn_rel_tax['cgst_tax_rate_conv'];
			$totaltaxamt2+=$trn_rel_tax['sgst_tax_rate_conv'];
			$totaltaxamt3+=$trn_rel_tax['igst_tax_rate_conv'];
		}
		...
		if($rel['stateid']==$comp_rel['stateid']){
			...
			$html.='<td style="text-align:right;font-size:13px;font-weight:bold" colspan="2">
			'.number_format($totaltaxamt3+$total_suntaxamt1,2).'
			</td>
			';
		}
		$html.='<td colspan="2" style=" text-align:right;font-size:13px;font-weight:bold">
		'.number_format($total1+$total_sun1,2).'
		</td>
		</tr>
		</table>';
		/*$html.='';*/
		...
		if(mysqli_num_rows($terms_qry_rs)){
			...
			<td style="text-align:left;"><strong>Terms & Conditions:</strong><br>';
			$t=1;
			while($term_rel=mysqli_fetch_assoc($terms_qry_rs)){
				$string=(nl2br($term_rel['tc_details']));
				$html.=$t.'. '.$term_rel['tc_name'].' : '.$string.'<br>';
				$t++;
			}
			$html.='</td>
			</tr>';	
		}
		$html.='<tr style="border-bottom: 0px; ">
