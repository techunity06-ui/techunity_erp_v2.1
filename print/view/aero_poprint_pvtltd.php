...
//Quotation Data
	$query="select po.*,usr.user_name,state.state_name,modesup.mode_dispatch,payterms.payment_terms as payment_term,l.l_name as vender_name,country.country_name,l.m_address as vender_address,l.ledger_code,l.cust_cont_name,l.gst_no as tin_no,l.cust_mobile as vender_mobile,l.stateid,state.gst_state_code,city.city_name,bran.branch_address,brnc.country_name as branch_country,brns.state_name as branch_state,brnci.city_name as branch_city,branz.zone_name,bran.branch_pincode,comp.company_name,le.l_name as con_ven,bm.branch_name as cons_bran,cur.currency_code,cur.currency_in_word_end from tbl_purchaseorder as po 
	...
	left join tbl_purchaseorder_aprv_log as log on log.purchaseorder_id=po.purchaseorder_id
...
//$sundrytax_rels = mysqli_fetch_assoc($sundrytax);
	...
	.page{
		...
		<tr style="border:none">
		    <td style= "border: none;"><b> (F/PUR/07 REV.1) </b></td>
		...
		<td rowspan="3" style="text-align:left; vertical-align:top; border:1px solid; width:50%;border-bottom: none;">
		...
		<td style="text-align:left;border:1px solid;"> purchase@aeroengg.com </td>
		</tr>
		<tr style="border-bottom: none;">
		<td colspan="3"><strong> With reference to the above, we are pleased to release our order for the following Item. Please acknowledge your acceptance of order by return mail. If we not received any reply within 3 days, then we shall assume you are accepted the P.O </strong></td>
		</tr>
		</table>
		</div>
		';
		$html.='<div>
				...
				<th style="width:8%;text-align:center;border:1px solid;">HSN Code</th>
				...
				<th style="width:13%;text-align:right;border:1px solid;">Basic Amount Rs.</th>
				</tr>
				</thead>
				<tbody> ';
		$qry="select trn.*,pre.pre_no,trn.product_qty as sh_qty,trn.product_conv_qty as sh_conv_qty,product.*,product.product_desc as scode,per.unit_name,per1.unit_name as base_unit_name,per2.unit_name as conv_unit_name FROM `tbl_purchaseordertrn` as trn 
		...
		left join tbl_request_product as rp on rp.rp_id=trn.po_ref_id 
		left join tbl_pre_trn as pre_trn on pre_trn.pre_trn_id=rp.pre_trn_id 
		left join tbl_pre as pre on pre.pre_id=pre_trn.pre_id 
		where purchaseordertrn_status=0 and purchaseorder_id=".$rel['purchaseorder_id']."  order by purchaseordertrn_id";
...
// 			}else{
				$pqty .= '<br/>'.number_format($row['sh_qty'],2,".","").' '.$row['base_unit_name'];	
// 			}
			...
			$disc_rate = ((float)$row['product_rate'] * (float)$row['discount_per']) / 100;
			...
			$html.= '<br><b>Wo Indent No.: </b>'.$row['pre_no'];
			$html.= '<br><b>Desc: </b>'.$row['product_des'];
			if($delivery_type == 'product_wise'){
...
// Initialize before the loop
			...
			// Inside the loop
			$sqr_ft = isset($row['sqr_ft']) ? (float)$row['sqr_ft'] : 0;
			$charges_qty1 = isset($charges_qty1) ? (float)$charges_qty1 : 0;

			$totalsqr = $totalsqr + $sqr_ft - $charges_qty1;
		...
		$html .= '</table></div><div><table style="font-size:12px;border-collapse: collapse;width:100%;" cellpadding="3" cellspacing="3"><tbody>';
		$html.='<tr>
			<td colspan="'.$cols.'" style="text-align:left;border:1px solid;color:Black; font-weight: bold;width:50%">Basic PO Amount: '.ucfirst(convert_number_to_words($total_product_amount,'Rupees')).'</td>
			<td colspan="4" style="text-align:right;border:1px solid; font-weight: bold;width:25%">Total Basic</td>
		...
		if($rel['stateid']==$set_head['stateid']){
			$html.='<tr>
				<!--<td colspan="'.$cols.'" rowspan="'.($rows+2).'" style="text-align:left;border:1px solid;color:Black; font-weight: bold;">GST Amount: '. ucfirst(convert_number_to_words($totaltaxable,$rel['currency_code'])).'	</td>-->
				<td colspan="'.$cols.'" rowspan="'.($rows+5).'" style="text-align:left;border:1px solid;color:Black; font-weight: bold;">1 If material will dispatch in part order then payment term will be calculated from last invoice of order.<br/>
				2. The prices mentioned in this Purchase Order shall remain firm and fixed till the complete execution of this order.<br/>
				3. Kindly follow the submitted QMS & OHSMS “Supplier’s Guidelines” strictly while supplying the materials.<br/>
				4. Material will be dually checked on quality parameters on receipt of materials as per our requirement before acceptance into the organization.<br/>
				5. The Company reserves the right to accept or reject the materials as per the set company standards & specifications defined in the order.<br/><br/>
				Rejection of Materials: In case of material is rejected, the same is to be replaced by you free of cost and to & fro transportation cost of the same will be to your account..
				<br/><br/>JURISDICTION :- THE ABOVE ORDER IS SUBJECT TO AHMEDABAD, INDIA ,JURISDICTION.
				</td>
				<td colspan="4" style="text-align:right;border:1px solid; font-weight: bold;">CGST </td>
				<td style="text-align:right;border:1px solid;font-weight: bold;">'.number_format(($total_cs_gst/2),2,".","").'</td>
			</tr>
			<tr>
				<td colspan="4" style="text-align:right;border:1px solid; font-weight: bold;">SGST </td>
				<td style="text-align:right;border:1px solid;font-weight: bold;">'.number_format(($total_cs_gst/2),2,".","").'</td>
					</tr>';
				while($row11=mysqli_fetch_assoc($result11))
				{
					$html.='<tr>
					<td colspan="4" style="text-align:right;border:1px solid; font-weight: bold;">'.$row11['l_name'].'</td>
				...
				{
					$html.='<tr>
						<td colspan="4" style="text-align:right;border:1px solid; font-weight: bold;">'.$row12['l_name'].'</td>
						<td style="text-align:right;border:1px solid;font-weight: bold;">'.number_format($row12['sundry_amount'],2,".","").'</td>
						</tr>';
				}
		}else{
			$html.='<tr>
				<!--<td colspan="'.$cols.'" rowspan="'.($rows+1).'" style="text-align:left;border:1px solid;color:Black; font-weight: bold;">GST Amount: '. ucfirst(convert_number_to_words($totaltaxable,$rel['currency_code'])).'		</td>-->
				<td colspan="'.$cols.'" rowspan="'.($rows+5).'" style="text-align:left;border:1px solid;color:Black; font-weight: bold;">1 If material will dispatch in part order then payment term will be calculated from last invoice of order.<br/>
				2. The prices mentioned in this Purchase Order shall remain firm and fixed till the complete execution of this order.<br/>
				3. Kindly follow the submitted QMS & OHSMS “Supplier’s Guidelines” strictly while supplying the materials.<br/>
				4. Material will be dually checked on quality parameters on receipt of materials as per our requirement before acceptance into the organization.<br/>
				...
				<td colspan="4" style="text-align:right;border:1px solid; font-weight: bold;">IGST </td>
		...
		$html.='<tr>
				<!--<td colspan="'.$cols.'" rowspan="3" style="text-align:left;border:1px solid;color:Black; font-weight: bold;">Total PO Amount: '.ucfirst(convert_number_to_words($rel['g_total'],$rel['currency_code'])).'</td>-->
				...
				<tr>
				<td colspan="4" style="text-align:right;border:1px solid; font-weight: bold;">Round Off</td>
				<td style="text-align:right;border:1px solid;font-weight: bold;">'.number_format($rel['round_of'],2,".","").'</td>
				</tr>
				<tr>
				<td colspan="4" style="text-align:right;border:1px solid; font-weight: bold;">Total Rs.</td>
				<td style="text-align:right;border:1px solid;font-weight: bold;">'.number_format($rel['g_total'],2,".","").'</td>
				</tr>';
				
				if($rel['tc_format']==1 && !empty($rel['po_condition'])){
			        $html.='<!--<center class="nextpage"></center>-->
			        <tr style="">
			        <td colspan="8"> 
        			<table width="100%" style="font-size:12px;border:none; width:100%;overflow:wrap;">
        			<tbody>
        		
        			<tr style="border: none;">
        			<td><h3><u>Terms & Conditions :</u></h3>'.$rel['po_condition'].'</td>
        			</tr>
        			</tbody>
        			</table>
        		...
        		if($rel['tc_format']==2){
                    ...
                    $terms_qry_rs=$dbcon->query($terms_qry);
                       if(brp_mysqli_num_rows($terms_qry_rs)){
                        ...
                        <tr style="">
			            <td colspan="8"> 
                        <table width="100%" style="font-size:12px;width:100%;overflow:wrap;border:none;"><tbody>
                        <tr style="border: none;"><td colspan="3" style="text-align:center; ">
				...
				$html.='
				<tr style="">
				<td colspan="'.$cols.'" rowspan="3" style="height:80px; text-align:left; vertical-align:top; border-left:1px solid; border-bottom:1px solid !important; font-weight: bold;"><strong>Remarks:</strong><br> '.$rel['remark'].'</td>
				<td colspan="5" style="text-align:center;vertical-align:top;border:0px !important;font-weight: bold;height:80px;">For, '.$set_head['company_name'].'
				<div><br><br><br><br><br>('.$rel['user_name'].')</div>
				...
				<!--page1 end-->';
				$html.='</tbody></table>
				<p style="text-align: center;">** This document is computer Generated and does not require Manual Signature or Company Seal. **</p>
				</div>';
...
// 			<tr>
// 			<td width="100%" style="width:100%;text-align:left;border:none;padding:5px;"><h3 style="text-align:center;"><u>Terms & Conditions :</u></h3>'.$rel['po_condition'].'</td>
// 			</tr>
// 			</tbody>
// 			</table>
// 			</div>';	
