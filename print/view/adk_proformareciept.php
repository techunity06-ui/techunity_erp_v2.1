...
//echo "<pre>";print_r($set_head);die();
    ...
    $header = get_header($dbcon,'text-align: center','100%','').'<table style="text-align:center;border:none">
...
$qry="select trn.*,product.*,unit_name FROM `tbl_proforma_trn` as trn left join product_mst as product on product.product_id=trn.product_id left join unit_mst as per on per.unitid=trn.unit_id where trancation_status=0 and invoice_id=".$rel['invoice_id']." group by trancation_id order by trancation_id desc";
...
$html.= '</td>
<td style="vertical-align:top; border-right:1px solid;white-space:nowrap;font-size:14px;text-align:center" >';
if($row['product_type']!='8'){ 
...
$qry="select trn.*,product.*, hsn.hsn_code,product.product_name,unit_name FROM `tbl_proforma_trn` as trn left join product_mst as product on product.product_id=trn.product_id left join unit_mst as per on per.unitid=trn.unit_id left join mst_hsn_code as hsn on product.product_hsn=hsn.hsn_id  where trancation_status=0 and invoice_id=".$rel['invoice_id']." group by trancation_id order by trancation_id desc";
//  echo $qry;die;
$result_qry=$dbcon->query($qry);
$rel_qry=mysqli_fetch_assoc($result_qry);
$html.='</td>
<td style="vertical-align:top;font-size:14px;text-align:center" >';
if($row['product_type']!='8'){ 
 $html.=$rel_qry['hsn_code'];
}
$html.='</td>
<td style="vertical-align:top;font-size:14px;text-align:center" >';
if($row['product_type']!='8'){ 
...
for($j=0; $j<$pr; $j++){
  ...
  <span style="font-weight:bold;font-size: 14px;">'.ucwords(convert_number_to_words_new($rel['g_total'],$rel['currency_id'],$rel['currency_in_word_end'],$rel['currency_in_word'])).'
  </span>
  <br><br>
  <!--<span style="font-weight:bold;font-size: 14px;">Payable Amount In Words : </span><br>
  <span style="font-weight:bold;font-size: 14px;">
    '.ucwords(convert_number_to_words_new($rel['payable_amt'])).' 
  </span>-->
  <br>
  <br>
  ...
  <table style="width:100%;border:none;padding:0px !important">
    ...
    <tr style="border:none;padding:0px !important">
        ...
        <td style="border:none;padding:0px !important"> : <span style="font-size:14px;">'.$set_head['bank_name'].'</span></td>
    </tr>
    <tr style="border:none;padding:0px !important">
        <td style="border:none;padding:0px !important"><span style="font-weight:bold;font-size: 14px"> Bank Address </span></td>
        <td style="border:none;padding:0px !important"> : <span style="font-size:14px;">'.$set_head['branch_name'].'</span></td>
    </tr>
    <tr style="border:none;padding:0px !important">
        <td style="border:none;padding:0px !important"><span style="font-weight:bold;font-size: 14px"> A/c No. </span></td>
        <td style="border:none;padding:0px !important"><span style="font-size:14px;"> : '.$set_head['ac_no'].'</span></td>
    </tr>
    <tr style="border:none;padding:0px !important">
        <td style="border:none;padding:0px !important"><span style="font-weight:bold;font-size: 14px"> IFSC Code </span></td>
        <td style="border:none;padding:0px !important"><span style="font-size:14px;"> : '.$set_head['ifcs'].'</span></td>
    </tr>
  </table>
...
<span style="font-weight:bold;font-size: 14px"> '.$rel['terms_condition'].' </span>
</td>
</tr>
<tr id="rawnone">
<td colspan="6" width="50%" style="border:1px solid">
...
if($set_head['authorized_signature']!=""){
    $html.='<img src="'.DOMAIN_F.'view/upload/signature/'.$set_head['authorized_signature'].'" style="height: 80px; width: 80px;"><br>';
}
$html.='<span style="font-weight:bold;font-size: 14px;"> AUTHORIZED SIGNATORY  
</span>
</td>
</tr>
';
...
$mpdf=new mPDF('','A4','0','calibri','10','10','105','40','1','1');
