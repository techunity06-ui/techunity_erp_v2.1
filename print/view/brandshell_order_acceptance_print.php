...
//>Plot No : 117 , Nr. GETCO Sub -Station , Old GIDC , Gundlav Valsad - 396035, Gujarat, India
	...
	if($companyConfiguration['sales_print_letterhead_per']==0){
		$header =get_header($dbcon,'text-align: center','250px','70px');
		$footer = '';
	}else{
	...
	.page{
		...
		$html.='
		<tr>
		<td colspan="7" style=" text-align:right;">Item Total Amount  </td>
		...
		if($rel['stateid']==$comp_rel['stateid']){
			...
			$html.='<tr>
			<td colspan="7" style=" text-align:right;">CGST %'.number_format(($gst_per)/2).'</td>
			<td  style=" text-align:center;">'.$currency_symbol.' '.number_format((($total_cs_gst/2)+$total_billsundy_gst_amount),2,".","").'</td>
			</tr><tr>
			<td colspan="7" style=" text-align:right;">SGST %'.number_format(($gst_per)/2).'</td>
			<td  style=" text-align:center;">'.$currency_symbol.' '.number_format((($total_cs_gst/2)+$total_billsundy_gst_amount),2,".","").'</td>
			</tr>';
		}else{
			  $total_billsundy_gst;
			$html.='<tr>
			<td colspan="7" style=" text-align:right;">IGST %'.number_format($gst_per).'</td>
		...
		$html .= '<tr>
		<td colspan="7" style=" text-align:right;font-size:14px;">Round Off  </td>
		<td  style=" text-align:center;font-size:14px;">'.indian_number($round_off,2).'</td>
		</tr>
		<tr>
		<td colspan="7" style=" text-align:right;font-size:14px;background-color:	#b3b3b3">Total Order Value (In Figure) </td>
		...
		<tr style="">
		<td colspan="8" style=" text-align:left;font-size:14px;">
		Total Order Value (In Words): <span style="font-weight:bold;font-size:13px;">'.(($rel['currency_id']=='1') ? convert_number_to_words_new($rel['g_total_conv']) : convert_number_to_words_new($rel['g_total_conv'],$currency_word_start)).'</span></td>
		</tr>
		<tr style="">
		<td colspan="8" style=" text-align:left;font-size:14px;">
		Total Tax (In Words): <span style="font-weight:bold;font-size:13px;">'.(($rel['currency_id']=='1') ? convert_number_to_words_new($gst_rate) : convert_number_to_words_new($rel['g_total_conv'],$currency_word_start)).'</span></td>
		</tr>
		<tr style="">
		<td colspan="8" style="border-bottom: 1px;text-align:left;font-size:14px;">
