...
if(strtolower($type) == 'pdf') {
 ...
 // echo "<pre>";print_r($query);die(); 
    ...
    $header =get_header($dbcon,'text-align: center','35%','15%');
    // $header ='<div style="text-align: center"><img src="'.DOMAIN_F.LOGO.$set_head['logo'].'" style="width:2.27in;" /></div>';
    ...
    <td colspan="2" style="border: none; font-size: 18px; font-weight: bold; text-align: right;">JOBWORK RECEIPT</td>
    <td style="border: none; font-size: 11px; font-weight: bold; text-align: right;"><input type="checkbox">&nbsp;Original&nbsp;&nbsp;<input type="checkbox">&nbsp;Duplicate&nbsp;&nbsp;<input type="checkbox">&nbsp;Triplicate</td>
    </tr>
    <tr>
    ...
    $qry="SELECT trn.*, product.product_name, per.unit_name,met.unit_name as met_unitname, process.process_name, unit1.unit_name as con_unit 
    FROM `tbl_job_work_trn` as trn 
    LEFT JOIN product_mst as product on product.product_id=trn.product_id 
    LEFT JOIN unit_mst as per on per.unitid=trn.rate_unit 
    LEFT JOIN unit_mst as unit1 on unit1.unitid=trn.product_con_unit 
    ...
    WHERE trn.job_work_trn_status=0 and trn.job_work_id=".$rel['job_work_id']." ORDER BY trn.job_work_trn_id";
    ...
    while($row=mysqli_fetch_assoc($result))
    {
        ...
        $q1 = "select GROUP_CONCAT(sp.po_req_no) AS work_order_no,GROUP_CONCAT(sp.sales_order_no) AS sales_order_no from tbl_job_work_sub_trn as strn
            left join tbl_request_product as rp on rp.rp_id = strn.rp_id
            left join tbl_set_main_process as sp on sp.sp_id = rp.sp_id
            where strn.job_work_sub_trn_status = 0 and strn.job_work_trn_id = " . $row['job_work_trn_id'];

        $r1 = brp_mysqli_fetch_assoc($dbcon->query($q1)); 
        $mrate=$row['pr_rate'];
        if($row['material_qty']!=0){
            $unit=$row['met_unitname'];
            $qty=$row['material_qty'];

        }else{
            $unit=$row['unit_name'];
            $qty=$row['product_base_qty'];
        }
        if($row['rate_unit']==$row['product_base_unit']){
            $units=$row['unit_name'];
            $qtys=$row['product_base_qty'];
        }else{
            $units=$row['con_unit'];
            $qtys=$row['product_con_qty'];
        }
        $mtotal=$qty*$mrate;

        $wo_so_details = "";
        if(!empty($r1['work_order_no'])){
            $wo_so_details .= "<p> <strong> Workorder No : </strong>" . $r1['work_order_no'] ."</p>";
        }

        if(!empty($r1['work_order_no'])){
            $wo_so_details .= "<p> <strong> Salesorder No : </strong>" . $r1['sales_order_no'] ."</p>";
        }
       /* '<br>'.$qtys.'
        <br>'.$units.'*/
        ...
        <td style="font-weight: bold; border-left: 1px solid;">'.$row['product_name'].'<p style="font-weight: normal;">'.$row['description'].'</p></td>
        <!--<td style="font-weight: bold; border-left: 1px solid;">'.$row['product_name'].'</td> -->
        <td style="text-align: center; border-left: 1px solid;">'.$row['process_name'].$wo_so_details.'<p></td>
        <td style="text-align: center; border-left: 1px solid;">'.$qty.'</td>
        <td style="text-align: center; border-left: 1px solid;">'.$unit.'</td>
        <td style="text-align: center; border-left: 1px solid;">'.$mrate.'</td>
        <td style="text-align: center; border-left: 1px solid; border-right: 1px solid;">'.$mtotal.'</td>
        </tr>';
    ...
    <td style="text-align: center;">'.indian_number($total,2).'</td>
    </tr>
    </tbody>
    </table>
    ...
    <tr>
    <td colspan = "2" style="font-size: 11px;">Complaints for shortage etc., If any, will not be considered. If not reported in writing within 24 hours after delivery of goods.</td>
    <td style="font-size: 12px; font-weight: bold;">Subject to Ahmedabad Jurisdiction</td>
    </tr>
    <tr style="border-right: 1px solid; border-left: 1px solid; border-top: none;">
    <td style="font-weight: bold; font-size: 14px; text-align: left;vertical-align: top;">Goods Dispatched Through<hr>Transport<hr>Goods once sold will be taken back</td>
    <td style="font-weight: bold; font-size: 14px; vertical-align: bottom; text-align: center; white-space: nowrap;">RECEIVER SIGNATURE WITH STAMP</td>
    <td style="font-weight: bold; font-size: 14px; text-align: right; white-space: nowrap;">For '.$set_head['company_name'].'<br><br><br><br><br>AUTHO. SIGNATORY</td>
    </tr>
    </tbody></table>';
