...
//Quotation Data
	...
	left join tbl_cust_contact_person as cont on cont.cust_contact_person_id = invoice.kind_attn
	...
	left join tbl_transport_transaction as trans on trans.transport_transaction_table_id = invoice.sales_order_id and transport_transaction_table='tbl_sales_order'
	left join transportation_details as td on td.id=trans.transport_id
...
//echo "<pre>";print_r($rel);die();
	...
	if($rel['consignee_id']==0){
		...
		Email Id: ".strtolower($rel_con['cust_email'])."
...
//>Plot No : 117 , Nr. GETCO Sub -Station , Old GIDC , Gundlav Valsad - 396035, Gujarat, India
	...
	.page{
		...
		<tr style="border:none; border-left:1px solid; border-right:1px solid; ">
		<td colspan="2" style=" text-align:left;">
		Sales Quote No: '.$rel['quotation_no'].'<!--Reference: '.$order_by.'--></td>
		<td style=" text-align:left;">
		Customer PO No: '.$rel['po_no'].'</td>
		</tr>
		<tr style="border:none; border-left:1px solid; border-right:1px solid; ">
		<td colspan="2" style=" text-align:left;">
		Contact Person: '.$contact_person.'<!--Payment Terms: '.$rel['payment_terms'].'--></td>
		<td style=" text-align:left;">
		...
		$ttax=$total_cs_gst+$total_cs_gst+$total_i_gst+$total_billsundy_gst;
		...
		Total Order Value (In Words): <span style="font-weight:bold;font-size:13px;">'.(($rel['currency_id']=='1') ? convert_number_to_words_new($rel['g_total_conv'],$rel['currency_id']) : convert_number_to_words_new($rel['g_total_conv'],$rel['currency_id'],$currency_word_start)).'</span></td>
		</tr>
		<tr style="">
		<td colspan="9" style=" text-align:left;font-size:14px;">
		Total Tax (In Words): <span style="font-weight:bold;font-size:13px;">'.(($rel['currency_id']=='1') ? convert_number_to_words_new($ttax,$rel['currency_id']) : convert_number_to_words_new($ttax,$rel['currency_id'],$currency_word_start)).'</span></td>
		</tr>
		<tr style="">
		...
		$html.='<table style="page-break-inside: avoid;border-bottom: 1px; " >';
		$html.='
		<!--<tr>
		...
		<tr style="border-bottom: 1px; ">
			<td rowspan="2" style=" text-align:left;font-size:14px;font-weight:bold;width:50%;">
		...
		<td style=" text-align:right;font-size:14px;font-weight:bold;width:50%;">
		For , '.$comp_rel["company_name"].'</td>
		</tr>
		<tr style="border-bottom: 0px;border-top: 0px; ">
		
		<td style=" text-align:right;font-size:12px;" >
		<img src="'.DOMAIN_F.'view/upload/signature/'.$comp_rel['authorized_signature'].'" height="100" width="100" class="img-thumbnail" /></td>
		</tr>
		<tr style="border-top: 0px; ">
		<td style=" text-align:right;font-size:12px;">
		</td>
...
//echo $trn_qry;
	...
	$mpdf=new mPDF('','A4','0','calibri','10','10','40','40','1','1');
