...
function quotation_print($dbcon, $quotation_id, $save_file)
{
	...
	if (strtolower($type) == 'pdf') {
		//Quotation Data
		$query = "select quot.*,per.c_con_fname,per.c_con_lname,qtrn.product_spec,qtrn.product_desc,per.c_con_mobile,per.c_con_email,cust.cust_name, cust.cust_email, cust.cust_mobile, inq.inquiry_no,inq.inquiry_date, ref.rb_name, cadd.c_add_state from tbl_quotation as quot
		...
		<tr style="border:0;">
			...
			<strong>Office & Workshop:</strong><br>'.$comp_rel["address"].'<br/>
			Mob.No : '.$comp_rel["contact_no"].'<br/>
			Email : '.$comp_rel["website"].'<br/>
			Web : '.$comp_rel["company_website"].'
		</td>
		
	<td style="width:40%; text-align:right;border:0;"> 
		
		<img src="../../view/img/logo/'.$comp_rel["logo"].'" style="height: 80px;" />
		</td>
		</tr>
			<tr style="border:none;">
				<td style="text-align: left;border: none;">Ref. No.: ' . $rel['inquiry_no'] . ' </td>
				<td style="text-align: right;border: none;">Date: ' .  date("d-m-Y",strtotime($rel['inquiry_date'])). ' </td>
			</tr>
		</table>
	';
    ...
    <table style="font-size:16px;width: 100%;border: none;">
        ...
        <tr style="border:none;">
            <td colspan="2" style="text-align: left;border: none; ">To,<br><b>' . $rel['cust_name'] . '<br></b>Address: ' . $rel['quot_address'] . '
			<br>State: ' . $rel['c_add_state'] . '
			</td>
        ...
        <tr style="border:none;">
            <td colspan="2" style="text-align: left; border: none;"><span style="text-decoration:underline;"><b>Kind Attention </b></span>:	' . $rel['c_con_fname'] . '' . $rel['c_con_lname'] . ' 
			 </td>
			
        </tr>   
        <tr style="border:none;">
            <td colspan="2" style="text-align: center; border: none; "><h3 style="text-align:center;text-decoration:underline;"><span style="text-decoration:underline;"><b>Sub  </b></span>: Quotation for &nbsp;' . $rel["quot_subject"] . '
			</h3><br>
        ...
        <tr style="border:none;">
            <td colspan="2" style="text-align: left; border: none; "><br>' . $quotation_print_content . '<br>
                </td>
    ...
    ';
	// <tr style="border:none;">
    //         <td colspan="2" style="text-align: left; border: none; "><span style="text-decoration:underline;"><b>Reference </b></span> : As per your discussion with &nbsp;' . $rel['quotation_ref'] . '<br>
	...
	// if ($inquiry_type != "2") {
		...
		$html .= '<h2 style="color:brown;">DESCRIPTION OF THE EQUIPMENT:</h2><br>“UNIPAVE” make ARAI tested & approved Asphalt Paver Finisher Machine comes with following
		specifications,<p>'.$rel['product_desc'].'</p>';
		$html .= '<h2 style="color:brown;">PRICE:</h2>
		<table cellpadding="5" cellspacing="5" style="border:0;font-size: 16px;">
			<tr style="border-left:0;border-right:0;">
			<th style="border:0;width: 10%;text-align:left;">Sr. No.</th>
			<th style="border:0;width: 20%;text-align:left;">Paver Model</th>
			<th style="border:0;width: 40%;text-align:left;">Basic Price Rs.</th>
			<th style="border:0;width: 15%;text-align:left;">Screed Model</th>
			<th style="border:0;width: 15%;text-align:left;">Quantity</th>
			</tr>text-align:left;
			';
		...
		while ($trn_rel = mysqli_fetch_assoc($trn_qry_rs)) {
			...
			$html .= ' 
			<tr style="border-left:0;border-right:0;">
			<td style="border:0;text-align:left;">'. $p . ' </td>
			<td style="border:0;text-align:left;color:brown;">'. $trn_rel['product_name'] . ' </td>
			<td style="border:0;text-align:left;">'. $currency_symbol . ' ' . $trn_rel['product_rate'] . '<br>('.ucwords(convert_number_to_words_new($trn_rel['product_rate'],$rel['currency_id'],$rel['currency_in_word_end'],$rel['currency_in_word'])).')</td>
			<td style="border:0;text-align:left;">'. $trn_rel['product_icode'] . ' </td>
			<td style="border:0;text-align:left;">'. $trn_rel['product_qty'] . ' </td>
			</tr>
		...
		<div style="text-align:left">
						<h3> <b>Thanking you and waiting for your early reply.<br>For, '.$comp_rel['company_name'].'<br><br><br>	
						'.$userData['user_name'].'<br>('.$userData['usertype_name'].')<br>Cell :'.$userData['user_phone'].'
						<br></b> </h3>
