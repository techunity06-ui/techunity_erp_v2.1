...
//>Plot No : 117 , Nr. GETCO Sub -Station , Old GIDC , Gundlav Valsad - 396035, Gujarat, India
	...
	/*if($companyConfiguration['sales_print_letterhead_per']==0){
		$header .= get_header($dbcon,'text-align: center','','50%');
		$footer = '';
	}else{
		$header .= get_header($dbcon,'text-align: center','','50%');
		$footer = '';
	...
	$header ='<table style:"border:0; border-color:white; width:100%">
		<tr style="border:0;">
		
		<td style="width:50%;  border:0;text-align:center; padding-top:20px">
    
        <img src="'.DOMAIN_F.LOGO.'/APSON_half.png" style="height: 50px; width: 200px;" />
...
//$header ='<div style="text-align:center;padding-top:15px"><img src="'.DOMAIN_F.LOGO.$comp_rel["logo"].'" style="width:8.27in;" /></div>';
	...
	.page{
		...
		while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
			...
			//echo $trn_rel['igst_tax_rate_conv'];
	        ...
	        if(in_array('item',$sales_pro_print)){
	            if(!empty($trn_rel['product_icode'])){
					$item_code = " -- (".$trn_rel['product_icode'].")";
				}
	        }
	        if(in_array('alias',$sales_pro_print)){
				if(!empty($trn_rel['product_alias_name'])){
			...
			<td style="vertical-align: top; text-align:center;">'.$currency_symbol.' '.indian_number($trn_rel['product_rate_conv'],2).'</td>
			<td style="vertical-align: top; text-align:center;">'.indian_number($trn_rel['discount_per'],2).' % <br>'.$currency_symbol.' ('.indian_number($trn_rel['product_discount_conv']).')</td>
			<td style="vertical-align: top; text-align:center;"> '.$currency_symbol.' '.indian_number($trn_rel['product_amount_conv'],2).'</td>
			</tr>';
		...
		<td  style=" text-align:right;">'.$currency_symbol.' '.indian_number($ttl_amt,2).'</td>
		</tr>';
		...
		while($row12=mysqli_fetch_assoc($result12))
		{
			...
			<td  style="text-align:right;">'.$currency_symbol.' '.number_format($row12['sundry_amount_conv'],2,".","").'</td>
			</tr>';
		...
		if($rel['stateid']==$comp_rel['stateid']){
			...
			<td  style=" text-align:right;">'.$currency_symbol.' '.number_format((($total_cs_gst/2)+$total_billsundy_gst_amount),2,".","").'</td>
			</tr><tr>
			<td colspan="8" style=" text-align:right;">SGST '.number_format(($gst_per)/2).'%</td>
			<td  style=" text-align:right;">'.$currency_symbol.' '.number_format((($total_cs_gst/2)+$total_billsundy_gst_amount),2,".","").'</td>
			</tr>';
			$gst_rate = number_format((($total_cs_gst/2)+$total_billsundy_gst_amount),2,".","") + number_format((($total_cs_gst/2)+$total_billsundy_gst_amount),2,".","");
		}else{
		  	$total_billsundy_gst;
			$html.='<tr>
			<td colspan="8" style=" text-align:right;">IGST '.number_format($gst_per).'%</td>
			<td  style=" text-align:right;">'.$currency_symbol.' '.number_format(($total_i_gst+$total_billsundy_gst),2,".","").'</td>
			</tr>';
			$gst_rate =number_format(($total_i_gst+$total_billsundy_gst),2,".","");
		}
		...
		while($row11=mysqli_fetch_assoc($result11))
		{
			...
			<td  style=" text-align:right;">'.$currency_symbol.' '.number_format($row11['add_sum'],2,".","").'</td>
			</tr>';
		}
		...
		<td  style=" text-align:right;font-size:14px;">'.$currency_symbol.' '.indian_number($round_off,2).'</td>
		</tr>
		<tr>
		<td colspan="8" style=" text-align:right;font-size:14px;background-color:	#b3b3b3">Total Order Value (In Figure) </td>
		<td  style=" text-align:right;font-size:14px;font-weight:bold;background-color:#b3b3b3">'.$currency_symbol.' '.number_format($rel['g_total_conv'],0,".","").'.00</td>
		</tr>';
		...
		Total Order Value (In Words): <span style="font-weight:bold;font-size:13px;">'.(($rel['currency_id']=='68') ? convert_number_to_words_new(number_format($rel['g_total_conv'],0,".",""),$rel['currency_id'],$currency_word_end,$currency_word_start) : convert_number_to_words_new(number_format($rel['g_total_conv'],0,".",""),$rel['currency_id'],$currency_word_end,$currency_word_start)).'</span></td>
		</tr>
		<tr style="">
		<td colspan="9" style=" text-align:left;font-size:14px;">
		Total Tax (In Words): <span style="font-weight:bold;font-size:13px;">'.(($rel['currency_id']=='68') ? convert_number_to_words_new($gst_rate,$rel['currency_id'],$currency_word_end,$currency_word_start) : convert_number_to_words_new($gst_rate,$rel['currency_id'],$currency_word_end,$currency_word_start)).'</span></td>
		</tr>
		<tr style="">
		...
		echo "select b.*,tl.ledger_hsn from tbl_bill_sundry_transaction as b
		...
		if($rel['stateid']==$comp_rel['stateid']){
			...
			$html.='<td  style="text-align:right;font-size:13px;font-weight:bold" colspan="2">
			'.$currency_symbol.' '.number_format($totaltaxamt3+$total_suntaxamt2,2).'
			</td>
			';
		}
...
// error_reporting(E_ALL);
	...
	$mpdf = new Mpdf(['format' => 'A4','margin_left' => 10,'margin_right' => 10,'margin_top' => 40,'margin_bottom' => 10,'margin_header' => 1,'margin_footer' => 1,'default_font' => 'calibri']);
