...
$type_mail = $_REQUEST['type'];
$type='pdf';
if(strtolower($type) == 'pdf') {
//Quotation Data
$query="select quot.*,per.c_con_fname,per.c_con_lname,per.c_con_mobile,per.c_con_email,cust.cust_name, cust.cust_email, cust.cust_mobile, inq.inquiry_no,inq.inquiry_date,usr.user_name from tbl_quotation as quot
...
//p($rel);
$quot_address = ($rel['quot_address']!='0')?nl2br($rel['quot_address']):'';
if(!$rel){
...
//p($comp_rel);
$header ='<img src="'.DOMAIN_F.LOGO.$comp_rel["logo"].'" style="width:7.57in;padding-top:25px;" />';  
...
$rel['quotation_valid_date'] = strtotime($rel['quotation_valid_date']);
$rel['quotation_valid_date'] = date("d/m/Y", $rel['quotation_valid_date']);
$html ='<html>
		<head>					
	...
	<table style="font-size:12px;border-collapse: collapse;width:100%;" cellpadding="5" cellspacing="5">
		<tr>
			<td colspan="3" style="text-align:center;font-size:15px;font-weight:bold;"> 
				QUOTATION / PERFORMA INVOICE'.$approve_status.'
			</td>
		</tr>
		<tr>
			<td rowspan="6" style="text-align:left;vertical-align:top;border:1px solid;"> 
				<strong>To,</strong><br/>
				<strong>'.$rel['cust_name'].'</strong><br/>
				'.$quot_address.'<br>
				<strong>Contact Person :</strong> <br>
				Person Name : '.$rel['c_con_fname'].' '.$rel['c_con_lname'].' <br>
				Mobile No : '.$rel['c_con_mobile'].' <br>
				Email : '.strtolower($rel['c_con_email']).' <br>
			</td>
		...
		<tr>
			<td style="text-align:left;border:1px solid;">Sales Person</td>
			<td>'.$rel['user_name'].'</td>
		</tr>
		<tr>
			<td style="text-align:left;border:1px solid">Quotation Validity</td>
			<td>'.$rel['quotation_valid_date'].'</td>
		</tr>
	</table>
	<table style="font-size:12px;border-collapse: collapse;width:100%;" cellpadding="3" cellspacing="3">
		<thead>
			<tr>
				<th style="width:2%;text-align:center;border:1px solid;">No.</th>
				<th style="width:50%;text-align:center;border:1px solid;">Product</th>
				...
				<th style="width:15%;text-align:center;border:1px solid;">Amount</th>
			</tr>
		</thead>
		<tbody>';
$trn_qry="select trn.*,pro.product_name,pro.product_type,unit.unit_name from tbl_quotation_trn as trn 
...
$p=1;$ttl_amt=0;$ttl_qty=0;$total_cs_gst=0;$total_i_gst=0;
...
while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
	...
	if($trn_rel['product_type']!=8){	
		$html.='<tr style="border:none;border-left:1px solid;border-right:1px solid;">';
				/*if($trn_rel['product_type']==8){		
					$packaging_charge = $trn_rel['product_name'];
					$html .='<td style="text-align:center;border:1px solid;vertical-align:top;"></td><td style="text-align:right;vertical-align:top;">'.$trn_rel['product_name'].'</td><td ></td><td></td><td></td>';
				}else{*/
					$html.='<td style="text-align:center;border:1px solid;vertical-align:top;">'.$p.'</td><td style="text-align:left;border:1px solid;vertical-align:top;"><strong>'.$trn_rel['product_name'].'</strong><br/>';
					$html.=$product_desc.'
					</td>
	...
	if($trn_rel['product_type']!=8){		
		if($trn_rel['act_amt_flag']!='1'){
			...
			if($packaging_charge==-1){
			}else{
				$ttl_amt+=$packaging_charge;
			}
		}
	}
...
for($j=0; $j<$pr; $j++)
{
	...
	$html.='<tr>
		...
		<td colspan="1" style="text-align:center;border:1px solid;"><strong>'.$ttl_amt.'</strong></td>
	</tr>';

	/* 
		Code By Umair: 29/06/2021
		Comment: Display service chanrge below 
		START
	*/
	$service_qry="select trn.*,pro.product_name,pro.product_type,unit.unit_name from tbl_quotation_trn as trn 
	left join product_mst as pro on pro.product_id=trn.product_id
	left join unit_mst as unit on unit.unitid=trn.unitid
	where trn.quot_trn_status=0 and pro.product_type=8 and trn.quotation_id=".$rel['quotation_id'];
	$service_qry_rs=$dbcon->query($service_qry);

	$service_charge = 0;
	if(brp_mysqli_num_rows($service_qry_rs) > 0 ){
		while($service_rel=mysqli_fetch_assoc($service_qry_rs)){
			$service_charge = $service_charge + $service_rel['product_amount'];
			$html.='<tr>
				<td colspan="3" style="text-align:center;border:1px solid;"></td>
				<td colspan="2" >
						'.$service_rel['product_name'].'
				</td>
				<td colspan="1" style="text-align:center;border:1px solid;">'.$service_rel['product_amount'].'</td>
			</tr>';
		}	
	}
	...
	/* END */
	$final_total = $ttl_amt + $service_charge;
    if($rel['qt_add_state']==$comp_rel['stateid']){
            $html .= '<tr>
                <td colspan="3" style=" text-align:right;font-size:12px;"></td>
                <td colspan="2">CGST</td>
                <td colspan="1" style="text-align:center;border:1px solid;">'.number_format(($total_cs_gst/2),2,".","").'</td>
            </tr>
            <tr>
                <td colspan="3" style=" text-align:right;font-size:12px;"></td>
        	...
        	$html .= '<tr>
                ...
                <td colspan="1" style="text-align:center;border:1px solid;">'.number_format(($total_i_gst),2,".","").'</td>
            </tr>';
    }
	...
	while($row11=mysqli_fetch_assoc($result11))
	{
		$html.='<tr>
                <td colspan="3" style=" text-align:right;font-size:12px;"></td>
                <td colspan="2">'.$row11['l_name'].'</td>
                <td colspan="1" style="text-align:center;border:1px solid;">'.number_format($row11['add_sum'],2,".","").'</td>
            </tr>';
	}
	...
	while($row12=mysqli_fetch_assoc($result12))
	{
		$html.='<tr>
                <td colspan="3" style=" text-align:right;font-size:12px;"></td>
                <td colspan="2">'.$row12['l_name'].'</td>
                <td colspan="1" style="text-align:center;border:1px solid;">'.number_format($row12['sundry_amount'],2,".","").'</td>
            </tr>';
	}
    $round_off = round($rel['g_total'])-$rel['g_total'];	
	$html.='<tr>
		<td colspan="3" style="border:1px solid;"><strong>Rs. '.convert_number_to_words_new(($rel['g_total'])).'</strong></td>
		<td colspan="2">
				Bill Amount: 
		</td>
		<td colspan="1" style="text-align:center;border:1px solid;">'.indian_number(($rel['g_total']),2).'</td>
	</tr>';
$html.='</tbody>
</table>
<table style="font-size:12px;border-collapse: collapse;width:100%;" cellpadding="5" cellspacing="5">';
...
if(mysqli_num_rows($terms_qry_rs)){
	$t=1;
	$html.='<tr><td colspan="3" width="100%" style="padding:5px;"><h3><strong>Terms: </strong></h3><br>';
	while($term_rel=mysqli_fetch_assoc($terms_qry_rs)){
	    $string=(nl2br($term_rel['tc_details']));
		$html.=$t.' &nbsp;&nbsp;&nbsp; '.$term_rel['tc_name'].'&nbsp;:&nbsp;&nbsp; '.$string.'<br><br>';
		$t++;
	}
	$html.='</td></tr>';
}
$html.='<tr style="height:80px;">
	<td colspan="2" style="height:80px;vertical-align:top;border-left:1px solid;border-bottom:1px solid;">
			<h2>GST NO. : 24AAACV9636J1Z0</h2><br>
			<h3>
				Our Bank : '.$comp_rel['bank_name'].' ( Branch : '.$comp_rel['branch_name'].' )<br>
				A/c No : '.$comp_rel['ac_no'].'<br>
				IFSC : '.$comp_rel['ifcs'].'
			</h3>
	</td>
	<td colspan="1" style="height:80px;vertical-align:top;text-align:center;">
			<h2>For, VIPUL COPPER PVT. LTD.</h2><br><br><br>
					...
					//$result1=$dbcon->query($query1);
							...
							while($rel1=mysqli_fetch_assoc($result1))
							{
								...
								$html.='<tr>
										...
										$html.='<td style="border-right:1px solid; border-bottom:1px solid;" >'.$rel1["product_name"].'</td>';				
										$html.='<td style="border-right:1px solid; border-bottom:1px solid;" >'.$pty.'</td>
...
//echo $html;exit;
		...
		if($type_mail){
		    $mpdf->Output('../quotation'.$quotation_id.'.pdf','f');
		     chmod('../quotation'.$quotation_id.'.pdf', 0777);
