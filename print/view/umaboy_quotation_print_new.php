...
.page{
	...
	if($inquiry_type==1){
		$trn_qry="select trn.*,pro.product_name,unit.unit_name, img.im_name from tbl_quotation_trn as trn 
		left join product_mst as pro on pro.product_id=trn.product_id
		left join unit_mst as unit on unit.unitid=trn.unitid
		left join tbl_product_images as img on img.im_product=trn.product_id
	...
	while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
		...
		$html.='<table style="width: 100%; font-size: 13px; margin-top: 30px;">
		...
		if(!empty($trn_rel['im_name'])){
			$html.='<tr style="border:none;">
			<td><img src="'.PRO_IMG_UPING.$trn_rel['im_name'].'" /></td>
			</tr>';
		}
		$html.='</table>
		</div>
		...
		if($rel['qt_add_state']==$comp_rel['stateid']){
			...
			<td style="border:1px solid;font-weight: bold;">'.indian_number(($total_cs_gst/2),2).'</td>
			</tr>
			<tr style="border:1px solid;">
			...
			<td style="border:1px solid;font-weight: bold;">'.indian_number($total_i_gst,2).'</td>
			</tr>';
		}
		$html.='<tr style="border:1px solid;">
