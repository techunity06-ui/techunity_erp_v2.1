...
.page{
	...
	if(!empty($rel['c_con_lname'])){
	    $html .= '<strong>Kind Attn : '.$rel['c_con_fname'].' '.$rel['c_con_lname'].' , '.$rel['cust_mobile'].'</strong><br>';
	}
...
$ps = 1;
	...
	while($sp_ro = mysqli_fetch_array($pr_se)){
		$html .= '<h3 style="text-decoration: underline; text-align: center;"><strong>'.$ps.') '.$sp_ro['product_name'].'</strong></h3><br>'.(($sp_ro['product_spec']) ? $sp_ro['product_spec'] : '').'<br>';
		$ps++;
	}
	...
	$html.='<br>'.$quotation_footer_content.'<br><br>
	<strong>For, '.$comp_rel['company_name'].'</strong><br>
	<p>Regards<br>
	<strong>'.$userData['user_name'] .'</strong><br>'. $userData['usertype_name'].'<br>
	'.$userPhone .'<br>'. $userEmail.'</p>
	</div>';
	/* Get Terms And Condition Start */
