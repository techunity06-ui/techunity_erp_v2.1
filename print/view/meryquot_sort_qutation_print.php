...
//Quotation Data
		$query="select quot.*,per.c_con_fname,per.c_con_lname,per.c_con_mobile,per.isd_id,per.c_con_email,per.c_con_job,cust.cust_name, cust.cust_email, cust.cust_mobile, inq.inquiry_no,inq.inquiry_date, ref.rb_name, cadd.c_add_state,cust.cust_gst from tbl_quotation as quot
...
//Amish Soni End 16-03-2021
		...
		//tax summary calculation start
				...
				'.$trn_rel['product_name'].'<br>Item Code : '.$item_code.'<br>'.$acces_desc.'
				</td>
			...
			if(!empty($quotation_footer_content)){
				$html.='<br /><div>'.$quotation_footer_content;
				$html.='</div>';
			}
