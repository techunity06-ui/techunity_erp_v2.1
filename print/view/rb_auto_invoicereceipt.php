...
$query="select invoice.*,country.country_name,state.state_name,cust.stateid,state.gst_state_code, city.city_name, cust.l_name, cust.m_address, type.invoice_type,cust_pincode,cust_mobile,gst_no,dispatch.mode_dispatch ,pay.payment_terms as terms
...
$qry_disc=$dbcon->query("select SUM(trn.product_discount) as discount FROM `tbl_invoicetrn` as trn WHERE trn.trancation_status=0 and invoice_id=".$rel['invoice_id']);
	$get_disc = brp_mysqli_fetch_assoc($qry_disc);
/* Check Discount is On or off Start */
if($get_disc['show_disc']>0){
	$colspan=5;
	$dynamicwidth=40;
}else{
	$colspan=6;
...
if($rel['is_sales_order']!=0){
	...
	if($rels['po_date']!="1970-01-01" && $rels['po_date']!="0000-00-00")
    {
    	$po_date=date('d-m-Y',strtotime($rels['po_date']));
    }
...
$sundrytax=$dbcon->query("select b.* from tbl_bill_sundry_transaction as b where b.sundry_voucher_id=".$rel['invoice_id']." and b.sundry_voucher_table='tbl_invoice' and b.isdelete='0' ");
	...
	while($sumsundrytax=brp_mysqli_fetch_assoc($sundrytax)){
		$total_sundrytax = $total_sundrytax + (($set_head['currency_id']==$_SESSION['currency_id']) ? $sumsundrytax['sundry_gst_amount'] : $sumsundrytax['sundry_gst_amount_conv']);
	}
if(!empty($rel['invoice_type'])){
    $invoice_type=$rel['invoice_type'];
}else{
    $invoice_type="TAX INVOICE";
}

$sel_t = $dbcon->query("select trn.*,t.id,t.transportation_name,cm.common_mst_name from tbl_transport_transaction as trn 
        left join transportation_details as t on t.id=trn.transport_id
        left join `tbl_common_mst` as cm on cm.common_mst_id = trn.transport_mode
											...
											<table  class="maintable headermain " id="table_head" width="100%">
												<tr style="border:none;">
													<td width="100%" style="border:none;">
														<img src="<?=ROOT.LOGO.$set_head['logo']?>"  style="width:85%;"/>
												...
												<table width="100%" class="maintable" style=" font-size:11px" id="invoice_type" >
															...
															<th colspan="11" style="padding:0px !important;">
																<table style="font-size:10px;border-collapse: collapse;border-top:none !important;" cellpadding="0" cellspacing="0" width="100%" >
																	...
																	<tr>
																		...
																		<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid" colspan="">: <?=$rel['order_no'];?> </td>
																		<td style="vertical-align:top;border-bottom:1px solid "><strong>Date </strong></td>
																	...
																	<tr>
																		...
																		<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid;">: <?=$rel['terms']?></td>
																	</tr>
																	<tr>
																		...
																		<td style="vertical-align:top;border-bottom:1px solid"><strong>Transport Mode</strong></td>							
																		<td style="vertical-align:top;border-bottom:1px solid;border-right:1px solid;">: <?=$r_t['common_mst_name']?></td>
																	</tr>
																	<tr>
																							
																		<td colspan="6" style="vertical-align:top;border-bottom:1px solid;border-right:1px solid;border-left:1px solid;"><strong>Reverse Charge</strong> : <?=(!empty($rel['reverse_charge'])?'Yes':'No')?></td>
																	</tr>
																	<tr id="rawnone">
																		...
																		{?>
																			<td colspan="2"  style="border-right:1px solid">
																				<b>Consignee : </b><br>
																				<strong><?=$cons_data['cust_name']?></strong><br>
																				<span style="font-weight:normal;">
													...
													<tr>
														...
														<?phpif($get_disc['discount']>0){ ?>
															...
															<th width="6%" style="text-align:center  !important;border:1px solid;border-top: none;">
																<strong>GST Amount<?=$currency_name;?></strong>
															</th>
													...
													<tbody style="border: none;">
														...
														//$tax_arr=explode(",",$row['tax_val']);
															...
															$gst_rate_conv = $row['cgst_tax_rate_conv']+$row['sgst_tax_rate_conv']+$row['igst_tax_rate_conv'];

															if($row['cgst_tax_rate_conv'] != 0 || $row['sgst_tax_rate_conv'] !=0){
																$total_cs_gst += ($rel['currency_id']==$_SESSION['currency_id']) ? $gst_rate : $gst_rate_conv;
															}else{
																$total_i_gst += ($rel['currency_id']==$_SESSION['currency_id']) ? $gst_rate : $gst_rate_conv;
															} ?>
															<tr style="height:25px">
																...
																<?phpif($get_disc['discount']>0){?>
																	...
																	<?}?>
																	<td style="text-align:right  !important; vertical-align:top;border-bottom-color:#FFFFFF;border-right:1px solid;">
																		<?=number_format(($row['product_rate_conv'] * $row['product_qty']),2,".","")?>
																	</td>
																	<td style="text-align:right  !important; vertical-align:top;border-bottom-color:#FFFFFF;border-right:1px solid;">
																		<?=$gst_per?>%
																	</td>
																	<td style="text-align:right;vertical-align:top;border-bottom-color:#FFFFFF;border-right:1px solid;">
																		<?=number_format($gst_rate_conv,2,".","")?>
																	</td>
																	<td style="text-align:right  !important; vertical-align:top;border-bottom-color:#FFFFFF;border-right:1px solid;">
																		<?=number_format($row['total_conv'],2,".","")?>
																	</td>
																</tr>
															...
															<tr style="height:20px">
																...
																<td style="border-top:1px solid;border-right:1px solid;text-align:right  !important;">
																	<strong><?=number_format($totalgst_rate,2,".","")?></strong>
																</td>
																<td style="border-top:1px solid;border-right:1px solid;text-align:right  !important;">
																	...
																	<table class="footer-table" width="100%">
																		...
																		<?php if($rel['stateid']==$set_head['stateid']){ ?>
																			<tr height="20px">
																				...
																				<td colspan="2" style="text-align:right  !important; border-top:1px solid;font-size:10px;border-left:1px solid; "><?=number_format((($total_cs_gst+$total_sundrytax)/2),2,".","")?></td>
																			</tr>
																			<tr height="20px">
																				<td  style="border-right:1px solid;border-top:1px solid; font-size:10px;" colspan="5">
																				</td>
																		...
																		<?php }else{ ?>
																			<tr height="20px">
																				...
																				<td colspan="2" style="text-align:right  !important; border-top:1px solid;font-size:10px;border-left:1px solid; "><?=number_format(($total_i_gst+$total_sundrytax),2,".","")?></td>
																			</tr>
																	...
																	<?php if($rel['formulaid']){
																		...
																		<tr height="20px">
																			<td style="border-right:1px solid;border-top:1px solid;font-size:10px;" colspan="<?=$colspan?>">
																				...
																				<?=ucwords(convert_number_to_words_new($rel['g_total_conv'],$rel['currency_id'],$currency_word_end, $currency_word_start))?>
																			</td>
																			<td colspan="3" style="border-top:1px solid;border-right:1px solid;font-size:10px;text-align:left  !important">
																				<strong>Grand Total<?=$currency_name;?></strong> :
																			</td>
																			<td colspan="2" style="text-align:right  !important; border-top:1px solid;font-size:10px;border-left:1px solid; ">
																				<strong><?=number_format($rel['g_total_conv'],0,".","").'.00'?></strong>
																			</td>
																		</tr>
																		<tr height="35px">
																...
																<tr>
																	...
																	<td colspan="5" style=" border-left:none;vertical-align:top;border-top:1px solid black;">
																		<center>
																			...
																			<br><img src="<?=DOMAIN_F.'view/upload/signature/'.$set_head['authorized_signature']?>" style="width: 100px; height: 100px;"><br>
...
function PrintMe(DivID) {
 ...
 if ($('input[name=logo]:Checked').val() == "1") {
 	$('#table_head').show();
 	$('#table_foot').show();
 	docprint.document.write('@media print{ @page { size:A4; margin: 0.1in <?=$set_head['letter_head_right_margin']?>in 0.1in <?=$set_head['letter_head_left_margin']?>in; } } ');
 }
 else
 {
