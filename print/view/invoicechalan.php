...
{	
	...
	$company_name1=$cons_data['company_name'];
												...
												<!--<thead>-->
													<tr>
														<td rowspan="4" width="55%" style="vertical-align:top;border:1px solid;padding-bottom:0px !important;">
															M/s,<br><strong><?=$company_name1?></strong><span style="font-weight:normal;"> <br><?=$cust_address?>,<br/><?=$city_name?>,<?=$state_name?>,<?=$country_name?><?php if(!empty($cust_pincode)){ ?> - <?=$cust_pincode?><?php } ?></span><br> <strong> GSTIN : <?=$gst_no?></strong>
														</td>
...
<script type="text/javascript"> 
	...
	function PrintMe(DivID) {
		...
		//	alert(<?=$set_head['letter_head_top_margin']?>);
  ...
  if ($('input[name=logo]:Checked').val() == "1") {
  	...
  	docprint.document.write(' @media print{ @page { size:A4; margin: <?=$set_head['letter_head_top_margin']?>px <?=$set_head['letter_head_right_margin']?>px <?=$set_head['letter_head_bottom_margin']?>px <?=$set_head['letter_head_left_margin']?>px; } }   ');
  
  }
  else{
    //  alert("s");
  	docprint.document.write(' @media print{ @page { size:A4; margin: <?php //=$set_head['letter_head_top_margin']?>140px <?php //=$set_head['letter_head_right_margin']?>10px <?php //=$set_head['letter_head_bottom_margin']?>10px <?php //=$set_head['letter_head_left_margin']?>15px; } }  #table_head, #table_foot { display:none }');
