...
if(strtolower($type) == 'pdf') {
	$query="SELECT sa.*, country.country_name, state.state_name, cust.stateid, state.gst_state_code, city.city_name, cust.l_name, cust.m_address, cust_pincode, cust_mobile, gst_no, cust.m_pan, cust.enable_sez,gd.gd_name from tbl_debitnote as sa 
		left join tbl_ledger as cust on cust.l_id=sa.vender_id 
		left join mst_godown as gd on gd.gd_id = sa.material_center
		left join country_mst as country on country.countryid=cust.countryid 
		left join state_mst as state on state.stateid=cust.stateid 
		left join city_mst as city on city.cityid=cust.cityid where sa.debitnote_id=$invoiceid";
	...
	$debitnote_date='';$lr_date='';$sale_return_eway_bill_date='';
	if($rel['debitnote_date']!="1970-01-01" && $rel['debitnote_date']!="0000-00-00")
	{
		$debitnote_date=date('d-m-Y',strtotime($rel['debitnote_date']));
	}
	
	$custLedgerDetails = get_cust_data_arr($dbcon,$rel['vender_id']);
	$company_config = getCompanyConfiguration($dbcon);

	$sundrytax=$dbcon->query("select b.* from tbl_bill_sundry_transaction as b where b.sundry_voucher_id=".$rel['debitnote_id']." and b.sundry_voucher_table='tbl_debitnote' and b.isdelete='0' ");
	$total_sundrytax=0;
	...
	$qry_disc=$dbcon->query("SELECT SUM(trn.product_discount) as discount FROM `tbl_debitnote_trn` as trn where debitnote_trn_status=0 and debitnote_id=".$rel['debitnote_id']);
	...
	$html ='<html>
	<head>					
	<title>Debit Note - '.$rel['debitnote_no'].'</title>
	<style type="text/css">
	/*
	.page{
		width:8.27in;
		...
		<td style="border:none;text-align: right; width:55%;" colspan="0"> <strong style="font-size:14px;">Debit Note</strong></td>
		...
		<td style="width:120px;border-bottom:1px solid; border-right:1px solid;border-top:1px solid;border-left:none !important;">: <strong>'.$rel['debitnote_no'].'</strong>
		</td>
		<td style="width:20px;border-bottom:1px solid;border-top:1px solid; border-right:none;"><strong>Date </strong>
		</td>
		<td style="width:120px;border-bottom:1px solid;border-right:1px solid;border-top:1px solid;border-left:none;">: <strong>'.$debitnote_date.'</strong>
		</td>
		<td  style="width:120px;border-bottom:1px solid;border-top:1px solid;border-right:none;"><strong>Material Center</strong>	
		</td>							
		<td colspan="2" style="border-bottom:1px solid;border-top:1px solid;border-right:1px solid;border-left:none;white-space: nowrap;">: '.$rel['gd_name'].'</td>
		</tr>
		...
		$qry="select trn.*,product.product_name,unit_name,hsn.hsn_code FROM `tbl_debitnote_trn` as trn 
		...
		where debitnote_trn_status=0 and debitnote_id=".$rel['debitnote_id']." order by debitnote_trn_id";
		...
		{
			$gst_per = $row['purchase_return_cgst_tax_per']+$row['purchase_return_sgst_tax_per']+$row['purchase_return_igst_tax_per'];
			$gst_rate = $row['purchase_return_cgst_tax_amt']+$row['purchase_return_sgst_tax_amt']+$row['purchase_return_igst_tax_amt'];

			if($row['purchase_return_cgst_tax_amt'] != 0 || $row['purchase_return_sgst_tax_amt'] !=0){
			...
			///if($company_config['tax_editable'] == 0 && $rel['sales_type'] == 1 ){
				...
				<td style="vertical-align:top;border-right:1px solid; text-align: center;">'.number_format($row['purchase_return_total_amount'],2,".","").'</td>';
			//}
			$html.='</tr>';
			$i++; 
			...
			$total+=$row['purchase_return_total_amount'];
		...
		where b.sundry_voucher_id=".$rel['debitnote_id']." and b.sundry_voucher_table='tbl_debitnote' and b.isdelete='0' and b.sundry_ledger_id=".TCS." ";
		...
		$qry11="select sum((tc.tax_per*trn.product_amount)/100) as add_sum ,l.l_name,l.l_id,t.tax_cat_id from tbl_debitnote_trn as trn 
		left join tbl_tax_category as t on t.tax_cat_id=trn.product_tax_cat 
		left join tbl_tax_category_details as tc on tc.tax_cat=t.tax_cat_id 
		left join tbl_ledger as l on l.l_id=tc.tax_id 
		where tc.tax_additional='1' and trn.debitnote_id=".$rel['debitnote_id']." and trn.debitnote_trn_status!=2 and tc.isdelete='0' group by tc.tax_id";
		...
		from tbl_bill_sundry_transaction as b left join tbl_ledger_bill_sundry as l on l.bill_sundry_id=b.sundry_ledger_id 
		left join tbl_ledger as le on le.l_id=b.sundry_ledger_id 
		where b.sundry_voucher_id=".$rel['debitnote_id']." and b.sundry_voucher_table='tbl_debitnote' and b.isdelete='0' and le.default_sundry='0' ";
		...
		//if($company_config['tax_editable'] == 0){
			/*if($rel['sales_type']==1){*/
				...
				$query="select sum(product_amount) as product_amount, trn.product_hsn_code as product_hsn_code, purchase_return_cgst_tax_per as cgst_tax_per, sum(purchase_return_cgst_tax_amt) as cgst_tax_rate, purchase_return_sgst_tax_per as sgst_tax_per, sum(purchase_return_sgst_tax_amt) as sgst_tax_rate, purchase_return_igst_tax_per as igst_tax_per,sum(purchase_return_igst_tax_amt) as igst_tax_rate FROM `tbl_debitnote_trn` as trn 
				
				where debitnote_trn_status=0 and debitnote_id=".$rel['debitnote_id']." group by trn.product_hsn_code";
				...
				$sundrytax1=$dbcon->query("select b.*,tl.ledger_hsn from tbl_bill_sundry_transaction as b
					...
					where b.sundry_voucher_id=".$rel['debitnote_id']." and b.sundry_voucher_table='tbl_invoice' and b.isdelete='0' ");
