...
//>Plot No : 117 , Nr. GETCO Sub -Station , Old GIDC , Gundlav Valsad - 396035, Gujarat, India
	...
	.page{
		...
		if($rel['inquiry_type'] == 2)
		{
			...
			$trn_qry="select *,product.product_name, hsn.hsn_code as product_hsn_code, product.image_name FROM `tbl_salesorder_project_trn` as trn 
		...
		$trn_qry="select *,product.product_name, hsn.hsn_code as product_hsn_code, product.image_name FROM `tbl_sales_ordertrn` as trn 
		left join product_mst as product on product.product_id=trn.product_id
		left join mst_hsn_code as hsn on hsn.hsn_id=product.product_hsn
		...
		while($trn_rel=mysqli_fetch_assoc($trn_qry_rs)){
			...
			$html.='<td border="0" style="border-radious: 0px; border: none!important;'.$align.'"><strong>'.$trn_rel['product_name'].''.$item_code.'</strong><br/>'.$product_desc.'</td> 
			</tr>
			</table>';
			...
			<td style="vertical-align: top; text-align:center;">'.without_comma_two_digit_amount($trn_rel['product_qty']).' </td>
			<td style="vertical-align: top; text-align:center;"> '.without_comma_two_digit_amount($trn_rel['product_rate']).'</td>
			<td style="vertical-align: top; text-align:center;">'.without_comma_two_digit_amount($trn_rel['discount_per']).' % <br>('.without_comma_two_digit_amount($trn_rel['product_discount']).')</td>
			<td style="vertical-align: top; text-align:center;"> '.without_comma_two_digit_amount($trn_rel['product_amount']).'</td>
			</tr>';
		...
		<td  style=" text-align:center;">'.indian_number($ttl_amt,2).'</td>
		</tr>';
		if($rel['stateid']==$comp_rel['stateid']){
			$html.='<tr>
			<td colspan="8" style=" text-align:right;">CGST</td>
			<td  style=" text-align:center;">'.number_format(($total_cs_gst/2),2,".","").'</td>
			</tr><tr>
			...
			<td  style=" text-align:center;">'.number_format(($total_i_gst),2,".","").'</td>
			</tr>';
		}
		...
		while($row11=mysqli_fetch_assoc($result11))
		{
			...
			<td  style=" text-align:center;">'.number_format($row11['add_sum'],2,".","").'</td>
			</tr>';
		}
		...
		while($row12=mysqli_fetch_assoc($result12))
		{
			...
			<td  style=" text-align:center;">'.number_format($row12['sundry_amount'],2,".","").'</td>
			</tr>';
		}
		...
		<td  style=" text-align:center;font-size:14px;font-weight:bold;background-color:	#b3b3b3">'.indian_number(($rel['g_total']),2).' </td>
		</tr>';
		...
		Total Order Value (In Words): <span style="font-weight:bold;font-size:13px;">'.(($rel['currency_id']=='1') ? convert_number_to_words_new($rel['g_total']) : convert_number_to_words($rel['g_total'],$currency_word_start)).'</span></td>
		</tr>
		<tr style="">
		<td colspan="9" style=" text-align:left;font-size:14px;">
		Total Tax (In Words): <span style="font-weight:bold;font-size:13px;">'.(($rel['currency_id']=='1') ? convert_number_to_words_new($gst_rate) : convert_number_to_words($rel['g_total'],$currency_word_start)).'</span></td>
		</tr>
		<tr style="">
		...
		if($rel['stateid']==$comp_rel['stateid']){
		 ...
		 $trn_qry_tax="select sum(product_amount) as product_amount,product_hsn_code,cgst_tax_per,sum(cgst_tax_rate) as cgst_tax_rate,sgst_tax_per,sum(sgst_tax_rate) as sgst_tax_rate,igst_tax_per,sum(igst_tax_rate) as igst_tax_rate FROM `tbl_sales_ordertrn` where sales_ordertrn_status=0 and sales_order_id='".$rel['sales_order_id']."' group by product_hsn_code";
		...
		while($trn_rel_tax=brp_mysqli_fetch_assoc($trn_qry_rs_tax)){
			...
			if($rel['stateid']==$comp_rel['stateid']){
				...
				'.number_format($trn_rel_tax['cgst_tax_rate'],2,".","").'
				</td>
				<td style="text-align:right;font-size:12px;">
				'.str_replace("SGST","",$trn_rel_tax['cgst_tax_per']).'
				</td>
				<td style="text-align:right;font-size:12px;">
				'.number_format($trn_rel_tax['sgst_tax_rate'],2,".","").'
				</td>';
				$k++;
				...
				<td style="text-align:right;font-size:12px;">
				'.number_format($trn_rel_tax['igst_tax_rate'],2,".","").'
				</td>';
				$k++;
