...
if(strtolower($type) == 'pdf') {
	...
	.page{
		...
		<td style="border:none;text-align: right; width:55%;" colspan="0"> <strong style="font-size:14px;">'.(($rel["invoice_type"]) ? $rel["invoice_type"] : 'TAX INVOICE').'</strong></td>
		...
		$html.='<tr>
		<td colspan ="3" rowspan="5"  style="width:50%;border-bottom:1px solid;border-top:1px solid;border-right:none;">
		<strong style="font-size:16px">'.$set_head['company_name'].'</strong><br>
			...
			Email   : '.strtolower($set_head['website']).'<br>
			Pan No : '.$set_head['pan_no'].'<br>
			Udyam Reg. No. : '.$set_head['lut_no'].'	
		</td>							
		
		<td style="width:11%;border:1px solid; border-right:none !important;"><strong>Invoice No. </strong>
		</td>
		<td style="width:16%;border-bottom:1px solid; border-right:1px solid;border-top:1px solid;border-left:none !important;">: <strong>'.$rel['invoice_no'].'</strong>
		</td>
		<td style="width:11%;border-bottom:1px solid;border-top:1px solid; border-right:none;"><strong>Date </strong>
		</td>
		<td style="width:13%;border-bottom:1px solid;border-right:1px solid;border-top:1px solid;border-left:none;">: <strong>'.date('d-m-Y',strtotime($rel['invoice_date'])).'</strong>
		</td>
		...
		<tr>
		<td colspan="3"  style="vertical-align:top;border-right:1px solid;border-left:1px solid;"><b>Bill to Party : </b><br/><strong>'.$rel['l_name'].'</strong><br><span style="font-weight:normal;">'.$rel['m_address'].'<br/>'.$rel['city_name'].', '.$rel['state_name'].', '.$rel['country_name'];
		...
		if($rel['enable_consignee']==1) {
			$html.='<td colspan="4"  style="vertical-align:top;border-right:1px solid"><b>Ship to Party : </b><br><strong>'.$rel['l_name'].'</strong><br><span style="font-weight:normal;">'.$rel['m_address'].'<br/>'.$rel['city_name'].', '.$rel['state_name'].', '.$rel['country_name'];
			if(!empty($rel['cust_pincode'])){
				$html.='-  '.$rel['cust_pincode'];
			}
			$html.='</span><br>';
			...
			$html.='<td colspan="4"  style="vertical-align:top;border-right:1px solid"><b>Consignee : </b><br><strong>'.$cons_data['company_name'].'</strong><span style="font-weight:normal;">'.$cons_data['m_address'].'<br/>'.$cons_data['city_name'].', '.$cons_data['state_name'].', '.$cons_data['country_name'];
		...
		if($rel['enable_consignee']==1) {
			$html.='<tr>
			<td colspan="3" style="border-right:1px solid;border-left:1px solid;"><strong>GSTIN: '.$rel['gst_no'].' <br>PAN No. : '.$rel['m_pan'].'</strong></td>
			<td colspan="4" style="border-right:1px solid; vertical-align: top;"><strong>GSTIN:  '.$rel['gst_no'].'</strong></td>
			</tr>
			<tr> 
			<td colspan="2" style="border-left:1px solid;border-bottom:1px solid;font-weight:normal;width:40%">State : '.$rel['state_name'].'</td>
			<td style="border-right:1px solid;text-align:left;border-bottom:1px solid;border-right:1px solid;font-weight:normal;width:10%">Code : '.$rel['gst_state_code'].'</td>
			<td colspan="3" style="text-align:left;border-bottom:1px solid;font-weight:normal;">State : '.$rel['state_name'].'</td>
			<td style="text-align:left;border-bottom:1px solid;border-right:1px solid;font-weight:normal;">Code : '.$rel['gst_state_code'].'</td>
			</tr>';
		} else {
			$html.='<tr>
			<td colspan="3" style="border-right:1px solid;border-left:1px solid;"><strong>GSTIN:'.$rel['gst_no'].' </strong></td>
			<td colspan="4" style="border-right:1px solid;"><strong>GSTIN: '.$cons_gst_no.'</strong></td>
			</tr>
			<tr> 
			<td colspan="2" style="border-left:1px solid;border-bottom:1px solid;font-weight:normal;white-space:nowrap;width:40%">State : '.$rel['state_name'].'</td>
			<td  style="border-right:1px solid;text-align:left;border-bottom:1px solid;border-right:1px solid;font-weight:normal;width:10%">Code : '.$cons_data['gst_state_code'].'</td>
			<td colspan="3" style="text-align:left;border-bottom:1px solid;font-weight:normal;">State : '.$cons_state_name.'</td>
		...
		$html.='<tr height="20px">
		<td style="border-right:1px solid;border-top:1px solid;"><strong>GST Amount(In Words):</strong> '.ucwords(convert_number_to_words_new($total_sundrytax+$total_i_gst+$total_cs_gst,$rel['currency_id'],$rel['currency_in_word_end'],$rel['currency_in_word'])).'</td>
		...
		<tr height="20px">
		<td style="border-right:1px solid;border-top:1px solid;"><strong>Rupees:</strong>'.ucwords(convert_number_to_words_new(round($rel['g_total']),$rel['currency_id'],$rel['currency_in_word_end'],$rel['currency_in_word'])).'</td>
