...
if(strtolower($POST['mode']) == "fetch") {
	...
	$appData = array();
	$i=1;
	$aColumns = array('pro.product_id','pro.product_name','pro.product_icode','pro.product_base_qty','pro.product_conv_qty','pro.product_base_unit','pro.product_conv_unit');
	...
	$sTable = "product_mst as pro";			
		...
		foreach($sqlReturn as $row) {
			...
			$row_data[] = '<a class="" data-original-title="View '.$row["product_id"].'" data-toggle="tooltip" data-placement="top">'.$row["product_name"].' -- ('.$row['product_icode'].')'.'</a>';
			...
			if(in_array(OPENING_STOCK_LIST_SLUG_CREATE,$bulkAccessArray)){
				$add = '<a class="btn btn-xs btn-primary" data-original-title="Add Stock" data-toggle="tooltip" data-placement="top" onClick="show_add_stock_modal('.$row["product_id"].','.$same_unit.','.$row['product_base_qty'].','.$row['product_conv_qty'].')"><i class="fa fa-plus"></i></a>';
			}
	...
	$sTable = "opening_stock_mst osm";			
		...
		foreach($sqlReturn as $row) {
			...
			$row_data[] = '<a class="" data-original-title="View '.$row["product_name"].'" data-toggle="tooltip" data-placement="top">'.$row["product_name"].' -- ('.$row['product_icode'].')'.'</a>';
			...
			if($row['approve_status'] == '1'){
		  		...
		  		$q1="select mst.used_base_stock from tbl_stock_trn mst where mst.stock_flage = 1 and mst.ref_id = ".$opening_stock_id ." and mst.ref_name = 'opening_stock' and product_id = ". $row['product_id']." and  stock_status = 0";
			
				$res11=$dbcon->query($q1);
				$row_stock=brp_mysqli_fetch_array($res11);
				$stock_apprv_btn = "";
				if(in_array(OPENING_STOCK_LIST_APPROVE,$bulkAccessArray)){
				if($row_stock['used_base_stock'] > 0){
			...
			$stiker="";
			if($row['approve_status'] == '1'){
		  		$stiker='<a class="btn btn-xs btn-info" data-original-title="Sticker Print" data-toggle="tooltip" data-placement="top" href="'.ROOT.INVENTORY_ROOT.'opening_sticker_common_print/'.$row['opening_stock_id'].'"><i class="fa fa-barcode"></i></a> ';
		  	}
			
			$row_data[] =$stock_apprv_btn.' '.$stiker;
	...
	}else if(strtolower($POST['mode'])=="check_batch_permission")
	{
		...
		$same_unit = $POST['same_unit'];

		$query="select product_base_unit,product_conv_unit, product_name, branch_id from product_mst where product_id=".$POST['product_id'];
		...
		while($product_process=brp_mysqli_fetch_assoc($rel_pro)){
			...
			$str .= '<div class="form-group row">
				...
				<div class="col-md-3 text-right">
					<h3> '. $product_process["process_name"].' </h3>
				</div>
				<div class="col-md-12 mtop20">
					<label class="col-md-3 control-label text-right"> Stock Qty</label>
			...
			$str .= '<div class="col-md-12 m-bot15">
						<div class="col-md-12">
							<div class="form-group">
								<label class="col-md-3 control-label text-right">Process Rate * <a href="#"  data-original-title="Total Rate for entered stock quantity." data-toggle="tooltip" data-placement="top"><i class="fa fa-info-circle fa-2x"></i></a></label>
								<div class="col-md-9 col-xs-11 getstock">
									<div style="display:flex;" class="col-md-6">
										<input type="number" class="form-control" name="pro_base_rate[]" id="base_rate'. $product_process["process_id"].'" onkeyup="process_convert_rate(1,\'rate\','.$product_process["process_id"].');" />
										<span style="color: red;font-weight: 600;margin: 10px;" class="btn btn-round btn-warning btn-xs unit_show"  id="unit_show_rate" > '.ucwords($_SESSION['currency_code']).' </span>
									</div>
									<div id="convert_unit_block"  class="col-md-6 convert_unit_block" style="display:none;" >
										<div style="display:flex;">
											<input type="number"  title="Enter Qty" min="0" id="conv_rate'. $product_process["process_id"].'" name="pro_conv_rate[]"  class="form-control" onkeyup="process_convert_rate(2,\'rate\','.$product_process["process_id"].');" />
											<span style="color: #105a03;font-weight: 600;margin: 10px;" class="btn btn-round btn-warning btn-xs convert_unit_show" id="convert_unit_show_rate"> '.ucwords($_SESSION['currency_code']).' </span>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="col-md-12">
						<div class="form-group">
							<label class="col-md-3 control-label text-right">Process Stock Rate * <a href="#"  data-original-title="Total Rate for entered stock quantity." data-toggle="tooltip" data-placement="top"><i class="fa fa-info-circle fa-2x"></i></a></label>
							<div class="col-md-9 col-xs-11 getstock">
								<div style="display:flex;" class="col-md-6">
									<input type="number" class="form-control" name="stock_base_rate[]" id="stock_base_rate'. $product_process["process_id"].'" onkeyup="process_convert_rate(1,\'stock\','.$product_process["process_id"].');" />
									<span style="color: red;font-weight: 600;margin: 10px;" class="btn btn-round btn-warning btn-xs unit_show" id="unit_show_rate" > '.ucwords($_SESSION['currency_code']).' </span>
								</div>
								<div id="convert_unit_block"  class="col-md-6 convert_unit_block" style="display:none;" >
									<div style="display:flex;">
										<input type="number"  title="Enter Qty" min="0" id="stock_conv_rate'. $product_process["process_id"].'" name="stock_conv_rate[]"  class="form-control" onkeyup="process_convert_rate(2,\'stock\','.$product_process["process_id"].');" />
										<span style="color: #105a03;font-weight: 600;margin: 10px;" class="btn btn-round btn-warning  btn-xs convert_unit_show" id="convert_unit_show_rate"> '.ucwords($_SESSION['currency_code']).' </span>
									</div>
								</div>
							</div>
				...
				$str .= '<div class="form-group">
									...
									<div class="col-md-5 col-xs-11">
                                        <input id="process_mfg_date" name="process_mfg_date[]" type="text" class="form-control default-date-picker" title="Date" value="'.date('d-m-Y').'" placeholder="MFG Date">
                                    </div>
								</div>
								</div>';	
			$i++;
	...
	else if(strtolower($POST['mode'])== "load_product_unit")
	{
		...
		$mfg_date = $POST['mfg_date'];
		$dt = get_exp_date_by_product($dbcon,$product_id,$mfg_date);
	   ...
	   $stock['mfg_date'] = date('Y-m-d',strtotime($mfg_date));
	   ...
	   $stock['base_rate'] = $base_rate ;
	   $stock['conv_rate'] = $conv_rate;
	   $stock['status'] = "0";
	   ...
	   $arr_base_rate = $POST['pro_base_rate'];
	   $arr_conv_rate = $POST['pro_conv_rate'];
	   $arr_stock_rate = $POST['stock_base_rate'];
	   $arr_stock_conv_rate = $POST['stock_conv_rate'];
	   $arr_mfg_date = $POST['process_mfg_date'];
		...
		// $process_stock = array_combine($arr_process_id, $arr_process_stock_qty);
		 ...
		 if($inserestimateid){	
			...
			*/
			...
			$query="select batch_wise_stock_manage from product_mst where product_id=".$product_id;
			...
			if($companyConfiguration['batch_wise_stock'] == '1' && $row['batch_wise_stock_manage'] == "1"){
				$arr['batch_wise_stock_manage'] =  1;
				if($companyConfiguration["batch_stock"] == '1'){
					$batch_no = update_batch_no($dbcon,$product_id);
				}
			}
			...
			$pro_stock['opening_stock_id'] = $inserestimateid;
			$pro_stock['product_id'] = $product_id;
			$pro_stock['branch_id'] = $branch_id;
			$pro_stock['location_id'] = $location_id;
			$pro_stock['opening_stock_unit'] = $POST['unitid'];
			$pro_stock['opening_stock_conv_unit'] = $POST['conv_unitid'];
			 foreach ($arr_process_ids as $key => $process_id) {
			   ...
			   $pro_stock['process_base_rate'] = (float)$arr_base_rate[$key];
			   $pro_stock['process_conv_rate'] = (float)$arr_conv_rate[$key];

			   $pro_stock['process_stock_base_rate'] = (float)$arr_stock_rate[$key] ;
			   $pro_stock['process_stock_conv_rate'] = (float)$arr_stock_conv_rate[$key];

			   $mfg_date = $arr_mfg_date[$key];
			   $dt = get_exp_date_by_product($dbcon,$product_id,$mfg_date);
			   $exp_date = date('Y-m-d',strtotime($dt));

			    $pro_stock['mfg_date'] = date('Y-m-d',strtotime($mfg_date));
	   			$pro_stock['exp_date'] = $exp_date;

			 if($arr_process_stock_qty[$key] != "" && $arr_process_stock_qty[$key] > 0){
			   $pro_add_id=add_record('process_opening_stock_mst', $pro_stock, $dbcon);
			 }	
		  	}
                	...
                	if($POST['approve_status'] == '1'){
						...
						if($cnt>0){
							...
							// if($row['opening_stock_qty'] > $cstock){
								...
								$info_stockadd['mfg_date']					= $row['mfg_date'];
								$info_stockadd['exp_date']					= $row['exp_date'];
									...
									while($rel3=mysqli_fetch_assoc($result1)){
										...
										// if($rel3['opening_stock_qty'] > $pstock){
											...
											if($stock > 0){
												...
												$conv_stock=$rel3['opening_stock_conv_qty'];
												...
												$process_stockadd['process_base_rate']				= $rel3['process_base_rate'];
												$process_stockadd['process_conv_rate']				= $rel3['process_conv_rate'];
												$process_stockadd['process_stock_base_rate']				= $rel3['process_stock_base_rate'];
												$process_stockadd['process_stock_conv_rate']				= $rel3['process_stock_conv_rate'];

												$process_stockadd['mfg_date']					= $rel3['mfg_date'];
												$process_stockadd['exp_date']					= $rel3['exp_date'];
			...
			if($cnt1 > 0){
				...
				while($rel2=mysqli_fetch_assoc($res1)){
					...
					/* $query3 = "select mst.*,pmst.process_name from process_opening_stock_mst mst 
						left join tbl_product_process as prpro on prpro.pr_process_id = mst.process_id
						left join process_mst as pmst on prpro.process_id=pmst.process_id
						where mst.opening_stock_id = ". $rel2['opening_stock_id']; */
						$query3 = "select mst.*,pmst.process_name from process_opening_stock_mst mst 
			left join process_mst as pmst on mst.process_id=pmst.process_id where mst.opening_stock_id = ". $rel2['opening_stock_id'];
			...
			$str .= '<div class="col-md-12 m-bot15">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-9 col-xs-11 getstock">
									<div style="display:flex;" class="col-md-6">
										<input type="number" class="form-control" name="pro_base_rate[]" id="base_rate'. $product_process["process_id"].'" onkeyup="process_convert_rate(1,\'rate\','.$product_process["process_id"].');" value="'.$product_process["process_base_rate"].'"/>
										<span style="color: red;font-weight: 600;margin: 10px;" class="btn btn-round btn-warning btn-xs " id="unit_show_rate" > '.ucwords($_SESSION['currency_code']).' </span>
									</div>
									<div id="convert_unit_block"  class="col-md-6 convert_unit_block" style="display:none;" >
										<div style="display:flex;">
											<input type="number"  title="Enter Qty" min="0" id="conv_rate'. $product_process["process_id"].'" name="pro_conv_rate[]"  class="form-control" onkeyup="process_convert_rate(2,\'rate\','.$product_process["process_id"].');" value="'.$product_process["process_conv_rate"].'"/>
											<span style="color: #105a03;font-weight: 600;margin: 10px;" class="btn btn-round btn-warning  btn-xs" id="convert_unit_show_rate"> '.ucwords($_SESSION['currency_code']).' </span>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="col-md-12">
						<div class="form-group">
							...
							<div class="col-md-9 col-xs-11 getstock">
								<div style="display:flex;" class="col-md-6">
									<input type="number" class="form-control" name="stock_base_rate[]" id="stock_base_rate'. $product_process["process_id"].'" onkeyup="process_convert_rate(1,\'stock\','.$product_process["process_id"].');" value="'.$product_process["process_stock_base_rate"].'"/>
									<span style="color: red;font-weight: 600;margin: 10px;" class="btn btn-round btn-warning btn-xs " id="unit_show_rate" > '.ucwords($_SESSION['currency_code']).' </span>
								</div>
								<div id="convert_unit_block"  class="col-md-6 convert_unit_block" style="display:none;" >
									<div style="display:flex;">
										<input type="number"  title="Enter Qty" min="0" id="stock_conv_rate'. $product_process["process_id"].'" name="stock_conv_rate[]"  class="form-control" onkeyup="process_convert_rate(2,\'stock\','.$product_process["process_id"].');" value="'.$product_process["process_stock_conv_rate"].'"/>
										<span style="color: #105a03;font-weight: 600;margin: 10px;" class="btn btn-round btn-warning  btn-xs" id="convert_unit_show_rate"> '.ucwords($_SESSION['currency_code']).' </span>
									</div>
								</div>
							</div>
