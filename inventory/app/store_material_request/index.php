...
{ 
    /*if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) */
	{
		...
		$aColumns = array('pro.product_icode','dr.drawing_number','pro.product_id','pro.product_base_unit','pro.product_conv_unit','pro.product_name','tc.cat_name','pro.product_status','pro.product_min_stock', 'IFNULL(reqqty,0) as reqqty', 'req_qty', 'base_stock_add','base_stock_minus','con_stock_add','con_stock_minus','(((IFNULL(base_stock_add,0)+IFNULL(con_stock_add,0))-(IFNULL(base_stock_minus,0)+IFNULL(con_stock_minus,0)))+IFNULL(reqqty,0) + IFNULL(req_qty,0)) as stock','pro.product_category');
		...
		$isJOIN = array("left join tbl_category as tc on pro.product_category=tc.cat_id",
			...
			"left join (select sum(ord.base_request_qty) as req_qty,ord.product_id from tbl_store_order_min_max as ord where ord.status=0 and wo_complete_status = 0  group by ord.product_id) as re_req on re_req.product_id=pro.product_id",

		"left join (select sum(req.rp_req_qty-req.used_rp_req_qty) as reqqty,req.rp_pid from tbl_request_product as req where req.status=0 and used_status=0 and finish_status = 0  group by req.rp_pid) as rereq on rereq.rp_pid=pro.product_id",
		...
		group by qc.product_id) as qc2 on qc2.product_id=pro.product_id and qc2.convert_unit=pro.product_conv_unit",
		...
		group by qc.product_id) as qc3 on qc3.product_id=pro.product_id and qc3.convert_unit=pro.product_conv_unit",
		...
		foreach($sqlReturn as $row) {
			...
			if($row['product_base_unit'] == $row['product_conv_unit']){
				...
				$stock_conv = convert_stock_new($dbcon,$stock,$row['product_id'],"conv_unit");
				$product_min_stock_conv = convert_stock_new($dbcon,$row['product_min_stock'],$row['product_id'],"conv_unit");
				$reqqty_conv = convert_stock_new($dbcon,$row['reqqty'],$row['product_id'],"conv_unit");
			}
			...
			if(in_array(STOCK_DETAIL_MINMAX_SLUG_CREATE,$bulkAccessArray)){
				/*$view='<a class="btn btn-xs btn-primary" data-original-title="" data-toggle="tooltip" data-placement="top" href="'.ROOT.'production/request_product/'.$row['product_id'].'"><i class="fa fa-paper-plane"></i> Request</a>';*/   //  hide by sanat for redirect in new  page

				$view='<a class="btn btn-xs btn-primary" data-original-title="" data-toggle="tooltip" data-placement="top" href="'.ROOT.INVENTORY_ROOT.'category_wise_min_max/'.$row['product_category'].'" target="_blank"><i class="fa fa-paper-plane"></i> Request</a>';
			}
