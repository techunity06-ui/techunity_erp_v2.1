...
$pro_search=explode(",", $production_pro_search);
		...
		if(brp_strtolower($POST['mode']) == "fetch_working") {
			...
			$check_branch = check_branch('tsr', $branch_id);
			 ...
			 $s_ql = "select sum(base_qty) as total_qty,sum(release_qty) as total_release_qty,branch.branch_name,p.product_name,pr.process_name,tc.cat_name, GROUP_CONCAT(tsr.p_id) AS pids,users.user_name,ap.batch_no, p.product_icode, dr.drawing_number, smain.po_req_no as work_order_no, ap.previous_process_id, p.product_id, ap.process_id, ap.process_unit,ap.branch_id,rp.priority_status from tbl_store_request as tsr
			...
			where tsr.store_request_status = 0 ".$Product_filter." ".$check_branch." and tsr.company_id=".$_SESSION['company_id']." group by tsr.product_id, rp.priority_status, tsr.branch_id, tsr.process_id, ap.batch_no,ap.extra_stock,ap.p_id" ;
				...
				if($remaining_qty > 0){
					...
					$str.='<tr>
							...
							/*if($rel['previous_process_id']=="0"){
								$matirial_available_qty=check_row_material_availability($dbcon,$rel["pids"],0);
							}else{
						
								$matirial_available_qty=production_process_reseve_stock($dbcon,$rel['process_unit'],$rel['branch_id'],$rel["pids"],$rel['product_id'],$pre_process_id,$process_reserve_id,$process_stock_id,0);
							}*/
			...
			where ap.p_id in (".$p_id.") and ap.p_status in(0,1)";
				...
				$working_qty=production_store_wise_start_count_using_p_id($dbcon,$p_id,0);
...
// echo $query1;
			...
			while($row=brp_mysqli_fetch_array($result1)){
				$start_qty=production_store_wise_start_count_using_p_id($dbcon,$row['p_id'],0);
				...
				if($remaining_qty > 0){
					...
					$pre_process_id = 0;
					if($rel['previous_process_id']!="0" && $rel['previous_process_id'] > 0){
						$apqry = "select process_id from tbl_allocate_process where p_id = " .  $row['previous_process_id'];
						$apres = $dbcon->query($apqry);
						$aprw = brp_mysqli_fetch_array($apres);
						$pre_process_id  = $aprw['process_id'];
					}
					...
					if($row['previous_process_id']=="0"){
						...
						$matirial_available_qty=production_process_reseve_stock($dbcon,$row['process_unit'],$row['branch_id'],$row["p_id"],$row['p_product_id'],$pre_process_id,$process_reserve_id,$process_stock_id,0);
					}
				 ...
				 // if($getspecialConfiguration['hermattic_permission']=="1") {
					 ...
					 // }
						$html .='
							...
							if(is_numeric($matirial_available_qty) &&  $matirial_available_qty > 0){
								$html.='<th class="text-success"> <strong> Material Available </strong> </th>';
							}else{
								$html.='<th class="text-danger"><strong> Material Pending </strong></th>';
							}							
						$html.='</tr>';
			...
			$html .='<tr>
						...
						<th colspan="3"><input type="text" name="total_req_qty" id="total_req_qty" class="form-control" value="'.$total_req_qty.'" readonly />  '.$unit.'</th>
					</tr>';
			$html .='</table>
			</div>';
			...
			$html .='
				...
				<div class="col-md-12 mtop20">
					<div class="col-md-6">
						...
						<div col-md-6>
							<select class="select2 disabled-select" title="To Godown" id="to_godown_id" name="to_godown_id" readonly>
							 '.get_last_node_godown_list($dbcon,'-111').'
							 </select>
						</div>
					</div>
					<div class="col-md-6">
	...
	$cnt=brp_mysqli_num_rows($result1);
		  ...
		  $query2 = "select ap.extra_stock,ap.p_id,res.godown_id,trp.*,p.product_name,p.batch_wise_stock_manage,bunit.unit_name,gd.gd_name,cunit.unit_name as conv_unit_name,p.product_base_unit,p.product_conv_unit from tbl_request_product as trp 
				left join product_mst as p on p.product_id=trp.rp_pid 
				...
				where ap.p_status !=2 and trp.status !=2 and trp.perent_id = " . $row['req_id'] . " and ap.p_id = ".$row['p_id']." and ap.p_id = " . $row['p_id']. " group by rp_id";
				...
				while($row2=brp_mysqli_fetch_array($result2)){
					...
					/* $gd_qry = "SELECT gd.gd_name,res.base_stock,res.convert_stock,(select IFNULL(sum(base_stock),0) from tbl_reserve_stock where stock_status != 2 and p_id = " . $row['p_id']." and stock_flage = 2 and perent_id = res.reserve_id) as base_stock_minus,(select IFNULL(sum(convert_stock),0)  from tbl_reserve_stock where stock_status != 2 and p_id = " . $row['p_id']." and stock_flage = 2 and perent_id = res.reserve_id) as convert_stock_minus FROM tbl_reserve_stock as res 
					LEFT JOIN mst_godown as gd ON gd.gd_id = res.godown_id
					WHERE res.stock_flage = 1 and res.stock_status != 2 and res.p_id = " . $row['p_id'] . " and  product_id = " . $row2['rp_pid'];
					$gd_result = $dbcon->query($gd_qry);*/
					
					$html .= '<input type="hidden" id="stock_process_id" value=""> 
					<div id="tr_release_row_'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" class="col-md-12" style="margin-top:10px; padding-bottom:10px; border-bottom: 1px solid #dadadd;">
							...
							<!--	<label class="col-md-5 control-label" style="color: #404040;font-weight: 600;">Request Qty :</label> 
								...
								<input type="text" class="form-control material_txt_qty_'.$i.' material_qty_'.($x+1).' col-md-7" name="material_qty'.$i.'[]" id="material_qty'.$i.'" data-work_order_no="'.$row["work_order_no"].'" value="'.$used_qty. '" data-pid="'.$row['p_id'].'" data-product_id = "'.$row2['rp_pid'].'" data-request_id = "'.$row2['rp_id'].'" data-req_qty="'.$used_qty.'" onchange="check_material_start_validation('.$i.');" onkeyup="material_convert_qty(2,'.$row2['rp_pid'].','.$i.');"/> <span class="col-md-5" style="color: #0e8400;font-weight: 600;margin-top:8px" > 
								' . $unitname .'</span>
								</div>	
								...
								<label class="col-md-12 control-label" style="color: #404040;font-weight: 600;">Request Qty : <span style="color: #0e8400;font-weight: 600;"> '.$used_qty.' ' . $unitname .'</span></label>
									<input type="hidden" id="mt_req_qty_'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" value="'.$used_qty.'" class="form-control material_txt_qty_'.$i.' material_qty_'.($x+1).' col-md-7" name="material_qty'.$i.'[]"  data-work_order_no="'.$row["work_order_no"].'" data-pid="'.$row['p_id'].'" data-product_id = "'.$row2['rp_pid'].'" data-request_id = "'.$row2['rp_id'].'" data-req_qty="'.$used_qty.'">
									...
									<input type="hidden" id="mt_req_qty_'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" value="'.$c_used_qty.'" class="form-control material_txt_qty_'.$i.' material_qty2_'.($x+1).' col-md-7" name="material_qty'.$i.'[]"  data-work_order_no="'.$row["work_order_no"].'" data-pid="'.$row['p_id'].'" data-product_id = "'.$row2['rp_pid'].'" data-request_id = "'.$row2['rp_id'].'" data-req_qty="'.$c_used_qty.'">
							...
							<div class="col-md-12">
							  <div class="col-md-12">
												<div class="form-group">
													<div class="col-md-12 col-xs-11">
														<table cellspacing="10" style=" border-spacing:10px;" class="display table table-bordered table-striped">
															...
															if($row2['batch_wise_stock_manage']=='1'){
																$html .='<td width="20%" >
																	<select class="form-control godown_select" name="godown_id" id="godown_id'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" title="Select Godown" onchange="load_batch_no(this.value,'.$row2['rp_pid'].','.$row2['process_unit'].','.$row2['p_id'].',\'\',\'\',\''.$row2['rp_id'].'\',\''.$row['req_id'].'\');">
																	'.load_available_stock_godown($dbcon,$row2['rp_pid'],0).'
																	</select>
																	</td><td>
																	<select class="form-control batch_no" name="batch_no" id="batch_no'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" title="Select Godown" onchange="get_stock_mfg_date('.$row2['batch_wise_stock_manage'].','.$row2['rp_pid'].','.$row2['process_unit'].','.$row2['p_id'].');
																	load_batch_godown('.$row2['batch_wise_stock_manage'].','.$row2['rp_pid'].','.$row2['process_unit'].','.$row2['p_id'].');load_stock_qty('.$row2['batch_wise_stock_manage'].','.$row2['rp_pid'].','.$row2['process_unit'].','.$row2['p_id'].','.$row2['rp_id'].');
																	">
																	'.get_load_stock_batch_no($dbcon,$row2['rp_pid'],"",$process_id,$previous_process_id).'
																	</select> </br>
																	<span class="text-danger bold ">MFG Date : </span><span class="text-danger bold" id="mfg_date'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'"></span>
																</td>';
															}else{
																$html .= '<td width="20%" >
																			<select class="form-control godown_select" name="godown_id" id="godown_id'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" title="Select Godown" onchange="load_stock_qty('.$row2['batch_wise_stock_manage'].','.$row2['rp_pid'].','.$row2['process_unit'].','.$row2['p_id'].','.$row2['rp_id'].')">
																			'.load_available_stock_godown($dbcon,$row2['rp_pid'],0).'
																			</select>
																		</td>';
															}
															...
															<div class="col-md-10">
															<input id="stock_qty'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" name="stock_qty" type="text" class="form-control numbersOnly" title="Stock Qty" value="" placeholder="Stock Qty" readonly >
															</div>
															...
															<input id="stock_qty2'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" name="stock_qty" type="text" class="form-control numbersOnly" title="Stock Qty" value="" placeholder="Stock Qty" readonly >
															</div>
															...
															<!--	<td width="20%" class="text-center">
																...
																<td width="30%" >
																<div class="col-md-10">
																<input id="release_qty'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" name="release_qty" type="number" class="form-control numbersOnly" title="Release Qty" value="" placeholder="Release Qty"  onkeyup="convert_qty(2,'.$row2['p_id'].','.$row2['rp_pid'].','.$row2['rp_id'].');">
																</div>
																<div class="col-md-2" style="padding: 0;margin-top: 5px;">
																<label class="col-md-12 control-label" style="color: #0e8400;font-weight: 600;padding: 0;"> ' . $unitname .'</span></label>
																<input type="hidden" name="unit_id" id ="unit_id'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" value="'.$row2['process_unit'].'"/>
																</div>
																<div class="col-md-10" style="margin-top: 10px;">
																<input id="release_qty2'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" name="release_qty" type="number" class="form-control numbersOnly" title="Release Qty" value="" placeholder="Release Qty" readonly>
																</div>
																<div class="col-md-2" style="padding: 0;margin-top: 15px;">
																<label class="col-md-12 control-label" style="color: #0e8400;font-weight: 600;padding: 0;"> ' . $conv_unitname .'</span></label>
																</div>
												...
												<div id="release_productdata'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'">';
												auto_add_reserve_godown_material_entry($dbcon,$row2,$used_qty,$row['previous_process_id']);
								$arr_mat_data = get_material_temp_data($dbcon,$row2['p_id'],$row2['rp_pid'],$row2['process_unit'],$row2['batch_wise_stock_manage'],0,$row2['rp_id']);	
											if($row2['extra_stock'] == '1'){
												$html .= '<input type="hidden" id="total_rel_material_'.$row2['p_id'].'_'.$row2['rp_pid'].'_'.$row2['rp_id'].'" value="'.$used_qty.'">';
											}			
							...
							<!--	<label class="col-md-5 control-label" style="color: #404040;font-weight: 600;"></label> 
								<div class="col-md-7" style="display:flex;margin-top:5px">
								<input type="text" readonly class="form-control conv_material_txt_qty_'.$i.' conv_material_qty_'.($x+1).' col-md-7" name="conv_material_qty'.$i.'[]" id="conv_material_qty'.$i.'" data-work_order_no="'.$row["work_order_no"].'" value="'.$c_used_qty. '" data-pid="'.$row['p_id'].'" data-product_id = "'.$row2['rp_pid'].'" data-request_id = "'.$row2['rp_id'].'" data-req_qty="'.$c_used_qty.'" /> <span class="col-md-5" style="color: #0e8400;font-weight: 600;margin-top:8px"> 
								' . $conv_unitname .'</span>
								</div> -->
							 ...
							 /* if(brp_mysqli_num_rows($gd_result) > 0){
							  		 $html.='<div class="col-md-12">
							  		<div class="col-md-2">
							  			<label class="col-md-5 control-label" style="color: #404040;font-weight: 600;">Godown</label> 
							  		</div>
							  		<div class="col-md-10">';
							  		
							  		while($gd_rw = brp_mysqli_fetch_assoc($gd_result)){
							  			$pen_base_stock = $gd_rw['base_stock'] - $gd_rw['base_stock_minus'];
							  			$pen_conv_stock =$gd_rw['convert_stock'] - $gd_rw['convert_stock_minus'];

							  			$html.= '<p style="margin:0"><strong>'.$gd_rw['gd_name'].' : </strong> <span style="color: #0e8400;font-weight: 600;"> 
								' . $pen_base_stock .' '. $unitname .'</span><span  style="color: #0e8400;font-weight: 600;margin-left:10px"> 
								' . $pen_conv_stock .' '. $conv_unitname .'</span>';
				...
				$row_2=brp_mysqli_fetch_array($res_2);
					...
					$html .= '<input type="hidden" id="stock_process_id" value="'.$row_2["process_id"].'"> 
					<div id="tr_release_row_'.$row['p_id'].'_'.$row['rp_pid'].'" class="col-md-12" style="margin-top:10px;">
							<div class="col-md-6">
							  ...
							  <div class="col-md-6">
								...
								<label class="col-md-12 control-label" style="color: #404040;font-weight: 600;">Request Qty : <span style="color: #0e8400;font-weight: 600;"> '.$req_qty.' ' . $unitname .'</span></label>
									<input type="hidden" id="mt_req_qty_'.$row['p_id'].'_'.$row['rp_pid'].'" value="'.$req_qty.'" class="form-control material_txt_qty_'.$i.' material_qty_'.($x+1).' col-md-7" name="material_qty'.$i.'[]"  data-work_order_no="'.$row["work_order_no"].'" data-pid="'.$row['p_id'].'" data-product_id = "'.$row['rp_pid'].'" data-request_id = "'.$row['req_id'].'" data-req_qty="'.$req_qty.'">
							...
							<div class="col-md-12">
							  <div class="col-md-12">
												<div class="form-group">
													<div class="col-md-12 col-xs-11">
														<table cellspacing="10" style=" border-spacing:10px;" class="display table table-bordered table-striped">
															...
															if($row['batch_wise_stock_manage']=='1'){
																$html .='<td width="20%" >
																	<select class="form-control godown_select" name="godown_id" id="godown_id'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" title="Select Godown" onchange="load_batch_no(this.value,'.$row['rp_pid'].','.$row['process_unit'].','.$row['p_id'].');
																	">
																	'.load_available_stock_godown($dbcon,$row['rp_pid'],0).'
																	</select>
																	</td><td>
																	<select class="form-control batch_no" name="batch_no" id="batch_no'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" title="Select Godown" onchange="get_stock_mfg_date('.$row['batch_wise_stock_manage'].','.$row['rp_pid'].','.$row['process_unit'].','.$row['p_id'].',1);
																	load_stock_qty('.$row['batch_wise_stock_manage'].','.$row['rp_pid'].','.$row['process_unit'].','.$row['p_id'].','.$row['req_id'].',1);
																	">'.get_load_stock_batch_no($dbcon,$row2['rp_pid'],"",$process_id,$previous_process_id).'
																	</select>
																	 </br>
																	<span class="text-danger bold">MFG Date : </span><span class="text-danger bold" id="mfg_date'.$row['p_id'].'_'.$row['rp_pid'].'"></span>
																</td>';
															}else{
																$html .= '<td width="20%" >
																<select class="form-control godown_select" name="godown_id" id="godown_id'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" title="Select Godown" onchange="load_stock_qty('.$row['batch_wise_stock_manage'].','.$row['rp_pid'].','.$row['process_unit'].','.$row['p_id'].','.$row['req_id'].',1)">
																'.load_available_stock_godown($dbcon,$row['rp_pid'],0).'
															</select>
																		</td>';
															}
															...
															$html .='
																...
																<td width="30%" >
																<div class="col-md-10">
																<input id="stock_qty'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" name="stock_qty" type="text" class="form-control numbersOnly" title="Stock Qty" value="" placeholder="Stock Qty" readonly >
																</div>
																<div class="col-md-2" style="padding: 0;margin-top: 5px;">
																<label class="col-md-12 control-label" style="color: #0e8400;font-weight: 600;padding: 0;"> ' . $unitname .'</span></label>
																...
																<div class="col-md-10" style="margin-top: 10px;">
																<input id="stock_qty2'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" name="stock_qty" type="text" class="form-control numbersOnly" title="Stock Qty" value="" placeholder="Stock Qty" readonly > </div>
																<div class="col-md-2" style="padding: 0;margin-top: 15px;">
																<label class="col-md-12 control-label" style="color: #0e8400;font-weight: 600;padding: 0;"> ' . $conv_unitname .'</span></label>
															...
															<!--	<td width="20%" class="text-center">
																...
																<td width="30%" >
																<div class="col-md-10">
															<input id="release_qty'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" name="release_qty" type="number" class="form-control numbersOnly" onkeyup="convert_qty(1,'.$row['p_id'].','.$row['rp_pid'].','.$row['req_id'].');" title="Release Qty" value="" placeholder="Release Qty">
															</div>
															<div class="col-md-2" style="padding: 0;margin-top: 5px;">
																<label class="col-md-12 control-label" style="color: #0e8400;font-weight: 600;padding: 0;"> ' . $unitname .'</span></label>
																<input type="hidden" name="unit_id" id ="unit_id'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" value="'.$row['process_unit'].'"/>
															</div>
															<div class="col-md-10" style="margin-top: 10px;">
																<input id="release_qty2'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" name="release_qty" type="number" class="form-control numbersOnly" title="Release Qty" value="" placeholder="Release Qty" readonly>
															</div>
															<div class="col-md-2" style="padding: 0;margin-top: 15px;">
																<label class="col-md-12 control-label" style="color: #0e8400;font-weight: 600;padding: 0;"> ' . $conv_unitname .'</span></label>
																<input type="hidden" name="unit_id" id ="unit_id'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" value="'.$row['process_unit'].'"/>
												...
												<div id="release_productdata'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'">';
												auto_add_reserve_godown_material_entry($dbcon,$row,$req_qty,$row['previous_process_id']);
								$arr_mat_data = get_material_temp_data($dbcon,$row['p_id'],$row['rp_pid'],$row['process_unit'],$row['batch_wise_stock_manage'],1,$row['req_id']);	
								if($row['extra_stock'] == '1'){
												$html .= '<input type="hidden" id="total_rel_material_'.$row['p_id'].'_'.$row['rp_pid'].'_'.$row['req_id'].'" value="'.$req_qty.'">';
											}				
											$html .= $arr_mat_data['material'].'
												</div>
												<div class="clearfix"></div>	
		...
		}else if(strtolower($POST['mode'])=="load_stock_qty")
		{
			...
			if($process_stock == '1'){
				...
				$stock = get_current_process_stock_new($dbcon, $product_id, $pre_process_id, $unit_id, $branch_id = "", $POST['godown_id'],$stock_id);

				// 	var_dump($stock);die;

					$rstock = get_material_temp_reserve_stock($dbcon, $product_id, $unit_id, $godown_id,$stock_id,$p_id,$stock_id);		
					...
					if($pro_row['product_base_unit'] == $pro_row['product_conv_unit']){
						...
						$conv_stock =  round_up($stock2,5);
						$arr['stock_2'] = $conv_stock;
					}else{
				...
				// if (in_array($get_pro_type_rel['product_type'], $product_type_arr)){
					...
					$rstock = get_material_temp_reserve_stock($dbcon, $product_id, $unit_id, $POST['godown_id'],$stock_id,$POST['p_id'],$stock_id);
					...
					$r_stock=reserve_stock_other_workorder($dbcon, $POST['product_id'],$POST['unit_id'], "", "","", "", $branch_id, $is_store_approval, $p_id, $godown_id, $stock_id,$customer_id = "");
                    // $rstock = round_up($rstock,5);

                    /*var_dump($current_stock);
                    var_dump($rstock);
                    var_dump($r_stock);*/
					$stock=$current_stock-$rstock-$r_stock;	
			...
			while($row_dstock=brp_mysqli_fetch_assoc($result_dstock)){
				...
				if($release_qty>0){
					...
					if($row['product_conv_unit']==$unit_id){
						...
						$base_stock=($rqty/$row_dstock['convert_stock'])*$row_dstock['base_stock'];
		...
		$str = get_material_temp_data($dbcon,$p_id,$product_id,$unit_id,$res['batch_wise_stock_manage'],$previous_process_id,$rp_id);
	...
	}else if(strtolower($POST['mode'])== "load_batch_no")
	{
		...
		$data = get_load_stock_batch_no($dbcon,$product_id,$godwn_id,$process_id,$previous_process_id);

		echo $data;
	}else if(strtolower($POST['mode'])== "batch_wise_stock_details")
	{
		$getspecialConfiguration=getspecialConfiguration($dbcon);
		...
		$str = '<div class="row">
			<div class="col-md-6 text-center mtop20">
				<h4> Product Name : '.$product_name.'</h4>
			</div>
			<div class="col-md-6 text-center mtop20">
				<h4> Required QTY : '. $req_qty . '  '. $unitname .'</h4>
			</div>
			</hr>
		</div>';
		...
		if($previous_process_id == '0'){
			...
			where stock_status!=2 and stock_flage = 1 and product_id = ".$product_id." and batch_no != '' ".$whr." group by batch_no HAVING pending_base_stock > 0";
		}else{
			 $query="select i.*,(cast(base_stock AS DECIMAL(22,5)) - IFNULL((select sum(base_stock) from tbl_process_stock_trn where stock_status = 0 and stock_flage = 2 and perent_id = i.process_stock_id),0)) as pending_base_stock,(cast(conv_stock AS DECIMAL(22,5)) - IFNULL((select sum(conv_stock) from tbl_process_stock_trn where stock_status = 0 and stock_flage = 2 and perent_id = i.process_stock_id),0)) as pending_conv_stock,group_concat(i.process_stock_id) as b_stock_id from tbl_process_stock_trn as i
			where stock_status!=2 and stock_flage = 1 and  process_id = ".$process_id ." and  product_id = ".$product_id." and batch_no != '' ".$whr." group by batch_no HAVING pending_base_stock > 0";
		}
		...
		$str .= '<div class="row">
			...
			$rs_cnt  = 	brp_mysqli_num_rows($rs_batch);
			if($rs_cnt > 0){
				$str .= '<input type="hidden" id="batch_wise_p_id" value="'.$p_id.'">';
				$str .= '<input type="hidden" id="batch_wise_godown_id" value="'.$godwn_id.'">';
				$str .= '<input type="hidden" id="batch_wise_parent_rp_id" value="'.$parent_rp_id.'">';
				$str .= '<input type="hidden" id="batch_wise_product_id" value="'.$product_id.'">';
				$str .= '<input type="hidden" id="batch_wise_process_id" value="'.$process_id.'">';
				$str .= '<input type="hidden" id="batch_wise_rp_id" value="'.$rp_id.'">';
				$str .= '<input type="hidden" id="batch_wise_unit_id" value="'.$unit_id.'">';
				$str .= '<input type="hidden" id="batch_wise_parent_rp_id" value="'.$parent_rp_id.'">';
				$str .= '<input type="hidden" id="batch_wise_previous_process_id" value="'.$previous_process_id.'">';
				$str .= '<input type="hidden" id="total_temp_batch_qty" value="'.$req_qty.'">';
				$str .= '<div class="row">
							<div class="col-md-6">
								<div class="col-md-6 text-right">
									<label> Godown Tranfer <a href="#"  data-original-title="IF YES THAN NON SELECTED BATCH STOCK WILL BE TRANSFER TO SELECTED GODOWN" data-toggle="tooltip" data-placement="top"><i class="fa fa-info-circle"></a></i> </label>
								</div>

								<div class="col-md-6 text-left">
									<label class="radio-inline"><input type="radio" name="opt_godown_transfer" checked value="0" onchange="toggle_res_batch_godown(0)">No</label>
									<label class="radio-inline"><input type="radio" name="opt_godown_transfer" value="1" onchange="toggle_res_batch_godown(1)">YES</label>
								</div>
							</div>
							<div class="col-md-6 res_batch_godown" style="display:none">
								<div class="col-md-4 text-right">
									<h4> Godown : </h4>
								</div>

								<div class="col-md-8">
									<select class="select2" title="To Godown" id="transfer_godown_id" name="transfer_godown_id">
				...
				$str .= '<div class="">
							<div class="col-md-12 text-center mtop20">
								<table cellspacing="10" style=" border-spacing:10px;" class="display table table-bordered table-striped">
									<tr id="field">
										<th width="5%" class="text-center chk_batch"><input id="checkAll" type="checkbox" onclick="checkAll();"  name="chk[]"/></th>
										<th width="20%" class="text-center">Batch No</th>
										<th width="20%" class="text-center">Godown</th>
										<th width="20%" class="text-center">Stock Qty</th>
										<th width="20%" class="text-center">Reserve Qty</th>
									</tr>
								<tbody id="field1">';
				...
				{	
					...
					$bt_value= "value='0'";
					$rstock = get_material_temp_reserve_stock($dbcon, $product_id, $rel['base_unit'], $godwn_id,"",$rel['p_id'],$rel['stock_id']);
					if($getspecialConfiguration['jet_technologies_permission'] == '1'){
						$bt_value= 'value="'.($rel['pending_base_stock'] - $rstock).'"';
					}
					if($rel['pending_base_stock'] - $rstock > 0){
						$str.='<tr>';
						$str.='<td><input type="checkbox" id="chk_batch'.$x.'" name="chk_batch[]" value="'.$x.'">';
						$str.='<td>'.$rel['batch_no'].'</td>';
						$str.='<td>'.get_godown_name($dbcon,$rel['godown_id']).'</td>';
						$str.='<td>'.($rel['pending_base_stock']- $rstock) .' '. $unitname .'</td>';
						$str.='<td><input type="text" class="form-control reserve_batch_qty" id="reserve_batch_qty'.$x.'" data-current_stock_qty="'.($rel['pending_base_stock']- $rstock).'" data-stock_id ="'.$rel['b_stock_id'].'" data-batch_no ="'.$rel['batch_no'].'" name="reserve_batch_qty[]" '.$bt_value.'>' .' '. $unitname .'</td>';
						$str.='</tr>';
						$x++;
					}
				}
				...
				$str .= '<p><strong>No Batch Data !!</strong></p>
				</div>
				</div>';
			}
			if($rs_cnt > 0){

				$str .='<div class="col-md-12 text-center" style="margin:25px;">
						<input type="button"  id="back_btn" name="back" class="btn btn-success" value="Release" onclick="batch_wise_stock_add();">
						
						<button type="button" id="batch_btn_close" onclick="batchcloseModal()" class="btn_close btn btn-danger" style="margin-left:10px" >Close</button>
					</div>';
			}
	...
	}else if(strtolower($POST['mode'])== "convert_qty")
	{
		...
		$row['hide_qty']=$ret_qty;
		echo json_encode($row);
	}else if(strtolower($POST['mode'])=="load_batch_godown")
	{
		$product_id=$POST['product_id'];
		$process_stock=$POST['process_stock'];
		$stock_id=$POST['stock_id'];
		...
		if($process_stock == '1'){
			$query = "SELECT godown_id FROM tbl_process_stock_trn where  product_id = ".$product_id." and  process_stock_id in (".$stock_id.")";
		}else{
			$query = "SELECT godown_id FROM tbl_stock_trn where product_id = ".$product_id." and stock_id in (".$stock_id.")";
		}

		$row = brp_mysqli_fetch_assoc($dbcon->query($query));

		echo json_encode($row);
	}else if(strtolower($POST['mode'])=="get_stock_mfg_date")
	{
		$product_id=$POST['product_id'];
		...
		if($process_stock == '1'){
			$qry = "SELECT mfg_date from tbl_process_stock_trn WHERE stock_status != 2 and stock_flage = 1 and process_stock_id in(".$stock_id.")";
		}else{
			$qry = "SELECT mfg_date from tbl_stock_trn WHERE stock_status != 2 and stock_flage = 1 and stock_id in(".$stock_id.")";
		}
		
		$row = brp_mysqli_fetch_assoc($dbcon->query($qry));
		if(!empty($row['mfg_date'])){
			$date = date('d-m-Y',strtotime($row['mfg_date']));
		}else {
			$date = "";
		}
		echo $date;
	}else if(brp_strtolower($POST['mode']) == "batch_wise_stock_fieldadd") {
			...
			$batch_no 		= $POST['batch_no'];
			$process_id 	= $POST['process_id'];
			$arr_release_qty	= $POST["qty"];
			$arr_stock_id 		= $POST['stock_id'];
			$arr_batch_no 		= $POST['batch_no'];
			$godown_transfer 		= $POST['godown_transfer'];
			$transfer_godown_id 		= $POST['transfer_godown_id'];
			$arr_transfer_stock_id 		= $POST['transfer_stock_id'];
			$arr_transfer_batch_no 		= $POST['transfer_batch_no'];
			...
			for($i=0;$i<count($arr_release_qty);$i++){
				$whr = '';
				$release_qty = $arr_release_qty[$i];
				if($is_process_stock == 1){
					$whr .= " and batch_no = '".$arr_batch_no[$i]."'";
					$whr .= " and process_stock_id in(".$arr_stock_id[$i].")";
					
					 $query_dstock = "select i.*,(cast(base_stock AS DECIMAL(22,5)) - IFNULL((select sum(base_stock) from tbl_process_stock_trn where stock_status = 0 and stock_flage = 2 and perent_id = i.process_stock_id),0)) as pending_base_stock,(cast(conv_stock AS DECIMAL(22,5)) - IFNULL((select sum(conv_stock) from tbl_process_stock_trn where stock_status = 0 and stock_flage = 2 and perent_id = i.process_stock_id),0)) as pending_conv_stock from tbl_process_stock_trn as i where stock_status=0 and stock_flage=1 and i.product_id=".$product_id." and i.godown_id=".$godown_id.$whr;
				}else{
					$whr .= " and batch_no = '".$arr_batch_no[$i]."'";
					$whr .= " and stock_id in(".$arr_stock_id[$i].")";
					$query_dstock = "select i.*,(cast(base_stock AS DECIMAL(22,5)) - IFNULL((select sum(base_stock) from tbl_stock_trn where stock_status = 0 and stock_flage = 2 and perent_id = i.stock_id),0)) as pending_base_stock,(cast(convert_stock AS DECIMAL(22,5)) - IFNULL((select sum(convert_stock) from tbl_stock_trn where stock_status = 0 and stock_flage = 2 and perent_id = i.stock_id),0)) as pending_conv_stock from tbl_stock_trn as i where stock_status=0 and stock_flage=1 and i.product_id=".$product_id." and i.godown_id=".$godown_id.$whr;
				}
			...
			if($godown_transfer == '1' && $godown_id != $transfer_godown_id){
				for($x=0;$x<count($arr_transfer_stock_id);$x++){
					$whr = '';
					if($is_process_stock == 1){
						$whr .= " and batch_no = '".$arr_transfer_batch_no[$x]."'";
						$whr .= " and process_stock_id in(".$arr_transfer_stock_id[$x].")";
						
						 $query_dstock = "select i.*,(cast(base_stock AS DECIMAL(22,5)) - cast(used_base_stock AS DECIMAL(22,5))) as pending_base_stock,(cast(conv_stock AS DECIMAL(22,5)) - cast(used_convert_stock AS DECIMAL(22,5))) as pending_conv_stock from tbl_process_stock_trn as i where stock_status=0 and stock_flage=1 and i.product_id=".$product_id." and i.godown_id=".$godown_id.$whr;
					}else{
						$whr .= " and batch_no = '".$arr_batch_no[$x]."'";
						$whr .= " and stock_id in(".$arr_stock_id[$x].")";
						$query_dstock = "select i.*,(cast(base_stock AS DECIMAL(22,5)) - cast(used_base_stock AS DECIMAL(22,5))) as pending_base_stock,(cast(convert_stock AS DECIMAL(22,5)) - cast(used_convert_stock AS DECIMAL(22,5))) as pending_conv_stock from tbl_stock_trn as i where stock_status=0 and stock_flage=1 and i.product_id=".$product_id." and i.godown_id=".$godown_id.$whr;
					}
					echo $query_dstock;
					echo "</br></br>"; 
					$result_dstock=$dbcon->query($query_dstock);
					...
					while($row_dstock=brp_mysqli_fetch_assoc($result_dstock)){
						...
						if($is_process_stock == 1){
							$perent_stock_id = $arr_reserve_stock['process_stock_id'];
							unset($arr_reserve_stock['process_stock_id']);	

							$arr_reserve_stock['base_stock'] =  $pending_stock;
							$arr_reserve_stock['conv_stock'] = $pending_conv_stock;
						}else{
							$perent_stock_id = $arr_reserve_stock['stock_id'];
							unset($arr_reserve_stock['stock_id']);
							$arr_reserve_stock['base_stock'] = $pending_stock;
						...
						$arr_reserve_stock['perent_id'] = $perent_stock_id;
						$arr_reserve_stock['stock_flage'] = 2;
						$arr_reserve_stock['ref_name'] = 'store_release_transfer';
						if($is_process_stock == 1){
							$new_rs_id=add_record('tbl_process_stock_trn', $arr_reserve_stock, $dbcon);
						}else{
							$new_rs_id=add_record('tbl_stock_trn', $arr_reserve_stock, $dbcon);
						}

						update_transfer_bach_wise_stock($dbcon,$perent_stock_id,$pending_stock,$pending_conv_stock,$is_process_stock);

						$arr_reserve_stock['stock_flage'] = 1;
						$arr_reserve_stock['ref_name'] = 'store_release_transfer';
						$arr_reserve_stock['godown_id'] = $transfer_godown_id;
						$arr_reserve_stock['perent_id'] = 0;
...
function update_transfer_bach_wise_stock($dbcon,$stock_id,$base_stock,$conv_stock,$is_process_stock){


	if($is_process_stock == '1'){
		$query = "SELECT process_stock_id,used_base_stock,used_convert_stock FROM tbl_process_stock_trn WHERE process_stock_id = " . $stock_id;
	}else{
		$query = "SELECT process_stock_id,used_base_stock,used_convert_stock FROM tbl_stock_trn WHERE stock_id = " . $stock_id;
	}

	$result = $dbcon->query($query);
	$row = brp_mysqli_fetch_assoc($result);

	$info['used_base_stock'] = $row['used_base_stock'] + $base_stock;
	$info['used_convert_stock'] = $row['used_convert_stock'] + $conv_stock;

	if($is_process_stock == '1'){
		$updateid=update_record('tbl_process_stock_trn', $info, "process_stock_id = " . $stock_id, $dbcon);
	}else{
		$updateid=update_record('tbl_stock_trn', $info, "stock_id = " . $stock_id, $dbcon);
	}
}


function get_material_temp_data($dbcon,$p_id,$product_id,$unit_id,$batch_wise_stock_manage,$previous_process_id,$rp_id=""){

	$where = "";
	$group = "";
	if(!empty($rp_id) && $rp_id > 0){
		$where = " and trn.rp_id = " .$rp_id;
	}
	...
	where status = 3 and p_id =" .$p_id . " and product_id =" . $product_id. $where." group by godown_id".$group;
	$result = $dbcon->query($query); 
/*echo $query;
echo "<br><br>";*/
	$str = "";
	...
	if(brp_mysqli_num_rows($result) > 0){
		$str .= '<div class="col-md-12">
					<table cellspacing="10" style=" border-spacing:10px;" class="display table table-bordered table-striped">
						...
						if($batch_wise_stock_manage == '1'){
							...
							$str .= '<th width="20%" class="text-center">Mfg Date</th>';			
						}
		...
		while($row = brp_mysqli_fetch_assoc($result)){
			if($previous_process_id == '0'){
				$mfg_qry = "SELECT mfg_date from tbl_stock_trn WHERE stock_status = 0 and stock_id in (".$row['stock_id'].")";
			}else{
				$mfg_qry = "SELECT mfg_date from tbl_process_stock_trn WHERE stock_status = 0 and process_stock_id in (".$row['stock_id'].")";	
			}

			$mfg_row = brp_mysqli_fetch_assoc($dbcon->query($mfg_qry));
			...
			if($batch_wise_stock_manage == '1'){
							...
							$str .= '<td>'.date("d-m-Y",strtotime($mfg_row['mfg_date'])).'</th>';			
						}
				...
				<td>'.$conv_qty.'</td>
				<td>'.$conv_unitname.'</td>
				<td class="text-center"><button class="btn btn-xs btn-danger" data-original-title="Delete" data-toggle="tooltip" data-placement="top" onClick="delete_material_temp_data('.$material_trn_id.','.$p_id.','.$product_id.','.$unit_id.','.$row['rp_id'].')"><i class="fa fa-trash-o"></i></button></td>
			</tr>';
			$x++;
		}
		$str .= '<input type="hidden" id="total_rel_material_'.$p_id.'_'.$product_id.'_'.$rp_id.'" value="'.$total_rel_qty.'">';
		$str .= "</tbody></table></div>";
	}	
...
function get_load_stock_batch_no($dbcon,$product_id,$godwn_id,$process_id,$previous_process_id){
	$str = "";
	/*$query="select batch_no,stock_id from tbl_stock_trn as trn
		...
		/* $query="select i.*,(IFNULL(sum(base_stock),0)-IFNULL(sum(used_base_stock),0)) as pending_base_stock,(IFNULL(sum(convert_stock),0)-IFNULL(sum(used_convert_stock),0)) as pending_conv_stock,group_concat(i.stock_id) as b_stock_id from tbl_stock_trn as i
			...
			$rs_batch=$dbcon->query($query);
			$str="";
			if(brp_mysqli_num_rows($rs_batch)>0)
			{
...
function auto_add_reserve_godown_material_entry($dbcon,$req_data,$qty,$previous_process_id){
	...
	if($previous_process_id == '0'){
		...
		$query_dstock = "select st.batch_no,i.*,(cast(i.base_stock AS DECIMAL(22,5)) - IFNULL((select IFNULL(sum(base_stock),0) as bstock from tbl_reserve_stock where stock_status != 2 and request_id =" .$req_data['rp_id']. " and stock_flage = 2 and perent_id = i.reserve_id and p_id=".$req_data['p_id'] ." and stock_id = i.stock_id),0)) as pending_base_stock,(cast(i.convert_stock AS DECIMAL(22,5)) - IFNULL((select IFNULL(sum(convert_stock),0) as cstock from tbl_reserve_stock where stock_status != 2 and stock_flage = 2 and request_id =" .$req_data['rp_id']. "  and p_id=".$req_data['p_id'] ." and stock_id = i.stock_id and perent_id = i.reserve_id),0)) as pending_conv_stock from tbl_reserve_stock as i left join tbl_stock_trn as st on st.stock_id = i.stock_id where i.stock_status != 2 and i.stock_flage=1 and i.ref_name!='store_release' and i.request_id =" .$req_data['rp_id']. " and i.product_id=".$req_data['rp_pid']." and i.p_id=".$req_data['p_id'];
	}else{
	...
	while($row_dstock=brp_mysqli_fetch_assoc($result_dstock)){
		...
		$st_base_stock = $row_dstock['base_stock'];
		$st_conv_stock = 0;

		if($previous_process_id == '0'){
			$st_conv_stock  = $row_dstock['convert_stock'];
		}else{
			$st_conv_stock  = $row_dstock['conv_stock'];
		}
		...
		if($release_qty>0 && $pending_stock > 0){
			if($pending_stock>=$release_qty){
				$rqty=$release_qty;
				$release_qty=$release_qty-$release_qty;
			}else{
			...
			$con_stock = ($base_stock/$st_base_stock) * $st_conv_stock;
			
			$info1['base_qty']		= $base_stock;
			$info1['conv_qty']		= $con_stock;
			if($previous_process_id == '0'){
				$info1['stock_id']		= $row_dstock['reserve_id'];
			}else{
				$info1['stock_id']		= $row_dstock['process_reserve_id'];
			}
				// var_dump($info1);
			if($base_stock > 0){
				$inserpoid=add_record('tbl_material_release_trn',$info1, $dbcon);	
