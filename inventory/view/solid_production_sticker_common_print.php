...
if(!in_array(PRODUCTION_STORE_LIST_APPROVE,$bulkAccessArray)){
	...
	$type_pri=$dbcon->real_escape_string($_REQUEST['rtype']);
...
<script type="text/javascript">
							...
							<section class="panel">
								...
								<div class="">
									<ul class="breadcrumb">
									  ...
									  <li class="active"><a href="<?=ROOT.'inventory/stock_general_list'?>"> Printing</a></li>
									</ul>
								</div>
							</section>
						</div>
   ...
   //QRcode End*/
	  ...
	  if($type_pri==1){
		//mixing
		$where=" and grn_trn.ref_name='mixing'";

	  }else if($type_pri==2){
		$where=" and grn_trn.ref_name='exe'";
	  }else if($type_pri==3){
		$where=" and grn_trn.ref_name='printing'";
	  }

        $query_inward="SELECT grn_trn.batch_no,pro.product_name,pro.product_icode,grn_trn.base_stock,unit.unit_name FROM tbl_stock_trn as grn_trn 
        	left join product_mst as pro on pro.product_id=grn_trn.product_id
        	left join unit_mst as unit on unit.unitid=grn_trn.base_unit
				where grn_trn.stock_flage=1 and grn_trn.stock_status=0 and grn_trn.ref_id= ".$stock_gen_id.$where;
				$rs_inward=$dbcon->query($query_inward);
				$s=1;
   ...
   ?>	
					...
					border-collapse:separate;font-weight:bold;" id="batch<?=$i?>" >
						...
						<tr style="font-size: 12px;">
							...
							<td  >: <?=$batch_rel['base_stock']?> <?=$batch_rel['unit_name']?></td>
						</tr>
						<!-- <tr style="font-size: 12px;">
