...
$("#store_release_add").on('submit',function(e) {

	var form = this;
	e.preventDefault();
	e.stopPropagation();	
	/* if (!$("#store_release_add").valid()) {
		return false;
	} */
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(response == '1') {
				...
				toastr.success("METERIAL RELEASE SUCCESSFULLY", "SUCCESS");
				
				//if(redirect_page=='working_process_detail_list' || redirect_page==''){
					window.location=root_domain+inventory_domain+'production_request_pending_material_list';
				//}else{
			...
			$('#store_release_add').trigger('reset');	
		},
...
function get_series_no(){
	...
	$.ajax({
		type: "POST",
		url: root_domain+inventory_domain+'app/allocate_process/',
