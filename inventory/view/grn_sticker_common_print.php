	...
	<body>
		<section id="container" >
			...
			<?
				...
				where grn_trn.grn_trn_status=0 and  grn_trn.grn_id = " .$grn_id;
				...
				while($batch_rel=mysqli_fetch_assoc($rs_inward)){
					if($x == 1){ ?>
						<?php
	$data_string='Item Name='.$batch_rel['product_name'].',Item Code='.$batch_rel['product_icode'].',Vendor Name='.$batch_rel['l_name'].',Purchase Bill No='.$batch_rel['invoice_no'].',Grn No='.$batch_rel['grn_no'].',Grn Date='.$batch_rel['grn_date'];
