...
function load_po_req_datatable()
{
	...
	if(po_type_status==1){
		...
		$("#po-req-table-done").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+purchase_domain+'app/purchase_order_req/',
			"fnServerParams": function ( aoData ) {
				aoData.push( 
					{ "name": "mode", "value": "fetch_done" },
...
function req_po_to_main_po(){
	...
	if(purchaseordertrn_id.length > 0){
		...
		$.ajax({
			type: "POST",
			url: root_domain+purchase_domain+'app/purchase_order_req/',
			data: { mode : "req_po_to_main_po", purchaseordertrn_id : purchaseordertrn_id, purchaseorder_id:purchaseorder_id },
			success: function(data){
				//console.log(data);
				//alert('OK');
				window.location=root_domain+purchase_domain+'po_req/'+purchaseorder_id;
				Unloading();
			}
		});
	}
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				...
				window.location=root_domain+purchase_domain+'po_req/'+vender_id+'/'+branch_id;
			}
			else if(arr.msg == '0') {
				...
				window.location=root_domain+purchase_domain+'po_req_list';
			}
...
function get_product(vender_id)
{
	...
	var sp_id = $("#sales_order_id").val();
	var product_cat = $("#product_cat").val();
	Loading();
	$.ajax({
		type: "POST",
		url: root_domain+purchase_domain+'app/purchase_order_req/',
		data: { mode : "get_product", vender_id:vender_id,workorder_id:workorder_id,sp_id:sp_id,product_cat:product_cat},
		success: function(response)
		{
...
function copy_qty(val,id){
	//alert(1)
	val = $("#pro_qty"+id).val();
	$("#product_alloc_qty"+id).val(val);
}
function copy_qty1(val,id){
	apr_qty = $("#pro_q"+id).val();
	$("#product_alloc_qty"+id).val(apr_qty);
}
function get_work_o_no(){
	var sp_id = $("#sales_order_id").val();
	$.ajax({
		type: "POST",
		url: root_domain+purchase_domain+'app/purchase_order_req/',
		data: { mode : "get_work_o_no",sp_id:sp_id},
		success: function(responce){
			$('#workorder_id').html(responce);
		}
	});
}

function show_remark(purchaseordertrn_id){
	Loading(true);
	$.ajax({
		type: "POST",
		url: root_domain+purchase_domain+'app/purchase_order_req/',
		data: { mode : "show_remark", purchaseordertrn_id:purchaseordertrn_id  },
		success: function(response)
		{
			$("#indent_remark_detail").modal("show");
			$("#indent_remark").html(response);
			Unloading();
		}
	});	
}