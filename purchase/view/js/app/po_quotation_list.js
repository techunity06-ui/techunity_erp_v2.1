...
$("#approve_indent_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+purchase_domain+'app/po_quotation_list/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				...
				window.location=root_domain+purchase_domain+arr.redirect;
			}
			else if(arr.msg == '0') {
...
function load_po_req_datatable()
{
	...
	datatable = $("#po-req-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+purchase_domain+'app/po_quotation_list/',
