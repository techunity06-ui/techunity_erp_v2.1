...
function delete_data_temp_products(){
	$.ajax({
		type: "POST",
		url: root_domain+purchase_domain+'app/purchase/',
		data: { mode : "delete_data_temp_products" },
		success: function(response)
		{
...
function addBillSundry(){
	...
	if(bill_sundry_value == 0)
	{
		...
		$.ajax({
	        ...
	        basic_amount:basic_amount,netamount:netamount,default_amount:bill_sundry_amount,invoice_id:eid,currency_enable:currency_enable,currency_id:currency_id,currency_rate:currency_rate,invoice_date:$('#po_date').val(),vender_id:vender_id,sales_type:$("#sales_type").val()},
	        success: function(response)
	        {
	        	//alert(response);
