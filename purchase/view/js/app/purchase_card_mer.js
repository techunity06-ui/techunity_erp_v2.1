...
$("#purchasecard_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+purchase_domain+'app/purchase_card_mer/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				...
				window.location=root_domain+purchase_domain+'po_card_list';
			}
			else if(arr.msg == '0') {
...
function load_purchase_card(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+purchase_domain+'app/purchase_card_mer/',
	...
	Loading(true);
		$.ajax({
			...
			success: function(response)
			{
				...
				$("#quotation_date").datepicker('setDate', null);
				$("#price").val('');
				$("#grate").val('');
				$("#lead_time").val('');
				$("#quotation_no").val('');
		...
		if(r) {
			...
			$.ajax({
				...
				success: function(response)
				{
					//console.log(response)
					if(response.trim() == "1") {
						toastr.success("PURCHASE CARD DELETE SUCCESSFULLY", "SUCCESS");
						load_purchase_card();
					}
					else if(response.trim() == "0") {
						toastr.warning("SOMETHING WRONG", "WARNING");
...
function add_pocard_apprv_hist(){
	...
	var form_data = {
		...
		party_purchase_id:$('#po_card_id').val()
	};
...
function showledger(){
	...
	$("#ledger_add_type").val('purchase_card');
	$("#ledger_name").focus();
}

function showledger1(){
	branch_id = $('#branch_id').val();
	if(!branch_id){
		toastr.warning("Choose Branch!!!", "ERROR");
		$('#branch_id').select2('focus');
	...
	$('#modal-add-ledger').modal('show');
	get_opening_balance('0');
	$("#ledger_add_type").val('purchase_card1');
	$("#ledger_name").focus();
}
