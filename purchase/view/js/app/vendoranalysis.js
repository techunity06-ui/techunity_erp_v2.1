...
function generate_chart()
{
	...
	$.getJSON(mainurl, function(json) {
		...
		function onClick(e){
			...
			$.ajax({
				type: "POST",
				url: root_domain+purchase_domain+'app/vendoranalysis/',
				data: { mode : "allanalysisreport", vendor_id:dvendor_id,rep_po_date:rep_po_date },
...
function generate_report_vendor_analysis() 
{
	...
	var from_delivery_date=$("#from_delivery_date").val();
	var to_delivery_date=$("#to_delivery_date").val();
	var report_wise = 'allanalysisreport';
	var to_po_date=$("#to_po_date").val();
	var rep_po_date=$("#rep_po_date").val();
	var rep_del_date=$("#rep_del_date").val();
	var item_status=0;
...
function report_require(val){
	if(val =='allanalysisreport'){
