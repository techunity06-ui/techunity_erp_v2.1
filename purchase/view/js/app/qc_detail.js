...
$("#qc_add").on('submit',function(e) {
	...
	if(acc > 0 && $("#qc_godown").val() == "" || $("#qc_godown").val() == '0'){
		toastr.warning("PLEASE SELECT ACCEPT QTY GODOWN", "ERROR");
		return false;
	}

	if(rej > 0 && $("#qc_reject_godown").val() == "" || $("#qc_reject_godown").val() == '0'){
		toastr.warning("PLEASE SELECT REJECT QTY GODOWN", "ERROR");
		return false;
	}

	if(repro > 0 && $("#qc_reporcess_godown").val() == "" || $("#qc_reporcess_godown").val() == '0'){
		toastr.warning("PLEASE SELECT REPROCESS QTY GODOWN", "ERROR");
		return false;
	}
...
function load_datatable()
{
	...
	datatable = $("#dynamic-table").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain+purchase_domain+'app/qc_detail/',
...
function save_store_confirmation(){
	...
	$("#auto_store_relese").val(auto_store_relese);
	$("#qc_add").submit();
}