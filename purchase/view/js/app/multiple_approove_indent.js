...
function get_pending_salesorder_no(){
	var branch_id = $("#branch_id").val();
	var val1 = '';
	$.ajax({
		type: "POST",
		url: root_domain+purchase_domain+'app/multiple_approove_indent/',
		data: { mode : "sales_order_no",  branch_id : branch_id},
		success: function(responce){
			
			$('#sales_order_no').html(responce);
			$("#sales_order_no").select2("val",val1);
		}
	});
}
...
function load_pending_indent(){
	...
	$.ajax({
		type: "POST",
		url: root_domain+purchase_domain+'app/multiple_approove_indent/',
		data: { mode : "load_pending_indent", work_ono:work_ono, indent_no:indent_no, product_id:product_id,sp_id:sales_order_no,branch_id:branch_id},
		success: function(responce){
			
			var resp=JSON.parse(responce);
			$('#multiple_appr_data').html(resp.html_resp);
			
			 for(var i = 1; i < resp.row; i++)
			 {
			CKEDITOR.replace('product_desc'+i, {
...
function get_work_order_no(id){
	var val1 = '';
	$.ajax({
		type: "POST",
		url: root_domain+purchase_domain+'app/multiple_approove_indent/',
		data: { mode : "get_work_o_no",  id : id},
		success: function(responce){
...
$("#multiple_indent_approove").on('submit',function(e) {
	...
	var branch_id_1 = $('input[name="branch_id_1[]"]').val();
	...
	$("input[name='che_box[]']:Checked").each(function(){
		...
		if($("#branch_id_1"+no).val() != ""){
			branch_id_1_arr[m++]=$("#branch_id_1"+no).val();
		}else{
			branch_id_1_arr[m++]='';
		}
		
		if($("#product_desc"+no).val() != ""){
			product_desc_arr[n++]=$("#product_desc"+no).val();
		}else{
			product_desc_arr[n++]='';
		}
	...
	if(approove_id==""){
		...
		var form_data = {
			...
			branch_id : branch_id_1_arr,
			product_desc : product_desc_arr,
...
function copy_qty(val,id){
	apr_qty = $("#apr_qty"+id).val();
	$("#approve_qty"+id).val(apr_qty);
}
