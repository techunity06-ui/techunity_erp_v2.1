<?php 
	...
	$query="select po.*,poo.purchaseorder_no,poo.purchaseorder_date,state.state_name,l.l_name as vender_name,country.country_name,l.m_address as vender_address,l.gst_no as tin_no,l.cust_mobile as vender_mobile,l.stateid,state.gst_state_code,city.city_name from tbl_pono as po 
		...
		left join tbl_purchaseorder as poo on poo.purchaseorder_id=po.purchaseorder_id
	...
	$order_date='';
	if($rel['order_date']!="1970-01-01" && $rel['order_date']!="0000-00-00" && $rel['order_date']!="")
	{
		$order_date=date('d-m-Y',strtotime($rel['order_date']));
	}
	if($rel['purchaseorder_date']!="1970-01-01" && $rel['purchaseorder_date']!="0000-00-00" && $rel['purchaseorder_date']!="")
	{
...
<section id="container" >
	...
	<div class="row">			
		...
		<div class="panel-body">
			<center>
				<a href="<?=ROOT.PURCHASE_ROOT.'purchase_list'?>" type="button" class="btn btn-info"><i class="fa fa-arrow-left"></i> Back to List</a>
			<center>
	<!--<center>-->
...
<input type="hidden" name="typename" id="typename" value="<?=$rel['invoice_type']?>">
			...
			<th colspan="11" style="padding:0px !important;">
				<table style="font-size:12px;border-collapse: collapse;border-top:none;" cellpadding="0" cellspacing="0" width="100%" >
					<!--<thead>-->	
						...
						<tr>
							<td width="61.50%" rowspan="5" style="vertical-align:top;border:1px solid;">
							<b>To, </b><br/>
							...
							<td width="20%" style="vertical-align:top;border:1px solid;border-right:none;"><strong>Purchase No </strong>
							</td>
							<td width="18.50%" colspan="" style="vertical-align:top;border:1px solid;border-left:none;"> : <strong><?=$rel['po_no']?></strong>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<td width="15%"  style="vertical-align:top;border:1px solid;border-right:none;"><strong>Purchase Order No </strong>
							</td>
							<td colspan="" style="vertical-align:top;border:1px solid;border-left:none;"> : <strong><?=$rel['purchaseorder_no']?></strong>
							</td>
						</tr>
						<tr>
							<td style="vertical-align:top;border:1px solid;border-right:none;white-space:nowrap;"><strong>Purchase Order Date </strong>
							</td>
							<td style="vertical-align:top;border:1px solid;border-left:none;"> : <strong><?=$purchaseorder_date?></strong>
							</td>
						</tr>
	...
	$qry="select trn.*,product.*,unit_name,group_concat(tax.tax_value) as tax_val,group_concat(tax.tax_name) as tax_name FROM `tbl_potrancation` as trn 
		...
		<tr style="">
					...
					<td style="text-align:right;vertical-align:top;border-bottom-color:#FFFFFF;border-right:1px solid;">
						<?php //=number_format($row['tax_amount1']+$row['tax_amount2'],2,".","")?>
		...
		<?php } ?>
			...
			<tr>
				...
				<table class="footer-table" width="100%">
					...
					<?php 
						$rt=$dbcon->query("select mst.*,t.tax_name,sum(tax_amount) as tamo from tbl_used_tax as mst 
						left join tbl_tax as t on t.tax_id=mst.tax_id 
						left join tbl_potrancation as trn on trn.potrancation_id=mst.used_transaction_id
						where trn.po_id='$purchaseorder_id' and tax_used_status=0 and table_name='tbl_potrancation' group by tax_id");
						while($rel1=mysqli_fetch_assoc($rt))
						{
						
							$rt1=$dbcon->query("select mst.*,t.tax_name,sum(tax_amount) as tamo from tbl_used_tax as mst 
							left join tbl_tax as t on t.tax_id=mst.tax_id 
							left join tbl_potrancation as trn on trn.potrancation_id=mst.used_transaction_id
							where trn.po_id='$purchaseorder_id' and mst.tax_id=".$rel1['tax_id']." and tax_used_status=0 and table_name='tbl_potrancation' ");
		...
		<?php 
				$total=($total)+$rel['packing']+$rel['misc_charges']; 
				
				if($rel['misc_charges']!="0.00"){ ?>
					<tr height="20px">
						<td style="border-right:1px solid;border-top:1px solid;" colspan="<?=$colspan?>">
							
						</td>
						<td colspan="3" style="border-top:1px solid;border-right:1px solid;text-align:left">Misc Charges </td>
						<td colspan="2" style="text-align:right; border-top:1px solid;border-right:1px solid; "><?=number_format($rel['misc_charges'],2,".","")?></td>
					</tr>
			<?php 	}
