<?php 
   ...
   //echo $_SERVER['REQUEST_URI'];
    ...
    if($purchase_type=='' || $vender_id==''){
        header('location:'.ROOT.'purchase_card_vendor');
        exit;
    }
    $sql = "SELECT `ppp`.`party_purchase_id`, `pm`.`product_name` ,`pm`.`product_icode` ,`pm`.`product_desc` ,`ppp`.`party_product`, `um`.`unit_name` FROM `tbl_product_party_purchase` as ppp left join product_mst as pm ON `ppp`.`party_product`=`pm`.`product_id` left join unit_mst as um ON `pm`.`product_conv_unit`=`um`.`unitid`  WHERE `ppp`.`party_id` = '".$vender_id."' ORDER BY `ppp`.`party_purchase_id` DESC";
                                                         ...
                                                         <tbody>
                                                            <?php 
                                                               ...
                                                               while($rel=mysqli_fetch_assoc($result))
                                                                { 
                                                                   $getItem = getItemPriceByVendorId($dbcon, $vender_id, $rel['party_product']); // parameters (connection, vendorid, productid)
                                                                     if($getItem){
                                                                 ...
                                                                 ?>
                                                            <tr onclick="get_item_information('<?=$rel['party_product']?>', '<?=$vender_id?>', '<?=$purchase_type?>')" class="item_<?=$i?>">
                                                        ...
                                                        <div class="col-md-6 bio-graph-info">
                                                           <div class="row">
                                                              ...
                                                              <div class="row">
                                                                 ...
                                                                 <?php } ?>
                                                                 <a href="<?=ROOT.PURCHASE_ROOT.'purchase_card_vendor'?>" type="button" class="btn btn-danger">Cancel</a>
