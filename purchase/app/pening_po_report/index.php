...
else if(strtolower($POST['mode']) == "po_product_report")
{
	...
	$where ='';
	$where.="  and podel.delivery_date >= '".date('Y-m-d',strtotime($s_date[0]))."' AND podel.delivery_date <= '".date('Y-m-d',strtotime($s_date[1]))."'";
	...
	$query = "select req.indent_no, req.indent_date, po.purchaseorder_no, po.purchaseorder_date,led.l_name,pro.product_name,ptrn.product_qty,ptrn.product_conv_qty,ptrn.product_rate,ptrn.purchaseordertrn_id,bun.unit_name as base_unit,cun.unit_name as conv_unit,us.user_name,(select IFNULL(sum(product_qty),0) as qty  from tbl_grn_sub_trn as chtrn  where chtrn.status=0 and chtrn.purchaseordertrn_id=ptrn.purchaseordertrn_id and ptrn.product_id=chtrn.product_id) as done_qty, (select IFNULL(sum(product_conv_qty),0) as qty  from tbl_grn_sub_trn as chtrn
		where chtrn.status=0 and chtrn.purchaseordertrn_id=ptrn.purchaseordertrn_id and ptrn.product_id=chtrn.product_id) as done_conv_qty, ptrn.unit_id, ptrn.conv_unit_id 

	from tbl_purchaseorder as po
	left join tbl_purchaseordertrn as ptrn on ptrn.purchaseorder_id = po.purchaseorder_id
	left join tbl_purchaseorder_delivery_date as podel on podel.purchaseordertrn_id = ptrn.purchaseordertrn_id
	...
	where po.status=0 and ptrn.purchaseordertrn_status=0 and podel.po_delivery_date_status=0 and ptrn.used_status=0 and po.po_approval_status=1 and po.revise_status=0 ".$where." group by ptrn.purchaseordertrn_id";
	...
	if(brp_mysqli_num_rows($result)>0){
		while($row = brp_mysqli_fetch_array($result)){
			...
			if($row['unit_id'] == $row['conv_unit_id']){
				...
				$pending_qty1 = '<strong style="color:green">'.number_format($pending_qty,4,'.','').' '.$row['base_unit'].'</strong>';
			}else{
				$product_qty = '<strong style="color:green">'.number_format($row['product_qty'],4,'.','').' '.$row['base_unit'].'</strong><br><strong style="color:orange">'.number_format($row['product_conv_qty'],4,'.','').' '.$row['conv_unit'].'</strong>';
				$pending_qty = $row['product_qty'] - $row['done_qty'];
				$pending_conv_qty = $row['product_conv_qty'] - $row['done_conv_qty'];
				$pending_qty1 = '<strong style="color:green">'.number_format($pending_qty,4,'.','').' '.$row['base_unit'].'<br><strong style="color:orange">'.number_format($pending_conv_qty,4,'.','').' '.$row['conv_unit'].'</strong>';

				$qty_conv_total = $qty_conv_total + $row['product_conv_qty'];
				$pqty_conv_total = $pqty_conv_total + $pending_conv_qty;	
			}
		...
		$str.='<tr>
			...
			<td style="text-align:right"><strong style="color:green">'.number_format($pqty_total,4,'.','').'</strong><br><strong style="color:orange">'.number_format($pqty_conv_total,4,'.','').'</strong></td>
