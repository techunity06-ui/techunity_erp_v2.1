...
$bulkAccessArray = canCheckPermissionAccess($dbcon, [
	...
	PURCHASE_BILL_APPROVE,
	PURCHASE_BILL_FINAL_APPROVE
]);
...
if (strtolower($POST['mode']) == "fetch") {
	...
	foreach ($sqlReturn as $row) {
		...
		if (in_array(PURCHASE_BILL_UPDATE, $bulkAccessArray)) {
			...
			$delete = '<button class="btn btn-xs btn-danger" data-original-title="Delete" data-toggle="tooltip" data-placement="top" onClick="delete_purchase(' . $row['po_id'] . ')"><i class="fa fa-trash-o"></i>delte</button>';
		}
	...
	if ($POST['currency_id'] == $_SESSION['currency_id']) {
		...
		$info['tds_amount_conv'] = (float) ($_POST['tds_amount'] ?? 0) * (float) $_POST['currency_rate'];
	...
	if ($inserpoid) {
		...
		foreach ($POST['bill_sundry_tax'] as $bill_sundry_tax_id => $bill_sundry_tax_amount) {
			...
			if ($POST['currency_id'] == $_SESSION['currency_id']) {
				...
				$info_sundry_tax['sundry_amount_conv'] = (float) $bill_sundry_tax_amount * (float) $_POST['currency_rate'];
			} else {
			...
			if ($bill_sundry_tax_id == 9870) {
				...
				if ($POST['currency_id'] == $_SESSION['currency_id']) {
					...
					$infogsttax['sgst_conv'] = (float) $bill_sundry_tax_amount * (float) $_POST['currency_rate'];
				} else {
...
*/
    ...
    try {
        ...
        $updateinvoiceid = update_record('tbl_pono', $info, "po_id=".$_POST['eid'], $dbcon);
        if(!$updateinvoiceid) {
            throw new Exception("Failed to update PO record");
        }
        
        // Update transaction record
        $updatetrancationid = update_record('tbl_potrancation', $info1, "po_id=".$_POST['eid'], $dbcon);
        if(!$updatetrancationid) {
            throw new Exception("Failed to update transaction record");
        }
        
        // Update Payment Reminder
        $updatermdrid = update_record('todo_mst', $informdr, "ref_id=".$_POST['eid']." and ref_table='tbl_pono'", $dbcon);
        
        $info_gen['genral_book_status'] = 2;
        $updateinvoiceid1 = update_record('tbl_general_book', $info_gen, "table_name='tbl_pono' and table_id=".$_POST['eid'], $dbcon);
        
        // Tax transaction
        $sel_itrn = $dbcon->query("SELECT * FROM tbl_potrancation WHERE po_id='".$_POST['eid']."' AND potrancation_status='2'");
        while ($r_itrn = brp_mysqli_fetch_array($sel_itrn)) {
            $info_tax_trn['tx_status'] = 2;
            $tax_update = update_record("tbl_tax_trn", $info_tax_trn, "tx_transaction_id='{$r_itrn['potrancation_id']}' AND tx_transaction_type='tbl_potrancation'", $dbcon);
            if(!$tax_update) {
                throw new Exception("Failed to update tax transaction");
            }
            
            $info_general['genral_book_status'] = 2;
            $gen_update = update_record('tbl_general_book', $info_general, "table_name='tbl_potrancation' AND table_id=" . intval($r_itrn['potrancation_id']), $dbcon);
            
            $info_grn['purchase_status'] = 0;
            
            if (!empty($r_itrn['grn_id']) && is_numeric($r_itrn['grn_id'])) {
                $grn_update = update_record('tbl_grn', $info_grn, "grn_id=" . intval($r_itrn['grn_id']), $dbcon);
            }
            if (!empty($r_itrn['grn_trn_id']) && is_numeric($r_itrn['grn_trn_id'])) {
                $grn_trn_update = update_record('tbl_grn_trn', $info_grn, "grn_trn_id=" . intval($r_itrn['grn_trn_id']), $dbcon);
            }
            if (!empty($r_itrn['grn_sub_trn_id']) && is_numeric($r_itrn['grn_sub_trn_id'])) {
                $grn_sub_update = update_record('tbl_grn_sub_trn', $info_grn, "grn_trn_sub_id=" . intval($r_itrn['grn_sub_trn_id']), $dbcon);
            }
            
            $info_used['po_grn_used_status'] = 2;
            $used_update = update_record('tbl_po_grn_used', $info_used, "potrancation_id=" . intval($r_itrn['potrancation_id']), $dbcon);
        }
        
        // Eway Bill Transaction
        $eway_trans['isdelete'] = 1;
        $updateinvoiceid2 = update_record('tbl_ewaybill_transaction', $eway_trans, "eway_bill_voucher_table='tbl_pono' and eway_bill_voucher_id=".$_POST['eid'], $dbcon);
        
        // Transport Transaction
        $transport_transaction['transportation_status'] = 1;
        $updateinvoiceid3 = update_record('tbl_transport_transaction', $transport_transaction, "transport_transaction_table='tbl_pono' and transport_transaction_table_id=".$_POST['eid'], $dbcon);
        
        // Salesman Transaction
        $salesman_transaction['isdelete'] = 1;
        $updateinvoiceid4 = update_record('tbl_salesman_transaction', $salesman_transaction, "transaction_table='tbl_pono' and transaction_table_id=".$_POST['eid'], $dbcon);
        
        // Insert LOG
        $log_entry = common_log_entry($dbcon, "invoice_add", 3, "tbl_pono", $_POST['eid']);
        
        // Cost Center Transaction
        // $info_cost['costcenter_status'] = 2;
        // $updateid1 = update_record("tbl_cost_center_transaction", $info_cost, "table_name='tbl_pono' and table_id=".$_POST['eid'], $dbcon);
        
        // TCS Deduction Transaction
        $info_tcs['isdelete'] = 1;
        $updateid1 = update_record("tbl_tcs_deduction_transaction", $info_tcs, "tcs_sale_id=".$_POST['eid'], $dbcon);
        
        // Bill Sundry Transaction
        $info_bsun['isdelete'] = 1;
        $updateid1 = update_record("tbl_bill_sundry_transaction", $info_bsun, "sundry_voucher_table='tbl_pono' and sundry_voucher_id='".$_POST['eid']."'", $dbcon);
        
        $sel_bsun = $dbcon->query("select * from tbl_bill_sundry_transaction where sundry_voucher_table='tbl_pono' and sundry_voucher_id='".$_POST['eid']."' and isdelete='1'");
	...
	if (($company_state['stateid'] == $POST['cust_stateid'])) {
		...
		$sgst_tax_rate_conv = ((float) $_POST['currency_rate'] * (float) $gst * (float) $_POST['product_amount']) / 100;
	} else {
	...
	if ($POST['currency_id'] == $company_state['currency_id']) {
		...
		$info1['product_rate_conv'] = (float) $_POST['product_rate'] * (float) $_POST['currency_rate'];
		$info1['product_discount_conv'] = (float) $POST['product_discount'] * (float) $POST['currency_rate'];
		$info1['product_amount_conv'] = (float) $_POST['product_amount'] * (float) $_POST['currency_rate'];
	...
	if (mysqli_num_rows($result) > 0) {
		...
		while ($rel = mysqli_fetch_assoc($result)) {
			...
			if ($rel['grn_id'] != 0) {
				$grn = '<br><strong style="color:red">GRN : </strong><strong style="color:green">' . $rel['grn_no'] . '</strong>';
			} else {
				$grn = "";
			}
			if ($rel['purchaseorder_no'] != "") {
				$po_no = '<br><strong style="color:red">PO No : </strong><strong style="color:green">' . $rel['purchaseorder_no'] . '</strong>';
			} else {
				$po_no = "";
			}
