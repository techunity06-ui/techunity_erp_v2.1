...
else if(strtolower($POST['mode']) == "allpurchasepaymenteport")
{
	$cust_id=$POST['cust_id'];
	$product_id=$POST['product_id'];
	...
	<tr style="border-bottom:0.5px #000 solid;">
	<td colspan="7">
	<strong>[ Vendor Purchase Bill Detail ]</strong>
	</td>
	</tr>';
   	$i=1;
	$total=0;
	$str.='<tr style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">
			<th width="5%" style="text-align:center">NO.</th>
			<th width="20%" style="text-align:center">Bill No</th>
			<th width="20%" style="text-align:center">Bill Date</th>
			<th width="20%" style="text-align:center">Party Name</th>
			<th width="5%" style="text-align:center">Net Amount</th>
			<th width="5%" style="text-align:center">Gross Amount</th>
			<th width="5%" style="text-align:center">Payment Cleared</th>
			<th width="5%" style="text-align:center">Payment Pending</th>
			</tr>';

			$p_bill_data=getpaymentbilldatabillnowise($dbcon,$POST);
			...
			if(count($p_bill_data)>0)
			{
				...
				foreach ($p_bill_data as $key => $re) {
					$pen_amt=$re["grossamt"]-$re["clearedpayment"];
					$net_amt=$net_amt+$re["billtotal"];
					$net_grs_amt=$net_grs_amt+$re["grossamt"];
					$net_cleared_amt=$net_cleared_amt+$re["clearedpayment"];
					$tot_pend_amt=$tot_pend_amt+$pen_amt;
					$str.='<tr style="border: 1px dashed #cccccc;">
					<td style="text-align:center">'.$j.'</td>
					<td style="text-align:center">'.$re["billno"].'</td>
					<td style="text-align:center">'.$re["billdate"].'</td>
					<td style="text-align:center">'.$re["l_name"].'</td>
					<td style="text-align:center">'.$re["billtotal"].'</td>
					<td style="text-align:center">'.$re["grossamt"].'</td>
					<td style="text-align:center">'.$re["clearedpayment"].'</td>
					<td style="text-align:center">'.$pen_amt.'</td>
					</tr>';
					$j++;
				}
				$str .='<tr>
				...
				<td style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:center;">'.number_format($net_amt,2).'</td>
				<td style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:center;">'.number_format($net_grs_amt,2).'</td>
				<td style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:center;">'.number_format($net_cleared_amt,2).'</td>
				<td style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:center;">'.number_format($tot_pend_amt,2).'</td>
				</tr>';
			}else{
				$str .='<tr>
				<td colspan="7" style="text-align:center">NO DATA FOUND  </td>
...
else if(strtolower($POST['mode']) == "billnowisepaymenteport")
{
	$cust_id=$POST['cust_id'];
	$product_id=$POST['product_id'];
	...
	$str.='<tr style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">
			...
			<th width="20%" style="text-align:center">Bill No</th>
			<th width="20%" style="text-align:center">Bill Date</th>
			<th width="20%" style="text-align:center">Vender Name</th>
			<th width="5%" style="text-align:center">Net Amount</th>
			<th width="5%" style="text-align:center">Gross Amount</th>
			<th width="5%" style="text-align:center">Payment Cleared</th>
			<th width="5%" style="text-align:center">Payment Pending</th>
...
else if(strtolower($POST['mode']) == "vendwiseledgerpaymentreport")
{
	$cust_id=$POST['cust_id'];
	$product_id=$POST['product_id'];
	...
	$query="SELECT tl.l_name as vendorname,tl.m_address,tp.order_no as billno,tp.po_date as billdate,tp.bill_booking_no,tp.vender_id,tp.po_booking_date as billbookingdate,tp.bill_status,tp.po_id,tp.vender_id
	FROM tbl_pono as tp
	inner JOIN tbl_ledger as tl ON tl.l_id=tp.vender_id";
	...
	if($POST['po_date_type']){
		if($POST['po_date_type']=='po'){
			...
			$startdate=$s_date[0];
			$enddate=$s_date[1];
			$query.=" where tp.po_date>='".date('Y-m-d',strtotime($startdate))."' and tp.po_date<='".date('Y-m-d',strtotime($enddate))."'";
		}else{
			$s_date=explode(' - ',$POST['rep_del_date']);
			$startdate=$s_date[0];
			$enddate=$s_date[1];
	...
	if(isset($POST['purchase_type_status'])){
		if($POST['purchase_type_id']){
			$query.=' and tp.purchase_bill_type='.$POST['purchase_type_id'];
		}
	}
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		while($vendor_list=mysqli_fetch_assoc($result))
		{
			...
			<tr>
			<td colspan="3"> 
			Bill Book Date : '.$vendor_list['billbookingdate'].'
			</td>
			<td colspan="3">
			</td>
			</tr>
			...
			$str.='
			<tr>
			<td colspan="3" style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;border-right: 0.5px #000 solid;">Bill Details</td>
			<td colspan="4" style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">Payment Details</td>
			</tr>
			<tr style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">
			<th width="20%" style="text-align:center">Bill No</th>
			<th width="20%" style="text-align:center;">Bill Date</th>
			<th width="5%" style="text-align:center;border-right: 0.5px #000 solid;">Amount</th>
			<th width="5%" style="text-align:center">Pay. Date</th>
			<th width="25%" style="text-align:center">Chq. NO</th>
			<th width="12%" style="">Chq. Date</th>
			<th width="13%" style="text-align:center">Amount Remark</th>
			</tr>
			';
			$p_bill_data=getpaymentbilldatawithledger($dbcon,$vendor_list['vender_id'],$POST);
			$j=1;
			if(count($p_bill_data)>0)
			{
				$net_amt=0;
				...
				foreach ($p_bill_data as $key => $re) {
					...
					// <td style="text-align:center">'.$re["billno"].'</td>
					// <td style="text-align:center;">'.$re["billdate"].'</td>
					// <td style="text-align:center;border-right: 0.5px #000 solid;">'.$re["billtotal"].'</td>
					// <td style="text-align:center">'.$payment_date.'</td>
					// <td style="text-align:center">'.$re["chqnumber"].'</td>
					// <td style="text-align:center">'.$re["chqdate"].'</td>
					// <td style="text-align:center">'.$re["payment_remark"].'</td>
					// </tr>';
					...
					<td style="text-align:center;" >'.$re["billdate"].'</td>
					<td style="text-align:center;border-right: 0.5px #000 solid;" >'.$re["billtotal"].'</td>';
					$rec_data=reciptdata($dbcon,$re["po_id"]);
					// echo "<pre>";
					// print_r($rec_data);
					if(count($rec_data)>0){
						$str.='<td colspan="4">';
						foreach ($rec_data as $key_1 => $re_1) {
							$clear_amt=$clear_amt+$re_1["clearpayment"];
							$payment_date='';
							if($re_1["payment_date"]!='0000-00-00 00:00:00'){
								$payment_date=$re_1["payment_date"];
								//$payment_date=date('Y-m-d',strtotime($re["payment_date"]));
							}
								$str.='
								<table width="100%">
						<tr>
						<td style="text-align:center" width="25%">'.$payment_date.'</td>
						<td style="text-align:center" width="25%">'.$re_1["chqnumber"].'</td>
						<td style="text-align:center" width="25%">'.$re_1["chqdate"].'</td>
						<td style="text-align:center" width="25%">'.$re_1["clearpayment"].'</td>
						</tr>
						</table>';
							}
							$str.="</td>";
				...
				$pend_amt=$net_amt-$clear_amt;
				$str .='<tr>
				<td colspan="1" style="text-align:center"></td>
				<td  style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:right;"><strong>Total :  </strong></td>
				<td style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:center;">'.number_format($net_amt,2).'</td>
				<td colspan="3" style="text-align:center"></td>
				<td style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:center;">'.number_format($clear_amt,2).'</td>
				</tr>
				<tr>
				<td></td>
				<td style="text-align:center">Pending Payment :</td>
				<td colspan="5">'.number_format($pend_amt,2).'</td>
				</td>
				';
			}else{
				$str .='<tr>
...
else if(strtolower($POST['mode']) == "vendorwisepaymentsummary")
{
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		while($vendor_list=mysqli_fetch_assoc($result))
		{
			...
			<tr>
			<td colspan="3">
			Purchase Bill No : '.$vendor_list['billno'].'
			</td>
			<td colspan="3"> 
			Bill Date : '.$vendor_list['billdate'].'
			</td>
			<td colspan="3">
			Status : '.$vendor_list['bill_status'].'
			</td>
			</tr>
			<tr>
			<td colspan="3">
			Bill Book No : '.$vendor_list['bill_booking_no'].'
			</td>
			<td colspan="3"> 
			Bill Book Date : '.$vendor_list['billbookingdate'].'
			</td>
			...
			';
			$p_bill_data=getpaymentbilldata($dbcon,$vendor_list['vender_id'],$POST);
...
else if(strtolower($POST['mode']) == "vendorwisepurchasebillsummary")
{
	//var_dump($POST);
	...
	<tr style="border-bottom:0.5px #000 solid;">
	<td colspan="9">
	<strong>[ Purchase Bill Summary Detail ]</strong>
	</td>
	</tr>';
	...
	// 		FROM tbl_purchaseorder as tp 
	// 		left JOIN tbl_purchaseordertrn as tpt ON tpt.purchaseorder_id=tp.purchaseorder_id
	// 		left JOIN tbl_ledger as tl ON tl.l_id=tp.vender_id where tpt.purchaseordertrn_status=0 group by tp.vender_id";
	// 		//$query.=' group by tp.vender_id';
	...
	// 		FROM tbl_purchaseorder as tp 
	// 		left JOIN tbl_purchaseordertrn as tpt ON tpt.purchaseorder_id=tp.purchaseorder_id
	// 		left JOIN tbl_ledger as tl ON tl.l_id=tp.vender_id where tpt.purchaseordertrn_status=0 and tp.purchaseorder_id in (".$venor_summary_arr['pos'].") group by tp.purchaseorder_id";
	...
	//echo $query.=' group by tp.po_id';
	$result=$dbcon->query($query);
	$i=1;

	$str.='<tr style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">
			<th width="5%" style="text-align:center">NO.</th>
			<th width="10%" style="text-align:center">Purchase Bill NO. <br> Purchase Date</th>
			<th width="10%" style="text-align:center">Vendor Name</th>
			<th width="10%" style="text-align:center">Grn No <br> Challan No</th>
			<th width="25%" style="text-align:center">Item Details</th>
			';

			$str.='<!--<th width="12%" style="text-align:center">UOM</th>-->
			<th width="12%" style="text-align:center">Quantity UP <br> Quantity UM</th>
			<th width="13%" style="text-align:center">Rate</th>
			<!--<th width="13%" style="text-align:center">Gross Rate</th>-->
	...
	if(brp_mysqli_num_rows($result)>0)
	{
		...
		while($vendor_list=brp_mysqli_fetch_array($result))
		{
			...
			//var_dump($vendor_list['po_id'],$vendor_list['vender_id']);
			$p_bill_data=getpurchasebilldata($dbcon,$vendor_list['po_id'],$vendor_list['vender_id'],$POST);
			/*echo "<pre>";echo "</pre>";*/
			// print_r($p_bill_data); die();
			...
			if(count($p_bill_data)>0)
			{
				...
				foreach ($p_bill_data as $key => $re) {
					...
					if($re['unit_id'] != $re['conv_unit_id']){
						$qty = '<span style="color:green"><strong>'.$re['billqty'].' '.$re['bunit_name'].'</strong></span> <br><span style="color:orange"><strong>'.$re['billcqty'].' '.$re['cunit_name'].'</strong></span>';
					}else{
						$qty = '<span style="color:green"><strong>'.$re['billqty'].' '.$re['bunit_name'].'</strong></span>';
					}
					//$pend_conv=convert_stock($dbcon,$pen,$re["product_id"],$unit);
					$tot_amt=$tot_amt+$re["product_amount"];
					$str.='<tr style="border: 1px dashed #cccccc;">
					<td style="text-align:center">'.$j.'</td>
					<td style="text-align:center">'.$vendor_list['billno'].'<br>'.$vendor_list['billdate'].'</td>
					<td style="text-align:center">'.$vendor_list['vendorname'].'</td>
					<td style="text-align:center">'.$re["grn_no"].'<br>'.$re["challan_no"].'</td>
				...
				<td colspan="2" style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:right;"><strong>Net Amount :  </strong></td>
		...
		$str .='<tr>
			...
			<td colspan="2" style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:right;"><strong>Net Amount :  </strong></td>
			<td style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;text-align:center;">'.number_format($tot_amt1,2).'</td>
			</tr>';
	}else
	{
		$str .='<tr>
...
else if(strtolower($POST['mode']) == "billnowisepurchasebillreport")
{
	$cust_id=$POST['cust_id'];
	$product_id=$POST['product_id'];
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		while($vendor_list=mysqli_fetch_assoc($result))
		{
			...
			<tr>
			<td colspan="2">
			<strong>Purchase Bill NO  : </strong>
			</td>
			<td>
			'.$vendor_list['billno'].'
			</td>
			...
			<tr>
			<td colspan="3">
			Vendor Name : '.$vendor_list['vendorname'].'
			</td>
			<td colspan="3"> 
			...
			$str.='<tr style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">
			<th width="5%" style="text-align:center">NO.</th>
			<th width="20%" style="text-align:center">Grn No <br> Challan No</th>
			<th width="5%" style="text-align:center">Item Details</th>
			';
			...
			$p_bill_data=getpurchasebilldatabillnowise($dbcon,$vendor_list['po_id'],$POST);
			// echo "<pre>";
			// print_r($p_bill_data);
			$j=1;
			if(count($p_bill_data)>0)
...
else if(strtolower($POST['mode']) == "purchasetypewisepurchasebillreport")
{
	$cust_id=$POST['cust_id'];
	$product_id=$POST['product_id'];
    ...
    $query="SELECT tl.l_name as vendorname,tl.m_address,tp.order_no as billno,tp.po_date as billdate,tp.bill_booking_no,tp.vender_id,tp.po_booking_date as billbookingdate,tp.bill_status,tpot.product_id,tp.po_id,tp.vender_id,tp.purchase_bill_type
	FROM tbl_pono as tp
	left JOIN tbl_potrancation as tpot ON tp.po_id=tpot.po_id
	inner JOIN tbl_ledger as tl ON tl.l_id=tp.vender_id";
	...
	$startdate=$s_date[0];
	$enddate=$s_date[1];
	$query.=" where potrancation_status=0 and tp.po_date>='".date('Y-m-d',strtotime($startdate))."' and tp.po_date<='".date('Y-m-d',strtotime($enddate))."'";

	// if($POST['po_date_type']){
	// 	if($POST['po_date_type']=='po'){
	// 		$startdate=$POST['from_po_date'];
	// 		$enddate=$POST['to_po_date'];
	// 		$query.=" where tp.po_date>='".date('Y-m-d',strtotime($startdate))."' and tp.po_date<='".date('Y-m-d',strtotime($enddate))."'";
	// 	}else{
	// 		$startdate=$POST['from_po_date'];//to_po_date//from_po_date
	// 		$enddate=$POST['to_po_date'];
	// 		$query.=" where tp.po_date>='".date('Y-m-d',strtotime($startdate))."' and tp.po_date<='".date('Y-m-d',strtotime($enddate))."'";
	// 	}
	// }
	...
	if($POST['item_id']){
			$query.=' and tpot.product_id='.$POST['item_id'];
	}
	...
	$str.='<tr style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">
			<th width="5%" style="text-align:center">NO.</th>
			<th width="13%" style="text-align:center">Purchase Type</th>
			<th width="10%" style="text-align:center">Bill No <br> Bill Date</th>
			<th width="10%" style="text-align:center">Grn No <br> Challan No</th>
			<th width="20%" style="text-align:center">Item Details</th>
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		while($vendor_list=mysqli_fetch_assoc($result))
		{
			/*$purchase_type=($vendor_list['purchase_bill_type']==1) ? 'General Purchase' : 'Import Purchase';*/
			/*$str.='<tr></tr>
			<tr>
			<td colspan="2">
			...
			if(count($p_bill_data)>0)
			{
				...
				foreach ($p_bill_data as $key => $re) {
					...
					$qty='';
					if($re['unit_id'] != $re['conv_unit_id']){
						$qty = '<span style="color:green"><strong>'.$re['billqty'].' '.$re['unit_name'].'</strong></span><br><span style="color:orange"><strong>'.$re['billcqty'].' '.$re['cunit_name'].'</strong></span>';
					}else{
						$qty = '<span style="color:green"><strong>'.$re['billqty'].' '.$re['unit_name'].'</strong></span>';
					}
					//$pend_conv=convert_stock($dbcon,$pen,$re["product_id"],$unit);
					$tot_amt=$tot_amt+$re["product_amount"];
					$str.='<tr style="border: 1px dashed #cccccc;">
					<td style="text-align:center">'.$j.'</td>
					<td style="text-align:center">-</td>
					<td style="text-align:center">'.$re["billno"].'<br>'.date('d-m-Y',strtotime($re["billdate"])).'</td>
					<td style="text-align:center">'.$re["grn_no"].'<br>'.$re['challan_no'].'</td>
					<td style="text-align:center">'.$re["product_name"].'</td>';

					$str.='<td style="text-align:center">'.$vendor_list['vendorname'].'</td>
					<td style="text-align:center">'.$qty.'</td>
					<td style="text-align:center">'.$re["product_rate"].'</td>
					<!--<td style="text-align:center">-</td>-->
					<td style="text-align:center">'.$re["product_discount"].'</td>
...
else if(strtolower($POST['mode']) == "vendorwisesummaryreport" || strtolower($POST['mode']) == "allsummary")
{
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		while($vendor_list=mysqli_fetch_assoc($result))
		{
			...
			left JOIN tbl_ledger as tl ON tl.l_id=tp.vender_id
			left JOIN unit_mst as um ON um.unitid=tpt.unit_id
			left JOIN product_mst as p ON p.product_id=tpt.product_id where tp.vender_id=".$vendor_list['vender_id'];
...
else if(strtolower($POST['mode']) == "vendorwiseratesummaryreport")
{
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		while($vendor_list=mysqli_fetch_assoc($result))
		{
			...
			$query="SELECT tsmp.po_req_no as workorderno,tsmp.po_req_date as workorderdate,tp.vender_id,tp.purchaseorder_id,tp.purchaseorder_no,tpt.unit_id,tpt.product_id,tpt.product_rate,tpt.product_disc, tl.l_name as vendorname, trp.indent_no,trp.indent_date,trp.indent_date,tpt.product_qty,tpt.used_qty,p.product_name,p.product_desc,tp.purchaseorder_due_date,(select sum(tbl_grn_trn.product_qty)  from tbl_grn_trn where tbl_grn_trn.purchaseordertrn_id=tpt.purchaseordertrn_id and tbl_grn_trn.product_id=tpt.product_id ) as pending,(product_qty - (select IFNULL(sum(tbl_grn_trn.product_qty),0) from tbl_grn_trn where tbl_grn_trn.purchaseordertrn_id=tpt.purchaseordertrn_id and tbl_grn_trn.product_id=tpt.product_id )) as remaining
			FROM tbl_purchaseorder as tp
			left JOIN tbl_purchaseordertrn as tpt ON tpt.purchaseorder_id=tp.purchaseorder_id
			left JOIN tbl_request_product as trpc ON tpt.po_ref_id=trpc.rp_id
			left JOIN tbl_set_main_process as tsmp ON tsmp.sp_id=trpc.sp_id
...
else if(strtolower($POST['mode']) == "itemwisepurchasebillreport")
{
	$s_date=explode(' - ',$POST['date']);
	$_SESSION['start']=$s_date[0];
	$_SESSION['end']=$s_date[1];
	...
	$query="SELECT group_concat(tp.po_id) as poids,tp.product_id,p.product_icode,p.product_name,p.product_desc FROM tbl_potrancation as tp left JOIN product_mst as p ON tp.product_id=p.product_id left JOIN  tbl_pono as tpt ON tpt.po_id=tp.po_id where tp.potrancation_status!='2' ";
	...
	if($POST['po_date_type']){
		if($POST['po_date_type']=='po'){
			...
			$startdate=$s_date[0];
			$enddate=$s_date[1];
			$query.=" and tpt.cdate>='".date('Y-m-d',strtotime($startdate))."' and tpt.cdate<='".date('Y-m-d',strtotime($enddate))."'";
		}else{
			$s_date=explode(' - ',$POST['rep_del_date']);
			$startdate=$s_date[0];
			$enddate=$s_date[1];
	...
	$str.='<tr style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">
			...
			$str.='<th width="20%" style="text-align:center">Item Name & Description </th>
			<th width="10%" style="text-align:center">Quantity UP <br> Quantity UM </th>
			<th width="10%" style="text-align:center">Rate</th>
			<!--<th width="10%" style="text-align:center">Gross Rate</th>-->
	...
	if(brp_mysqli_num_rows($result)>0)
	{
		...
		while($vendor_list=mysqli_fetch_assoc($result))
		{
			...
			$p_bill_data=getpurchasebilldataitemwiese($dbcon,$vendor_list['product_id'],$POST);
			...
			if(count($p_bill_data)>0)
			{
				...
				foreach ($p_bill_data as $key => $re) {
					...
					$qty = '';
					if($re['unit_id'] != $re['conv_unit_id']){
						$qty = '<span style="color:green"><strong>'.$re['billqty'].' '.$re['unit_name'].'</strong></span><br>
						<span style="color:orange"><strong>'.$re['billcqty'].' '.$re['cunit_name'].'</strong></span>';
					}else{
						$qty = '<span style="color:green"><strong>'.$re['billqty'].' '.$re['unit_name'].'</strong></span>';
					}
					//$pend_conv=convert_stock($dbcon,$pen,$re["product_id"],$unit);
					...
					$str.='<tr style="border: 1px dashed #cccccc;">
					<td style="text-align:center">'.$j.'</td>
					<td style="text-align:center">'.$re["billno"].'<br>'.$re["billdate"].'</td>
					<td style="text-align:center">'.$re["l_name"].'</td>';

					$str.='<td style="text-align:center">'.$vendor_list['product_name'].' ('.$vendor_list['product_icode'].')</td>
					<td style="text-align:center">'.$qty.'</td>
					<td style="text-align:center">'.$re["product_rate"].'</td>
					<td style="text-align:center">'.$re["product_disc"].'</td>
					<!--<td style="text-align:center">-</td>-->
...
else if(strtolower($POST['mode']) == "vendorwisepricelistreport")
{
	...
	else if(strtolower($POST['mode']) == "followupreport")
	{
		...
		left JOIN product_mst as p ON p.product_id=tpt.product_id";
		if($POST['vendor_id']!=''){
			$query.=" where tp.vender_id=".$POST['vendor_id'];
		}
		...
		if($POST['po_date_wise']){
			if($POST['po_date_wise']=='po' || $POST['po_date_wise']=='PO'){
	...
	else if(strtolower($POST['mode']) == "detailgrouppruchasebill")
	{
		$s_date=explode(' - ',$POST['date']);
		$_SESSION['start']=$s_date[0];
		$_SESSION['end']=$s_date[1];
		...
		$table_heading='<tr style="border-bottom:0.5px #000 solid;border-top:0.5px #000 solid;">
		<th width="9%" style="text-align:center"> NO </th>
		<th width="9%" style="text-align:center">Bill NO </th>
		<th width="9%" style="text-align:center">Vendor Name</th>
		<th width="20%" style="text-align:center">Item Description</th>';

		$table_heading.='<!--<th width="12%" style="text-align:center">UOM</th>-->
		<th width="12%" style="text-align:center">Qty</th>
		<th width="23%" style="text-align:center">Rate</th>
		<th width="23%" style="text-align:center">Amount</th>
		</tr>';
		$str .='
		<table  width="100%"   class="display">
		...
		<strong>[ Purchase Bill Group Wise Detail ]</strong>
		</td>
		</tr>';
		...
		if(count($POST['pr_type'])>0){
			if(isset($_POST['pr_cat']) && is_array($_POST['pr_cat']) && count($POST['pr_cat'])>0){
				foreach ($POST['pr_type'] as $key => $value) { 
					$cat_d="select * from tbl_category where cat_id=".$value;
					$cat_dr=brp_mysqli_fetch_array($dbcon->query($cat_d));
					if($value=='0'){
						$cat_dr['cat_name'] = 'Primary';
					}
					foreach ($POST['pr_cat'] as $key1 => $value1) {
						$vendor="select * from pro_ms_product_type where product_type_id=".$value1;
						$cat_data=brp_mysqli_fetch_array($dbcon->query($vendor));
						$str .='<tr></tr>
						<tr style="border-bottom: hidden;">
						<td>
						</td>
						<td colspan="" style="border-left: hidden;border-right: hidden;">
						<strong>Category : </strong>
						</td>
						<td colspan="" style="border-left: hidden;border-right: hidden;">
						<strong>'.$cat_dr['cat_name'].'</strong>
						</td>
						<td colspan="4">
						</td>
						</tr>';	 
						...
						<td colspan="" style="border-left: hidden;border-right: hidden;">
						<strong>'.$cat_data['product_type_name'].'</strong>
						</td>
						<td colspan="4">
						</td>
						</tr>';
						$str.=$table_heading;
						//$value as category and $value1 as product type
						$str.=getpgorupdetailreport($dbcon,$value,$value1,$POST);
					}
				}
				//getpgorupdetailreport();
			}else{
				...
				foreach ($POST['pr_type'] as $key => $value) {
					...
					$str.=$table_heading;
					$str.=getpgorupdetailreport($dbcon,$value,'',$POST);
				}
			...
			foreach ($POST['pr_cat'] as $key1 => $value1) {
				...
				$str.=$table_heading;
				$str.=getpgorupdetailreport($dbcon,'',$value1,$POST);
			}
		}
		$str .='				 
		</table>';
	...
	else if(strtolower($POST['mode']) == "summarygrouppruchasebill")
	{
		$s_date=explode(' - ',$POST['date']);
		$_SESSION['start']=$s_date[0];
		$_SESSION['end']=$s_date[1];
		...
		<tr style="border-bottom:0.5px #000 solid;">
		<td colspan="7">
		<strong>[Purchase Bill Group Wise Summary ]</strong>
		</td>
		</tr>';
		$str .=groupsummaryreport($dbcon,$POST);
		$str .='				 
		</table>';
		echo $str;
	}
