...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$appData = array();
		$i=1;
		$aColumns = array('sht.log_id','sht.po_no','sht.product_id','tc.cat_name','sht.short_close_qty','sht.short_close_reason','sht.date','pro.product_name','bms.branch_name','user.user_name','sht.user_id','sht.aproove_status','sht.po_trn_id','sht.po_id','unit.unit_name');
		...
		if($POST['approve_status'] == 1){
			...
			if( $cnt > 0){
				...
				if($cnt = brp_mysqli_num_rows($que_e)>0){
					$que = "select reqs.*,(select sum(used_qty) from tbl_purchaseorder_req_trn as sr where sr.purchaseordertrn_req_status=0 and sr.rp_id=rtrn.rp_id) as re_conv_qty,(select sum(product_conv_qty) from tbl_grn_sub_trn as inv where inv.status=0 and inv.rp_id = rtrn.rp_id)  as inw_conv_qty from tbl_purchaseordertrn as ptrn 
