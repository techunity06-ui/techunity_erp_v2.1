...
if (strtolower($POST['mode']) == "fetch") {
	...
	$query = "select apo.approve_no, apo.approve_date, apo.approve_qty, po.indent_no, delivery_date, pre.pre_no ,po.indent_date, po.rp_po_qty, unit.unit_name, spro.po_req_no, pmst.product_name, pmst.product_icode, dr.drawing_number, pmst.product_alias_name, tc.cat_name, po.rp_id, apo.approve_indent_id, apo.quotation_approve_status, bms.branch_name, us.user_name, apo.approve_indent_id
				from approve_indent as apo
				left join tbl_request_product as po on po.rp_id=apo.rp_id
				left join tbl_pre_trn as ptrn on ptrn.pre_trn_id=po.pre_trn_id
				left join tbl_pre as pre on pre.pre_id=ptrn.pre_id
	...
	if ($cnt > 0) {
		...
		while ($row = brp_mysqli_fetch_array($result)) {
			...
			$str .= '<tr>
						...
						<td class="text-center">' . $row['pre_no'] . '<br>' . date('d M, Y', strtotime($row['indent_date'])) . '</td>
	...
	$query = "select rtrn.po_quotationtrn_id,pro.product_name,rtrn.product_qty,rtrn.product_conv_qty,pre.pre_no,pre.pre_date,unit.unit_name as base_unit,cunit.unit_name as conv_unit,rtrn.remark, ain.approve_no,ain.approve_indent_id, ain.approve_date, req.indent_no,req.indent_date,rtrn.unit_id, rtrn.conv_unit_id from po_quotationtrn_ref as rtrn 
	...
	while ($row = brp_mysqli_fetch_array($result)) {
		...
		$str .= '<tr>
					...
					<td>' . $row['pre_no'] . '</td>
					<td>' . date('d-m-Y', strtotime($row['pre_date'])) . '</td>
	...
	$query = "select rtrn.po_quotationtrn_id,rtrn.ref_name,rtrn.product_id,pre.pre_no,pre_date,pro.product_name,rtrn.product_qty,rtrn.product_conv_qty,unit.unit_name as base_unit,cunit.unit_name as conv_unit, ain.approve_no, rtrn.remark,rtrn.product_rate, ain.approve_date, req.indent_no,req.indent_date,rtrn.unit_id,rtrn.delivery_date,rtrn.payment_days, rtrn.conv_unit_id 
			...
			left join tbl_pre_trn as pre_trn on pre_trn.pre_trn_id=req.pre_trn_id 
	        left join tbl_pre as pre on pre.pre_id=pre_trn.pre_id
			where rtrn.ref_name='supplier_quotation' and rtrn.ref_id='" . $POST['quotation_ref_id'] . "' and rtrn.vender_id = '" . $POST['vender_id'] . "' and rtrn.po_quotationtrn_status IN (0,2)";
	$str = '';
	$str .= '<table class="display table table-bordered table-striped" >
			<thead>
				<tr>
						...
						<td>
							<div>
								<span style="display: block; margin-bottom: 5px; font-weight: bold;">
									Enter Single Unit Price for 
									<span style="background-color: #ffe6e6; font-size: 1.2em; text-transform: uppercase; letter-spacing: 1px; color: #ff0000;">' . $row['conv_unit'] . '</span>
								</span>
							</div>
							<input type="number" class="form-control" id="p_rate_' . $row['po_quotationtrn_id'] . '" value="' . $row['product_rate'] . '" readonly/>
						</td>
						<td><input type="date" class="form-control .default-date-picker" id="d_date_' . $row['po_quotationtrn_id'] . '" value="' . $delivery_date . '" readonly/></td>
						<!--<td>' . $row['payment_days'] . '</td>-->
						<td><textarea class="form-control" id="remark_' . $row['po_quotationtrn_id'] . '" readonly>' . $row['remark'] . '</textarea></td>
						<td>
							
						    <input type="hidden" name="ref_name" id="ref_name" value="' . $row['ref_name'] . '">
							<input type="hidden" name="ref_trn" id="ref_trn" value="' . $row['po_quotationtrn_id'] . '">
							<input type="hidden" name="product_id" id="product_id" value="' . $row['product_id'] . '">
							

    						<button class="btn btn-xs btn-success" id="product_update_btn_' . $row['po_quotationtrn_id'] . '" data-original-title="Update" data-toggle="tooltip" onClick="product_update(' . $row['po_quotationtrn_id'] . ');" style="display:none;"><i class="fa fa-pluse">Update</i></button>
    						<button class="btn btn-xs btn-warning" id="product_edit_btn_' . $row['po_quotationtrn_id'] . '" data-original-title="Edit" data-toggle="tooltip" onClick="product_edit(' . $row['po_quotationtrn_id'] . ');"><i class="fa fa-pencil"></i></button>
    						<button class="btn btn-xs btn-danger" id="product_delete_btn_' . $row['po_quotationtrn_id'] . '" data-original-title="Delete" data-toggle="tooltip" onClick="product_delete(' . $row['po_quotationtrn_id'] . ');"><i class="fa fa-trash-o"></i></button>
						</td>
					</tr></div>';
			$t_rate += $row['product_rate'];
			$i++;
		}
	} else {
		$str .= '<tr>
	...
	$str .= '</tbody>
	            <input type="hidden" name="g_total" id="g_total" value="' . $t_rate . '">
				<input type="hidden" name="total" id="total" value="' . $t_rate . '">
			</table>';
	echo $str;
} else if (strtolower($POST['mode']) == "product_update") {
	/* var_dump($POST); */
	if ($POST['btnval'] == 2) {
	...
	$info['user_id'] = $_SESSION['user_id'];
	$info['company_id'] = $_SESSION['company_id'];
	// $info['supplier_detail_id'] = $POST['supplier_detail_id'];
	//var_dump($info);
	...
	while ($row = brp_mysqli_fetch_array($result)) {
		$str .= '<option value="' . htmlspecialchars($row['vender_id']) . '">' . htmlspecialchars($row['l_name']) . '</option>';
	}
	echo $str;
	...
	$info['delivery_priode'] = $POST['delivery_priode'];
	$info['discount'] = $POST['discount'];
	$info['payment_terms'] = $POST['payment_terms'];
	$info['ex_delivery'] = $POST['ex_delivery'];
	...
	// $info1['quotation_no']			= $POST['quotation_no'];
	...
	echo json_encode($arr);
} else if (strtolower($_POST['mode']) == "load_quotation_comparision") {
	$str = '';
	// Sanitize the input
	$quotation_ref_id = isset($_POST['quotation_ref_id']) ? $dbcon->real_escape_string($_POST['quotation_ref_id']) : '';

	// Query to fetch vendors and their quotation details
	$query = "SELECT led.l_name, sd.quotation_no, sd.quotation_date, tref.vender_id, tref.supplier_detail_id, sd.quotation_copm_aprv AS sup_aprv
              ...
              WHERE tref.quotation_copm = 1
              AND tref.po_quotationtrn_status IN (0,2)
              AND tref.ref_name = 'supplier_quotation'
              AND tref.ref_id = '$quotation_ref_id'
	...
	$query_pro = "SELECT tref.approve_indent_id, pro.product_name, tref.product_id
                  FROM po_quotationtrn_ref AS tref
                  ...
                  AND tref.ref_name = 'supplier_quotation'
                  AND tref.ref_id = '$quotation_ref_id'
                  GROUP BY tref.approve_indent_id";
	...
	$str .= '<table class="table table-striped">
            <tr>
                <th class="text-center" style="background-color:#2727b7;color:white;width:20%">Supplier Name</th>';

	// Loop through vendors to create headers
	foreach ($rows as $row) {
		$str .= '<th class="text-center">' . htmlspecialchars($row['l_name']) . '</th>';
	}
	...
	// Loop through vendors to display quotation numbers
	foreach ($rows as $row) {
		$str .= '<td class="text-center">' . htmlspecialchars($row['quotation_no']) . '</td>';
	}
	...
	foreach ($rows_pro as $row_pro) {
		$str .= '<tr>
            <td class="text-center" style="background-color:#2727b7;color:white">' . htmlspecialchars($row_pro['product_name']) . '</td>';

		// Array to store product rates for all vendors
		$product_rates = array();

		// Loop through vendors to fetch product rates
		foreach ($rows as $row) {
			$query_rate = "SELECT product_rate, po_quotationtrn_id, quotation_copm_aprv, quotation_copm
                           FROM po_quotationtrn_ref
                           WHERE product_id = " . (int) $row_pro['product_id'] . "
                           AND vender_id = " . (int) $row['vender_id'] . "
                           AND ref_name = 'supplier_quotation'
                           AND approve_indent_id = " . (int) $row_pro['approve_indent_id'] . "
                           AND ref_id = '$quotation_ref_id'";

			$result_rate = $dbcon->query($query_rate);
			$row_rate = brp_mysqli_fetch_array($result_rate);

			// Add product rate to array
			$product_rates[] = array(
				'product_rate' => $row_rate['product_rate'],
				'quotation_copm_aprv' => $row_rate['quotation_copm_aprv'],
				'quotation_copm' => $row_rate['quotation_copm'],
				'po_quotationtrn_id' => $row_rate['po_quotationtrn_id']
			);
		}

		// Find the lowest and highest product rate
		$lowest_rate = null;
		$highest_rate = null;
		foreach ($product_rates as $rate) {
			if ($lowest_rate === null || $rate['product_rate'] < $lowest_rate) {
				$lowest_rate = $rate['product_rate'];
			}
			if ($highest_rate === null || $rate['product_rate'] > $highest_rate) {
				$highest_rate = $rate['product_rate'];
			}
		}

		// Loop through vendors to display product rates and comparison indicators
		foreach ($product_rates as $rate) {
			$color = '';
			// Determine text color based on comparison with lowest rate
			if ($rate['product_rate'] <= $lowest_rate) {
				$color = 'green'; // Less expensive
			} elseif ($rate['product_rate'] >= $highest_rate) {
				$color = 'red'; // More expensive
			} elseif (($rate['product_rate'] < $highest_rate) && ($rate['product_rate'] > $lowest_rate)) {
				$color = 'blue'; // In between
			}

			// Checkbox/ radio button logic as per your requirement
			$checked = ($rate['quotation_copm_aprv'] == 1) ? 'checked' : '';
			$str .= '<td class="text-center" style="color:' . $color . '"><b>' . htmlspecialchars($rate['product_rate']) . '</b><br>';
			if ($rate['quotation_copm'] == 1) {
				$str .= '<input type="radio" name="abc' . $j . '" class="comapre_prod_wise" id="abc' . $k . '" value="' . htmlspecialchars($rate['po_quotationtrn_id']) . '" ' . $checked . ' style="width: 23px;height: 23px;margin-top: 0px;">';
			}
			$str .= '</td>';
			$k++;
		}
	...
	// Hidden input for count and final row for selecting quotation
	$str .= '<input type="hidden" name="cnt" id="cnt" value="' . ($j - 1) . '">
        <tr class="comapre_quot_wise">
            <td class="text-center" style="background-color:#2727b7;color:white"><strong>Select Quotation</strong></td>';

	$i = 1;
	foreach ($rows as $row) {
		$checked1 = ($row['sup_aprv'] == 1) ? "checked" : '';
		$str .= '<td class="text-center"><input type="radio" name="abc" class="" id="" value="' . htmlspecialchars($row['supplier_detail_id']) . '" ' . $checked1 . ' style="width: 23px;height: 23px;margin-top: 0px;"></td>';
		$i++;
	}
