...
if (brp_strtolower($POST['mode']) == "fetch") {
	...
	if ($f_type == '') {
		...
		$where .= " and DATE(complaint_date) >= '" . date('Y-m-d', strtotime($s_date[0])) . "' AND DATE(complaint_date) <= '" . date('Y-m-d', strtotime($s_date[1])) . "'";
	} else {
		if ($f_type == '2') {
	...
	$aColumns = array('comp.complaint_id', 'complaint_no', 'complaint_date', 'mdate', 'l.l_name', 'city.city_name', 'ctype.complaint_type_name', 'f.f_status_name', 'spm.spm_name', 'sp_part_status', 'comp.emp_id', 'complaint_status', 'followup_status', 'comp.cdate', 'comp.user_id', 'l1.l_name as emp_name', 'comp.cust_id', 'GROUP_CONCAT(pro_mstr.product_name SEPARATOR "<br>") as product_name', 'GROUP_CONCAT(tc.cat_name SEPARATOR "<br>") as cat_name'); // -ametr
	$sIndexColumn = "comp.complaint_id";

	$branch_id = ($_SESSION['user_type'] == '2' && isset($POST['branch_id']) && $POST['branch_id']) ? $POST['branch_id'] : $_SESSION['branch_id'];

	if (($usertype != '2' && $getspecialConfiguration['adk_permission'] == '1' && $usertype != '31') || ($usertype != '2' && $getspecialConfiguration['adk_permission'] == '0')) {
		$isWhere = array("complaint_status = 0 and (comp.emp_id='$emp_id' OR FIND_IN_SET($emp_id,comp.assign_cust_ids)) and comp.branch_id = $branch_id and comp.company_id in (0,$_SESSION[company_id])" . $where);
	} else {
		$whbr = check_branch('comp', $branch_id);
		$isWhere = array("complaint_status = 0 $whbr and comp.company_id in (0,$_SESSION[company_id])" . $where);
	}

	$sTable = " tbl_complaint as comp";

	$isJOIN = array('left join tbl_ledger as l on comp.cust_id=l.l_id', 'left join complaint_type_mst as ctype on ctype.complaint_type_id=comp.complaint_type_id', 'left join tbl_ledger as l1 on comp.emp_id=l1.l_id', 'left join tbl_followup_status as f on comp.followup_status=f.f_id', 'left join tbl_sp_part_status_mst as spm on spm.spm_id=comp.sp_part_status', 'left join city_mst as city on city.cityid=l.cityid', 'left join tbl_complaint_trn as comp_trn_mstr on comp_trn_mstr.complaint_id=comp.complaint_id and complaint_trn_status!=2', 'left join product_mst as pro_mstr on pro_mstr.product_id=comp_trn_mstr.product_id', 'left join tbl_category as tc on pro_mstr.product_category=tc.cat_id');
	$hOrder = "comp.mdate desc";
	$hGroupby = array("comp.complaint_id");
	...
	foreach ($sqlReturn as $row) {
		...
		if (in_array(COMPLAINT_SLUG_EDIT, $bulkAccessArray)) {
			$editLink = ROOT . SERVICE_ROOT . 'complaint_edit/' . $row['complaint_id'];
			$row_data[] = '<a class="" data-original-title="Edit ' . $row["complaint_no"] . '" data-toggle="tooltip" data-placement="top" href="' . $editLink . '">' . $row["sr"] . '</a>';
			$row_data[] = '<a class="" data-original-title="Edit ' . $row["complaint_no"] . '" data-toggle="tooltip" data-placement="top" href="' . $editLink . '">' . $row["complaint_no"] . '</a>';
			$row_data[] = '<a class="" data-original-title="Edit ' . $row["complaint_no"] . '" data-toggle="tooltip" data-placement="top" href="' . $editLink . '">' . date('d-m-Y h:i A', strtotime($row["complaint_date"])) . '</a>';
			$row_data[] = '<a class="" data-original-title="Edit ' . $row["complaint_no"] . '" data-toggle="tooltip" data-placement="top" href="' . $editLink . '">' . date('d M, Y', strtotime($row["mdate"])) . '</a>'; // -amter
			$row_data[] = '<a class="" data-original-title="Edit ' . $row["complaint_no"] . '" data-toggle="tooltip" data-placement="top" href="' . $editLink . '">' . $row["l_name"] . '</a>';
			$row_data[] = '<a class="" data-original-title="Edit ' . $row["complaint_no"] . '" data-toggle="tooltip" data-placement="top" href="' . $editLink . '">' . $row["city_name"] . '</a>';
		} else {
			$row_data[] = $row['sr'];
			$row_data[] = $row['complaint_no'];
			$row_data[] = date('d-m-Y h:i A', strtotime($row['complaint_date']));
		...
		$complain_btn = '';
		if (($_SESSION['user_type'] != '2' && $getspecialConfiguration['adk_permission'] == '1' && $_SESSION['user_type'] != '31' && $_SESSION['user_type'] != '38') || ($getspecialConfiguration['adk_permission'] == '0' && $_SESSION['user_type'] != '2')) {
			//add  complain Status
			if ($row['followup_status'] == '7' && $row['emp_id'] == $emp_id) {
			...
			//start button
			if ( ($row['followup_status'] == '2' || $row['followup_status'] == '3') && $row['emp_id'] == $emp_id) {
			...
			if ($row['followup_status'] == '1' || $row['followup_status'] == '6') {
				$complain_btn = '<a href="' . ROOT . SERVICE_ROOT . 'complaint_assign/' . $row['complaint_id'] . '" class="btn btn-xs btn-success" data-original-title="Assign Employee" data-toggle="tooltip" data-placement="top"><i class="fa fa-plus"></i></a>';
			}
			else if ($row['followup_status'] == '5' || $row['followup_status'] == '10') {
				$complain_btn = '<a href="' . ROOT . SERVICE_ROOT . 'complaint_assign/' . $row['complaint_id'] . '" class="btn btn-xs btn-success" data-original-title="Re-Assign Employee" data-toggle="tooltip" data-placement="top"><i class="fa fa-plus"></i></a>';
			} else {
				$complain_btn = '';
			}
			...
			$start_btn = "";
			if ($getspecialConfiguration['adk_permission'] == '1' && $_SESSION['user_type'] == '38' && $row['emp_id'] == $emp_id ) {
		...
		if (in_array(COMPLAINT_SLUG_VIEW, $bulkAccessArray)) {
			$view_btn = '<a href="' . ROOT . SERVICE_ROOT . 'complaint_history/' . $row['complaint_id'] . '" class="btn btn-xs btn-info" data-original-title="View Complaint History" data-toggle="tooltip" data-placement="top"><i class="fa fa-eye"></i></a>';
		}
	...
	$chkQuery = "SELECT tc.* FROM tbl_complaint tc JOIN tbl_complaint_trn tcr ON tc.complaint_id = tcr.complaint_id AND tc.cust_id = '" . $POST['cust_id'] . "' AND tcr.product_id = '" . $POST['product_first_id'] . "' AND tc.followup_status NOT IN (4,9)";
	...
	if ($count >= 0) {
		...
		$info['complaint_date']	= date('Y-m-d H:i:s', strtotime($POST['complaint_date']));
		...
		$info['media_id']		= $POST['media_id'];
		$info['cust_mobile_no']		= $POST['ledger_mobile'];
		$info['cust_address']		= $POST['ledger_address'];

		// Upload Document
		if(!empty($_FILES['file_attachment']['tmp_name']))
		{
		  $destination_folder_path = COMPLAINT_FILE_UPING;
		  $filename = upload_file_document($_FILES,$destination_folder_path);
		  $info['file'] = $filename;
		}

		$user_emp_id=getEmployeeIdUser($dbcon,$_SESSION['user_id']);
		$info['assign_cust_ids'] = $POST['f_emp'].','.$user_emp_id;

		// Add new field for ADK
		$info['pro_model_no']	= $POST['pro_model_no'];
		$info['prod_serial_no']	= $POST['prod_serial_no'];

		$inserid = add_record('tbl_complaint', $info, $dbcon, $branch_id);
		...
		if ($inserid) {
			...
			$updatetrnid = update_record('tbl_complaint_trn', $infotrn, "complaint_trn_status=3 and user_id=" . $_SESSION['user_id'], $dbcon, $branch_id);
		}
		...
		$inseridf = add_record('tbl_follow', $infofw, $dbcon, $branch_id);
		...
		if ($POST['ass_emp'] == 'yes') {
			...
			add_record('tbl_follow', $infoc, $dbcon, $branch_id);

			$infotrnc['followup_status']	= $POST['change_status'];
			$infotrnc['emp_id']	= $POST['f_emp'];
			$infotrnc['old_sp_part_status']	= brp_strtolower($POST['old_sp_part']);
			$infotrnc['sp_part_status']	= "3";
			$infotrnc['mdate']			= date("Y-m-d H:i:s"); // -ametr
			$updatetrnid = update_record('tbl_complaint', $infotrnc, "complaint_id=" . $inserid, $dbcon, $branch_id);
		}
		...
		if ($POST['sp_part_status'] == '1') {
			...
			for ($k = 0; $k < count($sp_array); $k++) {
				...
				$int2 = add_record('tbl_complain_spare_part', $infos, $dbcon, $branch_id);
				...
				if ($POST['old_sp_sent'][$loop_id] == 'yes') {
					...
					$table = 'tbl_complain_close_spare_part';
					$tableid = 's_id';
	...
	// Amish Soni Start 10-02-2021
	$branch_id = ($_SESSION['user_type'] == '2' && isset($POST['branch_id']) && $POST['branch_id']) ? $POST['branch_id'] : $_SESSION['branch_id'];
	// Amish Soni End 10-02-2021

	$updateid = update_record('tbl_complaint', $info, "complaint_id=" . $POST['eid'], $dbcon, $branch_id);

	$infoc['fl_date'] = date('Y-m-d H:i:s', strtotime($POST['complaint_date']));

	$updateid1 = update_record('tbl_follow', $infoc, "fl_cid=" . $POST['eid'] . " and fl_f_status=1", $dbcon, $branch_id);
	...
	if ($POST['sp_part_status'] == '1') {
		...
		for ($k = 0; $k < count($sp_array); $k++) {
			...
			add_record('tbl_complain_spare_part', $infos, $dbcon, $branch_id);
	...
	$info1['bom_id']		= $POST['bom_version_id'];
	// $info1['comp_remark']		= $POST['comp_remark'];
	$info1['comp_amount']		= $POST['comp_amount'];
	$info1['user_id']			= $_SESSION['user_id'];

	$table = 'tbl_complaint_trn';
	$tableid = 'complaint_trn_id';
...
} else if (brp_strtolower($POST['mode']) == "get_fist_bom") {
...
} else if (brp_strtolower($POST['mode']) == "refresh_complaint_data") {

	$dbcon->query("delete from tbl_complaint_trn where complaint_id='0' and user_id='$_SESSION[user_id]' and branch_id='$_SESSION[branch_id]'");
} else if (brp_strtolower($POST['mode']) == "load_product_trn_data") {

	if ($POST['complaint_id']) {
		$query = "select trn.*,pro.product_name, bom.bom_version_id,version.version_name from tbl_complaint_trn as trn
		left join product_mst as pro on pro.product_id=trn.product_id 
		left join tbl_bom as bom on bom.bom_id=trn.bom_id 
		left join pro_ms_bom_version as version on version.bom_version_id=bom.bom_version_id
	...
	$sel = '';
	$q = $dbcon->query("SELECT trn.*, pro.product_name FROM tbl_complaint_trn as trn LEFT JOIN product_mst as pro ON pro.product_id=trn.product_id WHERE complaint_trn_id= '$POST[complaint_trn_id]'");
	$r = brp_mysqli_fetch_assoc($q);

	$sql = $dbcon->query("SELECT bom.*, version.* FROM tbl_bom as bom LEFT JOIN pro_ms_bom_version as version ON version.bom_version_id = bom.bom_version_id WHERE bom.bom_product = '" . $r['product_id'] . "' AND bom.bom_status = 0 AND version.bom_version_status = 0");
	$response = '<option value="">Select Version</option>';
	while ($res = brp_mysqli_fetch_assoc($sql)) {
		if ($res['bom_id'] == $r['bom_id']) {
			$sel = "selected";
		}
		$response .= '<option value="' . $res['bom_id'] . '" ' . $sel . '>' . $res['version_name'] . '</option>';
	}
	...
	echo brp_json_encode($row);
} else if (brp_strtolower($POST['mode']) == "load_complaint_no") {
	$row = array();
	...
	echo brp_json_encode($row);
} else if (brp_strtolower($POST['mode']) == "load_model_service_status") {
	...
	$get_rel = brp_mysqli_fetch_assoc($dbcon->query($get_qry));
	...
	if ($POST['change_status'] == '8') {
		...
		$inserid = add_record('tbl_comp_flp_rmrk_trn', $infctrn, $dbcon, $branch_id);
		...
		$inserctrnid = add_record('tbl_comp_flp_rmrk_trn', $infctrn, $dbcon, $branch_id);
		...
		if ($POST['change_status'] == '4') {
			...
			$insercloseid = add_record('tbl_complain_close_detail', $infocl, $dbcon, $branch_id);
		}
		...
		if ($_FILES["file"]["tmp_name"]) {
			...
			$inserimgid = add_record('tbl_complaint_image', $info_image, $dbcon, $branch_id);
		}
		/* image entry close */
		...
		if ($payment_status == '0') {
			...
			$insertidptrn = add_record("payment_trn", $infoptrn, $dbcon, $branch_id);
		} else {
			$infoptrn['bill_id']		= $POST['comp_id_hid'];
			$infoptrn['bill_type']	= 'complaint';
			$infoptrn['paid_amount']	= $POST['c_amount'];
			...
			$insertidptrn = add_record("complain_payment_trn", $infoptrn, $dbcon, $branch_id);
	...
	echo brp_json_encode($row);
} else if (brp_strtolower($POST['mode']) == "get_complain_data") {
	...
	echo $html;
} else if (brp_strtolower($POST['mode']) == "show_complain_history") {
	...
	$output['aaData'] = $appData;
	echo brp_json_encode($output);
} else if (brp_strtolower($POST['mode']) == "show_complain_view") {
	...
	// 	where comp.complaint_status = 0 and comp.followup_status in(7) and comp.emp_id='$emp_id' and comp.branch_id = $_SESSION[branch_id] and comp.company_id in (0,$_SESSION[company_id])";
	// $p_qry_rs = $dbcon->query($p_qry);
	// $p_qry_num = brp_mysqli_num_rows($p_qry_rs);
	...
	echo $str;
} else if (brp_strtolower($POST['mode']) == "get_product_tree") {

	$q = "select * from tbl_complaint_trn where complaint_id='0' and user_id='$_SESSION[user_id]' and branch_id='$_SESSION[branch_id]'";
	$sql1 = $dbcon->query($q);
	if (brp_mysqli_num_rows($sql1) > 0) {
		$row = brp_mysqli_fetch_assoc($sql1);
		...
		echo brp_json_encode(array_values($data));
	}
} else if (brp_strtolower($POST['mode']) == "get_complaint_tree") {

	$bom = $POST['bom_id'];
	$product = $POST['product'];
	$eid = $POST['eid'];

	$getParentNodes = "select * from tbl_bomtrn where bom_id='$bom' and po_visible_status=0 and bom_trn_status='0'";

	$resParentNodes = $dbcon->query($getParentNodes);
	$response = '';
	if (brp_mysqli_num_rows($resParentNodes) > 0) {
		echo "<thead>
		...
		$cnt = 1;
		$counter_tree = 0;
		while ($parentNode = brp_mysqli_fetch_assoc($resParentNodes)) {

			$number = "1." . $cnt;
			echo '<tr>';

			get_tree_complain($dbcon, $parentNode['product_id'], $parentNode['p_bom_id'], 0, $cnt, $bom, $number, $eid, $parentNode['bom_trn_id']);
...
} else if (brp_strtolower($POST['mode']) == "update_spare_part") {
...
} else if (brp_strtolower($POST['mode']) == "load_ledger_detail") {
	...
	echo brp_json_encode($res);
} else if (brp_strtolower($POST['mode']) == "update_spare_part_old") {
	...
	$updatetrnid1 = update_record('tbl_complain_close_spare_part', $info, "s_id=" . $sc_id, $dbcon, $branch_id);
} else if (brp_strtolower($POST['mode']) == "get_bom_product") {

	$product = $POST['com_product'];
	$response = '<option value="">Select product</option>';

	$resParentNodes = $dbcon->query("SELECT * from tbl_bom where bom_product='$product' AND bom_status=0");
	if (brp_mysqli_num_rows($resParentNodes) > 0) {
		while ($getSql = brp_mysqli_fetch_assoc($resParentNodes)) {
			$sql = $dbcon->query("SELECT trn.*, pro.product_name from tbl_bomtrn as trn LEFT JOIN product_mst AS pro ON pro.product_id=trn.product_id where bom_id='" . $getSql['bom_id'] . "' AND bom_trn_status=0");
			if (brp_mysqli_num_rows($sql) > 0) {
				while ($res = brp_mysqli_fetch_assoc($sql)) {
	...
	echo $response;
} else if (brp_strtolower($POST['mode']) == "load_bom_version") {
	$sql = $dbcon->query("SELECT bom.*, version.* FROM tbl_bom as bom LEFT JOIN pro_ms_bom_version as version ON version.bom_version_id = bom.bom_version_id WHERE bom.bom_product = '" . $_POST['product_id'] . "' AND bom.bom_status = 0 AND version.bom_version_status = 0");
	$response = '<option value="">Select Version</option>';
	while ($res = brp_mysqli_fetch_assoc($sql)) {
		$response .= '<option value="' . $res['bom_id'] . '">' . $res['version_name'] . '</option>';
	}
	echo $response;
} else if (brp_strtolower($POST['mode']) == "generate_service_report") { // -ametr
	$s_date = explode(' - ', $POST['date']);
	...
	if ($com_data->num_rows > 0) {
		$i = 1;
		while ($re = brp_mysqli_fetch_assoc($com_data)) {
			...
			$model_name = brp_mysqli_fetch_array($model);
			...
			$last_visited_data = brp_mysqli_fetch_array($lvd);
			...
			$install_assign_data = brp_mysqli_fetch_array($iad);
			...
			$coe_end_date = brp_mysqli_fetch_array($coe);
	...
	echo $str;
} else if (brp_strtolower($POST['mode']) == "generate_service_report_new") { // -ametr
	$s_date = explode(' - ', $POST['date']);
	$start_date = date("Y-m-d", strtotime($s_date[0]));
	$end_date = date("Y-m-d", strtotime($s_date[1]));
	...
	$output['aaData'] = $appData;
	echo brp_json_encode($output);
} else if (brp_strtolower($POST['mode']) == "generate_service_user_report") { // -ametr
	$s_date = explode(' - ', $POST['date']);
	$start_date = date("Y-m-d", strtotime($s_date[0]));
	$end_date = date("Y-m-d", strtotime($s_date[1]));
	...
	if (!empty($POST['cust_id'])) {
		...
		$total_installation_count = brp_mysqli_fetch_assoc($dbcon->query($total_installation));
		...
		$total_complaint_count = brp_mysqli_fetch_assoc($dbcon->query($total_complaint));
		$str .= '<tr>
	...
	if ($com_data->num_rows > 0) {
		...
		while ($re = brp_mysqli_fetch_assoc($com_data)) {
			...
			$coe_end_data = brp_mysqli_fetch_array($coe);
			...
			$co_bill_amount = brp_mysqli_fetch_assoc($dbcon->query($co_bill_data));
	...
	echo $str;
} else if (brp_strtolower($POST['mode']) == "generate_service_user_stat_report") { // -ametr
	$str = '';
	if (!empty($POST['cust_id'])) {
		$total_installation = "Select count(complaint_id) as total_installation from tbl_complaint where emp_id='" . $POST["cust_id"] . "' AND complaint_status != 2 AND complaint_type_id = 1";
		$total_installation_count = brp_mysqli_fetch_assoc($dbcon->query($total_installation));
	...
	echo $str;
} else if (brp_strtolower($POST['mode']) == "generate_service_user_report_new") { // -ametr
	$s_date = explode(' - ', $POST['date']);
	$start_date = date("Y-m-d", strtotime($s_date[0]));
	$end_date = date("Y-m-d", strtotime($s_date[1]));
...
else if (brp_strtolower($POST['mode']) == "sortclose_complaint") {
	$comp_id = $POST['complaint_id'];
	$emp_id = $POST['employee_id'];
	$remark = $POST['remark'];
...
else if (brp_strtolower($POST['mode']) == "sortclose_spareparts") {
	$comp_id = $POST['complaint_id'];
	$emp_id = $POST['employee_id'];
...
//Amish Soni 17-09-2020
else if (brp_strtolower($POST['mode']) == "spare_report") {
	$s_date = explode(' - ', $POST['date']);
	$start_date = date("Y-m-d", strtotime($s_date[0]));
	$end_date = date("Y-m-d", strtotime($s_date[1]));
	...
	$sIndexColumn = "comp.complaint_id";
	$isWhere = array("comp.complaint_status = 0 and spare.s_paid_status='$report_type' and comp.branch_id = $_SESSION[branch_id] and comp.company_id in (0,$_SESSION[company_id])" . $where);
	$sTable = "tbl_complaint as comp";
...
//Amish Soni 17-09-2020
else if (brp_strtolower($POST['mode']) == "load_city") {
	$state_val = $POST['state_val'];
	echo $str = getcity($dbcon, $state_val, '');
}
//Amish Soni 18-09-2020
else if (brp_strtolower($POST['mode']) == "complaint_install_report") {
	$s_date = explode(' - ', $POST['date']);
	$start_date = date("Y-m-d", strtotime($s_date[0]));
	$end_date = date("Y-m-d", strtotime($s_date[1]));
...
//Amish Soni 18-09-2020
else if (brp_strtolower($POST['mode']) == "spare_part_sent_report") {
	$s_date = explode(' - ', $POST['date']);
	$start_date = date("Y-m-d", strtotime($s_date[0]));
	$end_date = date("Y-m-d", strtotime($s_date[1]));
	...
	$sIndexColumn = "comp.complaint_id";
	$isWhere = array("comp.complaint_status = 0 and spare.sp_sent_status = 'yes' $whbr and comp.company_id in (0,$_SESSION[company_id])" . $where);
	$sTable = "tbl_complaint as comp";
	$isJOIN = array('left join tbl_complain_spare_part as spare on spare.s_comp_id=comp.complaint_id', 'left join tbl_ledger as l on comp.cust_id=l.l_id', 'left join city_mst as city on city.cityid=l.cityid', 'left join product_mst as pro_mstr on pro_mstr.product_id=spare.comp_product_id', 'left join tbl_category as tc on pro_mstr.product_category=tc.cat_id', 'left join product_mst as pm on pm.product_id=spare.s_product');
	$hOrder = "comp.complaint_id desc";
	include($incPath . 'pagging.php');
...
//Amish Soni 18-09-2020
else if (brp_strtolower($POST['mode']) == "spare_part_receive_pending_report") {
	$s_date = explode(' - ', $POST['date']);
	$start_date = date("Y-m-d", strtotime($s_date[0]));
	$end_date = date("Y-m-d", strtotime($s_date[1]));
...
//Amish Soni 18-09-2020
else if (brp_strtolower($POST['mode']) == "customer_status_report") {
	$state_id = ($POST['state_id']) ? $POST['state_id']  : '';
	$city_id = ($POST['city_id']) ? $POST['city_id']  : '';
	$status_id = ($POST['status_id']) ? $POST['status_id']  : '';
	...
	$sIndexColumn = "comp.complaint_id";
	$whbr = check_branch('comp');
	$isWhere = array("comp.complaint_status = 0 and comp.followup_status='4' $whbr and comp.company_id in (0,$_SESSION[company_id])" . $where);
	$sTable = "tbl_complaint as comp";
...
//Amish Soni 21-09-2020
else if (brp_strtolower($POST['mode']) == "emp_performance_report") {
	$appData = array();
	$where = '';
	$i = 1;
	...
	$sIndexColumn = "comp.complaint_id";
	$whbr = check_branch('comp');
	$isWhere = array("comp.complaint_status = 0  $whbr and comp.company_id in (0,$_SESSION[company_id])" . $where);
	$sTable = "tbl_complaint as comp";
...
//Amish Soni 21-09-2020
else if (brp_strtolower($POST['mode']) == "outstanding_report") {
	$s_date = explode(' - ', $POST['date']);
	$start_date = date("Y-m-d", strtotime($s_date[0]));
	$end_date = date("Y-m-d", strtotime($s_date[1]));
	...
	$isWhere = array("tc.complaint_status = 0 and tct.inv_done_status = '1' AND tc.pay_status = '0' AND tc.followup_status = '4' and tc.branch_id = $_SESSION[branch_id] and tc.company_id in (0,$_SESSION[company_id])" . $where);
	...
	$service_pro_type = $companyConfiguration['service_pro_type'];
	$service_pro_search = $companyConfiguration['service_pro_search'];

	$complaint_type_id = $POST['complaint_type_id'];
	$cust_id = $POST['cust_id'];

	$whr = ' AND pro.product_type in(' . $service_pro_type . ')';
	$pro_search = explode(",", $service_pro_search);

	// if($complaint_type_id=='5' || $complaint_type_id==''){
	$query = "SELECT pro.product_id, pro.product_name, pro.product_icode, dr.drawing_number FROM product_mst as pro LEFT JOIN tbl_drawing as dr on dr.drawing_id = pro.drawing_id WHERE product_status=0 " . $whr . " order by product_name";
	// }else{
	// 	$query="SELECT sold_pro.product_id, pro.product_name, pro.product_icode, dr.drawing_number FROM tbl_cust_sold_pro as sold_pro LEFT JOIN product_mst as pro on pro.product_id=sold_pro.product_id LEFT JOIN tbl_invoicetrn as i on i.product_id=sold_pro.product_id LEFT JOIN tbl_drawing as dr on dr.drawing_id = pro.drawing_id WHERE cust_sold_pro_status=0 AND cust_id='".$cust_id."' AND sold_pro.company_id in(0,$_SESSION[company_id]) group by sold_pro.cust_sold_pro_id";
	// }
	$result = $dbcon->query($query);
	$i = 0;
	...
	// echo $query;
	echo brp_json_encode($row1);
} else if (brp_strtolower($POST['mode']) == "deletefile") {
  
    $tr = $dbcon->query("SELECT * FROM `tbl_complaint` WHERE `complaint_id`  = " . $POST['complaint_id']);

    if ($tr->num_rows > 0) {
   
      $r = $tr->fetch_assoc();
      $delete_file = $r['file'];
      $destination_path = COMPLAINT_FILE_UPING;
      ...
      $info = array();
      $info['file'] = '';
      $updateid = update_record('tbl_complaint', $info, "complaint_id=" . $POST['complaint_id'], $dbcon);
