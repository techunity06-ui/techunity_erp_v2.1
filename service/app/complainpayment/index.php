...
else if(brp_strtolower($POST['mode']) == "get_opn_bal") {
	$acc_id=$POST['acc_id'];
	if($acc_id==0)
	{	
...
else if(brp_strtolower($POST['mode']) == "load_data") {
...
else if(brp_strtolower($POST['mode']) == "load_totaldata") {
	$qry="select* from income_mst where incomeid=".$POST['invoice_id'];
	$total=brp_mysqli_fetch_assoc($dbcon->query($qry));	
	echo json_encode($total);
}
else if(brp_strtolower($POST['mode']) == "get_chequeno") {
	
	echo get_chequeno($POST['acc_id'],$dbcon);
}		
else if(brp_strtolower($POST['mode']) == "load_debit_note_amount") {
...
else if(brp_strtolower($POST['mode']) == "load_debit_note") {
		echo get_purcahsedebitnote($dbcon,$POST['vendor_id']);
}
else if(brp_strtolower($POST['mode']) == "get_complain") {
...
else if(brp_strtolower($POST['mode']) == "get_complain_pending_payment") {
