...
function get_first_bom() {
	...
	$.ajax({
		...
		success: function (resp) {
			...
			if (rel.bom) {
				$(".bom-product-spare-parts").removeClass("hidden");
			} else {
				$(".bom-product-spare-parts").addClass("hidden");
			}
			Unloading();
		}
	});
...
function edit_data(complaint_trn_id) {
	...
	$.ajax({
		...
		success: function (response) {
			...
			$("#bom_version_id").html(resp.bom_version);
...
function load_complaint_datatable() {
	...
	var branch_id = ($('#branch_id').length > 0) ? $('#branch_id').val() : '';
	...
	datatable = $("#complaint-table").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain + service_domain + 'app/complaint/',
		"fnServerParams": function (aoData) {
			aoData.push({ "name": "mode", "value": "fetch" }, { "name": "date", "value": date }, { "name": "fol_status", "value": fol_status }, { "name": "f_type", "value": f_type }, { "name": "fil_followup_status", "value": fil_followup_status }, { "name": "fil_followup_type", "value": fil_followup_type }, { "name": "emp_id", "value": emp_id }, { "name": "branch_id", "value": branch_id }); // -ametr
		},
...
function add_complain_status(complain_id, followup_status) {
	...
	$.ajax({
		...
		type: 'POST',
		url: root_domain + service_domain + 'app/complaint/', //Here you will fetch records 
...
$("#old_spare_part_update").on('submit', function (e) {
	...
	$.ajax({
		type: "POST",
		url: root_domain + service_domain + 'app/complaint/',
		data: { mode: "update_spare_part_old", sc_name: sc_name, sc_no: sc_no, sc_date: sc_date, sc_id: sc_id, c_type1: c_type1, c_remark1: c_remark1, branch_id: branch_id },
		success: function (response) {
			window.location = root_domain + service_domain + 'return_old_spare';
			Unloading();
			$('#save').prop("disabled", false);
		}
	});
...
function load_bom_version(id) {
	$.ajax({
		type: "POST",
		url: root_domain + service_domain + 'app/complaint/',
		data: { mode: "load_bom_version", product_id: id },
		success: function (resp) {
			$('#bom_version_id').html(resp);
		}
	});
}
function product_load() {
	...
	var mainurl = root_domain + service_domain + 'app/complaint/index.php?mode=product_load&complaint_type_id=' + complaint_type_id + '&cust_id=' + cust_id;
	$.getJSON(mainurl, function (json) {
		if (json) {
			var arr = new Array();
			var len = json[0].length;
			var lent = json[1].length;
...
function delete_editatble_file() {
    ...
    $.ajax({
        type: "POST",
        url: root_domain + service_domain + 'app/complaint/',
        data: { mode: "deletefile", complaint_id: edit_id },
        success: function (response) {
