...
function get_fixed_assets($dbcon, $where_date){
    ...
    //////////////////////////////////Maulik Code Add Group Entry///////////////
    $ca_sub_qry = "SELECT g_id AS ca_sub_group FROM `tbl_group` WHERE g_status = 0 And `g_pid`= ".FIXED_ASSETS;
...
//            
            ...
            foreach ($ca_entries as $ca_entry) {
                ...
                //if($amount > 0){
                    $str.='
                        <tr>
                            <td tabindex="1">
                                <a style="color: inherit;" class="noprint" href="group_detail_view.php?group_id='.$ca_entry["group_id"].'" ><strong>'.$ca_entry["group_name"].'</strong></a>
                                <span class="printshow" style="display:none;">'.$ca_entry["group_name"].'</span>
                            </td>
    ...
    $fa_query = "SELECT gb.ledger_id,led.l_name,led.l_id,(select sum(amount) from tbl_general_book where entry_type=1 and ledger_id=gb.ledger_id and genral_book_status=0) as cr_amount,(select sum(amount) from tbl_general_book where entry_type=2 and ledger_id=gb.ledger_id  and genral_book_status=0 ) as db_amount
    ...
    if($fa_entries_res){
        ...
        while($fa_entry = mysqli_fetch_assoc($fa_entries_res)){
                ...
                $amount = $fa_entry['db_amount'] - $fa_entry['cr_amount'];
                $amount = number_format((float)$amount, 2, '.', '');
                
                $str.='
                    <tr>
                        <td tabindex="1">
                            <a style="color: inherit;" class="noprint" href="ledger_detail_view.php?ledger_id='.$fa_entry["l_id"].'" >'.$fa_entry["l_name"].'</a>
                            <span class="printshow" style="display:none;">'.$fa_entry["l_name"].'</span>
                        </td>
    ...
    $fixed_assets['value'] = $fixed_assets_value+$ca_value;
    return $fixed_assets;
}
...
//            
    ...
    $fa_query = "SELECT gb.ledger_id,led.l_name,led.l_id,(select sum(amount) from tbl_general_book where entry_type=1 and ledger_id=gb.ledger_id and genral_book_status=0) as cr_amount,(select sum(amount) from tbl_general_book where entry_type=2 and ledger_id=gb.ledger_id  and genral_book_status=0 ) as db_amount
        ...
        WHERE led.l_status = ".ACTIVE."
            AND gb.genral_book_status = ".ACTIVE."
            AND gb.ref_date ".$where_date."
            AND led.`l_group` = ".CURRENT_ASSETS."
            AND led.company_id = ".$_SESSION['company_id']."
        GROUP BY gb.ledger_id";
       // return $fa_query;
    $fa_entries_res = $dbcon->query($fa_query);
    ...
    $current_assets['value'] = $ca_value+$fixed_assets_value;
...
function get_fixed_assets_entries($dbcon, $where_date){
    /*$query = "SELECT GROUP_CONCAT(ledgerids.l_id) as leger_id 
                ...
                LEFT join product_mst as pro on pro.product_id = pot.product_id 
...
function get_capital_account($dbcon, $start_date,$end_date){

    //////////////////////////////////Maulik Code Add Group Entry///////////////
    $cl_sub_qry = "SELECT g_id AS cl_sub_group FROM `tbl_group` WHERE g_status = 0 And `g_pid`= ".CAPITAL_ACCOUNT;
        ...
        if($sub_group_id) {
            ...
            if($cl_result){
                foreach ($cl_result as $value) {
                    $balance_type = $value['balance_typeid'];
                    $op_balance = ($balance_type=="1" ? ($value['opening_balance']) :-$value['opening_balance']);
                    $balance = $op_balance + ($value['creditamount']-$value['debitamount']);
            ...
            foreach ($cl_entries as $cl_entry) {
                ...
                //if($amount > 0){
                    $str.='
                        <tr>
                            <td tabindex="1">
                                <a style="color: inherit;" class="noprint" href="group_detail_view.php?group_id='.$cl_entry["group_id"].'" ><strong>'.$cl_entry["group_name"].'</strong></a>
                                <span class="printshow" style="display:none;">'.$cl_entry["group_name"].'</span>
                                </td>
    ...
    $capital_ac_qry = "select sum(opn_balance) as opening_balance,balance_typeid,
                sum(debitamount) as debitamount ,sum(creditamount) as creditamount,l_id,
                ...
                where l_status = ".ACTIVE." AND company_id = ".$_SESSION['company_id']." 
                    ...
                    AND cust.l_id IN (".$sub_ledger.") group by cust.l_id";
    //echo $capital_ac_qry;exit;
            ...
            foreach ($ca_entries as $ca_entry) {
                ...
                //if($amount > 0){
                    $str.='
                        <tr>
                            <td tabindex="1">
                            <a style="color: inherit;" class="noprint" href="ledger_monthly_view.php?ledger_id='.$ca_entry["ledger_id"].'" >'.$ca_entry["ledger_name"].'</a>
                            <span class="printshow" style="display:none;">'.$ca_entry["ledger_name"].'</span>
                            </td>
                            <td style="text-align: right;" '.$style.'>'.number_format($amount,2).'</td>
                        </tr>
                    ';
    ...
    $capital_account['value'] = $capital_account_value+$current_liabilities['value'];
...
function get_loans($dbcon, $where_date){
    ...
    //////////////////////////////////Maulik Code Add Group Entry///////////////
    $cl_sub_qry = "SELECT g_id AS cl_sub_group FROM `tbl_group` WHERE g_status = 0 And `g_pid`= ".LOANS_LIABILITY;
    $result = mysqli_query($dbcon,$cl_sub_qry);
    $cl_sub_groups = mysqli_fetch_all($result,MYSQLI_ASSOC);
    ...
    $loan_sub_qry = "SELECT g_id AS ca_sub_group FROM `tbl_group` WHERE  g_id=".LOANS_LIABILITY;
            ...
            foreach ($loans_entries as $loans_entry) {
                ...
                //if($amount > 0){
                    $str.='
                        <tr>
                            <td tabindex="1">
                                <a style="color: inherit;" class="noprint" href="ledger_monthly_view.php?ledger_id='.$loans_entry["ledger_id"].'" >'.$loans_entry["ledger_name"].'</a>
                                <span class="printshow" style="display:none;">'.$loans_entry["ledger_name"].'</span>
                            </td>
                            <td style="text-align: right;" '.$style.'>'. indian_number($amount,2).'</td>
                        </tr>
                    ';
    ...
    $loans_liability['value'] = $loan_value+$current_liabilities['value'];
...
function get_current_liabilities($dbcon, $start_date, $end_date){
    ...
    ///////////////////////////////////////Maulik Start Code////////////////////////////
    $loan_sub_qry = "SELECT g_id AS ca_sub_group FROM `tbl_group` WHERE  g_id=".CURRENT_LIABILITIES;
    $result = mysqli_query($dbcon,$loan_sub_qry);
    $loan_sub_groups = mysqli_fetch_all($result,MYSQLI_ASSOC);
    ...
    foreach ($loan_sub_groups as $loan_sub_group) {
        $sub_group_id = $loan_sub_group['ca_sub_group'];
        if($sub_group_id){
           // echo "SELECT group_concat(l_id) as ledger FROM `tbl_ledger` WHERE `l_group` = ".$sub_group_id;
            $ledger = $dbcon->query("SELECT group_concat(l_id) as ledger FROM `tbl_ledger` WHERE `l_group` = ".$sub_group_id)
                        ->fetch_object()->ledger;
                $loan_qry = "select sum(opn_balance) as opening_balance,balance_typeid,
                sum(debitamount) as debitamount ,sum(creditamount) as creditamount,
...
function get_suspence_account($dbcon, $where_date){
    ...
    $cl_sub_qry = "SELECT g_id AS cl_sub_group FROM `tbl_group` WHERE g_status = 0 And `g_pid`= ".SUSPENSE_ACCOUNTS;
    $result = mysqli_query($dbcon,$cl_sub_qry);
    $cl_sub_groups = mysqli_fetch_all($result,MYSQLI_ASSOC);
        ...
        foreach ($suspence_entries as $suspence_ac) {
            ...
            //if($amount && ($amount>0)){
                $str.='
                    <tr>
                        <td tabindex="1">
                            <a style="color: inherit;" class="noprint" href="ledger_monthly_view.php?ledger_id='.$suspence_ac["ledger_id"].'" >'.$suspence_ac["ledger_name"].'</a>
                            <span class="printshow" style="display:none;">'.$suspence_ac["ledger_name"].'</span>
                        </td>
    ...
    $suspence_account['value'] = $sa_value+$cl_value;
