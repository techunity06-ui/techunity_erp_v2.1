$(document).ready(function() {
	...
	$("#payroll_salary_slip_add").validate({
		rules: {
			employee_id: {
				required: true			
			},
...
$("#payroll_salary_slip_add").on('submit',function(e) {
	var form = this;
	e.preventDefault();
	e.stopPropagation();	
	if (!$("#payroll_salary_slip_add").valid()) {
		return false;
	}
	form.submitted = true;	
	Loading(true);	
	...
	$.ajax({
		cache:false,
		url: root_domain + hrms_domain + 'app/payroll_salary_slip/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				Unloading();
				toastr.success("PAYROLL SALARY SLIP ADDED SUCCESSFULLY", "SUCCESS");
				window.location=root_domain + hrms_domain + 'payroll_salary_slip_list';
			}else if(arr.msg == '0') {
			...
			$('#payroll_salary_slip_add').trigger('reset');	
		},
...
function delete_payroll_salary_slip(id) 
{
	...
	if(r) {
		...
		$.ajax({
			...
			success: function(response)
			{
				var arr = jQuery.parseJSON(response);
				if(arr.msg == "1") {
					toastr.success("PAYROLL SALARY SLIP DELETE SUCCESSFULLY", "SUCCESS");
					load_datatable();
					Unloading();
				}else if(arr.msg == "0") {
...
function load_datatable()
{
	...
	datatable = $("#dynamic-table").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain + hrms_domain + 'app/payroll_salary_slip/',
...
function changeStatus(id,p_status)
{
	...
	$.ajax({
		...
		success: function(response)
		{
			toastr.success("PAYROLL SALARY SLIP CHANGED SUCCESSFULLY", "SUCCESS");
			Unloading();
			load_datatable();
		}
	});
...
function getComponentEarning(){
	...
	$.ajax({
		type: "POST",
		url: root_domain + hrms_domain + 'app/payroll_salary_slip/',
		data: { mode : "payroll_salary_slip_earnings",comp_earn_id:comp_earn_id},
		success: function(response){
...
function getComponentDeduction(){
	...
	$.ajax({
		type: "POST",
		url: root_domain + hrms_domain + 'app/payroll_salary_slip/',
		data: { mode : "payroll_salary_slip_deductions",comp_dedu_id:comp_dedu_id},
		success: function(response){
			var data = jQuery.parseJSON(response);
...
function edit_earnings_data(id)
{
	Loading();
		$.ajax({
			...
			{
				var data = jQuery.parseJSON(response);
				$("#payroll_component_name_earnings").select2('val',data.payroll_component_id);
				$("#payroll_component_amount_earnings").val(data.payroll_component_amount);
				$("#edit_id").val(id);
