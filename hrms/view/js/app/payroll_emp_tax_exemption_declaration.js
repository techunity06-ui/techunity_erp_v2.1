...
$("#payroll_emp_tax_exemption_declaration_add").validate({
	rules: {
		employee_id: {
			required: true			
		},
...
$("#payroll_emp_tax_exemption_declaration_add").on('submit',function(e) {
	var form = this;
	e.preventDefault();
	e.stopPropagation();	
	if (!$("#payroll_emp_tax_exemption_declaration_add").valid()) {
		return false;
	}
	form.submitted = true;	
	Loading(true);	
	...
	$.ajax({
		cache:false,
		url: root_domain + hrms_domain + 'app/payroll_emp_tax_exemption_declaration/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				Unloading();
				toastr.success("PAYROLL EMPLOYEE TAX EXEMPTION DECLARATION ADDED SUCCESSFULLY", "SUCCESS");
				window.location=root_domain + hrms_domain + 'payroll_emp_tax_exemption_declaration_list';
			}
			else if(arr.msg == '0') {
			...
			{	
				toastr.success("PAYROLL EMPLOYEE TAX EXEMPTION DECLARATION UPDATED SUCCESSFULLY", "SUCCESS");		
				Unloading();
				window.location=root_domain + hrms_domain + 'payroll_emp_tax_exemption_declaration_list';
			}
			$('#payroll_emp_tax_exemption_declaration_add').trigger('reset');	
		},
...
function delete_payroll_emp_tax_exemption_declaration(id) 
{
		...
		if(r) {
			...
			$.ajax({
				...
				success: function(response)
				{
					console.log(response)
					if(response.trim() == "1") {
						toastr.success("PAYROLL EMPLOYEE TAX EXEMPTION DECLARATION DELETE SUCCESSFULLY", "SUCCESS");
...
function load_datatable()
{
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain + hrms_domain + 'app/payroll_emp_tax_exemption_declaration/',
...
function add_field()
{
	...
	if($("#declared_amount").val()==="")
	{		
		toastr.warning("Enter Declared Amount", "ERROR")
		return false;
	}
	Loading();	
	$.ajax({
			...
			data: { 
				...
				declared_amount:$("#declared_amount").val()
			},
			success: function(response)
			{
...
function show_data()
{
	...
	$.ajax({
		...
		success: function(data){
			$('#payroll_employee_tax_declaration').html(data);				
			Unloading();
		}		
	});
}
	...
	Loading();
		$.ajax({
			...
			success: function(response)
			{
				...
				$("#exemption_display_category").select2('val',data.exemption_category);
				...
				$("#declared_amount").val(data.declared_amount);
				$("#edit_id").val(id);
				$('#addrow').val('Update');
				Unloading();
			}
...
function changeStatus(id,p_status)
{
	...
	$.ajax({
		...
		success: function(response)
		{
			toastr.success("PAYROLL EMPLOYEE TAX EXEMPTION DECLARATION STATUS CHANGED SUCCESSFULLY", "SUCCESS");
			Unloading();
			load_datatable(); 
		}
	});
