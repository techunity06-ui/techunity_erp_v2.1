<?php 
	...
	$form="Employee Tax Exemption Proof Submission";
	$companyID = $_SESSION['company_id'];
	$usertype=$_SESSION['user_type'];
	if(strpos($_SERVER[REQUEST_URI], "payroll_emp_exemption_proof_submission_edit")==false)
	{
		$mode="Add";
	...
	else
	{
		...
		$query="select payrolltaxproof.* from payroll_emp_tax_exemption_proof_submission as payrolltaxproof 
				left join tbl_company as comp on comp.company_id = payrolltaxproof.company_id
				left join payroll_tax_exemption_proofs as payempproof on payempproof.payroll_emp_tax_exem_proof_subm_id = payrolltaxproof.id
				where `payrolltaxproof`.`id` = $payroll_id and `payrolltaxproof`.`company_id` = $companyID";
...
<html lang="en">
	...
	<body>
							...
							<section class="panel">
								...
								<div class="">
									<ul class="breadcrumb">
										...
										<li ><a href="<?= ROOT . HRMS_ROOT . 'payroll_emp_exemption_proof_submission_list'?>"><?=$form?> List</a></li>
									</ul>
								</div>
							</section>
						</div>	
						...
						<div class="col-sm-12">
							<section class="panel">
								...
								<div class="panel-body">
									<form class="form-horizontal" role="form" id="payroll_emp_exemption_proof_submission_add" action="javascript:;" method="post" name="payroll_emp_exemption_proof_submission_add" enctype="multipart/form-data">
										<div class="">
											<div class="col-md-12">
												<?php if($mode == "Edit"){ ?>
													<div class="col-md-6">
							 					...
							 					<?php } else { ?>
													<div class="col-md-6">
														<div class="form-group">
																...
																<?php
																$series_id = '';
																$query = $dbcon->query("SELECT `invoicetype_id`,`taxinvoice_start`,`format_value`,`end_format_value` FROM `tbl_invoicetype` WHERE `status` = 0 and `invoice_type`='EMP TAX EXEM PROOF SUBMISSION' and company_id = $companyID and `type_id` = '16' order by invoicetype_id ");
												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<div class="col-md-8 col-xs-12">
															<input id="submission_date" name="submission_date" type="text" class="form-control default-date-picker" title="Enter Submission Date" placeholder="Enter Submission Date" value="<?php if($mode=='Edit'){ echo $rel['submission_date'];} ?>" >
														</div>
													</div>
												</div>
											</div>
											<hr style="margin-top: 0px; margin-bottom:0px; border: 1px solid #eee"><br>
											<h4>Tax Exemption Proofs </h4>
											<h6>Employee Tax Exemption Proofs</h6>
											<div class="col-md-12">
												<div class="form-group">
													<table cellspacing="10" style="border-collapse:inherit; " id="payroll_emp_tax_declaration" class="display table table12 table-striped table-bordered">
														...
														<tr id="field1">
															...
															<td data-label="Type Of Proof" style="vertical-align:top;">
																<input type="text"  name="type_of_proof" title="Enter Type Of Proof" placeholder="Type Of Proof" id="type_of_proof" class="form-control" />
															</td>
															<td data-label="Actual Amount" style="vertical-align:top;">
																<input type="text"  name="actual_amount" title="Enter Actual Amount" placeholder="Actual Amount" id="actual_amount" class="form-control" />
															</td>
											...
											<div id="payroll_employee_proof_submission"></div>
											<hr style="margin-top: 0px; margin-bottom:0px; border: 1px solid #eee"><br>
											<h4>Attachments </h4>
											<div class="col-md-12 margin_row">
												<div class="col-md-6">
													<div class="form-group">
														...
														<div class="col-md-8 col-xs-12">
															<input type="file" id="attachments" name="attachments"  class="form-control" title="Select Attachment" >
															<div class="col-md-3">
																<?php if($mode=='Edit') { ?>
																	<img src="<?php echo ROOT . HRMS_ROOT .'upload/proof_submission_file/'.$rel['attachment']; ?>" width="150" height="150" />
																<?php } ?>
															</div>
														</div>
													</div>
											...
											<div class="col-md-12 margin_row">
												<div class="col-md-6">
													<div class="form-group">
														<label class="col-md-4 control-label">Total Actual Amount </label>
														<div class="col-md-8 col-xs-12">
															<input id="total_actual_amount" name="total_actual_amount" type="text" class="form-control" title="Enter Total Actual Amount" placeholder="Enter Total Actual Amount" value="<?php if($mode=='Edit'){ echo $rel['total_actual_amount'];} ?>" >
														</div>
													</div>
												</div>
												<div class="col-md-6">
											...
											<div class="col-md-12">
												<center>
													...
													<a href="<?= ROOT . HRMS_ROOT . 'payroll_emp_exemption_proof_submission_list'?>" type="button" class="btn btn-danger">Cancel</a>
												</center>
											</div>		
										</div>
		...
		<?php include_once('../../include/include_js_file.php');?>   
			<script src="<?= ROOT . HRMS_ROOT ?>js/app/payroll_emp_exemption_proof_submission.js?<?=time()?>"></script>
		<script>
			$(".select2").select2({
				width: '100%'
			});
			...
			$(document).ready(function(){
				$(document).on("keyup","#actual_amount", function(){
					var decla_am = $(this).val();
					$("#total_actual_amount").val(decla_am);
				});
			});
		</script>
		<?php if($mode == 'Edit'){ 
