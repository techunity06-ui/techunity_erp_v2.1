...
<body>
						...
						<div class="col-sm-3">
							<section class="panel">
								...
								<div class="panel-body">
									<form role="form" id="payroll_emp_other_income_add" action="javascript:;" method="post" name="payroll_emp_other_income_add">
										<div class="form-group">
											<label class="control-label">Series</label>
											<?php
												$series_id = '';
												$query = $dbcon->query("SELECT `invoicetype_id`,`taxinvoice_start`,`format_value`,`end_format_value` FROM `tbl_invoicetype` WHERE `status` = 0 and `invoice_type`='EMPLOYEE OTHER INCOME' and company_id = $companyID and `type_id` = '16' order by invoicetype_id ");
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" id="income_source" name="income_source" placeholder="Enter Income Source" />
										</div>
										<div class="form-group">
											<label for="income_amount">Income Amount*</label>
											<input type="text" class="form-control" id="income_amount" name="income_amount" placeholder="Enter Income Amount" required="" />
										</div>
										<div class="form-group">
	...
	<!-- Modal -->
	<div class="modal colored-header info" id="ModalEditEmpOtherIncome" role="dialog" data-keyboard="false" data-backdrop="static">
		<div class="modal-dialog custom-width">
			<div class="modal-content">
				...
				<form id="FormEditEmpOtherIncome" role="form" method="post" novalidate>
					<div class="modal-body form">
							...
							<div class="form-group">
								<label class="control-label">Payroll Period*</label>
								<select id="edit_payroll_period_id" class="select2" name="payroll_period_id" required>
									<option selected disabled value="">SELECT PAYROLL PERIOD</option>
									<?php
										$query = $dbcon->query("SELECT `id`,`payroll_period_name` FROM `payroll_period` WHERE `status` = 0 and company_id = $companyID order by payroll_period_name");
										while ($r = $query->fetch_assoc()) {
							...
							<div class="form-group">
								<label for="income_source">Income Source</label>
								<input type="text" class="form-control" id="edit_income_source" name="income_source" placeholder="Enter Income Source" />
							</div>
							<div class="form-group">
								<label for="income_amount">Income Amount*</label>
								<input type="text" class="form-control" id="edit_income_amount" name="income_amount" placeholder="Enter Income Amount" required="" />
							</div>
							<div class="form-group">
								<label for="status">Status*</label>
	...
	<script src="<?= ROOT . HRMS_ROOT ?>js/app/payroll_emp_other_income.js?<?=time()?>"></script>
	<script>
		$(".select2").select2({
			width: '100%'
		});
