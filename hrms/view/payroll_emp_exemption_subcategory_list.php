...
$title = 'Employee Tax Exemption Sub Category';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
						...
						<div class="col-sm-3">
							<section class="panel">
								...
								<div class="panel-body">
									<form role="form" id="payroll_emp_exemption_sub_category_add" action="javascript:;" method="post" name="payroll_emp_exemption_sub_category_add">
										<div class="form-group">
											<label for="parent_id">Parent Category*</label>
											<select id="parent_id" class="select2" name="parent_id">
												<option selected disabled value="">SELECT PARENT CATEGORY</option>
												<?php
													$query = $dbcon->query("SELECT `id`,`parent_id`,`category_name` FROM `payroll_emp_tax_exemption_cat_sub` WHERE `status` = 0 and `parent_id` = 0");
													while ($r = $query->fetch_assoc()) {
														if($rel['parent_id'] == $r['id']){
															$parentIDS = 'selected';
														}else{
															$parentIDS = '';
														}
														echo '<option value="' . $r['id'] . '" '.$parentIDS.'>' . $r['category_name'] . '</option>';
													}
												?>
											</select>
										</div>							 
	...
	<!-- Modal -->
	<div class="modal colored-header info" id="ModalEditEmpExemptionSubCategory" role="dialog" data-keyboard="false" data-backdrop="static">
		<div class="modal-dialog custom-width">
			<div class="modal-content">
				...
				<form id="FormEditEmpExemptionSubCategory" role="form" method="post" novalidate>
					<div class="modal-body form">
							<div class="form-group">
								<label for="parent_id">Parent Category*</label>
								<select id="edit_parent_id" class="select2" name="parent_id">
									<option selected disabled value="">SELECT PARENT CATEGORY</option>
									<?php
										$query = $dbcon->query("SELECT `id`,`parent_id`,`category_name` FROM `payroll_emp_tax_exemption_cat_sub` WHERE `status` = 0 and `parent_id` = 0");
										while ($r = $query->fetch_assoc()) {
	...
	<script src="<?= ROOT . HRMS_ROOT ?>js/app/payroll_emp_exemption_subcategory.js?<?=time()?>"></script>
	<script>
		$(".select2").select2({
			width: '100%'
		});
