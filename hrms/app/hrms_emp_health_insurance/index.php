...
if(strtolower($POST['mode']) == "fetch") {
	...
	$aColumns = array('emphealthinsurance.id', 'emphealthinsurance.health_insurance_name', 'emphealthinsurance.status', 'comp.company_name');
	$sIndexColumn = "emphealthinsurance.id";
	$isWhere = array("emphealthinsurance.status IN (0,1) and emphealthinsurance.company_id = $companyID".check_user('emphealthinsurance'));
	$sTable = "hrms_emp_health_insurance as emphealthinsurance";			
	$isJOIN = array("left join tbl_company as comp on comp.company_id=emphealthinsurance.company_id");
	$hOrder = "emphealthinsurance.health_insurance_name ASC";
	...
	foreach($sqlReturn as $row) {
		...
		$row_data[] = $row['health_insurance_name'];
		if($row['status']=='0'){
	...
	$row['res']='';
	$tr = $dbcon -> query("SELECT `id`,`health_insurance_name`,`status` FROM `hrms_emp_health_insurance` WHERE `health_insurance_name` = '$POST[health_insurance_name]'");
	if($tr->num_rows > 0) {
		$r = $tr -> fetch_assoc();
		if($r['status'] != 0) {
			$info['status']=0;
			$updateid=update_record('hrms_emp_health_insurance', $info,"id=".$r['id'] , $dbcon);
		...
		$info['health_insurance_name'] = $POST['health_insurance_name'];
		$info['status'] = $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");
		$inserid = add_record('hrms_emp_health_insurance', $info, $dbcon);

		if($inserid) {
			if(strtolower($POST['model'])=="model") {
				$query="select * from hrms_emp_health_insurance where id=".$inserid;
	...
	$q = $dbcon -> query("SELECT * FROM `hrms_emp_health_insurance` WHERE `id` = '$POST[id]'");
	$r = $q->fetch_assoc();
	echo json_encode($r);
}
	...
	if($_POST['token'] == $_SESSION['token']) {
		...
		$info['status'] = $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");			
		$updateid = update_record('hrms_emp_health_insurance', $info,"id=".$POST['eid'] , $dbcon);
