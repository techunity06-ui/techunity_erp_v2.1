...
{ 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			$aColumns = array('id', 'designation_name', 'status', 'comp.company_name');
			$sIndexColumn = "id";
			$isWhere = array("status IN (0,1) and design.company_id = $companyID".check_user('design'));
			$sTable = "hrms_designation as design";			
			$isJOIN = array("left join tbl_company as comp on comp.company_id=design.company_id");
			$hOrder = "design.designation_name ASC";
			...
			echo json_encode( $output );
				...
				$row['res']='';
				$tr = $dbcon -> query("SELECT `id`,`designation_name`,`status` FROM `hrms_department` WHERE `designation_name` = '$POST[designation_name]'");
				if($tr->num_rows > 0) {
					$r = $tr -> fetch_assoc();
					if($r['status'] != 0) {
						$info['status']=0;
						$updateid=update_record('hrms_designation', $info,"id=".$r['id'] , $dbcon);
							if($updateid)
							{
									$row['res']='1';
							}
					...
					$info['designation_name']= $POST['designation_name'];
					$info['status'] = $POST['status'];
					$info['created_at']		= date("Y-m-d H:i:s");
					$info['updated_at']		= date("Y-m-d H:i:s");
					
					$inserid = add_record('hrms_designation', $info, $dbcon);
					if($inserid)
					{
						if(strtolower($POST['model'])=="model")
						{
							$query="select * from hrms_designation where id=".$inserid;
			...
			$q = $dbcon -> query("SELECT * FROM `hrms_designation` WHERE `id` = '$POST[id]'");
			$r = $q->fetch_assoc();
			echo json_encode($r);
		}
			...
			if($_POST['token'] == $_SESSION['token']) {
				...
				$info['status'] = $POST['status'];
				$info['created_at']		= date("Y-m-d H:i:s");				
				$updateid=update_record('hrms_designation', $info,"id=".$POST['eid'] , $dbcon);
				if($updateid)
					echo "1";
				else
					echo "0".$dbcon->error;
