...
{ 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			echo json_encode( $output );
				...
				$row['res']='';
				$tr = $dbcon -> query("SELECT `id`,`department_name`,`status` FROM `hrms_department` WHERE `department_name` = '$POST[department_name]'");
				if($tr->num_rows > 0) {
					$r = $tr -> fetch_assoc();
					if($r['status'] != 0) {
						$info['status']=0;
						$updateid=update_record('hrms_department', $info,"id=".$r['id'] , $dbcon);
							if($updateid)
							{
									$row['res']='1';
							}
					...
					$info['department_name']= $POST['department_name'];
					$info['status'] = $POST['status'];
					$info['updated_at']		= date("Y-m-d H:i:s");
					$inserid = add_record('hrms_department', $info, $dbcon);
					if($inserid)
					{
						if(strtolower($POST['model'])=="model")
						{
							$query="select * from hrms_department where id=".$inserid;
			...
			$q = $dbcon -> query("SELECT * FROM `hrms_department` WHERE `id` = '$POST[id]'");
			$r = $q->fetch_assoc();
			echo json_encode($r);
		}
			...
			if($_POST['token'] == $_SESSION['token']) {
				...
				$info['status']	= $POST['status'];
				$info['updated_at']		= date("Y-m-d H:i:s");				
				$updateid=update_record('hrms_department', $info,"id=".$POST['eid'] , $dbcon);
				if($updateid)
					echo "1";
				else
					echo "0".$dbcon->error;
