...
if(strtolower($POST['mode']) == "fetch") {
	...
	$aColumns = array('id', 'employee_type_name', 'status', 'comp.company_name');
	$sIndexColumn = "id";
	$isWhere = array("status IN (0,1) and depart.company_id = $companyID".check_user('depart'));
	$sTable = "hrms_emp_type as depart";			
	$isJOIN = array("left join tbl_company as comp on comp.company_id=depart.company_id");
	$hOrder = "depart.employee_type_name ASC";
	...
	foreach($sqlReturn as $row) {
		...
		$row_data[] = $row['employee_type_name'];
		if($row['status']=='0'){
	...
	$row['res']='';
	$tr = $dbcon -> query("SELECT `id`,`employee_type_name`,`status` FROM `hrms_emp_type` WHERE `employee_type_name` = '$POST[employee_type_name]'");
	if($tr->num_rows > 0) {
		$r = $tr -> fetch_assoc();
		if($r['status'] != 0) {
			$info['status']=0;
			$updateid=update_record('hrms_emp_type', $info,"id=".$r['id'] , $dbcon);
		...
		$info['employee_type_name'] = $POST['employee_type_name'];
		$info['status'] = $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");
		$inserid = add_record('hrms_emp_type', $info, $dbcon);

		if($inserid) {
			if(strtolower($POST['model'])=="model") {
				$query="select * from hrms_emp_type where id=".$inserid;
	...
	$q = $dbcon -> query("SELECT * FROM `hrms_emp_type` WHERE `id` = '$POST[id]'");
	$r = $q->fetch_assoc();
	echo json_encode($r);
}
	...
	if($_POST['token'] == $_SESSION['token']) {
		...
		$info['status'] = $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");			
		$updateid = update_record('hrms_emp_type', $info,"id=".$POST['eid'] , $dbcon);
