...
if(strtolower($POST['mode']) == "fetch") {
	...
	$aColumns = array('id', 'payroll_period_name', 'payroll_start_date', 'payroll_end_date', 'status', 'comp.company_name');
	$sIndexColumn = "id";
	$isWhere = array("status IN (0,1) and payrollperiod.company_id = $companyID".check_user('payrollperiod'));
	$sTable = "payroll_period as payrollperiod";			
	$isJOIN = array("left join tbl_company as comp on comp.company_id=payrollperiod.company_id");
	$hOrder = "payrollperiod.payroll_period_name ASC";
	...
	foreach($sqlReturn as $row) {
		...
		$row_data[] = $row['payroll_start_date'];
		$row_data[] = $row['payroll_end_date'];
		if($row['status']=='0'){
	...
	$row['res']='';
	$tr = $dbcon -> query("SELECT `id`,`payroll_period_name`, `payroll_start_date`, `payroll_end_date`, `status` FROM `payroll_period` WHERE `payroll_period_name` = '$POST[payroll_period_name]'");
	if($tr->num_rows > 0) {
		$r = $tr -> fetch_assoc();
		if($r['status'] != 0) {
			$info['status']=0;
			$updateid=update_record('payroll_period', $info,"id=".$r['id'] , $dbcon);
		...
		$info['payroll_period_name'] = $_POST['payroll_period_name'];
		$info['payroll_start_date'] = date('Y-m-d', strtotime($_POST['payroll_start_date']));
		$info['payroll_end_date'] = date('Y-m-d', strtotime($_POST['payroll_end_date']));
		$info['status'] = $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");
		$inserid = add_record('payroll_period', $info, $dbcon);

		if($inserid) {
			if(strtolower($POST['model'])=="model") {
				$query="select * from payroll_period where id=".$inserid;
	...
	$q = $dbcon -> query("SELECT * FROM `payroll_period` WHERE `id` = '$POST[id]'");
	$r = $q->fetch_assoc();
	echo json_encode($r);
}
	...
	if($_POST['token'] == $_SESSION['token']) {
		...
		$info['status'] = $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");			
		$updateid = update_record('payroll_period', $info,"id=".$POST['eid'] , $dbcon);
