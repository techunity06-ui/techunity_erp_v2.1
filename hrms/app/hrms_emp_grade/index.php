...
if(strtolower($POST['mode']) == "fetch") {
	...
	$aColumns = array('id', 'employee_grade_name', 'status', 'comp.company_name');
	$sIndexColumn = "id";
	$isWhere = array("status IN (0,1) and empgrade.company_id = $companyID".check_user('empgrade'));
	$sTable = "hrms_emp_grade as empgrade";			
	$isJOIN = array("left join tbl_company as comp on comp.company_id=empgrade.company_id");
	$hOrder = "empgrade.employee_grade_name ASC";
	...
	$tr = $dbcon -> query("SELECT `id`,`employee_grade_name`,`status` FROM `hrms_emp_grade` WHERE `employee_grade_name` = '$POST[employee_grade_name]'");
	if($tr->num_rows > 0) {
		$r = $tr -> fetch_assoc();
		if($r['status'] != 0) {
			$info['status']=0;
			$updateid=update_record('hrms_emp_grade', $info,"id=".$r['id'] , $dbcon);
		...
		$info['employee_grade_name'] = $POST['employee_grade_name'];
		$info['leave_policy_id'] = $POST['leave_policy_id'];
		$info['salary_structure_id'] = $POST['salary_structure_id'];
		$info['status']= $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");
		$inserid = add_record('hrms_emp_grade', $info, $dbcon);
		if($inserid) {
			if(strtolower($POST['model'])=="model") {
				$query="select * from hrms_emp_grade where id=".$inserid;
	...
	$q = $dbcon -> query("SELECT * FROM `hrms_emp_grade` WHERE `id` = '$POST[id]'");
	$r = $q->fetch_assoc();
	echo json_encode($r);
}
	...
	if($_POST['token'] == $_SESSION['token']) {
		...
		$info['status']= $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");			
		$updateid = update_record('hrms_emp_grade', $info,"id=".$POST['eid'] , $dbcon);
