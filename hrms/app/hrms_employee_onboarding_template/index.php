...
include_once("../../../include/hrms_common_functions.php");
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			$sTable = "hrms_employee_onboarding_template as hrmsempsepalist";			
			...
			foreach($sqlReturn as $row) {
				...
				if($row['id']!='0'){
					if($edit_btn_per) {
						$edit_btn='<a class="btn btn-xs btn-warning" data-original-title="Edit" data-toggle="tooltip" data-placement="top" href="'. ROOT . HRMS_ROOT .'hrms_employee_onboarding_template_edit/'.$row['id'].'"><i class="fa fa-pencil"></i></a>';
					}
					if($delete_btn_per) {
						$delete_btn='<button class="btn btn-xs btn-danger" data-original-title="Delete" data-toggle="tooltip" data-placement="top" onClick="delete_hrms_employee_onboarding_template('.$row['id'].')"><i class="fa fa-trash-o"></i></button>';
					}
				}
				if($other_btn_per) {
					if($row['status'] == '0')
			...
			$insertempseparateid=add_record('hrms_employee_onboarding_template', $info, $dbcon);

			updateSeries($dbcon, 'id', 'hrms_employee_onboarding_template', 'EMPLOYEE ONBOARDING TEMPLATE');
					
			$info_update['status']	= 0;
			$info_update['emp_onboarding_temp_id']	= $insertempseparateid;
			$updateemponboardingid=update_record('hrms_employee_onboarding_activities', $info_update,"status=3 and user_id=".$_SESSION['user_id'] , $dbcon);
			...
			$info['employee_grade_id']	= $POST['employee_grade_id'];
			$info['status']	= $POST['status'];
			$updateid=update_record('hrms_employee_onboarding_template', $info,"id=".$POST['eid'] , $dbcon);
			...
			$info['status']	= 2;
			$updateemponboardingid=update_record('hrms_employee_onboarding_template', $info,"id=".$POST['eid'] , $dbcon);	
			$updateemponboardingactivityid=update_record('hrms_employee_onboarding_activities', $info1,"emp_onboarding_temp_id=".$POST['eid'] , $dbcon);
			...
			if($updateemponboardingid)
				...
				if(empty($POST['edit_id'])){
					if(empty($POST['eid'])){
						...
						$info1['status']	= '0';
						$info1['emp_onboarding_temp_id'] = $POST['eid'];
					}
				}else{
					if(empty($POST['eid'])){
						$info1['status']	= '3';
				...
				$table='hrms_employee_onboarding_activities';
				$tableid='id';
			...
			if(empty($POST['est_id'])){
				$query="select hrmsempsepalist.*,user.user_name,usertype.usertype_name from hrms_employee_onboarding_activities as hrmsempsepalist 
				...
				left join users as user on user.user_id = hrmsempsepalist.activity_user_id
				left join tbl_usertype as usertype on usertype.usertype_id = hrmsempsepalist.activity_role_id
		 		where `hrmsempsepalist`.`status` = 0 and `hrmsempsepalist`.`emp_onboarding_temp_id`=".$POST['est_id']." and `hrmsempsepalist`.`user_id` = $userID and `hrmsempsepalist`.`company_id` = ".$companyID;
			}
				...
				while($rel=mysqli_fetch_assoc($result)){
				 	echo '<tr id="fieldtr'.$rel['id'].'" >
						...
						<td data-label="ACTION" style="vertical-align:top">
							<button type="button" class="btn btn-round btn-warning btn-xs" onclick="edit_employee_onboarding_template_data('.$rel['id'].');" ><i class="fa fa-pencil"></i></button>
							<button type="button" class="btn btn-round btn-danger btn-xs" onclick="delete_employee_onboarding_template_data('.$rel['id'].');" id="fieldremove'.$i.'">X</button>
						</td>	
					</tr>';
					$i++;
				}
		...
		{
			$ids = $POST['id'];
			$q = $dbcon -> query("select * from hrms_employee_onboarding_activities as hrmsempsepaactivities
		...
		else if(strtolower($POST['mode'])== "delete_data")
		{
			$row=array();
			$info['status'] = 2;	
			$updateid=update_record("hrms_employee_onboarding_activities", $info,"id=".$POST['eid'] , $dbcon);
			if($updateid)
				$row['res']="1";
			else
				$row['res']="0";
