...
if(strtolower($POST['mode']) == "fetch") {
	...
	$aColumns = array('hrappraisal.id', 'hrmsapptemp.appraisal_template_title', 'hrappraisal.status', 'comp.company_name');
	$sIndexColumn = "hrappraisal.id";
	$isWhere = array("hrappraisal.status IN (0,1) and hrappraisal.company_id = $companyID".check_user('hrappraisal'));
	$sTable = "hrms_appraisal as hrappraisal";			
	$isJOIN = array("left join tbl_company as comp on comp.company_id=hrappraisal.company_id",
					"left join hrms_appraisal_template as hrmsapptemp on hrmsapptemp.id=hrappraisal.appraisal_template_id");
	$hOrder = "hrappraisal.id ASC";
	...
	$row['res']='';
	$tr = $dbcon -> query("SELECT `id`,`appraisal_template_id`,`status` FROM `hrms_appraisal` WHERE `appraisal_template_id` = '$POST[appraisal_template_id]'");
	if($tr->num_rows > 0) {
		$r = $tr -> fetch_assoc();
		if($r['status'] != 0) {
			$info['status']=0;
			$updateid=update_record('hrms_appraisal', $info,"id=".$r['id'] , $dbcon);
		...
		$info['appraisal_template_id'] = $POST['appraisal_template_id'];
		$info['status'] = $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");
		$inserid = add_record('hrms_appraisal', $info, $dbcon);

		updateSeries($dbcon, 'id', 'hrms_appraisal', 'HRMS APPRAISAL');

		if($inserid) {
			if(strtolower($POST['model'])=="model") {
				$query="select * from hrms_appraisal where id=".$inserid;
	...
	$q = $dbcon -> query("SELECT * FROM `hrms_appraisal` WHERE `id` = '$POST[id]'");
	$r = $q->fetch_assoc();
	echo json_encode($r);
}
	...
	if($_POST['token'] == $_SESSION['token']) {
		...
		$info['status'] = $POST['status'];
		$info['updated_at']	= date("Y-m-d H:i:s");			
		$updateid = update_record('hrms_appraisal', $info,"id=".$POST['eid'] , $dbcon);
