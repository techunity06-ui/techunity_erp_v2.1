							...
							<form role="form" id="balty_add" action="javascript:;" method="post" name="balty_add">
								  ...
								  <div class="form-group">
									  <label for="size_id">Size*</label>
									  	<select class="select2" name="size_id" id="size_id" >
								  ...
								  <div class="form-group">
									  <label for="batch_size_id">Batch Size*</label>
									  <select class="select2" name="batch_size_id" id="batch_size_id" >
									 	 	<option>Select Batch Size</option>
                                            <?php echo load_batch_size($dbcon, $where,$rel['mixing_batch_size']);?>
										</select>
								  </div>
								  <div class="form-group">
									  <label for="balty_name">Batch Qty*</label>
									  <input type="text" class="form-control" id="batch_qty" name="batch_qty" placeholder=" Batch Qty">
								  </div>
...
<div class="modal colored-header info" id="ModalEditAccount" role="dialog" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog custom-width">
		<div class="modal-content">
			...
			<form id="FormEditCurrency" role="form" method="post" novalidate>
				...
				<div class="form-group">
						<label for="size_id">Size*</label>
						<select class="select2" name="edit_size_id" id="edit_size_id" >
							<option>Select Size</option>
							<?php echo load_size($dbcon, $where,$rel['extrusion_size']);?>
						</select>
				</div>
				<div class="form-group">
					<label for="batch_size_id">Batch Size*</label>
						<select class="select2" name="edit_batch_size_id" id="edit_batch_size_id" >
							<option>Select Batch Size</option>
							<?php echo load_batch_size($dbcon, $where,"");?>
						</select>
				</div>
				<div class="form-group">
					<label for="balty_name">Batch Qty*</label>
					<input type="text" class="form-control" id="edit_batch_qty" name="edit_batch_qty" placeholder=" Batch Qty">
				</div>
	...
	<script src="<?=ROOT.ADMINISTRATION_ROOT ?>js/app/batch_calculation_mst.js?<?=time()?>"></script>
<script>
