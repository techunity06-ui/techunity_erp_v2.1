...
$("#class_add").on('submit',function(e) {
	...
	var bonnet_nut=$("#bonnet_nut").val();
	var branch_id=$("#abranch_id").val();
	// var code=$("#code").val();
	var form_data = {
		bonnet_nut: bonnet_nut,
		branch_id: branch_id,
		mode:$("#mode").val(),
		is_ajax: 1
	};	
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/bonnet_nut_mst/',
		type: "POST",
		data: form_data,
...
$("#FormEditunit").on('submit',function(e) {
	...
	var form_data = {
		eid :$("#edit_id").val(),
		bonnet_nut: $("#edit_bonnet_nut").val(),
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.bonnet_nut_id);
			$("#edit_bonnet_nut").val(obj.bonnet_nut);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/bonnet_nut_mst/',
