...
$("#class_add").on('submit',function(e) {
	...
	var testing_std=$("#testing_std").val();
	var branch_id=$("#abranch_id").val();
	// var code=$("#code").val();
	var form_data = {
		testing_std: testing_std,
		branch_id: branch_id,
		mode:$("#mode").val(),
		is_ajax: 1
	};	
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/testing_std_mst/',
		type: "POST",
		data: form_data,
...
$("#FormEditunit").on('submit',function(e) {
	...
	var form_data = {
		eid :$("#edit_id").val(),
		testing_std: $("#edit_testing_std").val(),
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.testing_std_id);
			$("#edit_testing_std").val(obj.testing_std);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/testing_std_mst/',
