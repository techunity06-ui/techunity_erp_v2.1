...
$("#Common_add").on('submit',function(e) {
	...
	var common_mst_desc=$("#common_mst_desc").val();

	var form_data = {
		Common_name: Common_name,
		common_category_id: common_category_id,
		common_mst_desc:common_mst_desc,
		mode:'Add',
		is_ajax: 1
	};	
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/common_mst/',
		type: "POST",
		data: form_data,
...
$("#FormEditCommon").on('submit',function(e) {
	...
	var form_data = {
		...
		common_mst_desc: $("#edit_common_mst_desc").val(),
		mode:'edit',
		is_ajax: 1
	};	
...
function edit_common(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_common_mst_desc").val(obj.common_mst_desc);
...
function load_Common_datatable(){
	...
	var catgory_id = $('#common_category_ids').val();
	...
	datatable = $("#common-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/common_mst/',
			"fnServerParams": function ( aoData ) {
				aoData.push( 
					{ "name": "mode", "value": "fetch" },
					{"name": "category_id", "value": catgory_id }					
				);
			},
