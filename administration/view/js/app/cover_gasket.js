...
$("#class_add").on('submit',function(e) {
	...
	var cover_gasket=$("#cover_gasket").val();
	var branch_id=$("#abranch_id").val();
	// var code=$("#code").val();
	var form_data = {
		cover_gasket: cover_gasket,
		branch_id: branch_id,
		mode:$("#mode").val(),
		is_ajax: 1
	};	
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/cover_gasket_mst/',
		type: "POST",
		data: form_data,
...
$("#FormEditunit").on('submit',function(e) {
	...
	var form_data = {
		eid :$("#edit_id").val(),
		cover_gasket: $("#edit_cover_gasket").val(),
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.cover_gasket_id);
			$("#edit_cover_gasket").val(obj.cover_gasket);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/cover_gasket_mst/',
