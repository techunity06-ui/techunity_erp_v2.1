...
$("#class_add").on('submit',function(e) {
	...
	var bonnet_bolt=$("#bonnet_bolt").val();
	var branch_id=$("#abranch_id").val();
	// var code=$("#code").val();
	var form_data = {
		bonnet_bolt: bonnet_bolt,
		branch_id: branch_id,
		mode:$("#mode").val(),
		is_ajax: 1
	};	
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/bonnet_bolt_mst/',
		type: "POST",
		data: form_data,
...
$("#FormEditunit").on('submit',function(e) {
	...
	var form_data = {
		eid :$("#edit_id").val(),
		bonnet_bolt: $("#edit_bonnet_bolt").val(),
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.bonnet_bolt_id);
			$("#edit_bonnet_bolt").val(obj.bonnet_bolt);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/bonnet_bolt_mst/',
