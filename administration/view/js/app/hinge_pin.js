...
$("#class_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/hinge_pin_mst/',
		type: "POST",
		data: form_data,
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.hinge_pin_id);
			$("#edit_hinge_pin").val(obj.hinge_pin);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/hinge_pin_mst/',
