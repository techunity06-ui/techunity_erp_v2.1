...
$("#make_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/makemst/',
		type: "POST",
		data: form_data,
...
function edit_test(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(id);				
			$("#edit_make_name").val(obj.make_name);
...
function load_make_datatable(){
	...
	datatable = $("#make-table").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain+administration_domain+'app/makemst/',
