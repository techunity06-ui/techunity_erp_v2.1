...
$("#class_add").on('submit',function(e) {
	...
	var gland_packing=$("#gland_packing").val();
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/gland_packing_mst/',
		type: "POST",
		data: form_data,
...
$("#FormEditunit").on('submit',function(e) {
	...
	var form_data = {
		eid :$("#edit_id").val(),
		gland_packing: $("#edit_gland_packing").val(),
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.gland_packing_id);
			$("#edit_gland_packing").val(obj.gland_packing);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/gland_packing_mst/',
