...
function add_cust_address()
{
	...
	$.ajax({
		...
		url: root_domain + crm_domain +'app/customer/',
		type: "POST",
		data: { mode : mode , c_add_address:c_add_address, c_add_country:c_add_country,c_add_state:c_add_state,c_add_city:c_add_city,cust_id:cust_id,c_addr_defult:c_addr_defult,edit_id:edit_id },
...
function add_cust_contact()
{
	...
	$.ajax({
		...
		url: root_domain + crm_domain +'app/customer/',
		type: "POST",
		data: { mode : mode , con_first:con_first, con_last:con_last, com_email:com_email,con_mobile:con_mobile,con_phone:con_phone,con_job:con_job,cust_id:cust_id,edit_con_id:edit_con_id,cust_person_model:cust_person_model,cust_ref_id:cust_ref_id },
		success: function(responsevalue)
		{	
			if(responsevalue.trim() == '1') {
				Unloading();
