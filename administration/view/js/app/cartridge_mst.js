$(document).ready(function() {
	...
	$("#cartridge_add").validate({
		...
		messages: {
			...
			cartridge_name: {
				required: "Please Enter Cartridge Name "
			},
			code: {
				required: "Please Enter Cartridge Code "
			},
		}
	}); 
...
$("#cartridge_add").on('submit',function(e) {
	var form = this;
	e.preventDefault();
	e.stopPropagation();	
	if (!$("#cartridge_add").valid()) {
		return false;
	}		
	form.submitted = true;	
	Loading(true);	
	...
	var cartridge_name=$("#cartridge_name").val();
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/cartridge_mst/',
		type: "POST",
		data: form_data,
...
$("#FormEditunit").on('submit',function(e) {
	...
	var form_data = {
		eid :$("#edit_id").val(),
		cartridge_name: $("#edit_cartridge_name").val(),
		code: $("#edit_code").val(),
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.cartridge_id);
			$("#edit_cartridge_name").val(obj.cartridge_name);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/cartridge_mst/',
