$(document).ready(function() {
	...
	$("#impregnation_add").validate({
		...
		messages: {
			...
			code: {
				required: "Please Enter Style Code "
			},
		}
	}); 
...
$("#impregnation_add").on('submit',function(e) {
	var form = this;
	e.preventDefault();
	e.stopPropagation();	
	if (!$("#impregnation_add").valid()) {
		return false;
	}		
	form.submitted = true;	
	Loading(true);	
	...
	var impregnation_name=$("#impregnation_name").val();
	var branch_id=$("#abranch_id").val();
	var code=$("#code").val();
	var form_data = {
		impregnation_name: impregnation_name,
		code: code,
		branch_id: branch_id,
		mode:$("#mode").val(),
		is_ajax: 1
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/impregnation_mst/',
		type: "POST",
		data: form_data,
		success: function(response)
		{
			...
			$('#impregnation_add').trigger('reset');
		},
...
$("#FormEditunit").on('submit',function(e) {
	...
	var form_data = {
		eid :$("#edit_id").val(),
		impregnation_name: $("#edit_impregnation_name").val(),
		code: $("#edit_code").val(),
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.impregnation_id);
			$("#edit_impregnation_name").val(obj.impregnation_name);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/impregnation_mst/',
