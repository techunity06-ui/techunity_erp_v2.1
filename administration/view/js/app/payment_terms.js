...
$(document).ready(function() {
		datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/payment_terms/',
...
$("#payment").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/payment_terms/',
		type: "POST",
		data: form_data,
		success: function(response)
		{
			...
			if(responsevalue.trim() == '1') {
				...
				$('#payment_terms').append('<option value='+data.terms_id+'>'+data.payment_terms+'</option>');
