...
$("#class_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/screen_mst/',
		type: "POST",
		data: form_data,
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.screen_id);
			$("#edit_screen").val(obj.screen);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/screen_mst/',
