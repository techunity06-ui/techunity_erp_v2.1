...
$("#class_add").on('submit',function(e) {
	...
	var cover_stud=$("#cover_stud").val();
	var branch_id=$("#abranch_id").val();
	// var code=$("#code").val();
	var form_data = {
		cover_stud: cover_stud,
		branch_id: branch_id,
		mode:$("#mode").val(),
		is_ajax: 1
	};	
	...
	$.ajax({
		cache:false,
		url: root_domain+administration_domain+'app/cover_stud_mst/',
		type: "POST",
		data: form_data,
...
$("#FormEditunit").on('submit',function(e) {
	...
	var form_data = {
		eid :$("#edit_id").val(),
		cover_stud: $("#edit_cover_stud").val(),
...
function edit_parameter(id)
{
	...
	editReq = $.ajax({
		...
		success: function(response)
		{
			...
			$("#edit_id").val(obj.cover_stud_id);
			$("#edit_cover_stud").val(obj.cover_stud);
...
function load_impregnation_msteter_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+administration_domain+'app/cover_stud_mst/',
