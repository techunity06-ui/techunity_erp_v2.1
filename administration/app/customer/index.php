...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$appData = array();
		$i=1;
		$aColumns = array('cust_id', 'cc.cc_name', 'party_type', 'cust_name', 'cust_email', 'cust_mobile', 'cust_gst', 'cust_status','cust.cdate','cust.user_id','cust.cust_owner');
	...
	//	$info['edit_id']=$POST['edit_id'];
		...
		while($row=mysqli_fetch_assoc($sel))
		{
			...
			echo '<td style="white-space: nowrap;">'.$row['c_con_mobile'].'</td>';
