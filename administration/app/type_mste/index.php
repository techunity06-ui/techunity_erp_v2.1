  ...
  //  if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
		    ...
		    $bulkAccessArray = canCheckPermissionAccess($dbcon, [
		    	ADMINISTRATOR_TYPE_MSTE_UPDATE,
		        ADMINISTRATOR_TYPE_MSTE_DELETE
		    ]);
			...
			foreach($sqlReturn as $row) {
				...
				$edit_btn=''; $delete_btn='';  
				if(in_array(ADMINISTRATOR_TYPE_MSTE_UPDATE,$bulkAccessArray)){
				...
				if(in_array(ADMINISTRATOR_TYPE_MSTE_DELETE,$bulkAccessArray) && $row['is_deletable']=='0'){
			...
			$tr = $dbcon -> query("SELECT `type_id`,`type_name`,`type_status`,`company_id` FROM `mst_typefi` WHERE `type_name` ='".$POST['impregnation_name']."' and company_id = '".$_SESSION['company_id']."' and `type_status`='0'");
			...
			if($cnt>0) {
				...
				$info['type_name']			= $_POST['type_desc'];							
				$info['code']				= $_POST['code'];							
				$info['cdate']				= date("Y-m-d H:i:s");
				$info['user_id']			= $_SESSION['user_id'];
				$info['company_id']			= $_SESSION['company_id'];
				$inserid=add_record('mst_typefi', $info, $dbcon, $branch_id);
				if($inserid) {
					if(strtolower($POST['mode']) == "add") {
						$resp['msg'] = "1";
					} else {
						$zone_qry="select * from mst_typefi where type_id=".$inserid; 
			...
			$q = $dbcon -> query("SELECT * FROM `mst_typefi` WHERE `type_id` = '$POST[id]'");
			$r = $q->fetch_assoc();
			echo json_encode($r);
		}
			...
			$tr = $dbcon -> query("SELECT `type_id`,`type_name`,`type_status`,`company_id` FROM `mst_typefi` WHERE `type_id` != '".$POST['eid']."' and company_id = '".$_SESSION['company_id']."' and `type_name` ='".$POST['impregnation_name']."' and `type_status`='0'");
			if($tr->num_rows > 0) {
				echo "-1";
			} else {
				$info['type_name']			= $_POST['impregnation_name'];							
				$info['code']				= $_POST['code'];							
				$info['cdate']				= date("Y-m-d H:i:s");
				$info['user_id']			= $_SESSION['user_id'];
				$info['company_id']			= $_SESSION['company_id'];
				$updateid=update_record('mst_typefi', $info,"type_id=".$POST['eid'] , $dbcon, $branch_id);
				if($updateid)
					echo "1";
				else
					echo "0".$dbcon->error;
				...
				$info['type_status']='2';
				$updateid=update_record('mst_typefi', $info,"type_id=".$POST['eid'] , $dbcon);
