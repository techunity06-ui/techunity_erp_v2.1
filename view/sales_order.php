<?php 
    ...
    $pro_search = $set_conf['sales__pro_search'];
	...
	if(strpos($_SERVER[REQUEST_URI], "salesorderedit")==true)
	{
		...
		//echo $cust_id=copy_ledger_cust($dbcon,$rel_q['quotation_id']);
	}
	else
	{
	...
	<body>
										...
										<div class="">
											...
											<div class="col-md-12">
												...
												<div class="col-md-4">
													<div class="form-group">
														...
														<div class="col-md-8 col-xs-12">
															<input id="po_date" name="po_date" type="text" class="form-control default-date-picker valid" title="Purchase Order Date" placeholder="Purchase Order Date" value="<?=$po_date?>" placeholder="Purchase Order Date" <?=$required?> >
														</div>
													</div>
												</div>
												<div class="col-md-4">
													<?php echo getBranchBox($dbcon, $branch_id, $edit_branch_id, false, true, ''); ?>	
												</div>
											</div>
											<div class="col-md-12">
												<div class="col-md-4">
											...
											<div class="col-md-5">
												...
												<?php $tcs_applicable = $dbcon->query("SELECT tcs_applicable FROM tbl_finance_setting as comp WHERE company_id=".$_SESSION['company_id'])->fetch_object()->tcs_applicable; 
                                                    if($tcs_applicable) {?>
                                                        <div class="form-group">
                                                                <label class="col-md-3 control-label">Select Formula</label>
                                                                <div class="col-md-6 col-xs-12">
		...
		<?php 
			...
			if($smode=="quotation_mode"){
				echo "<script>load_quotation_details('".$quotaion_id."')</script>";
			}
		?>
		<script>
