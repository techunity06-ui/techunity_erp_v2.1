...
if(brp_strtolower($POST['mode']) == "fetch") {
		...
		$bom_rel2=brp_mysqli_fetch_assoc($dbcon->query($bom2));
			...
			if($extra_stock == '1'){
				...
				if($rel["status"]!=0){	
					...
					}else
					{
						...
						// $po_qty=convert_stock($dbcon,$po_qty_conv,$rel['rp_pid'],"base_unit");
						$po_qty_conv=convert_stock($dbcon,$po_qty,$rel['rp_pid'],"conv_unit");
			...
			//pathik production convert unit end
										...
										<div class="col-md-9">
											<div id="rbase'.$rel["rp_id"].'" style="'.$basedisplay.'">
												<input type="number" min="0" class="form-control" name="res_qty'.$rel["rp_id"].'" id="res_qty'.$rel["rp_id"].'" onkeydown="return numericonly(event)" onkeyup="convert_unit_fun('.$rel["rp_id"].','.$res_qty_sh.',\'base_unit\');error_check('.$rel["rp_id"].','.$res_qty_sh.')" value="'.$rel["reserve_stock"].'" '.$reserv_read_only.' />
											</div>
											</div>
											<div class="col-md-2" style="margin-top: 8px;margin-left: -20px;">
										<strong>'.$rel["base_unit_name"].'</strong>
										...
										<div class="col-md-9" >
										<input type="number" min="0" class="form-control" name="base_po_qty'.$rel["rp_id"].'" id="base_po_qty'.$rel["rp_id"].'" onkeypress="return isNumberKey(event)" onkeyup="convert_unit_fun('.$rel["rp_id"].','.$po_qty_sh.',\'base_unit\');"  value="'.$po_qty.'" '.$po_read_only.' />
										<div class="col-md-12">
										<span style="display:none;" class="error" id="po_qty_err'.$rel["rp_id"].'" ></span>
										</div>
										</div>
										...
										{
											get_child_tree($dbcon,$rp_id,$jobwork_type);
			...
			*/
			...
			if(empty($set_process_rel['sp_id'])){
				...
				left join tbl_product_process on tbl_product_process.pr_process_id = pro_bom_process.pr_process_id WHERE  pro_bom_process.process_status = 0 and prover.bom_version_id='".$POST['bom_version_id']."' AND bom.bom_product='".$POST['eid']."'"; 
	...
	//for ($x = 0; $x <= 100; $x++) {	
		...
		while($rel1=brp_mysqli_fetch_assoc($result1)){  
			...
			WHERE pro_bom_process.process_status = 0 and  bom.bom_product='".$rel1['product_id']."' and bom.bom_id=".$info_sub['bom_id']." AND pro_bom_process.pr_process_id != ''"; 
...
// die;
			...
			min_max_bom_show($dbcon,$rel1['p_bom_id'],$base_qty,$i,$call,$space,$inserid,$inserid_sub,'',$POST['branch_id'],$jobwork_type,$sales_customer_id,$sales_order_id,$store_order_id);
			$i++;
		} 
			...
			else
			{
				...
				if(brp_mysqli_num_rows($rs_custw_b)>0)
				{
					while($product_process=brp_mysqli_fetch_assoc($rs_custw_b))
					{
						...
						$queryw="select * from tbl_product_process where pr_process_id=".$product_process['pr_process_id'];
						$rs_custw=$dbcon->query($queryw);	
						$relw=brp_mysqli_fetch_array($rs_custw);
  ...
  <div class="col-md-12">
	...
	<input type="hidden" id="selected_process_ids" class="form-control" placeholder="Selected Process" value="'.$selected_process_ids.'">
		...
		// Process Parameter
				...
				$process_id = $POST['process_id'];
				$query="select pr_process_id from tbl_product_process where product_id=".$product_id." and process_id=".$process_id;
				$result=$dbcon->query($query);
				$count=brp_mysqli_num_rows($result);
				if($count > 0){
					$arr['msg']="exist";
...
/*	else if(brp_strtolower($POST['mode']) == "get_product_process_data") {
	...
	//$end = $start+1;
				...
				function get_child_tree($dbcon,$rp_id,$jobwork_type)
				{
			...
			$q="SELECT rpro.*,tc.cat_name,pro.product_name,pro.product_icode,dr.drawing_number,pro.product_min_stock,pro.product_setting_check,bunit.unit_name as base_unit_name,cunit.unit_name as conv_unit_name,pro.image_name,pro.product_id,pro.product_base_qty,pro.product_conv_qty,rpro.customer_id, ptm.product_type_name,pro.product_type,pro.reorder_qty, bom.bom_version_id,pro.product_base_unit FROM `tbl_request_product` as rpro
				...
				//pathik production convert unit end
							...
							{ 
								...
								if($rel['status']==3){
									$sub_product_button='<a class="btn btn-success btn-xs" data-original-title="" id="sub_prd_btn'.$rel["rp_id"].'" data-toggle="tooltip" data-placement="top" onclick="add_sub_product('.$rel["rp_id"].','.$rel["product_id"].','.$POST['eid'].','.$rel['product_base_unit'].');" ><i class="fa fa-plus"></i></a>';
								}
								else{
									$sub_product_button='';
								}
			...
			//pathik production convert unit end
								...
								echo '<tr '.$bclolr.' id='.$rel["rp_id"].' class='.$rel['rp_pid'].'>
								<td>'.$rel["sr_no"].'</td>
								...
								<div class="col-md-9">
									<div id="rbase'.$rel["rp_id"].'" style="'.$basedisplay.'">
										<input type="number" min="0" class="form-control numbersOnly" name="res_qty'.$rel["rp_id"].'" id="res_qty'.$rel["rp_id"].'" onkeydown="return numericonly(event)" onkeyup="convert_unit_fun('.$rel["rp_id"].','.$res_qty_sh.',\'base_unit\');error_check('.$rel["rp_id"].','.$res_qty_sh.')" value="'.$rel["reserve_stock"].'" '.$reserv_read_only.' />
									</div>
									</div>
											<div class="col-md-2" style="margin-top: 8px;margin-left: -20px;">
										<strong>'.$rel["base_unit_name"].'</strong>
				...
				//$rp_id = $rel['rp_id'];
								...
								if(brp_mysqli_num_rows($child_result)>0)
								{
									...
									get_child_tree($dbcon,$child_rel['perent_id'],$jobwork_type);
