...
th {
  ...
  <section id="container" >
				...
				<section class="panel">
				  <header class="panel-heading">
					...
					<div class="panel-body">
						<form role="form" id="user_template_permission_add" action="javascript:;" method="post" name="user_template_permission_add">
								...
								<div class="form-group col-md-12">
										<div class="col-md-3"></div>
									    <div class="col-md-2" style="text-align:right;"> 
											<label for="vendor_name">Template Name</label>
									 	</div>
									 	<div class="col-lg-3">
									 		<select class="select2" onchange="load_user_menu(null,this.value)" id="template_id" name="template_id">
												<?php
													echo getTemplateName($dbcon,"");
												?>
									  		</select>
									 	</div>
									 	<div class="col-md-3"><b>NOTE: -</b> If template id selected then user permission worked accordingly and if template id is not selected then custom permission apply for specific users.</div> 
								</div>
								<br>
								<h2><span>OR</span></h2>
								<br>
	...
	<script src="<?=ROOT?>js/app/user_template_permission.js?<?=time()?>"></script>
<script>
$(".select2").select2({
	width: '100%'
});
