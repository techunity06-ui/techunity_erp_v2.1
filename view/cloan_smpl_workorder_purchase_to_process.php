<?php
	...
	$qryse='SELECT rp_po_qty,in_process_qty_main,sp_id FROM `tbl_set_main_process` WHERE rp_po_qty>0 and `sp_status`!=2;)';
	$resultse=$dbcon->query($qryse);
	while($rowse=mysqli_fetch_assoc($resultse))
	{

		$info_sub['rp_po_qty'] = 0;	
		$info_sub['in_process_qty_main'] = $rowse['rp_po_qty'];	
		...
		{
			$info_sub1['rp_po_qty'] 			= 0;	
			$info_sub1['rp_po_base_qty'] 		= 0;	
			$info_sub1['in_process_qty'] 		= $row['rp_po_qty'];	
			$info_sub1['in_process_conv_qty'] 	= $row['rp_po_qty'];	
			$info_sub1['indent_no'] 			= 0;	
			$info_sub1['indent_status'] 		= 0;	
