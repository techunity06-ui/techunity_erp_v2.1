<?php 
	...
	//echo $id;
	$query="select ap.*,p.product_name,p.product_type,pr.process_name,dqty,group_concat(ap.p_id ORDER BY ap.p_id ASC) as allo_id from tbl_allocate_re_process as ap left join product_mst as p on p.product_id=ap.p_product_id left join process_mst as pr on pr.process_id=ap.process_id left join (select sum(pt_qty) as dqty,pt_alloc_id from tbl_allocate_re_process_trn group by pt_alloc_id) as apta on apta.pt_alloc_id=ap.p_id where ap.p_product_id IN (".$id.") and ap.pr_process_type=".$process_type." and ap.process_id=".$process_id." group by ap.p_product_id, ap.process_id";
	...
	$set_pro="select ap.*,r.rp_req_no,jo.j_vendor,j_chalan_no,pr_rate from tbl_allocate_process as ap
	left join tbl_request_product as r on r.rp_id=ap.p_ref_id
	left join tbl_jobwork as jo on jo.j_alloc_process_id=ap.p_id
	where p_id=".$rel['pr_process_id'];
	$set_rel_pro=mysqli_fetch_assoc($dbcon->query($set_pro));

	$ven_id=$set_rel_pro['j_vendor'];
	$chno=$set_rel_pro['j_chalan_no'];
	$pr_rate=$set_rel_pro['pr_rate'];
	...
	//$pending_qty=$rel['pen_qty']-$rel['dqty'];
	$product_id=$id;
...
<html lang="en">
	...
	<body>
							...
							<section class="panel" >
								...
								<div class="" style="padding:20px !important;">
								  <ul class="breadcrumb">
									  ...
									  <li><a href="<?=ROOT.'invoice_list'?>">Start Re-Process </a></li>
								  </ul>
								</div>
							</section>
						</div>	
									...
									<form class="form-horizontal" role="form" id="start_allocate_add" action="javascript:;" method="post" name="start_allocate_add">
										<div class="row">
											...
											<div class="col-md-4">
												...
												<div class="col-md-6 col-xs-11">
													<input type="text" class="form-control" id="pr_process_id2" name="pr_process_id2" value="<?=$rel['process_name']; ?>" readonly />
												</div>
											</div>
											<div class="col-md-4">
											...
											<div class="col-md-4">
												<div class="form-group">
												  <label class="col-md-4 control-label">Jobwork No*</label>
													<div class="col-md-6 col-xs-11">
														<input id="pr_job_no" name="pr_job_no" type="text" class="form-control required valid" title="Date" value="" placeholder="Jobwork No" >
													</div>
												</div>	
											</div>
											<div class="col-md-12"></div>
											...
											<div class="col-md-4">
												<div class="form-group">
													<label class="col-md-4 control-label" style="white-space:nowrap;">Available Machine*</label>
													<div class="col-md-6 col-xs-11">
														<input type="text" name="machine_no" id="machine_no" class="form-control" value="<?=$pending_qty;?>"  />
											...
											<div class="col-md-12">
												...
												<div class="col-md-4">
													<div class="form-group">
														...
														<div class="col-md-6 col-xs-11">
															<input type="number" name="pr_rate" id="pr_rate" class="form-control" value="<?=$pr_rate?>" required />
														</div>
													</div>
												</div>
												<div class="col-md-4">
													<div class="form-group">
														...
														<div class="col-md-6 col-xs-11">
															<input type="text" name="pr_chalan_no" id="pr_chalan_no" class="form-control" value="<?=$chno?>"  />
														</div>
													</div>
												</div>
											</div>
										...
										<input type='hidden' name='pr_process_id' id='pr_process_id' value='<?=$rel['pr_process_id'];?>' />
										<input type="hidden" name="process_unit" id="process_unit" value="<?=$set_rel_pro['process_unit']?>" />

										<input type="hidden" name="max_available_qty" id="max_available_qty" value="<?=$pending_qty?>" />
									</form>
								</div>	
							</section>
						</div>
		...
		?>   
		<script src="<?=ROOT?>js/app/allocate_reprocess.js"></script>
		<script>
			$(".select2").select2({
				width: '100%'
			});
