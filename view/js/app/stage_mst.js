	...
	$.ajax({
		...
		success: function(resnse)
		{
			...
			if(responsevalue.trim() == '1') {
				...
				Unloading();
				window.location=root_domain+'stage_list';
				//show_unit_data();
				//load_pro_tbl();
			}
			if(responsevalue.trim() == '2') {
				
				toastr.success("Stage Updated SUCCESSFULLY", "SUCCESS")
				$('#product_add').trigger('reset');
		...
		if(r) {
			...
			$.ajax({
				...
				success: function(resnse)
				{
					...
					if(resnse.trim() == "1") {
						toastr.success("Stage DELETE SUCCESSFULLY", "SUCCESS");
						load_pro_tbl();
						Unloading();
					}
					else if(resnse.trim() == "0") {
...
//	var product_type= $("input[name='product_type1']:checked").val();
	...
	var datatable = $("#product-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+'app/stage_mst/',
