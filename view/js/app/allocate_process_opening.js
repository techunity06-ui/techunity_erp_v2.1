...
function load_datatable()
{
	...
	$.ajax({
			type: "POST",
		url: root_domain+'app/allocate_process_opening/',
		data: { mode : "fetch_opening",process_id:process_id,process_type:process_type },
...
function add_allocate_process_opening(aid,pid,pname,pqty,qc_id,process_id,ref_type,pr_process_type,pro_type)
{
	//alert(st_time);
	//alert(pname);
	
	$('#table_show_allocate_process_opening').modal("show");
...
function start_process(pid,pname,pqty,product_id,pro_type)
{
	...
	$('#table_start_allocate_process_opening').modal("show");
...
/*function show_material_list(product_id,pro_type,pqty){
	...
	$("#material_details").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain+'app/allocate_process_opening/',
	...
	$.ajax({
		...
		success: function(response)
		{
			if(response=='0'){
				...
				load_datatable();
				$('#table_start_allocate_process_opening').modal("hide");
				Unloading(); 
			}
...
function start_process_allocation()
{
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			$('#pr_pr_qty').val('');
			//alert(response);
			$('#table_show_allocate_process_opening').modal("hide");
			load_datatable();
			//$('#dynamic-table').html(response);
			Unloading();
