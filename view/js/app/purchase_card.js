...
$("#purchasecard_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+'app/purchase_card/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
		success: function(response)
		{
			...
			{	
				...
				Unloading();
				window.location=root_domain+'purchase_card_list';
...
function load_po_datatable()
{
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+'app/purchase_card/',
...
function get_vendor_details(tab){
 ...
 function get_vendor_name(vid) {
 	...
 	$.ajax({
		...
		success: function(data){
				window.location.href=root_domain+'purchase_card';
				Unloading();
			}		
	});
 }
