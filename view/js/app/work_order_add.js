...
function next_page(){
	...
	//location.href= root_domain+'direct_workorder/'+product_id+'/'+branch_id+'/'+bom_version_id+'/'+qty;
	//return false;

		$.ajax({
			type: "POST",
			url: root_domain+'app/work_order_add/',
			data: { mode : "save_work_order_product",product_id:product_id,qty:qty,bom_version_id:bom_version_id,branch_id:branch_id,extra_stock:extra_stock},
			...
			/*alert(response);
				...
				if(arr.res==="1"){
						...
						var sp_id = arr.sp_id;
						location.href= root_domain+'direct_workorder/'+product_id+'/'+branch_id+'/'+bom_version_id+'/'+qty+'/'+sp_id;
						return false;
					}else{
	...
	$.ajax({
			type: "POST",
			url: root_domain+'app/work_order_add/',
			data: { mode : "show_next_page",product_id:product_id,qty:qty,request_id:request_id,branch_id:branch_id,bom_version_id:bom_version_id},
			success: function(response)
			{
				$(".second_page").show();
				$(".first_page").hide();
