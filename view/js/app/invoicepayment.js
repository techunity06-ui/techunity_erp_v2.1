	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				...
				{
					window.location=root_domain+'invoicepayment_list';
				}
			}
			if(arr.msg == '2') {
				Unloading();
			...
			{	
				...
				if (typeof arr.eid != 'undefined')
				{
					window.location=root_domain+'invoicepayment_list/'+arr.eid;
				}
				else
				{
					window.location=root_domain+'purchase_list';
...
Loading();
var emode=$('#invoicepayment_add #mode').val();
	$.ajax({
	type: "POST",
	url: root_domain+'app/invoicepayment/',
	data: { mode : "load_data", partyid :partyid,emode:emode,eid:$('#invoicepayment_add #eid').val(),tax_deduct:$('#tax_deducted_flag').is(':checked')},
...
function load_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+'app/invoicepayment/',
