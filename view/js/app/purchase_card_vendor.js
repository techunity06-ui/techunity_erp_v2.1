...
$("#purchasecard_add").on('submit',function(e) {
	...
	$.ajax({
		cache:false,
		url: root_domain+'app/purchase_card_vendor_wise/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
...
function load_po_datatable()
{
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+'app/purchase_card_vendor_wise/',
...
function get_vendor_details(tab){
 ...
 function get_vendor_name(vid) {
 	...
 	$.ajax({
		...
		success: function(data){
				window.location.href=root_domain+'purchase_card_vendor_wise';
				Unloading();
			}		
	});
 }
