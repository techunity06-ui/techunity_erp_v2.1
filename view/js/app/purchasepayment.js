...
$("#purchasepayment_add").on('submit',function(e) {
	...
	{
		toastr.warning("ENTER USE AMOUNT IN PURCHASE/EXPENSE","WARNING");
		return false;
	}
	...
	if(excess_amount>0)
	{	
		var r= confirm("Would you like to store credits amount of "+excess_amount+" as over payment to this vendor?");
		if(!r) {
			return false;
		}
	}
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				...
				{
					window.location=root_domain+'purchasepayment_list';
				}
			}
			if(arr.msg == '2') {
				Unloading();
			...
			{	
				...
				if (typeof arr.eid != 'undefined')
				{
					window.location=root_domain+'purchasepayment_list/'+arr.eid;
				}
				else
				{
					window.location=root_domain+'purchase_list';
...
$("#payment_credit_amount").on('submit',function(e) {
	...
	if($('#total_used_payment').val()==="")
	{
		toastr.warning("ENTER USED AMOUNT","WARNING");
		return false;
	}
	var excess_amount=parseFloat($('#total_excess_payment').val());
	if(excess_amount>0)
...
Loading();
var emode=$('#purchasepayment_add #mode').val();
	$.ajax({
	type: "POST",
	url: root_domain+'app/purchasepayment/',
	data: { mode : "load_data", partyid :partyid,emode:emode,eid:$('#purchasepayment_add #eid').val(),tax_deduct:$('#tax_deducted_flag').is(':checked')},
...
function load_datatable(){
	...
	datatable = $("#dynamic-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+'app/purchasepayment/',
...
function getAllBankByGroup(bank)
{
	//alert(bank);
	if(bank=='27')
	{
		...
		$.ajax({
			type: "POST",
			url: root_domain+'app/invoicepayment/',
			data: { mode : "bank_by_group", bank :bank },
			success: function(response)
			{
				//console.log(response);
				//alert(response);
				$('#form_bank_group').show();
				$('#bank_group').html(response);
				Unloading();
			}
		});	
	}
