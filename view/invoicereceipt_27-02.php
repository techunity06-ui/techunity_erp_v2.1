<?php 
	...
	$query123="select l_name,l_id from tbl_ledger as pro where l_status=0  and l_group in (31) and print_priority!=0 and company_id = $_SESSION[company_id] order by print_priority";
	$rs_dispatch1112=$dbcon->query($query123);
	$cnt1=mysqli_num_rows($rs_dispatch1112);
...
<div class="col-lg-12 table-responsive" id="receipt_print">	
	...
	<thead>
			...
			<th colspan="<?=$hed?>" style="padding:0px !important;">
				<table style="font-size:10px;border-collapse: collapse;border-top:none;" cellpadding="0" cellspacing="0" width="100%" >
					<thead>	
						...
						<b style="font-size:12px;">Details Of Receiver (Bill to) : </b><br/>
						...
						<b style="font-size:12px;">Details Of Consignee (Shipped to) : </b><br>
							<strong><?=$rel['l_name']?></strong>
							<span style="font-weight:normal;">   <br>
							<?=$rel['m_address']?>
							 <br>
		...
		<tr>
			...
			<th rowspan="2" width="<?=$dynamicwidth?>%" style="text-align:center;border:0.5px #ccc solid;border-top: none;" >
				<strong>Particulars Description</strong>
			</th>
		...
		<?php	$query11="select l_name,l_id from tbl_ledger as pro where l_status=0  and l_group in (31) and print_priority!=0 and company_id = $_SESSION[company_id] order by print_priority";
			$rs_dispatch11=$dbcon->query($query11);
			while($rel112=mysqli_fetch_assoc($rs_dispatch11))
			{ ?>
	...
	<tbody style="border: 1px solid;">
		...
		<tr style="height:30px">
					...
					while($rel112=mysqli_fetch_assoc($rs_dispatch11))
					{ 
						$taxd=print_tax($dbcon,$rel112['l_id'],$row['trancation_id'],"per");
						$taxd1=print_tax($dbcon,$rel112['l_id'],$row['trancation_id'],"amount");
					?>
						<td style="text-align:right;vertical-align:top;border-bottom-color:#FFFFFF;border-right:0.5px #ccc solid;">
							<?=$taxd?>
							<?php //=$rel112['l_id']?>
		...
		<?php } ?>
			<tr style="height:20px">
				...
				<?php	$query11="select l_name,l_id from tbl_ledger as pro where l_status=0 and print_priority!=0 and l_group in (31) and company_id = $_SESSION[company_id] order by print_priority";
					...
					while($rel112=mysqli_fetch_assoc($rs_dispatch11))
					{ 
						$taxd2=total_print_tax($dbcon,$rel112['l_id'],$rel['invoice_id']);
					?>
			...
			<tr>
				...
				?>
					...
					?>
						...
						<!--<td style="border-top:0.5px #ccc solid;border-right:0.5px #ccc solid;text-align:right;"><strong><?=$rel112['l_name']?></strong></td>
						<td style="border-top:0.5px #ccc solid;border-right:0.5px #ccc solid;text-align:right;"><strong><?=$taxd2?></strong></td>-->
				...
				<?php 
					...
					<tr height="20px">
						<td colspan="5" style="border-top:0.5px #ccc solid;border-right:0.5px #ccc solid;text-align:left"><strong>Amount Chargable(In Word):</strong> <?=ucwords(convert_number_to_words($total))?></td>
