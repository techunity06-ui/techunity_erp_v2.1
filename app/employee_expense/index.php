...
$bulkAccessArray = canCheckPermissionAccess($dbcon, [
	FINANCE_EMPLOYEE_EXPENSE_STATUS,
	FINANCE_EMPLOYEE_EXPENSE_DELETE,
]);
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		// -ametr
		$_SESSION['last_action_user']=$POST['emp_name'];
		$_SESSION['last_action_status']=$POST['emp_status'];
		...
		//Amish Soni 02-09-2020
		$hOrder = "expense_approve_status, expense_date desc"; // -ametr
		include('../../include/pagging.php');
		$appData = array();
		$id=1;
		foreach($sqlReturn as $row) {
			$row_data = array();
			if(in_array(FINANCE_EMPLOYEE_EXPENSE_STATUS,$bulkAccessArray)){
				$row_data[] = '<a href="'.ROOT.'expense_status/'.$row['ex_id'].'" class="">'.$row["sr"].'</a>';
				$row_data[] ='<a href="'.ROOT.'expense_status/'.$row['ex_id'].'" class="">'.date("d/m/Y",strtotime($row["expense_date"])).'</a>';
				$row_data[] = '<a href="'.ROOT.'expense_status/'.$row['ex_id'].'" class="">'.$row["exp_name"].'</a>'; 
				$row_data[] = '<a href="'.ROOT.'expense_status/'.$row['ex_id'].'" class="">'.$row["user_name"].'</a>'; 
				$row_data[] = '<a href="'.ROOT.'expense_status/'.$row['ex_id'].'" class="">'.$row["complaint_no"].'</a>'; 
				$row_data[] = '<a href="'.ROOT.'expense_status/'.$row['ex_id'].'" class="">'.$row["customer_name"].'</a>'; 
				$row_data[] = '<a href="'.ROOT.'expense_status/'.$row['ex_id'].'" class="">'.$row["paid_amount"].'</a>';
			} 
			...
			$del_btn = '';
			$status_btn = '';
			if(in_array(FINANCE_EMPLOYEE_EXPENSE_DELETE,$bulkAccessArray)){
			...
			if(in_array(FINANCE_EMPLOYEE_EXPENSE_STATUS,$bulkAccessArray)){
				$status_btn='<a href="'.ROOT.'expense_status/'.$row['ex_id'].'" class="btn btn-primary btn-xs">Change Status</a> '.$del_btn;
			}
			$row_data[] = $status_btn.'&nbsp;'.$del_btn;
			$appData[] = $row_data;
			$id++;
		}
