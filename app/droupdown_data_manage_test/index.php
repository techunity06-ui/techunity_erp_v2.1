  ...
  //  if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			$where='';
		    if($branch_id){
		        $where .= check_branch('state',$branch_id);
		    }
		 
			$appData = array();
			$i=1;
			$aColumns = array('state.stateid', 'country.country_name', 'state.state_name','state.gst_state_code', 'state.state_status');
				...
				if($tr->num_rows > 0) {
					...
					if($r['state_status'] != 0) {
						...
						$row['res']='-1';
							...
							$info['gst_state_code']= $POST['product_id'];							
			...
			//$q = $dbcon -> query("SELECT product_id as id,product_name as text FROM `product_mst` WHERE 1 ");
			//$r = $q->fetch_assoc();
