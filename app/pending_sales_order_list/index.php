	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		if($POST['po_type_status']!='2'){
			if($POST['po_type_status']=='3'){
				$wherenew=' and quot.po_approve_status=3';
			}else{
				$wherenew=' and quot.po_approve_status!=3';
			}
		}
		...
		$isWhere = array("quot.quotation_status = 0 and quot.revise_status=0 and quot.approve_status=1 and stage_prob>=90 and quot.company_id in (0,$_SESSION[company_id])".$where.$wherenew);
		$sTable = "tbl_quotation as quot";
		$isJOIN = array('left join tbl_customer as cust on cust.cust_id=quot.cust_id', 'left join tbl_inquiry as inq on inq.inquiry_id=quot.inquiry_id', 'left join users as usr on usr.user_id=quot.quot_won_user_id');
		...
		foreach($sqlReturn as $row) {
			...
			$sql=$dbcon->query("SELECT * FROM print_setup_mst WHERE print_type = 3 AND approve_status = 1 AND status = 0 ORDER BY priority");
...
//				}
			...
			$sqls=$dbcon->query("SELECT * FROM print_setup_mst WHERE print_type = 1 AND approve_status = 1 AND status = 0 ORDER BY priority");
			while($ress = mysqli_fetch_assoc($sqls)){
				if(in_array($ress['id'],$menu_show_permissions)) {
					$print_btn.='<a class="btn btn-xs btn-primary" data-original-title="'.$ress['print_name'].'" data-toggle="tooltip" data-placement="top" target="_blank"  href="'.ROOT.PRINT_ROOT.$ress['page_path'].'/'.$row['quotation_id'].'" style="background: '.$ress['icon_color'].'; border-color: '.$ress['icon_color'].';"><i class="'.$ress['fa_icon'].'"></i></a>';	
				}
			}
			
			// $print_btn='<a class="btn btn-xs btn-info" data-original-title="View Quotation" data-toggle="tooltip" data-placement="top" target="_blank" href="'.ROOT.CRM_ROOT.'quotation_print/'.$row['quotation_id'].'"><i class="fa fa-print"></i></a>'; 
