    ...
    //if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			{	
				...
				update_record('tbl_product_party_purchase', $updateRate, "party_id='".$POST['vender_id']."' and party_product = '".$POST['product_id']."' " , $dbcon);
		...
		else if(strtolower($POST['mode'])== "get_item_selected_information")
		{
			...
			$type = $POST['type'];
			$purchase_rate_info = getItemPriceByVendorId($dbcon, $vendor_id, $product_id);
