...
//include("../../config/image.php");
		...
		$str .='<tr>
			...
			$qry='select * from tbl_general_book as payment
				where payment.genral_book_status=0 and payment.company_id='.$_SESSION['company_id'].' and ref_date>="'.date('Y-m-d',strtotime($s_date[0])).'" and ref_date<="'.date('Y-m-d',strtotime($s_date[1])).'" and payment.ledger_id='.$POST['cust_id'].'';
