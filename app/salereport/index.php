...
{ 
    //if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		$set="select * from tbl_company where company_id=".$_SESSION['company_id'];
			...
			$str='';
		 $qry='Select invoice.invoice_id,invoice.invoice_no,invoice.invoice_date,g_total,sum(tax_amount1) as tax_amount1,sum(tax_amount2) as tax_amount2,sum(tax_amount3) as tax_amount3,invtrn.tax_name1,invtrn.tax_name2,invtrn.tax_name3,cust.company_name,cust.gst_no,(select sum(product_amount) from tbl_invoicetrn as trn where trancation_status=0 and formulaid in (2,1) and trn.invoice_id=invoice.invoice_id )as taxable_amt,(select sum(product_amount) from tbl_invoicetrn as trn where trancation_status=0 and formulaid in (3,4) and trn.invoice_id=invoice.invoice_id )as taxable_amt1,(select sum(product_amount) from tbl_invoicetrn as trn where trancation_status=0 and formulaid=21 and trn.invoice_id=invoice.invoice_id )as taxable_amt2  from tbl_invoice as invoice left join tbl_invoicetrn as invtrn on invtrn.invoice_id=invoice.invoice_id inner join tbl_customer as cust on invoice.cust_id=cust.cust_id where invoice_status=0 and invtrn.trancation_status=0 and invoice_date>="'.date('Y-m-d',strtotime($s_date[0])).'" and invoice_date<="'.date('Y-m-d',strtotime($s_date[1])).'"'.$where.' and invoice.company_id='.$_SESSION['company_id'].' group by invoice.invoice_id order by invoice_no';
			...
			$rs_tax=$dbcon->query($query);
			 ...
			 $str .='<div id="report" class="col-md-12" name="report" width="100%">
					...
					<tr>
						<td colspan="6"><strong>Sale Tax Report</strong></td>
						
						<td colspan="'.(4+$tax_col).'" style="text-align:right">Date
					...
					<tr>
						...
						<th style="text-align:center" width="5%">Taxable Amount</br> 18%</th>
						<th style="text-align:center" width="5%">Taxable Amount</br> 28%</th>
						<th style="text-align:center" width="5%">Taxable Amount</br> 5%</th>';
				...
				if(mysqli_num_rows($result)>0)
				{
					...
					{	
						...
						$str.='<tr>
						  ...
						  <td data-label="Taxable Amount" style="text-align:right">'.indian_number($re["taxable_amt1"]).'</td>
						  <td data-label="Taxable Amount" style="text-align:right">'.indian_number($re["taxable_amt2"]).'</td>';
						$cnt=1;							
						...
						$total_taxable1=$total_taxable1+$re["taxable_amt1"];
						$total_taxable2=$total_taxable2+$re["taxable_amt2"];
					}
				}
				else
				{
			...
			$str .='<tr>
						 ...
						 <td data-label="Total" style="text-align:right">
							<label><strong>'.indian_number($total_taxable1).'</strong></label>
						</td>
						<td data-label="Total" style="text-align:right">
							<label><strong>'.indian_number($total_taxable2).'</strong></label>
						</td>';						
				for($i=0;$i<count($tax_arr['name']);$i++)
					{
			...
			$str	.='<td data-label="Total" style="text-align:right">
						...
						<tr style="font-size:20px">
						 <td class="tfhide" colspan="7" style="text-align:right"> <strong>Total Tax Payable</strong></td>
						 <td data-label="Total Tax Payable" style="text-align:right" colspan="'.(3+$tax_col).'">
