    ...
    //if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		$_SESSION['expense']['filter']['payment_status']=$POST['report'];
			...
			foreach($sqlReturn as $row) {
				$row_data = array();
				//$row_data[] = $id;
				$row_data[] = '<a class="" data-original-title="Edit '.$row["invoice_no"].'" data-toggle="tooltip" data-placement="top" href="'.ROOT.'expense-update/'.$row['expenseid'].'">'.$row["invoice_no"].'</a>';
				$row_data[] = '<a class="" data-original-title="Edit '.$row["invoice_no"].'" data-toggle="tooltip" data-placement="top" href="'.ROOT.'expense-update/'.$row['expenseid'].'">'.$row["company_name"].'</a>';
				$row_data[] = '<a class="" data-original-title="Edit '.$row["invoice_no"].'" data-toggle="tooltip" data-placement="top" href="'.ROOT.'expense-update/'.$row['expenseid'].'">'.date('d M, Y',strtotime($row["expense_date"])).'</a>';				
				//$row_data[] = $row['city_name'];
				$row_data[] = '<a class="" data-original-title="Edit '.$row["invoice_no"].'" data-toggle="tooltip" data-placement="top" href="'.ROOT.'expense-update/'.$row['expenseid'].'">'.$row["g_total"].'</a>';
			...
			if($ap_status=="1"){
				add_general_book_entry($dbcon,"tbl_expense_detail",$inserestimateid,1,$info['emp_id'],$info['paid_amount'],$general_book_id,$POST['expense_date']);
				
				add_general_book_entry($dbcon,"tbl_expense_detail_account",$inserestimateid,1,$info['exp_accountid'],$info['paid_amount'],$general_book_id,$POST['expense_date']);
			}
			...
			//if($_POST['token'] == $_SESSION['token']) 
						...
						//$general_book_id=get_general_book_id($dbcon,'tbl_expense_detail',$POST['eid'],$info['vendorid']);
						
						//if($ap_status=="1"){
							//add_general_book_entry($dbcon,"tbl_expense_detail",$POST['eid'],1,$info['vendorid'],$info['paid_amount'],$general_book_id,$POST['expense_date']);
			...
			$updatetrancationid_gen=update_record('tbl_general_book', $info_gen,"table_name='tbl_expense_detail' and table_id=".$POST['eid'] , $dbcon);
