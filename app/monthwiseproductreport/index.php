...
if(strtolower($POST['mode']) == "generate_report") {
    ...
    $str .='<table  class="display table table-bordered table-striped" id="data_list">
                <tr id="logo" class="logo" style="display:none">
                    ...
                    <tr>
                            <td colspan="7"><strong>Month Wise Sale Product Report</strong></td>
        ...
        $qry="SELECT product_name,
                sum(case when 4 = MONTH(invoice_date) then product_qty else 0 end) '4',
                sum(case when 5 = MONTH(invoice_date) then product_qty else 0 end) '5',
                sum(case when 6 = MONTH(invoice_date) then product_qty else 0 end) '6',
                sum(case when 7 = MONTH(invoice_date) then product_qty else 0 end) '7',
                sum(case when 8 = MONTH(invoice_date) then product_qty else 0 end) '8',
                sum(case when 9 = MONTH(invoice_date) then product_qty else 0 end) '9',
                sum(case when 10 = MONTH(invoice_date) then product_qty else 0 end) '10',
                sum(case when 11= MONTH(invoice_date) then product_qty else 0 end) '11',
                sum(case when 12 = MONTH(invoice_date) then product_qty else 0 end) '12',
                sum(case when 1 = MONTH(invoice_date) then product_qty else 0 end) '1',
                sum(case when 2 = MONTH(invoice_date) then product_qty else 0 end) '2',
                sum(case when 3 = MONTH(invoice_date) then product_qty else 0 end) '3',
                trn.product_id 
            FROM `tbl_invoicetrn` as trn 
            left join  product_mst as product on product.product_id=trn.product_id 
            left join tbl_invoice as invoice on invoice.invoice_id=trn.invoice_id 
            where trn.trancation_status=0 and product_status=0 
                and invoice.invoice_date between '".date('Y-m-d',strtotime($date['start_date']))."' and '".date('Y-m-d',strtotime($date['end_date']))."' and invoice.company_id=".$_SESSION['company_id']." 
                and product.product_type in(0,2) and product.company_id in(0,".$_SESSION['company_id'].") 
