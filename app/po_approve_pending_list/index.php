...
//	{
		...
		switch($POST['po_approval_status']){
			case "0":
			$where.="  and po.po_approval_status=0";
			break;
		...
		if($getapprovalsetting['auto_approval']==1){
			...
			$appData = array();
			$i=1;
			$aColumns = array('purchaseorder_id','purchaseorder_no','l.l_name','city.city_name','bms.branch_name','purchaseorder_date','g_total','paid_amount','status','purchase_status','po.cdate','po.userid','po.po_type_status','po.po_req_status','po_approval_status');
