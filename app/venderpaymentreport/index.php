...
if(strtolower($POST['mode']) == "generate_report") {
        if(!empty($POST['vender_id'])){
            foreach ($POST['vender_id'] as $value) {
                $sub_ledger[]['ledger_id'] = $value;
            }
        } else {
            $sub_groups = implode(',',get_sub_group($dbcon, SUNDRY_CREDITORS));
...
//                                        $custmob = $rel['cust_mobile'];
//                                }else{
//                                        $custmob='-';
//                                }
//						 $op_balance=($rel['balance_typeid']=="1"?-($rel['opening_balance']):$rel['opening_balance']);
//						$balance=($op_balance+$rel['debitamount']+$rel['podebitamount']+$rel['excessdebitamount'])-($rel['creditamount']+$rel['pocreditamount']+$rel['excesscreditamount']);
//						 $str.='<tr>
//								<td data-label="SR. NO." style="text-align:center">'.$i.'</td>
//								<td data-label="PARTY NAME" style="text-align:left">'.$rel["company_name"].'-'.$rel['city_name'].'</td>
//								<td data-label="PHONE NO." style="text-align:left">'.$custmob.'</td>';
//								
//						//$str.='<td style="text-align:center">'.abs($rel['opening_balance']).($rel['balance_typeid']=="1"?' CR':($rel['balance_typeid']=="2"?' DR':'')).'</td>';
//						//$str.=' <td style="text-align:center">'.abs($rel['debitamount']).'</td>';
//						 // $str.='<td style="text-align:center">'.abs($rel['creditamount']).'</td>';
//						   
//							if($balance>0)
//							{
//								//$balancetype='DR';
//								$str.='<td data-label="DEBIT" style="text-align:right">'.number_format(abs($balance),2,".","").'</td><td>-</td>';
...
//								$str.='<td></td><td>-</td>';
//							}
//							//$str .='<td style="text-align:center">'.abs($balance).' '.$balancetype.'</td>
//							$str .='</tr>';
//						$i++;
					}*/
                        $str.='<tr>
