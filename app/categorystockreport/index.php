...
if(strtolower($POST['mode']) == "main_cat") {
	...
	$query = "select * from tbl_category where cat_status=0 and cat_pid=0";
	...
	if(brp_mysqli_num_rows($execu)>0){
		...
		while($row = brp_mysqli_fetch_array($execu)){
			 $cat_r = category_recusive($dbcon,$row['cat_id'],$frmdate,$todate,[]);
...
else if(strtolower($POST['mode']) == "sub_cat")
{		
	$s_date=explode(' - ',$POST['date']);
	$_SESSION['start']=$s_date[0];
	$_SESSION['end']=$s_date[1];
	...
	if(brp_mysqli_num_rows($execu)>0){
		...
		$i=1;
		while($row = brp_mysqli_fetch_array($execu)){
			$cat_r = category_recusive($dbcon,$row['cat_id'],$frmdate,$todate,$stock="");
			//echo "<pre>123";
			//print_r($cat_r);
			$str.='<tr>
				<td><a data-original-title="Sub Category Report" data-toggle="tooltip" data-placement="top" class="noprint" href="'.ROOT.'sub_category_report/'.$row['cat_id'].'">'.$i.'</a> <span class="printshow" style="display: none;">'.$i.'</span></td>
				<td><a data-original-title="Sub Category Report" data-toggle="tooltip" data-placement="top" class="noprint" href="'.ROOT.'sub_category_report/'.$row['cat_id'].'">'.$row['cat_name'].'</a> <span class="printshow" style="display: none;">'.$row['cat_name'].'</span></td>
				<td><a data-original-title="Sub Category Report" data-toggle="tooltip" data-placement="top" class="noprint" href="'.ROOT.'sub_category_report/'.$row['cat_id'].'"><strong style="color:green">'.$cat_r['base_opn_stock'].'</strong><br><strong style="color:orange">'.$cat_r['conv_opn_stock'].'</strong></a> <span class="printshow" style="display: none;"><strong style="color:green">'.$cat_r['base_opn_stock'].'</strong><br><strong style="color:orange">'.$cat_r['conv_opn_stock'].'</strong></span></td>
				<td><a data-original-title="Sub Category Report" data-toggle="tooltip" data-placement="top" class="noprint" href="'.ROOT.'sub_category_report/'.$row['cat_id'].'"><strong style="color:green">'.$cat_r['base_closing_stock'].'</strong><br><strong style="color:orange">'.$cat_r['conv_closing_stock'].'</strong></a> <span class="printshow" style="display: none;"><strong style="color:green">'.$cat_r['base_closing_stock'].'</strong><br><strong style="color:orange">'.$cat_r['conv_closing_stock'].'</strong></span></td>
				<td><a data-original-title="Sub Category Report" data-toggle="tooltip" data-placement="top" class="noprint" href="'.ROOT.'sub_category_report/'.$row['cat_id'].'"><strong style="color:green">'.$cat_r['base_opn_rate'].'</strong><br><strong style="color:orange">'.$cat_r['conv_opn_rate'].'</strong></a> <span class="printshow" style="display: none;"><strong style="color:green">'.$cat_r['base_opn_rate'].'</strong><br><strong style="color:orange">'.$cat_r['conv_opn_rate'].'</strong></span></td>
		...
		$cat_pro1 = category_wise_pro_stock($dbcon,$parent_id,$frmdate,$todate,$stock="");

		$str .= '<table class="table table-bordered" style="width:100%;margin-top:30px">
			<tr>
				<td colspan="6" style="text-align:center"><strong>Category Name : '.get_category_by_id($dbcon,$parent_id).'</strong></td>
			</tr>
			<tr>
				<th>Sr. No.</th>
				<th>Product Name</th>
		...
		if(brp_mysqli_num_rows($cat_pro1)>0){
			$i=1;
			while($row = brp_mysqli_fetch_array($cat_pro1)){
				...
				$str.='<tr>
					...
					<td><strong style="color:green">'.$row['base_rate'].'</strong><br><strong style="color:orange">'.$row['conv_rate'].'</strong></td>
		...
		$cat_pro = category_wise_pro_stock($dbcon,$parent_id,$frmdate,$todate,$stock="");
		$str .= '<table class="table table-bordered" style="width:100%;margin-top:30px">
			<tr>
				<td colspan="6" style="text-align:center"><strong>Category Name : '.get_category_by_id($dbcon,$parent_id).'</strong></td>
			</tr>
