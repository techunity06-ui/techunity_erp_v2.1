			...
			$set_head=mysqli_fetch_assoc($dbcon->query($set));		
				$str .='
				 ...
				 <tbody>';
				  ...
				  $qry='Select company_name,receipt_date,paymentmode.payment_mode,bank.bank_name,acc.acc_name,acc.branch_name,res_trn.paid_amount as receive,cheque_dtl,inv.po_no from tbl_receipt as receipt 
					...
					where receipt.status=0 and res_trn.status=0 '.$wher.' '.$where.' and   receipt_date>="'.date('Y-m-d',strtotime($s_date[0])).'" and receipt_date<="'.date('Y-m-d',strtotime($s_date[1])).'" and res_trn.purchase_id !=0 and receipt.company_id='.$_SESSION['company_id'].'  order by receipt_date';
