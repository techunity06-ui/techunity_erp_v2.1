...
if(strtolower($POST['mode']) == "generate_report") {
        ...
        $str .='  <table  class="display table table-bordered  table-striped" id="data_list">
                        ...
                        <tr>
                            <td colspan="4"><strong>Monthly Sale Payment Report</strong></td>
                    ...
                    $qry='Select cust.l_name as company_name,receipt_date,paymentmode.l_name as bank_name,paymentmode.acc_name,receipt.cheque_dtl,res_trn.total_amount as receive,inv.invoice_no from tbl_receipt as receipt 
                            left join tbl_receipt_trn as res_trn on res_trn.receipt_id=receipt.receipt_id 
                            left join tbl_invoice as inv on inv.invoice_id=res_trn.invoice_id 
                            left join tbl_ledger as paymentmode on paymentmode.l_id=receipt.payment_mode_id 
                            left join tbl_ledger as cust on receipt.cust_id=cust.l_id  
                        where receipt.status=0 and res_trn.status=0 '.$wher.' and receipt_date>="'.date('Y-m-d',strtotime($s_date[0])).'" and receipt_date<="'.date('Y-m-d',strtotime($s_date[1])).'" and res_trn.invoice_id !=0 and receipt.company_id='.$_SESSION['company_id'].'
                    ...
                    if(mysqli_num_rows($result1)>0)
                    {
                        ...
                        while($re=mysqli_fetch_assoc($result1)){	
                            $str.='<tr>
                                ...
                                <td data-label="Sr. NO." style="text-align:center">'.$re["invoice_no"].'</td>
					...
					$str .="<tr>
                                                    ...
                                                    <td data-label='Total' style='text-align:center'>".indian_number($total,2)."</td>
                                                </tr>";
				}
				else
				{
