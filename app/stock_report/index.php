...
$getspecialConfiguration=getspecialConfiguration($dbcon);
				...
				$appData = array();
				$i=1;
				$aColumns = array('pro.product_icode, dr.drawing_number, pro.product_id, pro.product_base_unit, un.unit_name, c_un.unit_name AS conv_unit_name, pro.product_name, pro.product_status, pro.product_conv_unit, pro.batch_wise_stock_manage,
...
LEFT JOIN tbl_stock_trn AS qc ON qc.product_id = pro.product_id AND qc.company_id = " . $_SESSION['company_id']);
				...
				foreach($sqlReturn as $row) {
					...
					if($getspecialConfiguration['fusiontech_permission'] == '1'){
					...
					$row_data[] = '<a data-original-title="Product Stock Details" data-toggle="tooltip" data-placement="top" target="_blank" href="'.ROOT.REPORT_ROOT.'product_ledger_new/'.$row['product_id'].'">'.number_format($stock,3,'.','').' '.$row['unit_name'].'<br>'.number_format($conv_stock,3,'.',''). ' '.$row['conv_unit_name']  .'</a>'; 
					$row_data[] = '<a data-original-title="Reserve Stock Details" data-toggle="tooltip" data-placement="top" target="_blank" href="'.ROOT.REPORT_ROOT.'reserve_stock_report/'.$row['product_id'].'">'. number_format(abs($res_stock),3,'.','') .' '.$row['unit_name'].'<br>'.number_format(abs($res_conv_stock),3,'.',''). ' '.$row['conv_unit_name'] .'</a>'; 
					...
					$row_data[] = '<a data-original-title="Product Stock Details" data-toggle="tooltip" data-placement="top" target="_blank" href="'.ROOT.REPORT_ROOT.'product_ledger_new/'.$row['product_id'].'">'.number_format($free_base_stock,3,'.','').' '.$row['unit_name'] .'<br>'.number_format($free_conv_stock,3,'.',''). ' '.$row['conv_unit_name'] .'</a>'; 

					if($getspecialConfiguration['rb_auto_permission']!=1){
						$row_data[] = '<a data-original-title="Product Stock Details" data-toggle="tooltip" data-placement="top" target="_blank" href="'.ROOT.REPORT_ROOT.'product_ledger_new/'.$row['product_id'].'">'.number_format($base_rate,3,'.','').'<br>'.number_format($conv_rate,3,'.','').'</a>';
					}
