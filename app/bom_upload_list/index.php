...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$appData = array();
		$i=1;
		$aColumns = array('so_trn.sales_ordertrn_id','so.sales_order_no','so.sales_order_date','led.company_name','pro.product_name','so_trn.product_qty','so_trn.product_id','so_trn.unit_id','so_trn.with_out_stock_invoice');
		$sIndexColumn = "so_trn.sales_ordertrn_id";
		$isWhere = array("so_trn.sales_ordertrn_status = 0 and so_trn.invoice_status=0 and so.approve_status=3");
		$sTable = "tbl_sales_ordertrn as so_trn";
		$isJOIN = array("left join tbl_sales_order as so on so.sales_order_id=so_trn.sales_order_id","left join tbl_ledger as led on led.l_id=so.cust_id","left join product_mst as pro on pro.product_id=so_trn.product_id");
		...
		foreach($sqlReturn as $row) {
			...
			if($row['with_out_stock_invoice']=="0"){
				...
				if($reserve_stock>0){
					...
					$so_to_inv_btn='<a class="btn btn-xs btn-primary" data-original-title="Add Invoice" data-toggle="tooltip" data-placement="top" href="'.ROOT.'invoiceso/'.$row['sales_ordertrn_id'].'"><i class="fa fa-plus-circle"></i>add Invoice</a>';
				}
			}else{
