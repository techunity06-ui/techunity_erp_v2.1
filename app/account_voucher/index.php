...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		foreach($sqlReturn as $row) {
			$row_data = array();
			$row_data[] = '<a class="" data-original-title="Edit" data-toggle="tooltip" data-placement="top" href="'.ROOT.'account-voucher-update/'.$row['voucher_mstid'].'">'.$row["sr"].'</a>';
	...
	if($updatetrnid)
		...
		if(empty($POST['edit_id'])) {
			...
			$vtrn_id=$POST['edit_id'];
		}
		
		if(!empty($POST['voucher_mstid'])) {
			$table_name="account_voucher_trn";
			$query1="SELECT * FROM `account_voucher_trn` as trn 
			left join account_voucher_mst as ac on ac.voucher_mstid=trn.voucher_mstid 
			where trn.trn_status=0 and trn.voucher_mstid=".$POST['voucher_mstid'];
			$result1=$dbcon->query($query1);
			$rel1=mysqli_fetch_assoc($result1);
			
			$general_book_id=get_general_book_id($dbcon,$table_name,$vtrn_id,$POST['l_id']);
			//if($POST['type_id']==1){$amou=$rel['cr_amount'];}else{$amou=$rel['dr_amount'];}
			$infoled['ref_date']		= date("Y-m-d",strtotime($rel1['voucher_date']));
			$infoled['table_name']		= $table_name;
			$infoled['table_id']		= $vtrn_id;
			$infoled['entry_type']		= $POST['type_id'];
			$infoled['ledger_id']		= $POST['l_id'];
			$infoled['amount']			= $POST['input_amt'];
			$infoled['user_id']			= $_SESSION['user_id'];
			$infoled['cdate']			= date("Y-m-d H:i:s");
			$infoled['company_id']		= $_SESSION['company_id'];
			
			if(empty($general_book_id)){
				$inserid_gen=add_record("tbl_general_book", $infoled, $dbcon);
			}else{
				$updateid=update_record('tbl_general_book', $infoled,"general_book_id=".$general_book_id , $dbcon);
			}
		}
	...
	else if(strtolower($POST['mode'])== "delete_data")
	{
		...
		$updateid=update_record("tbl_general_book", $info1, "table_name='account_voucher_trn' and table_id=".$POST['eid'], $dbcon);
