...
if(strtolower($POST['mode']) == "generate_report") 
{
        ...
        $str .='<table  class="display table table-bordered table-striped" id="data_list">
            ...
            $ledger_qry = 'SELECT distinct(ledger.l_id),ledger.l_name FROM tbl_excess as ex  
                ...
                WHERE ledger.l_status = 0 and ledger.company_id = '.$_SESSION['company_id'].' and ledger.l_group IN ('.SUNDRY_DEBTORS.') '.$ledger_where;
            ...
            if($ledger_data){
                ...
                foreach ($ledger_data as $i => $ledger) {
                    $excess_amount = $dbcon->query('select sum(excess_amount) as excess_amount 
                        ...
                        where inv.status=0 and inv.company_id = '.$_SESSION['company_id'].' and excess_type=1 AND inv.cust_id= '.$ledger['l_id'].' 
                            and inv.excess_amount>(select IFNULL(sum(total_amount),0) as qty from tbl_receipt_trn as trn where status=0 and payment_type=2 and inv.excess_id=trn.excess_id)
                        ')->fetch_object()->excess_amount;
                    
                    $total_excess_amount += $excess_amount;
                    
                    if($excess_amount){
                        $adjustable_amount = $dbcon->query('select sum(g_total) as adjustable_amount 
                            FROM tbl_invoice as inv 
                            WHERE invoice_status=0 AND cust_id= '.$ledger['l_id'].' and inv.company_id = '.$_SESSION['company_id'].'
                                and inv.g_total>(select IFNULL(sum(total_amount),0) as qty from tbl_receipt_trn as trn where status=0 and inv.invoice_id=trn.invoice_id)
                            ')->fetch_object()->adjustable_amount;
                        ...
                        $str .='<tr>
                                ...
                                <td style="text-align:right;">
                                    <a style="color: inherit;" class="link_dash" href="'.ROOT.'unadjusted_advance/'.$ledger['l_id'].'" target="_blank">
...
else if(strtolower($POST['mode']) == "generate_chart") 
{
    ...
    $query = "select sum(excess_amount) as excess_amount ,inv.cust_id, ledger.l_name 
            ...
            left join tbl_receipt as rep on rep.receipt_id=inv.receipt_id 
            where inv.status=0 and excess_type=1 and inv.company_id = ".$_SESSION['company_id']." ".$where."
                and inv.excess_amount>(select IFNULL(sum(total_amount),0) as qty from tbl_receipt_trn as trn where status=0 and payment_type=2 and inv.excess_id=trn.excess_id)
