...
*/
			...
			if($inserpoid){
				...
				//foreach($_FILES["file"]["tmp_name"] as $files){
					...
					$ext = end($test);
					$name = $dr_id.'_'.$revision_insertid.'_'.rand(100, 999) . '.' . $ext;
			...
			$sql = "SELECT * FROM `tbl_drawing` WHERE drawing_number='".$drawing_number."' $where ";	
			...
			echo '<section class="panel">
                     <div class="panel-body bio-graph-info">
                         ...
                         <div class="row">
                             ...
                             <div class="bio-row">
                                 <p><span>Prepared Date </span>: '.date('d-M-Y', strtotime($rel["cdate"])).'</p>
                             </div>
                             <div class="bio-row">
                                 <p><span>Modified By </span>: '.$rel["last_modify_by"].'</p>
                             </div>
                             <div class="bio-row">
                                 <p><span>Modified Date</span>: '.date('d-M-Y', strtotime($rel["mdate"])).'</p>
                             </div>
			...
			echo '<div class="col-md-12" >
					<table cellspacing="10" style="border-spacing:10px;" class="display table table-bordered table-striped">
						<tr id="field">
							...
							<th class="text-center" width="8%">Opening Stock</th>
						</tr>
					';
			$i=1;$process_type="";
			while($rel=brp_mysqli_fetch_assoc($vrow)){
				if($rel["process_type"]=="1"){ $process_type="Inhouse";}else{$process_type="Job Work";}
				echo '<tr id="field'.$i.'">
							<th class="text-center" width="25%">'.$rel["process_name"].'</th>
							<th class="text-center" width="8%">'.$rel["process_priority"].'</th>
							<th class="text-center" width="8%">'.$process_type.'</th>
							<th class="text-center" width="8%">
								<input id="process_opening_stock'.$i.'" name="process_opening_stock[]" type="number" class="form-control" title="Opening Stock Qty" value="" placeholder="Opening Stock Qty" >
							</th>
						</tr>';
				$i++;
			}
