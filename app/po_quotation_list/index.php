...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		//$branch=$_SESSION['branch_id'];
			...
			$appData = array();
			$i=1;
			$aColumns = array('apo.approve_no','apo.approve_date','apo.approve_qty','po.indent_no','delivery_date','po.indent_date','po.rp_po_qty','unit.unit_name','spro.po_req_no','pmst.product_name','tc.cat_name','po.rp_id','apo.approve_indent_id', 'apo.quotation_approve_status', 'bms.branch_name');
			$sIndexColumn = "apo.approve_indent_id";
			$isWhere = array("apo.approve_indent_status=0 ".$where);
			$sTable = "approve_indent as apo";			
			$isJOIN = array('left join tbl_request_product as po on po.rp_id=apo.rp_id','left join tbl_set_main_process as spro on spro.sp_id=po.sp_id','left join branch_mst as bms on bms.branch_id=apo.branch_id','left join product_mst as pmst on pmst.product_id=po.rp_pid','left join tbl_category as tc on pmst.product_category=tc.cat_id','left join unit_mst as unit on unit.unitid=apo.approve_unit');
			...
			foreach($sqlReturn as $row) {
				...
				if(in_array(PO_QUOTATION_ADD,$bulkAccessArray)){
					if($row['quotation_approve_status']=='0'){
						$add_po_btn='<a class="btn btn-xs btn-primary" data-original-title="ADD Quotation" data-toggle="tooltip" data-placement="top" href="'.ROOT.'po_quotation_add/'.$row['approve_indent_id'].'"><i class="fa fa-plus"></i></a>';
					}
				}
				if(in_array(PO_QUOTATION_VIEW,$bulkAccessArray)){
					$view_quo = '<a class="btn btn-xs btn-warning" data-original-title="View" data-toggle="tooltip" data-placement="top" href="'.ROOT.'po_quotation_vendor_list/'.$row['approve_indent_id'].'"><i class="fa fa-eye"></i></a>';
				}
				if(in_array(PO_QUOTATION_APPROVE,$bulkAccessArray)){
					$approve_po_btn='<a class="btn btn-xs btn-success btn-flat" data-original-title="Approve" data-toggle="tooltip" data-placement="top" href="'.ROOT.'po_quotation_approve/'.$row['approve_indent_id'].'"><i class="fa fa-plus">Approve</i></a>';
				}	
