...
if(strtolower($POST['mode']) == "load_purchase_po_com") {
    ...
    if($POST['pur_amount_filter']==0){
    	$query="SELECT m.month,(SELECT sum(trn.product_qty) FROM tbl_purchaseordertrn as trn LEFT JOIN `tbl_purchaseorder` as po ON trn.purchaseorder_id = po.purchaseorder_id
    	where po.status=0 and po.po_aproove_finance=1 and MONTH(STR_TO_DATE(m.month,'%M')) = MONTH(po.purchaseorder_date) and po.purchaseorder_date between '".$date['start_date']."' and '".$date['end_date']."' AND trn.purchaseordertrn_status = 0) as pototal, 
    	(SELECT sum(trns.product_qty) FROM tbl_potrancation as trns LEFT JOIN `tbl_pono` as pur on pur.po_id = trns.po_id
    	...
    	where po.status=0 and po.po_aproove_finance=1 and MONTH(STR_TO_DATE(m.month,'%M')) = MONTH(po.purchaseorder_date) and po.purchaseorder_date between '".$date['start_date']."' and '".$date['end_date']."') as pototal, 
   ...
   /* while($chart= mysqli_fetch_assoc($tar_counter))
	...
	echo $table1;
			...
			//$date=get_sdate($POST['c_year']);
	$POST['end_date']="2021-12-31";
	$POST['start_date']="2021-01-01";
			//$user_ids=check_user_chein($dbcon,$POST['user_id'],1);
	if($POST['vendor_filter']==0){
		$query="select sum(trn.product_qty) as purchase_total,itrn.l_name as pg_name from tbl_potrancation as trn LEFT JOIN tbl_pono as pro on pro.po_id = trn.po_id
		...
		$query_total="select sum(trn.product_qty) as purchase_total from tbl_potrancation as trn LEFT JOIN tbl_pono as pro ON pro.po_id = trn.po_id	where pro.status=0 and trn.potrancation_status = 0 and DATE_FORMAT(pro.po_date,'%Y-%m-%d') BETWEEN CAST('".date('Y-m-d',strtotime($POST['start_date']))."'AS DATE) and CAST('".date('Y-m-d',strtotime($POST['end_date']))."'AS DATE) AND pro.company_id IN (0,".$_SESSION['company_id'].") ";
	}else{
	...
	if($POST['product_filter']==0){
		$query="select sum(itrn.product_qty) as purchase_total,p.product_name as pg_name from tbl_pono as pro
		...
		$query_total="select sum(itrn.product_qty) as purchase_total from tbl_pono as pro
		left join tbl_potrancation as itrn on itrn.po_id=pro.po_id
		left join product_mst as p on p.product_id=itrn.product_id
		where pro.status=0 and itrn.potrancation_status=0
	...
	if($POST['pur_cat_filter']==0){
		...
		where pro.status=0 and itrn.potrancation_status=0
		and DATE_FORMAT(pro.po_date,'%Y-%m-%d') BETWEEN CAST('".date('Y-m-d',strtotime($POST['start_date']))."'AS DATE) and CAST('".date('Y-m-d',strtotime($POST['end_date']))."'AS DATE) AND pro.company_id IN (0,".$_SESSION['company_id'].") group by p.product_category order by purchase_total desc limit 0,20";
	...
	echo json_encode($result_remainder);
			...
			left join tbl_purchaseordertrn as trn on trn.purchaseordertrn_id = del.purchaseordertrn_id
			left join tbl_purchaseorder as po on po.purchaseorder_id = trn.purchaseorder_id
			where po_delivery_date_status=0 and trn.purchaseordertrn_status=0 and trn.purchaseorder_id!=0 and po.po_approval_status=1 and trn.used_status=0 ".$where."' del.grn_status=0 and del.company_id=".$_SESSION['company_id'];

			$over_due_7days_inword_res=mysqli_fetch_assoc($dbcon->query($over_due_7days_inword));
			
			if(empty($over_due_7days_inword_res['overdue_in'])){$over_due_7days_inword_res['overdue_in']=0;}
			if(empty($today_over_due_inword_res['overdue_in'])){$today_over_due_inword_res['overdue_in']=0;}
			if(empty($over_due_inworde_res['overdue_in'])){$over_due_inworde_res['overdue_in']=0;}
