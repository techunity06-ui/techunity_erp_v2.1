...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$tr = $dbcon -> query("SELECT `expense_id`,`expense_name`,`expense_status` FROM `expense_mst` WHERE expense_status=0 and `expense_name` ='".$POST['expense_name']."' and `expense_head_id` ='".$POST['expense_head_id']."' ");
		if($tr->num_rows > 0) {
			$resp['msg'] = '-1';
		}
		else {
			...
			$info['exp_opn_balance']	= $POST['exp_opn_balance'];							
			$info['cdate']		= date("Y-m-d H:i:s");
			$info['user_id']	= $_SESSION['user_id'];
			$info['company_id']	= $_SESSION['company_id'];
			$inserid=add_record('expense_mst', $info, $dbcon);
			
			$info_l['l_name']=$POST['ledger_name'];
			$info_l['l_group']=$POST['expense_head_id'];
			$info_l['l_form']='expense_form';
			$info_l['l_form_id']=$inserid;
			$info_l['l_form_table']='expense_mst';
			...
			if($inserid){
				...
				if(strtolower($POST['expense_model']) == "expense_model"){
					$zone_qry="select * from expense_mst where expense_id=".$inserid; 
		...
		$q = $dbcon -> query("SELECT * FROM `expense_mst` WHERE `expense_id` = '$POST[expense_id]'");
		$r = $q->fetch_assoc();
		echo json_encode($r);
	}
		...
		$info['user_id']	= $_SESSION['user_id'];
		$info['company_id']	= $_SESSION['company_id'];
		$updateid=update_record('expense_mst', $info,"expense_id=".$POST['eid'] , $dbcon);
		...
		$updateid=update_record('expense_mst', $info,"expense_id=".$POST['expense_id'] , $dbcon);
