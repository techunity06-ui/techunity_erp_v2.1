...
if($_POST != NULL) {
	...
	if(strtolower($POST['mode']) == "fetch") {
		...
		$appData = array();
		$i=1;
		$aColumns = array('quot.quotation_id', 'cust.cust_name', 'pro.product_name', 'trn.product_qty',  'quot.qt_delivery_date','quot.quotation_no', 'quot.quotation_date');
		$sIndexColumn = "quot.quotation_id";
		$isWhere = array("quot.quotation_status = 0 and payment_approve_status=1 and trn.quot_trn_status=0 and trn.inv_done_status=0 ".$where);
		$sTable = "tbl_quotation as quot";
		$isJOIN = array('left join tbl_customer as cust on cust.cust_id=quot.cust_id', 'left join tbl_quotation_trn as trn on trn.quotation_id=quot.quotation_id', 'left join product_mst as pro on pro.product_id=trn.product_id');
		...
		foreach($sqlReturn as $row) {
			...
			$row_data[] = date('d M, Y',strtotime($row['qt_delivery_date']));
			//$row_data[] = date('d M, Y',strtotime($row['qt_delivery_date']));
