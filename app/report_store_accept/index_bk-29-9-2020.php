  ...
  //  if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "fetch") {
			...
			$appData = array();
			$i=1;
			$aColumns = array('l.l_id', 'l.l_name', 'l.l_group','cit.city_name','l.user_id','g.g_name','l.l_form','l.l_status');
			$sIndexColumn = "l_id";
		...
		else if(strtolower($POST['mode']) == "generate_report_ledger") 
		{
			...
			if(!empty($POST['g_id'])){ $con=" and g_id=".$POST['g_id'];}else{ $con="";}
		...
		else if(strtolower($POST['mode']) == "generate_report_ledger_detail") 
		{
			$l_id=$POST['l_id'];
			$s_date=explode(' - ',$POST['date']);
			if(!empty($POST['showledger_id'])){ $con=" and l_id=".$POST['showledger_id'];}else{$con="";}
			$query="select l.* from tbl_ledger as l where l_status=0 ".$con." and l.l_group='$l_id'";
			$qry=$dbcon->query($query);
			
			$cnt=1;$str='';$total_cr="";$total_dr="";
			while($row=mysqli_fetch_assoc($qry))
			{
