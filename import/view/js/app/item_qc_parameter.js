$(document).ready(function() {
	$("#product_qc_importfile").validate({
		rules: {
			excel_file:{
				required:true
			}
		...
		messages: {
			excel_file: {
				required: "Select Product QC Parameter Csv file",
			}
		}
	});
});

$("#product_qc_importfile").on('submit',function(e) {
	var form = this;
	e.preventDefault();
	e.stopPropagation();	
	if (!$("#product_qc_importfile").valid()) {
		return false;
	}
	form.submitted = true;	
	Loading();	
	...
	$.ajax({
		cache:false,
		url: root_domain+import_domain+'app/item_qc_parameter/',
		type: "POST",
		data: form_data,
		contentType: false,
		processData:false,
		...
		{	
			...
			else if(response == '-1')
			{
				...
				$('#product_qc_importfile').trigger('reset');
				Unloading();				
			}
			else if(response == '0')
			{
