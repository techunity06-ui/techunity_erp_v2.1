...
{ 
	/*if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) */
	{
		...
		if(strtolower($POST['mode']) == "generate_report_min_new") {
			$appData = array();
			$i=1;
			$aColumns = array('so.order_id','pro.product_icode', 'dr.drawing_number','so.doc_no','so.cdate','pro.product_name','so.product_id','bov.bom_no','bom.bom_id','so.base_request_qty','pro.bom_required','bov.version_name','bom.bom_version_id');
			$sIndexColumn = "so.order_id";
			$isWhere = array("1 and so.bom_status=0 AND so.status = 0 and so.wo_complete_status=0  AND so.company_id =". $_SESSION['company_id']);
			$sTable = " tbl_store_order_min_max as so";
			$isJOIN = array("left join product_mst as pro on pro.product_id=so.product_id 
				left join tbl_drawing as dr on dr.drawing_id = pro.drawing_id
				left join pro_ms_bom_version as bov on bov.product_id = so.product_id and is_default_bom=1",
				"left join tbl_bom as bom on bom.bom_version_id=bov.bom_version_id and bom.bom_status = 0");
			$hOrder = "so.order_id desc";
			...
			foreach($sqlReturn as $row) {
				$row_data = array();			
				$row_data[] = '<input type="checkbox" chk name="chk[]" data-soid="'.$row['order_id'].'" data-bomid="'.$row['bom_id'].'" data-bom_version_id="'.$row['bom_version_id'].'" value="'.$row['product_id'].'"/>';
				$row_data[] = $row['doc_no'];
				...
				$add_button='<a href="'.ROOT.PRODUCTION_ROOT.'bom_assign_store_order/'.$row['product_id'].'/'.$row['order_id'].'/'.$row['bom_id'].'"><button type="button" class="btn btn-xs btn-success" data-original-title="Assign Bom" data-toggle="tooltip" data-placement="top" ><i class="fa fa-plus"></i></button></a>';
				$view='';
			...
			$store_order_id_array =  $_POST['store_order_id'];
			$product_array = $_POST['product_id'];		
			$bom_array = $_POST['bom_id'];		
			$bom_version_array = $_POST['bom_version_id'];		
			...
			foreach($product_array as $prd)
			{
				...
				if(!empty($bom_array[$i]))
				{
					...
					$info_update['bom_id']		= $bom_array[$i];
					$info_update['bom_version_id']		= $bom_version_array[$i];
					
					$updateid=update_record('tbl_store_order_min_max', $info_update,"order_id=".$store_order_id_array[$i] , $dbcon);
			...
			$info_b['bom_version_id']			=$bom_version_id;
			$updatebomassign=update_record('tbl_store_order_min_max', $info_b, "order_id=".$order_id , $dbcon);
			if($updatebomassign){
				$arr['msg']="1";
			}
			else{
