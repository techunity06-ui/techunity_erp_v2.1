...
if(brp_strtolower($POST['mode'])== "generate_report"){
	...
	if($company_config['jobwork_grn'] == '0'){
				$field=",GROUP_CONCAT(job.job_work_id) as jobwork_id,GROUP_CONCAT(job.chalan_no) as chalan_no,GROUP_CONCAT(res.job_card_no) as job_card_no,GROUP_CONCAT(job_trn.job_work_trn_id) as job_trn_id,sum(job_strn.product_base_qty) as total_qty";

				$lfjoin = 'left join tbl_job_work_trn as job_trn on job_trn.job_work_id=job.job_work_id
				...
				$where.= 'job.grn_complete_status="0" and job_trn.grn_complete_status="0" '.$ser_pro.' '.$ser_ven.' '. $check_branch .' and job_strn.job_work_sub_trn_status = 0 and job.job_work_type=2 and job.job_work_status="0" and job_trn.job_work_trn_status=0 and job.company_id='.$_SESSION['company_id'];
				
				$group=array("job_trn.product_base_unit","job_trn.process_id","job_trn.product_id","job.vender_id");
			}else{
				$field=",job.job_work_id as jobwork_id,job.chalan_no as chalan_no,res.job_card_no as job_card_no,GROUP_CONCAT(job_trn.job_work_trn_id) as job_trn_id,sum(job_strn.product_base_qty) as total_qty";
				...
				left join tbl_ledger as led on led.l_id=job.vender_id
				left join process_mst as pmst on pmst.process_id=job_trn.process_id';
				$where .= 'job.grn_complete_status="0" and job_strn.job_work_sub_trn_status = 0 and job_trn.grn_complete_status="0" '.$ser_pro.' '.$ser_ven.' '. $check_branch .' and job.job_work_type=2 and job.job_work_status="0" and job_trn.job_work_trn_status=0 and job.company_id='.$_SESSION['company_id'];
				$group=array("job.job_work_id","job_trn.product_id");
			}

	
	$appData = array();
	$i=1;
	$aColumns = array('produ.product_name','produ.product_icode','pmst.process_name','job_trn.description','tc.cat_name','led.l_name'.$field);
	...
	foreach($sqlReturn as $rel) {
		...
		//if(in_array(PRODUCTION_PENDING_JOBCARD_SLUG_CREATE,$bulkAccessArray)){
			$view = '<a class="btn btn-xs btn-success" data-original-title="Add GRN" data-toggle="tooltip" data-placement="top" href="'.ROOT.INVENTORY_ROOT.'grn_add_job/'.$job_trn_id.'" ><i class="fa fa-plus"></i></a>';
		//}	
		...
		$row_data[] = $rel['product_name']." -- (".$rel['product_icode'].")".'<p>'.$rel['description'].'</p>';
		$row_data[] = $rel['process_name'];
		$row_data[] = $cat_name;
		$row_data[] = $rel['l_name'];
		$row_data[] = $rel['total_qty'];
	...
	/*if(brp_strtolower($POST['mode']) == "generate_report") {
			...
			if($company_config['jobwork_grn'] == '0'){
				...
				where job.grn_complete_status="0" and job_trn.grn_complete_status="0" '.$ser_pro.' '.$ser_ven.' '. $check_branch .' and job_strn.job_work_sub_trn_status = 0 and job.job_work_type=2 and job.job_work_status="0" and job_trn.job_work_trn_status=0 and job.company_id='.$_SESSION['company_id'].' group by job_trn.product_base_unit,job_trn.process_id,job_trn.product_id,job.vender_id';
			}else{
