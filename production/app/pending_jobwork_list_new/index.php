...
if(brp_strtolower($POST['mode']) == "fetch") {
	...
	if($companyConfiguration['po_work_order_wise'] == 1){
		$group=",ap.p_ref_id";
		$left="left join tbl_request_product as res on res.rp_id=ap.p_ref_id  left join tbl_set_main_process as setm on setm.sp_id=res.sp_id";
		$pera=",setm.po_req_no,setm.po_req_date,setm.sales_order_no,res.job_card_no";
	}else if($companyConfiguration['po_work_order_wise'] == 0 && $companyConfiguration['production_start_type'] == 0){
		$group=",ap.p_ref_id";
		$left="left join tbl_request_product as res on res.rp_id=ap.p_ref_id  left join tbl_set_main_process as setm on setm.sp_id=res.sp_id";
		$pera=",setm.po_req_no,setm.po_req_date,setm.sales_order_no,res.job_card_no";
	}
			...
			$appData = array();
			$i=1;
			$aColumns = array('p.product_type', 'p.product_name', 'tc.cat_name' , 'pro.process_name','branc.branch_name', 'sum(ap.p_qty) as ap_qty', 'sum(ap.pen_qty) as apen_qty', 'IFNULL(end_qty,0) as end_qty', 'IFNULL(strtt_qty,0) as strtt_qty', 'GROUP_CONCAT(ap.p_id ORDER BY `ap`.`p_id` ASC) as allocate_id','GROUP_CONCAT(res.job_card_no) as job_card_no','ap.*','p.product_icode'.$pera);
			...
			foreach($sqlReturn as $rel) {
				...
				$min_working_qty=production_start_count_using_p_id($dbcon,$allocate_id);
					...
					where trn.short_close = 0 AND job_work_trn.short_close = 0 AND job_work_sub_trn_status = 0 and p_id IN (".$allocate_id.")  and job_work_trn.job_work_trn_status in (0,1)";
             ...
             if($total_start_qty > 0){
                 $start_qtttyy = $total_start_qty;
                 if($job_trn_qty > $total_start_qty){
                     $start_qtttyy = $job_trn_qty;
                 }
             }else{
                 $start_qtttyy = $job_trn_qty;
             }
				...
				if($min_working_qty - $jobwork_working_qty > 0){
					if($min_working_qty > 0)
					{
						...
						$row_data[] = $rel['product_name'] . " -- (".$rel['product_icode'].")";
												$row_data[] =$cat_name;
						...
						$row_data[] = $rel['ap_qty'];
						$row_data[] = $min_working_qty - $jobwork_working_qty;
						// $row_data[] = $total_start_qty + $jobwork_working_qty;
						$row_data[] = $start_qtttyy;
						...
						if($min_working_qty > 0)
						{
							$add_po_btn='<a class="btn btn-xs btn-primary" data-original-title="Create Jobwork" data-toggle="tooltip" data-placement="top" href="'.ROOT.PRODUCTION_ROOT.'create_job_work_new/'.$rel['process_id'].'/'.$rel['p_product_id'].'/'.$jjparty.'/'.$rel['branch_id'].'/'.$rel['p_id'].'"><i class="fa fa-plus"></i></a>';
						}else{
							$add_po_btn='';
						}	
						$row_data[] = $add_po_btn;
		...
		else if(brp_strtolower($POST['mode'])== "get_process_list")
		{
			...
			$branch_id = $POST['branch_id'];
			 ...
			 $qry = "SELECT pro.process_name,ap.process_id as process_id FROM tbl_allocate_process as ap left join product_mst as p on p.product_id=ap.p_product_id left join (select sum(pt_qty) as end_qty,pt_alloc_id from tbl_allocate_process_trn where p_status=1 group by pt_alloc_id) as apta on apta.pt_alloc_id=ap.p_id left join (select sum(pt_qty) as strtt_qty,pt_alloc_id from tbl_allocate_process_trn where p_status=0) as apta1 on apta1.pt_alloc_id=ap.p_id left join process_mst as pro on ap.process_id=pro.process_id left join branch_mst as branc on branc.branch_id=ap.branch_id where ( 1 AND ap.pr_process_type='2' and ap.p_status in(0,1) and ap.company_id= ".$_SESSION['company_id']." and ap.p_status IN (0,1) and ap.p_product_id = " .$product_id ." and ap.branch_id = ".$branch_id.") Group by ap.p_product_id, ap.process_id,ap.branch_id ORDER BY ap.p_id asc";
		...
		else if(brp_strtolower($POST['mode'])== "load_invoiceno_jobwork")
		{
			...
			$row['invoiceno'] = load_common_no($dbcon,OUTSIDE_JOB_WORK);
		...
		else if(brp_strtolower($POST['mode'])== "get_jobwork_detail_data")
		{
			...
			$allocate_id = $rel['allocate_id'];
				...
				$q = "SELECT IFNULL(sum(product_base_qty),0) as used_qty FROM `tbl_job_work_sub_trn` where job_work_sub_trn_status = 0 and release_status = 0 and p_id IN (".$allocate_id.") " . $where;
				$job_trn=$dbcon->query($q);
				$job_trn_result = brp_mysqli_fetch_assoc($job_trn);
				...
				$pd_qty = $min_working_qty - $job_trn_qty;
			...
			$qry = "SELECT p.product_name,ap.p_product_id as product_id, (IFNULL(ap.pen_qty,0)) as rem_qty  FROM tbl_allocate_process as ap left join product_mst as p on p.product_id=ap.p_product_id left join (select sum(pt_qty) as end_qty,pt_alloc_id from tbl_allocate_process_trn where p_status=1 group by pt_alloc_id) as apta on apta.pt_alloc_id=ap.p_id left join (select sum(pt_qty) as strtt_qty,pt_alloc_id from tbl_allocate_process_trn where p_status=0) as apta1 on apta1.pt_alloc_id=ap.p_id left join process_mst as pro on ap.process_id=pro.process_id left join branch_mst as branc on branc.branch_id=ap.branch_id where ( 1 AND ap.pr_process_type='2' and ap.p_status in(0,1) and ap.company_id= ".$_SESSION['company_id']." and ap.p_status IN (0,1) and ap.branch_id = ".$POST['branch_id'].") Group by ap.p_product_id, ap.process_id,ap.branch_id  having rem_qty > 0  ORDER BY ap.p_id asc";
	...
	$query="select job_trn.*,product.product_name,pro.process_name,u.unit_name,job_sub_trn.p_id,munit.unit_name as munit_name from tbl_job_work_trn as job_trn
		...
		where job_trn.branch_id = ".$POST['branch_id']." and job_trn.user_id = ".$_SESSION['user_id']." and job_trn.job_work_trn_status = 3 group by job_trn.job_work_trn_id";
		$result=$dbcon->query($query);
	...
	else if(brp_strtolower($POST['mode'])== "preedit")
	{
		...
		$branch_id = isset($POST['branch_id']) ? $POST['branch_id'] : $_SESSION['branch_id'];
			...
			if($job_work_id){
				update_series_no_using_type_id($dbcon,OUTSIDE_JOB_WORK,$_SESSION['company_id'],$branch_id1);
					...
					if($companyConfiguration['store_approval'] == '0'){
		...
		{
			$q = "SELECT GROUP_CONCAT(job_work_trn_id) as jobwork_trn_ids from tbl_job_work_trn where job_work_trn_status = 3";
	       	$job_trn=$dbcon->query($q);
          	$job_trn_result = brp_mysqli_fetch_assoc($job_trn);
          	$job_trn_ids = $job_trn_result['jobwork_trn_ids'];
			if (!empty($job_trn_ids)) {
				$info['job_work_trn_status'] = 2;   
				...
				echo "0"; // No IDs found, nothing to update
			}
		}
		else if(brp_strtolower($POST['mode'])== "auto_add_temp_data")
		{
			...
			$conditions = "pr_process_type='2' and ap.p_status IN (0,1) and ap.company_id=".$_SESSION['company_id'];
			...
			if (!empty($branch_where)) {
				$conditions .= " $branch_where";
			}

			$cnt=1;
			$query1="SELECT SQL_CALC_FOUND_ROWS ap.*, sum(ap.p_qty) as ap_qty, sum(ap.pen_qty) as apen_qty, p.product_type, p.product_name,p.product_base_unit,p.product_conv_unit, IFNULL(end_qty,0) as end_qty, IFNULL(strtt_qty,0) as strtt_qty, pro.process_name, tc.cat_name, GROUP_CONCAT(`ap`.`p_id` ORDER BY `ap`.`p_id` ASC) allocate_id FROM tbl_allocate_process as ap
			...
			if($query->num_rows > 0){
				...
				//$min_working_qty = working_qty_avalable($dbcon,$rel['process_id'], $rel['pr_process_type'], $rel['p_product_id'], $rel['p_status'], $rel['previous_process_id'],$branch_id);
					...
					$q = "SELECT IFNULL(sum(trn.product_base_qty),0) as used_qty FROM `tbl_job_work_sub_trn` as trn  
					left join tbl_job_work_trn as job_work_trn on job_work_trn.job_work_trn_id =  trn.job_work_trn_id
					where job_work_sub_trn_status = 0 and p_id IN (".$allocate_id.")  and job_work_trn.job_work_trn_status = 1";
				$job_trn=$dbcon->query($q);
				$job_trn_result = brp_mysqli_fetch_assoc($job_trn);
				$jobwork_working_qty = $job_trn_result['used_qty'];
				...
				$query="select p_id,p_qty,start_qty,p_ref_id from tbl_allocate_process where p_id in (".$allocate_id.")";
				$result=$dbcon->query($query);
				$cnt=brp_mysqli_num_rows($result);
				if($cnt){
					$allocate_process_qty=0;
					...
					/*	var_dump('start_qty : '.$start_qty);	
						...
						if($working_qty!="0" && $allocate_process_qty!="0"){
							...
							/*	$tmp_info['job_work_trn_id']   = $job_work_trn_id;
								$tmp_info['product_id']   = $product_id;
								$tmp_info['process_id']   = $process_id;
								$tmp_info['p_id']   = $info_jobwork_sub_trn['p_id'];
								$tmp_info['qty']   		= $info_jobwork_sub_trn['product_base_qty'];
								$tmp_info['unit_id']   	= $info_jobwork_sub_trn['product_base_unit'];
								$tmp_info['cdate']		= date("Y-m-d H:i:s");
								$tmp_info['user_id']	= $_SESSION['user_id'];
								$tmp_info['company_id']	= $_SESSION['company_id'];		

								$insertid=add_record('tbl_jobwork_qty_tmp', $tmp_info, $dbcon);*/
	...
	$date = date('Y-m-d');
	 ...
	 left join tbl_jobwork_rate_card as mst on mst.jobwork_card_id = trn.jobwork_card_id
	...
	$query="select rp.job_card_no, sp.po_req_no,ap.p_id,rp.rp_id,(ap.p_qty - (select IFNULL(sum(product_base_qty),0) from tbl_job_work_sub_trn where job_work_sub_trn_status = 0  and temp_delete = 0 and p_id = ap.p_id)) as pending_qty from tbl_allocate_process as ap LEFT JOIN tbl_request_product as rp on rp.rp_id = ap.p_ref_id LEFT JOIN tbl_set_main_process as sp on rp.sp_id = sp.sp_id where ap.p_status in (0,1) AND ap.p_id in(".$POST['p_id'].") having pending_qty > 0";
	...
	$min_working_qty=production_start_count_using_p_id($dbcon,$p_id);

	$q = "SELECT IFNULL(sum(trn.product_base_qty),0) as used_qty FROM `tbl_job_work_sub_trn` as trn  
				left join tbl_job_work_trn as job_work_trn on job_work_trn.job_work_trn_id =  trn.job_work_trn_id
				where job_work_sub_trn_status = 0 and p_id IN (".$p_id.")  and job_work_trn.job_work_trn_status IN (0,1) AND trn.release_status = 0 and temp_delete = 0";
	$job_trn=$dbcon->query($q);
	$job_trn_result = brp_mysqli_fetch_assoc($job_trn);
	$jobwork_working_qty = $job_trn_result['used_qty'];
