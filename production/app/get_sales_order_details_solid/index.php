...
{ 
	/*if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) */
	{
		...
		$aColumns = array('mst.product_icode', 'dr.drawing_number','so.sales_order_no','so.sales_order_date','led.l_name','so_trn.product_qty','so_trn.sales_ordertrn_id','mst.product_name','uns.unit_name','tc.cat_name','so.delivery_date','bran.branch_name','so_trn.product_id','so_trn.work_order_qty','so_trn.unit_id','(IFNULL(product_qty,0)-IFNULL(stock_add,0)) as pending_qty','so.jobwork_type','mst.bom_required','so_trn.production_branch_id','so.cust_id','so_trn.description','so.cust_id','mst.printing_material','pmst.product_base_unit as punit','mst.printing_req');


		$sIndexColumn = "so_trn.sales_ordertrn_id";
		$isWhere = array("so_trn.sales_ordertrn_status=0 and so_trn.production_status=0 and  so.order_accept_status = 1 and so_trn.short_close_status=0 and so_trn.invoice_status=0 and so.approve_status=3".$where_db.$pro_branch);

		$sTable = "tbl_sales_ordertrn as so_trn";

		$isJOIN = array("left join tbl_sales_order as so on so.sales_order_id=so_trn.sales_order_id","left join tbl_ledger as led on led.l_id=so.cust_id","left join product_mst as mst on mst.product_id=so_trn.product_id","left join unit_mst as uns on uns.unitid=so_trn.unit_id","left join tbl_category as tc on mst.product_category=tc.cat_id","left join (select IFNULL(sum(qc.product_qty),0) as stock_add,qc.sales_ordertrn_id from tbl_sales_order_production_trn as qc 
			where qc.sales_order_production_status=0 group by qc.sales_ordertrn_id) as qc on qc.sales_ordertrn_id=so_trn.sales_ordertrn_id","left join branch_mst as bran on bran.branch_id=so_trn.branch_id","left join tbl_drawing as dr on dr.drawing_id = mst.drawing_id","left join product_mst as pmst on pmst.product_id=mst.printing_material");
		...
		foreach($sqlReturn as $row) {
			...
			$view='';$view1 = "";$apprv_btn='';$stock_allocate='';$view_desc='';
			...
			$pcstock=get_current_stock_new($dbcon,$row["printing_material"],$row["punit"],"",$customer_id);
			$prstock=reserve_stock($dbcon,$row["printing_material"],$row["punit"],"","","","","","","","","",$customer_id);
			$psto=$pcstock-$prstock;
			if($psto>0 && $row['printing_req']==1){
				$view_allo='<button class="btn btn-xs btn-success" data-original-title="Sales Order Detail" data-toggle="tooltip" data-placement="top" type="button" onclick="open_so_trn_modal_allocate('.$row['sales_ordertrn_id'].','.$row["product_id"].','.$row["printing_material"].')">Printing Stock Allocate('.$psto.')</button>';
			}
			//}
			//$row_data[] = $view1.' '.$view.' '.$apprv_btn.' '.$stock_allocate.' '.$view_desc;
			$row_data[] = $view_desc." ".$view_allo;
	...
	}else if(strtolower($POST['mode']) == "generate_report_min_new_mul"){
		
		$appData = array();
		$i=1;
		//+IFNULL(qc_total_rejected,0)

		$aColumns = array('sum(so_trn.product_qty) as soqty','mst.product_name','uns.unit_name','tc.cat_name','so_trn.product_id','so_trn.unit_id','(IFNULL(sum(product_qty),0)-IFNULL(stock_add,0)) as pending_qty','IFNULL(stock_add,0) as stock_add','mst.product_min_stock','mst.printing_material','mst.printing_req','pmst.product_base_unit as punit');
		...
		$isJOIN = array("left join tbl_sales_order as so on so.sales_order_id=so_trn.sales_order_id",
						...
						"left join (select IFNULL(sum(qc.product_qty),0) as stock_add,qc.product_id from tbl_sales_order_production_trn as qc where qc.sales_order_production_status=0 group by qc.product_id) as qc on qc.product_id=so_trn.product_id",
						"left join product_mst as pmst on pmst.product_id=mst.printing_material"
				);
		
		$hOrder = "so_trn.product_id desc";
		$hGroupby = array("so_trn.product_id");
		$having=" pending_qty > 0";
		include($include.'pagging.php');
		$appData = array();
		$id=1;
		...
		foreach($sqlReturn as $row) {
			...
			$csto=$cstock-$rstock;
			$row_data = array();
			$pqty=($row['soqty']+$row['product_min_stock'])-($csto+$row['stock_add']);
			$row_data[] = $row['product_name'];
			$row_data[] = ($row['cat_name']!=null) ? $row['cat_name'] : 'PRIMARY';
			$row_data[] = $row['soqty'] . ' '.$row['unit_name'];
			$row_data[] = $row['stock_add'] . ' '.$row['unit_name'];
			$row_data[] = $row['product_min_stock'] . ' '.$row['unit_name'];
			$row_data[] = $csto . ' '.$row['unit_name'];
			$row_data[] = $pqty . ' '.$row['unit_name'];
			$view_desc='<button class="btn btn-xs btn-primary" data-original-title="Sales Order Detail" data-toggle="tooltip" data-placement="top" type="button" onclick="open_so_trn_modal_mul('.$row['product_id'].','.$pqty.')"><i class="fa fa-eye"></i></button>';
			...
			$view_allo="";
			if($psto>0 && $row['printing_req']==1){
				$view_allo='<button class="btn btn-xs btn-success" data-original-title="Sales Order Detail" data-toggle="tooltip" data-placement="top" type="button" onclick="open_so_trn_modal_allocate_mul('.$pqty.','.$row["product_id"].','.$row["printing_material"].')">Printing Stock Allocate('.$psto.')</button>';
			}
	...
	else if(strtolower($POST['mode'])== "delete_data_stock")
	{
		...
		where mst.attach_status=0 and mst.design_dept=1 and mst.sales_order_id=".$res['sales_order_id'];
			...
			echo $str;
			 ...
			 $query_img 	= "select bsiz.sales_order_no,bsiz.sales_order_date,pro.product_name,product_qty,mst.product_id,mst.sales_ordertrn_id from tbl_sales_ordertrn as mst 
					left join tbl_sales_order as bsiz on bsiz.sales_order_id=mst.sales_order_id
					left join product_mst as pro on pro.product_id=mst.product_id
					where mst.sales_ordertrn_id=".$POST['sales_ordertrn_id'];
			$result_img = $dbcon->query($query_img);
			$row = brp_mysqli_fetch_array($result_img);
			$arr['sales_order_no']=$row['sales_order_no'];
			$arr['sales_order_date']=date('d-m-Y',strtotime($row['sales_order_date']));
			$arr['product_name']=$row['product_name'];
			$arr['product_qty']=$row['product_qty'];
			$arr['product_id']=$row['product_id'];
			$arr['sales_ordertrn_id']=$row['sales_ordertrn_id'];
			echo json_encode($arr);
		}
		else if(strtolower($POST['mode']) == "preview_solid_planning1_mul") {
			$query_img 	= "select pro.product_name,pro.product_id from product_mst as pro 
			...
			$arr['product_id']=$row['product_id'];
			echo json_encode($arr);
		}else if(strtolower($POST['mode']) == "preview_solid_planning") {

			 $query_img 	= "select pmp.product_name as printing_material_name,balty_name,mst.printing_req,emp.product_name as extrusin_material_name,siz.size_name,bsiz.batch_size_name,bsiz1.batch_size from product_mst as mst 
				left join product_mst as pmp on pmp.product_id=mst.printing_material
				left join solid_balty_mst as bal on bal.balty_id=mst.printing_balty
				left join product_mst as emp on emp.product_id=mst.extrusion_material
				left join solid_size_mst as siz on siz.size_id=mst.extrusion_size
				left join solid_batch_size_mst as bsiz on bsiz.batch_size_id=mst.mixing_batch_size
				left join solid_batch_calculation_mst as bsiz1 on bsiz1.batch_size_id=bsiz.batch_size_id and bsiz1.size_id=siz.size_id and bsiz1.status=0 
		where mst.product_id=".$POST['product_id'];
			$result_img = $dbcon->query($query_img);
			$row = brp_mysqli_fetch_array($result_img);
			$batch_qty=$POST['qty']/$row['batch_size'];
			$ac_batch_qty=ceil($batch_qty);
			$qty=$ac_batch_qty*$row['batch_size'];
			$html="<table class='table' border=1>
				<tr>
					<td style='width:30%'>Printing1</td>
					<td style='width:30%'>".$row['printing_material_name']."</td>
					<td style='width:30%'>".$row['balty_name']."</td>
					<td style='width:30%'>".$qty."</td>
				</tr>
				<tr>
					<td>Extrusion</td>
					<td>".$row['extrusin_material_name']."</td>
					<td>".$row['size_name']."</td>
					<td>".$qty."</td>
				</tr>
				<tr>
					<td>Mixing</td>
					<td></td>
					<td>".$row['batch_size_name']."</td>
					<td>".$ac_batch_qty."(".$qty."/".$row['batch_size'].")</td>
				</tr>
			</table>";
			$arr['html']=$html;
			$arr['qty']=$qty;
	...
	else if(strtolower($POST['mode'])== "save_reserve_stock")
	{
		...
		//end wip stock
	}else if(strtolower($POST['mode'])== "save_solid_planning"){
		$query_img 	= "select pmp.product_base_unit,mst.mixing_batch_size,mst.extrusion_size,mst.printing_material,mst.printing_balty,mst.extrusion_material,pmp.product_name as printing_material_name,balty_name,mst.printing_req,emp.product_name as extrusin_material_name,siz.size_name,bsiz.batch_size_name,bsiz1.batch_size from product_mst as mst left join product_mst as pmp on pmp.product_id=mst.printing_material left join solid_balty_mst as bal on bal.balty_id=mst.printing_balty left join product_mst as emp on emp.product_id=mst.extrusion_material left join solid_size_mst as siz on siz.size_id=mst.extrusion_size left join solid_batch_size_mst as bsiz on bsiz.batch_size_id=mst.mixing_batch_size left join solid_batch_calculation_mst as bsiz1 on bsiz1.batch_size_id=bsiz.batch_size_id and bsiz1.size_id=siz.size_id and bsiz1.status=0 where mst.product_id=".$_POST['product_id'];
			$result_img = $dbcon->query($query_img);
			$row = brp_mysqli_fetch_array($result_img);

			$batch_qty=$POST['pqty']/$row['batch_size'];
			$info_w['sales_order_trn_id'] 		= $POST['sales_order_trn_id'];
			$info_w['product_id'] 				= $_POST['product_id'];
			$info_w['req_batch'] 				= $batch_qty;
			$info_w['printing_material'] 		= $row['printing_material'];
			$info_w['printing_balty	'] 			= $row['printing_balty'];
			$info_w['printing_req'] 			= $row['printing_req'];
			$info_w['extrusion_material'] 		= $row['extrusion_material'];
			$info_w['extrusion_size'] 			= $row['extrusion_size'];
			$info_w['mixing_batch_size'] 		= $row['mixing_batch_size'];
			$info_w['calculation_batch_qty'] 	= $row['batch_size'];
			$info_w['planning_qty'] 			= $POST['pqty'];
			
			$info_w['cdate']				=date("Y-m-d");
			$info_w['company_id']			=$_SESSION['company_id'];
			$info_w['userid']				=$_SESSION['user_id'];
			$inserinvoiceidexp1=add_record('solid_production_planning', $info_w, $dbcon);

			if(!empty($info_w['sales_order_trn_id'])){
				// $info['sales_ordertrn_id']		= $info_w['sales_order_trn_id'];
				// $info['product_id']				= $info_w['product_id'];
				// $info['product_qty']			= $info_w['planning_qty'];
				// $info['request_id']				= $inserinvoiceidexp1;
				// $info['unit_id']				= $row['product_base_unit'];
				// $info['user_id']				= $_SESSION['user_id'];
				// $info['cdate']					= date("Y-m-d");
				// $info['company_id']				= $_SESSION['company_id'];
				
				// $inser1=add_record('tbl_sales_order_production_trn', $info, $dbcon);
				$whre=" and so_trn.sales_ordertrn_id=".$POST['sales_order_trn_id'];
			}//else{
				//sales order pllaning ma pending hoy aeni loop fervi entry karav
				$pqty=$info_w['planning_qty'];
					$que="select (IFNULL(product_qty,0)-IFNULL(stock_add,0)) as pending_qty,so_trn.sales_ordertrn_id from tbl_sales_ordertrn as so_trn
						...
						where so_trn.sales_ordertrn_status=0 and so_trn.production_status=0 and so.order_accept_status = 1 and so_trn.short_close_status=0 and so_trn.invoice_status=0 and so.approve_status=3 and so_trn.product_id=".$info_w['product_id']." ".$whre." having pending_qty > 0";
					$rs_di=$dbcon->query($que);
					while($re=brp_mysqli_fetch_assoc($rs_di)){

						if($re['pending_qty']>0 && $pqty>0){
							if($re['pending_qty']>=$pqty){
								$suqty=$pqty;
							}else{
								$suqty=$re['pending_qty'];
							}
							$pqty=$pqty-$suqty;
							$info['sales_ordertrn_id']		= $re['sales_ordertrn_id'];
							$info['product_id']				= $info_w['product_id'];
							$info['product_qty']			= $suqty;
							$info['request_id']				= $inserinvoiceidexp1;
							$info['unit_id']				= $row['product_base_unit'];
							$info['user_id']				= $_SESSION['user_id'];
							$info['cdate']					= date("Y-m-d");
					...
					if($pqty>0){	
						$infom['product_id']			= $info_w['product_id'];
						$infom['product_qty']			= $pqty;
						$infom['request_id']			= $inserinvoiceidexp1;
						$infom['unit_id']				= $row['product_base_unit'];
						$infom['user_id']				= $_SESSION['user_id'];
						$infom['cdate']					= date("Y-m-d");
						$infom['company_id']			= $_SESSION['company_id'];
						
						$inser1=add_record('tbl_min_max_production_trn', $infom, $dbcon);
					}
			//}
			

			if($inserinvoiceidexp1){
				$arr['msg']="1";
			}else{
				$arr['msg']="0";
			}
	...
	}else if(strtolower($POST['mode'])== "open_so_trn_modal_allocate"){
		$query_img 	= "select bsiz.sales_order_no,bsiz.sales_order_date,pro.product_name,product_qty,mst.product_id,mst.sales_ordertrn_id from tbl_sales_ordertrn as mst 
					left join tbl_sales_order as bsiz on bsiz.sales_order_id=mst.sales_order_id
					left join product_mst as pro on pro.product_id=mst.product_id
					where mst.sales_ordertrn_id=".$POST['sales_ordertrn_id'];
	...
	else if(strtolower($POST['mode'])== "open_so_trn_modal_allocate_mul"){
		$query_img 	= "select pro.product_name from product_mst as mst 
					where mst.product_id=".$POST['product_id'];
			$result_img = $dbcon->query($query_img);
			$row = brp_mysqli_fetch_array($result_img);
	...
	else if(strtolower($POST['mode'])== "open_batch_modal_pro"){
		$query_img 	= "select pro.product_name from product_mst as pro 
					where pro.product_id=".$POST['sub_product_id'];
		 	$result_img = $dbcon->query($query_img);
		 	$row = brp_mysqli_fetch_array($result_img);
		 
		$html='<table cellspacing="10" style="border-spacing:10px; table-layout: fixed;" id="product_list" class="display table table-bordered table-striped">
			<tr>
				<th colspan="3" style="text-align: center;">'.$row["product_name"].'</th>
			</tr>
			<tr>
				<td>Batch No</td>
				<td>Qty</td>
			...
			<tr>
				<td>
					...
					'.get_batch_printing_allocate($dbcon,$_POST['sub_product_id']).'
					</select>
				</td>
				<td>
					<input type="number" class="form-control" value="" id="smqty" name="smqty" />
					<input type="hidden" class="form-control" value="" id="smqty_hide" name="smqty_hide" />
				</td>
				<td>
					<button class="btn btn-primary" data-original-title="Save" data-toggle="tooltip" data-placement="top" type="button" onclick="allocate_temp_add();">Add</button>
				</td>
			</tr>
		</table>';
		...
		// 	$arr['product_id']=$row['product_id'];
			$arr['htmlq']=$html;
			echo json_encode($arr);
	}else if(strtolower($POST['mode'])== "load_batch_temp"){
		$html='<table cellspacing="10" style="border-spacing:10px; table-layout: fixed;" id="product_list" class="display table table-bordered table-striped">
			<tr>
				<td>Batch No</td>
				<td>Qty</td>
		...
		$query_img 	= "select batch_no,qty,batch_temp_id from solid_printing_batch_temp as mst 
						where mst.status=3 and product_id=".$POST['product_id']." and sub_product_id=".$POST['sub_product_id'];
			$result_img = $dbcon->query($query_img);
			while($row = brp_mysqli_fetch_array($result_img)){
				$html.='<tr>
					<td>'.$row["batch_no"].'</td>
					<td>'.$row["qty"].'</td>
					<td>
						<button class="btn btn-xs btn-danger" data-original-title="Delete" data-toggle="tooltip" data-placement="top" onClick="delete_batch_stock_entry('.$row['batch_temp_id'].')"><i class="fa fa-trash-o"></i></button>
					</td>
				</tr>
				';
			}
	...
	}else if(strtolower($POST['mode'])== "delete_batch_stock_entry"){
		$info['status'] = 2;			
		$updateid=update_record('solid_printing_batch_temp', $info,"batch_temp_id=".$_POST['batch_temp_id'] ,$dbcon);
		if($updateid){
			$arr['msg']=1;
		}else{
			$arr['msg']=0;
		...
		echo json_encode($arr);
	}else if(strtolower($POST['mode'])== "allocate_temp_add"){
		$query_img 	= "select batch_no from tbl_stock_trn as mst 
						where stock_id=".$POST['batch_id'];
			$result_img = $dbcon->query($query_img);
			$row = brp_mysqli_fetch_array($result_img);
		

		$infom['product_id']			= $POST['product_id'];
		$infom['sub_product_id']		= $POST['sub_product_id'];
		$infom['stock_id']				= $POST['batch_id'];
		$infom['batch_no']				= $row['batch_no'];
		$infom['qty']					= $POST['qty'];
		$infom['status']				= 3;
		$infom['userid']				= $_SESSION['user_id'];
		$infom['cdate']					= date("Y-m-d");
		
		$inser1=add_record('solid_printing_batch_temp', $infom, $dbcon);
		if($inser1){
			$arr['msg']=1;
		}else{
			$arr['msg']=0;
		...
		echo json_encode($arr);
	}else if(strtolower($POST['mode'])== "save_solid_allocate"){
		$query_img 	= "select sum(qty) as aqty,pro.printing_balty,pro.product_base_unit from solid_printing_batch_temp as mst 
						left join product_mst as pro on pro.product_id=mst.product_id
						where mst.status=3 and mst.product_id=".$POST['product_id']." and mst.sub_product_id=".$POST['sub_product_id'];
			$result_img = $dbcon->query($query_img);
			$row = brp_mysqli_fetch_array($result_img);

			$info_w['sales_order_trn_id'] 		= $POST['sotrn_allo'];
			$info_w['product_id'] 				= $POST['product_id'];
			$info_w['printing_material'] 		= $POST['sub_product_id'];
			$info_w['printing_balty	'] 			= $row['printing_balty'];
			$info_w['printing_req'] 			= 1;
			$info_w['planning_qty'] 			= $row['aqty'];
			$info_w['printing_allocate_qty'] 	= $row['aqty'];
			
			$info_w['cdate']				=date("Y-m-d");
			$info_w['company_id']			=$_SESSION['company_id'];
			$info_w['userid']				=$_SESSION['user_id'];
			$insert_id=add_record('solid_production_planning', $info_w, $dbcon);


			if(!empty($info_w['sales_order_trn_id'])){
				$whre=" and so_trn.sales_ordertrn_id=".$POST['sotrn_allo'];
			}
			$pqty=$info_w['planning_qty'];
					$que="select (IFNULL(product_qty,0)-IFNULL(stock_add,0)) as pending_qty,so_trn.sales_ordertrn_id from tbl_sales_ordertrn as so_trn
							left join tbl_sales_order as so on so.sales_order_id=so_trn.sales_order_id
			...
			$query 	= "select mst.stock_id,mst.qty,mst.sub_product_id,pro.product_base_unit,pro.product_conv_unit,sto.used_base_stock,sto.used_convert_stock,mst.batch_temp_id from solid_printing_batch_temp as mst 
						left join product_mst as pro on pro.product_id=mst.sub_product_id
						left join tbl_stock_trn as sto on sto.stock_id=mst.stock_id
						where mst.status=3 and mst.product_id=".$POST['product_id']." and mst.sub_product_id=".$POST['sub_product_id'];
			$result = $dbcon->query($query);
			while($row_1 = brp_mysqli_fetch_array($result)){

				$info_dil15['reserve_date']		= date("Y-m-d");
				$info_dil15['product_id']		= $row_1['sub_product_id'];
				$info_dil15['godown_id']		= 1;
				$info_dil15['base_unit']		= $row_1['product_base_unit'];
				$info_dil15['base_stock']		= $row_1['qty'];
				$info_dil15['convert_unit']		= $row_1['product_conv_unit'];
				$info_dil15['convert_stock']	= $row_1['qty'];
				$info_dil15['stock_flage']		= 1;
				$info_dil15['ref_name']			= "exe";
				$info_dil15['ref_id']			= 0;
				$info_dil15['cdate']			= date("Y-m-d h:i:s");
				$info_dil15['user_id']			= $_SESSION['user_id'];
				$info_dil15['company_id']		= $_SESSION['company_id'];
				$info_dil15['p_id']				= $insert_id;
				$info_dil15['stock_id']			= $row_1['stock_id'];
				
				$inserd=add_record("tbl_reserve_stock",$info_dil15,$dbcon);
				
				$infor1w['used_base_stock']= $row5['used_base_stock']+$row_1['qty'];
				$infor1w['used_convert_stock']= $row5['used_convert_stock']+$row_1['qty'];
				$updateid_k=update_record("tbl_stock_trn",$infor1w,"stock_id=".$row_1['stock_id'],$dbcon);

				$infr1w['status']= 0;
				$infr1w['solid_production_planning_id']= $insert_id;
				$updateid_k=update_record("solid_printing_batch_temp",$infr1w,"batch_temp_id=".$row_1['batch_temp_id'],$dbcon);
			}
			if($insert_id){
				$arr['msg']=1;
			}else{
		...
		$query 	= "select base_stock from tbl_stock_trn as mst 
						where mst.stock_status!=2 and stock_flage=1 and mst.stock_id=".$POST['stock_id'];
			$result = $dbcon->query($query);
		$row = brp_mysqli_fetch_array($result);

		$query_1 	= "select IFNULL(sum(base_stock),0) as used_stock from tbl_stock_trn as mst 
						where mst.stock_status!=2 and stock_flage=2 and mst.perent_id=".$POST['stock_id'];
			$result_1 = $dbcon->query($query_1);
		$row_1 = brp_mysqli_fetch_array($result_1);
		
		$query_2 = "select IFNULL(sum(base_stock),0) as rplase from tbl_reserve_stock as mst 
						where mst.stock_status!=2 and stock_flage=1 and mst.stock_id=".$POST['stock_id'];
			$result_2 = $dbcon->query($query_2);
		$row_2 = brp_mysqli_fetch_array($result_2);

		$query_3 = "select IFNULL(sum(base_stock),0) as rmin from tbl_reserve_stock as mst 
						where mst.stock_status!=2 and stock_flage=2 and mst.stock_id=".$POST['stock_id'];
			$result_3= $dbcon->query($query_3);
		$row_3= brp_mysqli_fetch_array($result_3);
		$stock=($row['base_stock']-$row_1['used_stock'])-($row_2['rplase']-$row_3['rmin']);
			$arr['stock']=$stock;
...
function add_requst_rnd($dbcon,$product_id,$sales_ordertrn_id,$qty,$version_id)
{
	...
	if($inserid)
	{
		...
		$info2['shortclose_remark']		= '';
						...
						left join pro_bom_process on pro_bom_process.bom_version_id = prover.bom_version_id
						left join tbl_product_process on tbl_product_process.pr_process_id = pro_bom_process.pr_process_id
						WHERE bom.bom_product='".$rel1['product_id']."'"; 
