...
if(strtolower($POST['mode']) == "fetch") {
	...
	$appData = array();
	$i=1;
	$aColumns = array('job.job_work_id','job.chalan_no','job.job_work_no','job.job_work_date','l.l_name','job.vehicle_no','branch.branch_name','job.g_total'.$pera);
	$sIndexColumn = "job.job_work_id";
	$isWhere = array(' job.is_reprocess and '.$where);
	...
	foreach($sqlReturn as $row) {
		...
		$app_btn=''; $print_btn='';
		$app_btn = '<a class="btn btn-xs btn-success" data-original-title="Create Chalan" data-toggle="tooltip" data-placement="top" href="'.ROOT.PRODUCTION_ROOT.'create_reprocess_jobwork_chalan/'.$row['job_work_id'].'" ><i class="fa fa-plus"></i></a>';
