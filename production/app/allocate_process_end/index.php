  ...
  //  if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) 
	{
		...
		if(strtolower($POST['mode']) == "end_process") {
			...
			*/
		...
		}else if(strtolower($POST['mode']) == "load_scrap") {

			$scr 	= "select sc.*,(select sum(qty) from tbl_scrap_add where alloc_id = sc.alloc_id) as used_qty from tbl_scrap_add as sc where sc.status=0 and sc.alloc_id=".$POST['alloc_id'];
			$sc_r = brp_mysqli_fetch_assoc($dbcon->query($scr));
			$scrap_detail = "";
			$scrap_detail .= '
			<input type="hidden" name="sc_used_qty" id="sc_used_qty" value="'.$sc_r['used_qty'].'">
			<table class="display table table-bordered table-striped" style="margin-top:20px">
				<tr>
					<th>Expected Scrap</th>
					<th>Rate</th>
					<th>Scrap Recieved</th>
					<th>Scrap Code</th>
				</tr>';
			if(mysqli_num_rows($dbcon->query($scr))>0){
				$scrap_detail .= '	<tr>
						<td>'.$sc_r['alloc_id'].'</td>
						<td>'.$sc_r['product_id'].'</td>
						<td>'.$sc_r['process_id'].'</td>
						<td>'.$sc_r['qty'].'</td>
					</tr>';
			}else{
				$scrap_detail .= '	<tr>
			...
			$info2['scrap_expected_qty']= $POST['scrap_expected_qty'];
			$info2['rate']						= $POST['rate'];
