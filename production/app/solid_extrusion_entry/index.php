...
$pro_search=explode(",", $production_pro_search);
		...
		$aColumns = array('pro.product_name','bsm.size_name','so_trn.extrusion_size','so_trn.printing_material','IFNULL(sum(planning_qty-excluding_allocate_qty),0) as pending_qty','IFNULL(sum(excluding_allocate_qty-excluding_allocate_approve_qty),0) as pending_allo_qty','IFNULL(sum(excluding_allocate_approve_qty-excluding_complate_qty),0) as pending_end_qty');
		...
		$isJOIN = array("left join product_mst as pro on pro.product_id=so_trn.printing_material","left join solid_size_mst as bsm on bsm.size_id=so_trn.extrusion_size");
		
		$hOrder = "pro.product_name";
		//$hGroupby = "so_trn.printing_material,so_trn.extrusion_size";
		$hGroupby =array("so_trn.printing_material","so_trn.extrusion_size");
		...
		foreach($sqlReturn as $row) {
			...
			if($POST['stage1']==0){
				...
				$view_desc='<button class="btn btn-xs btn-primary" data-original-title="Sales Order Detail" data-toggle="tooltip" data-placement="top" type="button" onclick="open_allo_modal('.$row['printing_material'].','.$row['extrusion_size'].')"><i class="fa fa-eye"></i></button>';
			}else{
				$view_desc='<button class="btn btn-xs btn-primary" data-original-title="Sales Order Detail" data-toggle="tooltip" data-placement="top" type="button" onclick="open_exe_end_modal('.$row['printing_material'].','.$row['extrusion_size'].')"><i class="fa fa-eye"></i></button>';
			}
		...
		where gd.status=0 and gd.printing_material=".$POST['product_id']." and gd.extrusion_size=".$POST['size']." and excluding_allocate_qty>excluding_allocate_approve_qty";
		$rel=$dbcon->query($q);
		while($row=mysqli_fetch_array($rel)){
			 $q1="select gd.*,sto.batch_no from tbl_reserve_stock as gd 
				...
				where gd.stock_status=0 and gd.ref_name='mixing' and gd.stock_flage=1 and gd.p_id=".$row['solid_production_planning_id']." and gd.base_stock>gd.approve_base_stock";
			$rel1=$dbcon->query($q1);
		...
		$q1="select gd.reserve_id,gd.base_stock,gd.approve_base_stock,gd.p_id,spa.excluding_allocate_approve_qty,spa.solid_production_planning_id,spa.calculation_batch_qty from tbl_reserve_stock as gd 
			...
			$info['excluding_allocate_approve_qty']= $row1['excluding_allocate_approve_qty']+$row1['calculation_batch_qty'];
		...
		$q="select pro.product_name,bsm.size_name,IFNULL(sum(excluding_allocate_approve_qty-excluding_complate_qty),0) as pending_qty from solid_production_planning as gd 
		left join product_mst as pro on pro.product_id=gd.printing_material
		left join solid_size_mst as bsm on bsm.size_id=gd.extrusion_size
		where gd.status=0 and gd.printing_material=".$POST['product_id']." and gd.extrusion_size=".$POST['size']." group by gd.printing_material,gd.extrusion_size";
		$rel=$dbcon->query($q);
		...
		$info_dil['printing_material']			= $_POST['product_id'];
		$info_dil['size_id']					= $_POST['size_id'];
		...
		$inserid_k=add_record("tbl_exe_end",$info_dil,$dbcon);
		...
		$q="select IFNULL(excluding_allocate_approve_qty-excluding_complate_qty,0) as pending_qty,solid_production_planning_id,excluding_complate_qty from solid_production_planning as gd 
		where gd.status=0 and gd.printing_material=".$_POST['product_id']." and gd.extrusion_size=".$_POST['size_id'];
		$rel=$dbcon->query($q);
		while($row=mysqli_fetch_array($rel))
		{
			if($row['pending_qty']>0)
			{
				if($end_qty>0)
				{
					...
					$info['excluding_complate_qty']= $row['excluding_complate_qty']+$useqty;
					...
					$info_dil1['exe_end_id']							= $inserid_k;
					...
					$inserid_ks=add_record("tbl_exe_end_trn",$info_dil1,$dbcon);
					//deduction start
					$ddqty=$info_dil1['end_qty'];
					$q2="select gd.*,spa.calculation_batch_qty from tbl_reserve_stock as gd 
					left join solid_production_planning as spa on spa.solid_production_planning_id=gd.p_id
					where gd.stock_status=0 and stock_flage=1 and approve_base_stock>used_base_stock and gd.ref_name='mixing' and gd.p_id=".$info_dil1['solid_production_planning_id'];
					$rel2=$dbcon->query($q2);
					while($row2=mysqli_fetch_array($rel2))
					{
						...
						if($ddqty>0 && $ptqty>0)
						{
							...
							$ddqty=$ddqty-$row2['calculation_batch_qty'];
							...
							$info_dil13['base_stock']		= $row2['base_stock'];
							$info_dil13['convert_unit']		= $row2['convert_unit'];
							$info_dil13['convert_stock']	= $row2['convert_stock'];
							$info_dil13['stock_flage']		= 2;
							$info_dil13['ref_name']			= "exe";
					...
					//reserve start
						...
						$q4="select * from solid_production_planning as gd 
						where gd.solid_production_planning_id=".$info_dil1['solid_production_planning_id'];
						$rel4=$dbcon->query($q4);
						$row4=mysqli_fetch_array($rel4);
						
						if($row4['printing_req']==1)
						{
							//var_dump("no");
							$q5="select * from tbl_stock_trn as gd 
								where stock_status=0 and stock_flage=1 and cast(base_stock AS DECIMAL(50,5))>cast(used_base_stock AS DECIMAL(50,5)) and ref_name='exe' and gd.ref_id=".$inserid_k;
								$rel5=$dbcon->query($q5);
								while($row5=mysqli_fetch_array($rel5))
								{
									$ppshqty=$row5['base_stock']-$row5['used_base_stock'];
									if($ppshqty>0 && $rrqty>0)
									{
										...
										$info_dil15['product_id']		= $row5['product_id'];
										$info_dil15['godown_id']		= 1;
										$info_dil15['base_unit']		= $row5['base_unit'];
										$info_dil15['base_stock']		= $sssqty;
										$info_dil15['convert_unit']		= $row5['convert_unit'];
										$info_dil15['convert_stock']	= $sssqty;
										...
										$info_dil15['stock_id']			= $row5['stock_id'];
										
										$inserd=add_record("tbl_reserve_stock",$info_dil15,$dbcon);
										$infor1w['used_base_stock']= $row5['used_base_stock']+$sssqty;
										$infor1w['used_convert_stock']= $row5['used_convert_stock']+$sssqty;
										$updateid_k=update_record("tbl_stock_trn",$infor1w,"stock_id=".$row5['stock_id'],$dbcon);
										
										$queryup="select printing_allocate_qty from solid_production_planning as gd 
										where solid_production_planning_id=".$info_dil1['solid_production_planning_id'];
										$relup=$dbcon->query($queryup);
										$rowup=mysqli_fetch_array($relup);
								

										$infor1wp['printing_allocate_qty']= $rowup['printing_allocate_qty']+$sssqty;
										$updateid_kx=update_record("solid_production_planning",$infor1wp,"solid_production_planning_id=".$info_dil1['solid_production_planning_id'],$dbcon);
							 ...
							 $q6="select gd.sales_order_production_trn_id,gd.sales_ordertrn_id,IFNULL(gd.allocate_qty,0) as allocate_qty,gd.product_qty,spa.printing_material,pro.product_base_unit,pro.product_conv_unit from tbl_sales_order_production_trn as gd
								left join solid_production_planning as spa on spa.solid_production_planning_id=gd.request_id
								left join product_mst as pro on pro.product_id=spa.printing_material
								where gd.sales_order_production_status=0 and cast(gd.product_qty AS DECIMAL(50,5))>cast(gd.allocate_qty AS DECIMAL(50,5)) and gd.request_id=".$info_dil1['solid_production_planning_id'];
								$rel6=$dbcon->query($q6);
								while($row6=mysqli_fetch_array($rel6))
								{
									$soallocate=$row6['allocate_qty'];
										...
										while($row7=mysqli_fetch_array($rel7))
										{
											if($sssqty>0)
											{
												...
												$queryup="select printing_allocate_qty from solid_production_planning as gd 
													...
													$infor1wp['printing_allocate_qty']= $rowup['printing_allocate_qty']+$stuqty;
													$infor1wp['printing_allocate_approve_qty']= $rowup['printing_allocate_qty']+$stuqty;
													$infor1wp['printing_complate_qty']= $rowup['printing_allocate_qty']+$stuqty;
													$updateid_kx=update_record("solid_production_planning",$infor1wp,"solid_production_planning_id=".$info_dil1['solid_production_planning_id'],$dbcon);
													$soallocate=$soallocate+$stuqty;
													$infor1wpa['allocate_qty']= $soallocate;
													$updateid_kxa=update_record("tbl_sales_order_production_trn",$infor1wpa,"sales_order_production_trn_id=".$row6['sales_order_production_trn_id'],$dbcon);
											}
										}
									}
								}
