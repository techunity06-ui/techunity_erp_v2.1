	...
	<thead>
		<tr>
			...
			<th style="text-align:left;border: none;"> 
				<=$set_head['address']?> 
				< if($set_head['contact_no']){?><br/>Contact No. <=$set_head['contact_no']?><}?>
				< if($set_head['website']){?><br/>E-Mail: <=$set_head['website']?><}?>
			</th>
		</tr>
	</thead>
</table>-->
...
<table width="100%" class="maintable" style="font-size: 12px;" id="invoice_type" >
	...
	//$result1=$dbcon->query($query1);
		...
		while($rel1=mysqli_fetch_assoc($result1))
		{
			...
			<?=bom_show_print($dbcon,$rel1['p_bom_id'],$rel1['product_base_qty'],$i,$call,$space,$main_bom_id);?>
