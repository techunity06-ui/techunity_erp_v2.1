<?php 
	...
	$query="select p.product_name,pr.process_name,ap.branch_id,ap.process_unit,umst.unit_name,ap.p_product_id,ap.process_id,ap.product_version,ap.batch_no from tbl_allocate_process as ap 
...
<!DOCTYPE html>
<html lang="en">
	<head><title>Start Process</title>
	...
	<body onload="show_material_list();">
										...
										<?php if($company_config['batch_wise_stock'] == '1' && $company_config['batch_process'] == '0'){ 
											...
											<div class="col-md-4">
												<div class="form-group">
													...
													<div class="col-md-6 col-xs-11">
														<input id="batch_no" name="batch_no" type="text" class="form-control" title="Enter <?=$lbl_batch?>" value="<?=$rel['batch_no']?>" placeholder="<?=$lbl_batch?>" required readonly>
													</div>
												</div>
											</div>
		...
		?>   
		<script src="<?=ROOT.PRODUCTION_ROOT?>js/app/production_process_start.js"></script>
		<script>
			$(".select2").select2({
				width: '100%'
			});
