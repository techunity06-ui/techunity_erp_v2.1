...
$type=$dbcon->real_escape_string($_REQUEST['id']);
	...
	$query='select job_trn.product_id, job.vender_id, produ.product_name, produ.product_icode, led.l_name from tbl_job_work as job 
				...
				where job.grn_complete_status="0" and job_trn.grn_complete_status="0"  and job_strn.job_work_sub_trn_status = 0 and job.job_work_type=2 and job.job_work_status="0" and job_trn.job_work_trn_status=0 and job.company_id='.$_SESSION['company_id'].' group by job_trn.product_id,job.vender_id';
	$result = $dbcon->query($query);		

	$product_list = '<option value="">Select Product</option>';
	$vender_list = '<option value="">Select Vendor</option>';

	while($row = brp_mysqli_fetch_assoc($result)){

		$product_list .= '<option value="'.$row['product_id'].'">'.$row['product_name'].' -- ('. $row['product_icode'].')</option>';
		$vender_list .= '<option value="'.$row['vender_id'].'">'.$row['l_name'].'</option>';
	}
...
<html lang="en">
	...
	<body>
		...
		<script src="js/app/pending_job_work.js"></script>
		<script>
			$(".select2").select2({
				width: '100%'
			});
