...
<html lang="en">
	<head>
		<title>PRODUCTION REPORT LIST</title>
					...
					<div class="row">			
						<div class="col-sm-12">
									...
									<?php 
										...
										$querymenu1re1="SELECT menumaster.*, CASE WHEN parent_id = 0 THEN id ELSE parent_id END AS sort FROM menu_master_access as menumaster WHERE menumaster.status=0 and menu_path='#production_report_list' ORDER BY sort, id";
									...
									<ul class="sub ulpad0">
										...
										?> 
											<?php if(in_array($rel_menu1re['accessdata'], $user_access_permission)){ ?>
												...
												<a  class="two"  href="<?=ROOT.strtolower($rel_menu1re['route_path_name'])?>" target="_blank"><?=ucwords(strtolower($rel_menu1re['menu_name']))?></a></li>
												</div>
											<?php } ?>
									<?php } ?>
									</ul>
