...
<html lang="en">
	<head>
	<title>Production Store Release Material</title>
	...
	<body onload="show_material_list();">
							...
							<section class="panel" >
								...
								<div class="" style="padding:20px !important;">
								  <ul class="breadcrumb">
									  ...
									  <li><a href="<?=ROOT.PRODUCTION_ROOT.'process_detail_list/'.$rel['process_id'].'/'.$rel['pr_process_type'];?>">Store Approval Pending </a></li>
								  </ul>
								</div>
							</section>
						</div>	
										...
										<div class="row">
											...
											<div class="col-md-12">
												<div class="col-md-6 col-md-offset-4">  	
													...
													<input type="submit" id="sp_btn" name="submit" class="btn btn-success" value="Aprrove Qty" />
												</div>
											</div>		
										</div>
		...
		?>   
		<script src="<?=ROOT.PRODUCTION_ROOT?>js/app/production_store_approve.js"></script>
		<script>
			$(".select2").select2({
				width: '100%'
			});
		...
		<?php
		if($mode=="add_store_approve" && $count_job==0){
			echo "<script>get_series_no()</script>";
			echo "<script>load_jobcard_no()</script>";
		} 
		?>
	</body>
</html>
