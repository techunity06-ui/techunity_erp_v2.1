...
function load_datatable()
{
	...
	$.ajax({
			type: "POST",
			url: root_domain+production_domain+'app/store_request_details_list/',
...
function store_request_using_model(p_ids) 
{
	Loading();
	var process_id=$('#process_id').val();
			$("#store_request_data").show();
				$("#store_request_material_detail").hide();
	$.ajax({
			type: "POST",
			url: root_domain+production_domain+'app/store_request_details_list/',
			data: { mode : "store_request_using_model",p_ids:p_ids,process_id:process_id},
			success: function(response)
			{
				//alert(response);
...
function store_request(){
    ...
    $.ajax({
		type: "POST",
		url: root_domain+production_domain+'app/production_store_request/',
		data: { mode : mode,pid:bomObj.pidChecked,pid_wise_start_qty:bomObj.start_qtyChecked,max_available_qty:max_available_qty,pending_qty:pending_qty,product_base_unit:product_base_unit,branch_id:branch_id,product_id:product_id,process_id:process_id,product_version:product_version,remark:remark,start_qty:start_qty,rp_id:bomObj.rp_id },
		success: function(response)
		{
			if(response == '1') {
					toastr.success("PROCESS REQUEST SUCCESSFULLY", "SUCCESS");
			}
			else if(response == '0') {
...
function check_material_stock(p_id){
	Loading();
	$.ajax({
			type: "POST",
			url: root_domain+production_domain+'app/store_request_details_list/',
			data: { mode : "check_material_stock",p_id:p_id},
			success: function(response)
			{
				if(response.trim() == '1'){
					toastr.success("Material Available", "SUCCESS");
				}else{
					toastr.warning("Material Pending", "ERROR")
				}
				Unloading();
			}
	});
...
function check_process_stock(unit_id,branch_id,p_id,product_id,process_id){
	Loading();
	$.ajax({
			type: "POST",
			url: root_domain+production_domain+'app/store_request_details_list/',
			data: { 
				mode : "check_process_stock",
				unit_id:unit_id,
				branch_id:branch_id,
				p_id:p_id,
				product_id:product_id,
