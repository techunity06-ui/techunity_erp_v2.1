...
$("#store_request_add").on('submit',function(e) {
	
	var form = this;
	e.preventDefault();
	e.stopPropagation();	
	/* if (!$("#store_request_add").valid()) {
		return false;
	} */
		...
		success: function(response)
		{
			...
			if(response == '1') {
				...
				//if(redirect_page=='working_process_detail_list' || redirect_page==''){
					window.location=root_domain+production_domain+'store_request_detail_list/'+process_id_hid+'/1';
				//}else{
			...
			$('#store_request_add').trigger('reset');	
		},
