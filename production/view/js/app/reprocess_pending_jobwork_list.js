...
function load_jobwork_pending_datatable()
{
	...
	datatable = $("#jobwork-table").dataTable({
			...
			"iDisplayLength": 10,
			"sAjaxSource": root_domain+production_domain+'app/reprocess_pending_jobwork_list/',
...
function get_series_no_jobwork(){
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'app/reprocess_pending_jobwork_list/',
	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				...
				window.location=root_domain+production_domain+"reprocess_pending_jobwork_list";
...
function jobwork_no_confirm_msg(){
	var jobwork_total_qty = $("#jobwork_total_qty");

	if(jobwork_total_qty == '0'){
		toastr.warning("PLEASE ENTER JOBWORK QTY", "WARNING");
		return false;
	}
	...
	$('#old_jobwork_modal').modal('show');
}



function save_store_confirmation(){
	var add_old_jobwork_no = $("input[name='sel_jobwork_no']:checked").val();

	$("#add_old_jobwork_no").val(add_old_jobwork_no);
	$("#jobwork_add").submit();
