	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(arr.msg == '1') {
				...
				window.location=root_domain+production_domain+'get_sales_order_details';
...
function show_data() {
	...
	datatable = $("#dynamic-table1").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain+production_domain+'app/store_order_design_department/',
...
function assign_standard_bom()
{
	...
	else
	{
		...
		$("input:checkbox").each(function () {
			if ($(this).is(":checked")) {
				if(typeof $(this).attr("value") != 'undefined')
				{
					...
					bomObj.bomversionChecked.push($(this).attr("data-bom_version_id"));
		...
		$.ajax({
			...
			data: { mode : "assign_standard_bom", product_id:bomObj.pidChecked,store_order_id:bomObj.soidChecked,bom_id:bomObj.bomidChecked,bom_version_id:bomObj.bomversionChecked},
