...
function show_data() {
	...
	datatable = $("#dynamic-table1").dataTable({
		...
		"iDisplayLength": 10,
		"sAjaxSource": root_domain+production_domain+'app/solid_extrusion_entry/',
...
function open_allo_modal(product_id,size){
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'/app/solid_extrusion_entry/',
		data: { mode : "preview_solid_allocate", product_id:product_id,size:size},
...
function open_exe_end_modal(product_id,size){
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'/app/solid_extrusion_entry/',
		data: { mode : "open_exe_end_modal", product_id:product_id,size:size},
		success: function(response){
			var arr = jQuery.parseJSON(response);
			$('#solid_exe_entry').modal('show');
			$("#pro_name").html(arr.product_name);
			$("#batchsiz").html(arr.size_name);
...
function save_solid_exe_planning(){
	...
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'/app/solid_extrusion_entry/',
		data: { mode : "save_solid_exe_planning", product_id:product_id,size_id:size_id,rolsize:rolsiz_arr,rolqty:rolqty_arr,finish_qty:finish_qty},
		success: function(response){
			var arr = jQuery.parseJSON(response);
			$('#solid_exe_entry').modal('hide');
			show_data();
			window.location=root_domain+inventory_domain+'solid_production_sticker_common_print/'+arr.id+'/2';
			Unloading();
		}		 
	});
}