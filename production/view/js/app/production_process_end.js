	...
	$.ajax({
		...
		success: function(response)
		{
			...
			if(response == '1') {
				...
				//alert('fdj');
				//window.location=root_domain+production_domain+'process_detail_list/'+process_id_hid+'/'+process_type_hid;
				
				//if(redirect_page=='working_process_detail_list' || redirect_page==''){
					window.location=root_domain+production_domain+'working_process_detail_list/'+process_id_hid+'/2';
				//}else{
...
function show_material_list_23_6_21()
{
	...
	//if(max_start_qty<=max_available_qty){
		...
		$.ajax({
		type: "POST",
		url: root_domain+production_domain+'app/allocate_process_end/',
	...
	if(qty>0){
		$.ajax({
			...
			success: function(data){
					...
					$('#mod_per_div_sec1').html(obj.str);
					//load_scrap();
					Unloading();
				}		
			});
	}else{
		toastr.info("Enter Product Qty", "INFO");
	}
}
function load_scrap(){
	...
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'app/allocate_process_end/',
		data: { mode : 'load_scrap',alloc_id:sallo_id},
		success: function(data){
				obj = jQuery.parseJSON(data);
				$('#scrap_detail').html(obj.scrap);
				Unloading();
			}		
		});
}
