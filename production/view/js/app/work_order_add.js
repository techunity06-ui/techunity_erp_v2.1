...
function next_page(){
	...
	var ext_stock_vendor_id = 0;
	if(extra_stock == 1){
		if($("#ext_stock_vendor_id").val()==""){
				toastr.warning("Select Supplier", "ERROR");
				return false;
		}else{
			ext_stock_vendor_id = $("#ext_stock_vendor_id").val();
		}
	}
	...
	Loading();
		$.ajax({
			type: "POST",
			url: root_domain+production_domain+'app/work_order_add/',
			data: { mode : "save_work_order_product",product_id:product_id,qty:qty,bom_version_id:bom_version_id,branch_id:branch_id,extra_stock:extra_stock,ext_stock_vendor_id:ext_stock_vendor_id,priority_status:priority_status},
	...
	/*	alert(response);
				...
				if(arr.res==="1"){
						...
						var sp_id = arr.sp_id;
						location.href= root_domain+production_domain+'direct_workorder/'+product_id+'/'+branch_id+'/'+bom_version_id+'/'+qty+'/'+sp_id;
						return false;
					}else{
...
function check_already_work_order (){
	var product_id=$("#product_id").val();
	var branch_id=$("#branch_id").val();
	if(branch_id!="" && branch_id!=0){	
		$.ajax({
				type: "POST",
				url: root_domain+production_domain+'app/work_order_add/',
				data: { mode : "check_already_work_order",product_id:product_id,branch_id:branch_id},
				success: function(response)
				{
					if(response==="1"){
	...
	//alert(rp_id);
	 $.ajax({
			...
			success: function(response)
			{
				...
				if(response==="1"){
					toastr.success("ADDED SUCCESSFULLY", "SUCCESS");
					window.location=root_domain+production_domain+'job_card_list';
				}else{
					toastr.warning("SOMETHING WRONG", "ERROR")
				}
			}
...
function view_process(rp_id,parent_flag){
	
	/*if(parent_flag == "0")
	{
		toastr.warning("Please Add Product Under This Product", "WARNING");
		return false;
	}
	else
	{*/
...
function add_work_order_product(product_id,qty){
	...
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'app/work_order_add/',
		data: { mode : "add_work_order_product",product_id:product_id,qty:qty,product_name:product_name},
		success: function(response)
		{
...
function add_sub_product(rp_id,sub_product_id,main_product_id){
	var eid=$("#eid").val();
	var qty = $("#req_qty").val();
	var product_name=$("#product_name").val();
	$("#mtype").text('Add');
	...
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'app/work_order_add/',
		data: { mode : "add_work_order_sub_product",sub_product_id:sub_product_id,rp_id:rp_id,qty:qty,main_product_id:main_product_id,product_name:product_name},
		success: function(response)
		{
...
function save_work_order_product()
{
	...
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'app/work_order_add/',
		data: { mode : "save_work_order_product",main_product_id:main_product_id,wo_product_id:wo_product_id,qty:qty,product_qty:product_qty},
		success: function(response)
		{
...
function save_work_order_sub_product()
{
	...
	$.ajax({
		type: "POST",
		url: root_domain+production_domain+'app/work_order_add/',
		data: { mode : "save_work_order_sub_product",sub_product_id:sub_product_id,main_product_id:main_product_id,qty:qty,rp_id:rp_id,product_qty:product_qty},
		success: function(response)
		{
			if(response==="1"){
					toastr.success("ADDED SUCCESSFULLY", "SUCCESS");
...
function delete_work_order_product(prd_id,rp_id,parent_delete_flag)
{
    ...
    else
    {
		...
		type: "POST",
		url: root_domain+production_domain+'app/work_order_add/',
		data: { mode : "delete_work_order_product",rp_id:rp_id,parent_delete_flag:parent_delete_flag},
		success: function(response)
		{
			if(response==="1"){
					toastr.success("DELETED  SUCCESSFULLY", "SUCCESS");
					next_page(prd_id,qty);
					return false;
				}else{
					toastr.warning("SOMETHING WRONG", "ERROR")
				}
...
function get_reorder_qty()
{
	...
	$.ajax({
			type: "POST",
			url: root_domain+production_domain+'app/work_order_add/',
			data: { mode : "get_reorder_qty",product_id:product_id},
			success: function(response)
			{
					$('#reorder_qty').val(response.trim());
					Unloading();		
			}
	});
...
function toggle_vendor_list(stock_type){
	if(stock_type == 1){
		$(".vendor_row").show();
	}else{
		$(".vendor_row").hide();
