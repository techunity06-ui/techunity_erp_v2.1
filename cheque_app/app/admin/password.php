...
//    if(@isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],DOMAIN) !== false) {
		...
		if(strtolower($POST['type']) == "fetch") {
			$appData = array();
			$i=1;
			$aColumns = array('acc_id', 'acc_bank', 'bank_name', 'bank_branch','acc_name', 'acc_number', 'acc_chequeno', 'acc_chequeleft','acc_status', 'acc_tmst', 'acc_of');
			$sIndexColumn = "acc_id";
			...
			if($_POST['token'] == $_SESSION['token']) {
				$tr = $dbcon -> query("SELECT `acc_id`,`acc_status` FROM `coro_accounts` WHERE `acc_bank` = '$POST[bank]' AND `acc_number` = '$POST[number]' AND `acc_of` = '$_SESSION[user_id]'");
				if($tr->num_rows > 0) {
					$r = $tr -> fetch_assoc();
					if($r['cust_status'] == 0) {
					...
					$query = $dbcon -> query("INSERT INTO `coro_accounts`(`acc_bank`, `acc_name`, `acc_number`, `acc_chequeno`, `acc_chequeleft`,`acc_of`) VALUES ('$POST[bank]','$POST[name]','$POST[number]','$POST[cheque]','$POST[cheque_num]','$_SESSION[user_id]')");
					if($query)
						echo "1";
					else
						echo "0";
